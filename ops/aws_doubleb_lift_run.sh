#!/bin/sh
# Evidence-aware exact Singular membership runner for the double-B P(p).
# Usage: aws_doubleb_lift_run.sh TAG TIMEOUT_SECONDS
set -eu
...
generator=$repo_root/cases/max12_912_order3_double_b_p_membership_20260824/generate_lift.py

python3 "$generator" > "$input"
{
  echo "tag=$tag"
  echo "backend=Singular-lift-slimgb"
...
set +e
timeout --signal=TERM --kill-after=300 "$cap" nice -n 5 Singular -q < "$input" > "$result" 2> "$stderr"
rc=$?
set -e
