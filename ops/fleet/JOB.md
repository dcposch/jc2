...
`job.sh` is an opt-in worker-side pilot. The corrected version passed
different-model review and all nine registered no-CAS AWS regression groups
on September13; see the [test and scope record](../../box/execution-reliability-pilot-root-20260913/RESULT.md).
It replaces interactive, separately timed phase launch and observation with
one bounded service. This does not migrate or qualify the frozen F10
scientific harness; each scientific workload still needs its own registration
and applicable source/result checks. The [prior documentation snapshot](../../history/JOB-before-20260913T0910.md)
preserves the exact pre-test-status bytes recorded in the earlier packets.
