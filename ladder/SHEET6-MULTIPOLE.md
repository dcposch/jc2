...
> allowed. Actual-weight Corollary 7.1 and the multipole selected-exit sum
> are **GREEN** at MFE inequality scope: Definition 3.3's contact quotient
> makes off-tree witnesses uniquely attach to the rootward-closed pole-path
> union, so witnesses are globally distinct and a shared suffix is counted
> once (`f55a00f5...`, integration `c74fc0f9...`). MP8 therefore contains
> that selected-exit inequality plus its local `lambda=0` anatomy and the
> concrete witness;
> its literal (22) itemization, fixed `(22-cl)` ledger, equality/slack
> rhetoric, and categorical “no refinement can charge” conclusion are
> quarantined. The old `td=6` root enumeration omitted `l>=1`, but the
> replacement is analytic, not that enumeration: a genuine root meet is
> Proposition 9.3 case I and every actual incoming parent satisfies
> `X_root=mu(1-w)=A/B`, hence `0<w<1`. In the `td=6,m=2` sector, MP1,
> MP4, and MP5/D5 carry each forced row-1 chain to the last nonroot parent
> with exact depth alphabet `W={2}`. Thus the pole-chain root-meet branch
> there is excluded (`645b9424...`). Root `M=1` itself remains
> legal, and mixed-`mu`/off-axis root completeness at broader degrees and
> any overall `td=6` exclusion remain open.

**Historical status (2026-08-12, SHEET6-MP-REVIEW.md):
the then-filed MP0--MP8/Theorem O package was promoted and MP9 was said to
stand on an `l`-independent `kappa<nu` kill; the "finite book" rider was demoted to
...
Historical update 2026-08-12: the closure lemma was promoted — SHEET6-DEPTH.md
...
restored for the nonroot jump layer. The 2026-08-28 corrections above and in
SHEET6-DEPTH withdraw MP8's global equality claims and the old phase-3 root
count, while restoring both the selected-exit shared inequality and the
finite all-`M=1` root layer through separately reviewed repairs.
Mixed-`mu`/off-axis root completeness remains outside them.**
...
The local/nonroot seam remains useful. **Kill half:** for `m>=2` poles the
corrected Proposition 8.4 machinery applies on the nonroot shared suffix at
and below the global meet `G*` when `G*!=(0,y)`, and the merge calculus pins
entry values, `M=1` cascades, and resonant jump anatomy. Specified local
first-exit sets in a pure `M=1` segment can have zero local charge, but no
global budget-transparency or equality claim is made. **Obstruction half:**
the verbatim multi-pole analogue of Proposition 8.4 on pre-merge segments is
false, and the `(r,nu,l)=(2,3,1)` resonant pattern remains a local exact
witness. The selected-exit shared inequality is promoted, with shared
suffixes and merge arrivals deduplicated in the actual contact tree. The
`td=6,m=2` pole-chain root-meet branch is separately excluded by
MP1+MP4+MP5/D5, the reviewed depth alphabet, and the mixed-root edge theorem.
The full mixed-`mu`/off-axis root-aware reach book at broader degrees remains
pending; this paragraph does not assert a finite exhaustive book or exclude
`td=6`.
...
**MP2 (restored nonroot kill, general m).** If `G* != (0,y)`, then
`M_F != 1` for every `F in U \ {(0,y)}` with `F <= G*` (the nonroot trunk
and `G*` itself). The root is outside corrected Proposition 8.4.

**MP3 (nonroot escape set).** Among nonroot vertices, those escaping MP2
are exactly `{F in U:F>G*}`: all strict ancestors of `G*`. Separately,
`(0,y)` has no corrected-Proposition-8.4 `M!=1` constraint.
...
**MP8 (global selected-exit inequality; equality quarantined).** Let
`D_F` be the actual selected positive exit-orbits at each vertex of the
set-theoretic pole-path union `U^full`, excluding pole arrivals and the
rootward continuation. Definition 3.3's contact quotient and the reviewed
attachment lemma make their cv witnesses pairwise distinct; repaired
Propositions 6.7--6.8 exclude every remaining down direction. Applying
actual-weight `(C7.1*)` once, with the x-side witness counted once, gives
`sum_(F in U^full) lambda_F^exit <= td-1-psi`. A shared suffix occurs once,
never once per pole path. The pure `M=1` anatomy may still give local
`lambda_F=0`, and the explicit resonant witness remains valid. MP8 does not
itemize printed (22), assert `(22-cl)`, infer slack equality, or claim that
no refinement/repartition can charge the ancestry.
...
**MP9 (m = 2, b = (1,1) sharpening; td-uniform).** Both chains are M=1/λ=0 single-simple-orbit to the meet, μ = (1,1).
 ...
 (Root meet, G\* = (0,y)) [+H1 for the local clauses]: `l=0` is
impossible and `l=1` is locally solvable. Proposition 9.3(d), at child
`(nu,kappa-bar,D)=(1,1,l_f)`, forces `kappa-bar<nu` at both parents. The
old l-free `td=6` reach count is invalid. The corrected case-I handshake
requires `0<w<1` at every actual parent (and root top-degree cancellation
specializes this to `w=l/(2+l)` after MP5 gives `mu=(1,1)`). MP1+MP4+MP5/D5
give the exact row-1 parent alphabet `W={2}`, so the `td=6,m=2` pole-chain
root-meet branch is analytically empty; no AWS census is used for
that conclusion.
...
### D3 (MP2, MP3; historical root-inclusive proof scope superseded)

> **Canonical scope.** Take a nonroot `F<=G*`. The restored contradiction
> kills only nonroot `F<=G*`; `(0,y)` is a separate endpoint and has no
> Proposition-8.4 `M!=1` conclusion.
...
### D5 (MP5)
Downward induction from an M = 1 pre-merge vertex H, K = H° still pre-merge: (a) St 8.4 (p. 42) at the pair (K, H = K + c): mult(p_K^{red}, c) | M_H = 1 — the chain orbit is simple. (b) St 8.2 (p. 41) at that μ = 1 arrival: deg q_K > deg p_K; then for every root c′ of p_K, deg q·mult(p,c′) ≥ deg q > deg p, so every root is searrow — **no northeast roots exist at K at all**. (c) A non-chain root c′ has a realizable microstep by corrected St 3.18. Since K ∈ Ta& and a second root gives deg p_K > 1, repaired Prop 6.7 makes that microstep positive; the raw St 6.2 computation makes it searrow, with deg p = i·mult(p^{red}, εc′) ≥ i ≥ 2. Repaired Prop 6.8 (`c3d6ff92...`, sweep r2 `581219e0...`) then manufactures a pole above it **on that branch**, and in particular carries the direction to its next searrow vertex (Prop 3.2). That pole is necessarily one of the m poles of T_{a,pole} (this consumes no pole budget, which is why it scales to every m, replacing L1a's m=2-only third-pole counting); its chain visits K through direction c′, making K a merge vertex — contradiction with K strictly pre-merge. So p_K = single simple ν-orbit, dp = ν. (d) Root law + eta law [P2] (exact order/residue counting on (iv): at a p-root of mult μ, mult(q) = 1 forced; off-p q-roots simple; η-residue b ≡ 1 mod ν): dq = nν + 1, M_K = gcd(ν, nν+1) = 1 — verbatim the thesis's own step in St 9.6's proof (p. 52). (e) λ_K = 0: no direction in Ta% exists (b); the q-extras at non-roots of p have no tree continuation (St 3.18). (f) No ray separates: all full-pattern roots lie in one ν-orbit with a unique continuing representative (St 3.18; Prop 5.6's proof, p. 27). At the merge arrival: St 8.4 at (G_i, H_i) gives μ_i = 1. ∎
...
> **2026-08-28 root correction.** At `u=0`, obtain
> `k_f=(r+l)l_f` by direct top-degree comparison in reduced identity (iv),
> not by citing Statement 8.2 at the root.
...
### D8 (MP8; selected-exit inequality promoted, historical equality ledger superseded)

> Canonical proof: use `U^full=union_j I_(P_j)([0,v_j])`. Definition 3.3
> makes every ray's intersection with `U^full` a rootward interval. The
> reviewed attachment repair, with its pole-endpoint truncation fix, assigns
> each selected positive exit a unique cv witness and makes the assignment
> injective over all vertices. Repaired Propositions 6.7--6.8 remove down
> directions as pole arrivals. Actual-weight `(C7.1*)` plus the one x-side
> witness therefore gives `sum lambda_F^exit<=td-1-psi`, counting shared
> suffixes once (`c74fc0f9...`). The historical paragraph below is retained
> only for its local zero-exit anatomy; its equality/itemization and
> no-refinement conclusions remain quarantined.
...
**Open, stated, NOT claimed:** mixed merges with all μ_e ≥ 2 obey only MP6(b)/(d)-subadditivity (k = 0 needs some μ_e = 1; the l ≥ 1 forcing does not apply) — possible only for m ≥ 3 downstream of an earlier jump; the m ≥ 3 pattern-level escape (M = m via lν ≡ −1 mod m) is arithmetic existence only, r-edge H1 ratio-consistency unverified; mixed-`mu`/off-axis root completeness remains open, while the all-`M=1` `td=6,m=2` root meet is closed analytically; jump vertices are unkillable by the current local printed-tier tests (witness O), but this is not a global equality/no-refinement claim.
...
**Corrected local Theorem O.** No printed-tier argument can prove `M_F!=1`
on pre-merge segments (it is false: MP5 forces `M=1` there whenever
`b_i=1`), and the cell `(r,nu,l)=(2,3,1)`, `M=2` at `td=6,m=2` solves
Proposition 8.1(iv) exactly with rigid coefficients
`a_1/a_2=2+-sqrt(3)` and passes the listed local pattern/order constraints.
The `l` extra q-orbits are invisible to those local tests except through
`deg q`. The selected-exit shared inequality is now promoted through the
global contact-tree attachment theorem. It does not turn the local
zero-exit anatomy into a global equality/no-refinement statement, and full
finite-book conclusions are not part of this corrected theorem.
...
3. **Budget kill** ("each `M=1` occurrence costs at least one"): not
   established. Local transition exit sets can have zero charge, but the
   old stronger “no repartition can charge” conclusion used invalid equality
   `(22)` and is withdrawn.
4. **`lambda_root>=1` transported to `m` poles:** unresolved globally.
   Local merge anatomy can give `lambda_F^exit=0`; this does not prove a
   global equality or force zero actual mass at every refinement.
...
2. **Close the m = 2 root-merge residual at td = 7, 8** (l odd,
   `k_f=(2+l)l_f`, both `kappa-bar<nu`) using the corrected case-I depth
   and reach equations; the historical 2POLE phase-3 menu is not reusable.
...
- The k = 0 manufacture (D5c/D6a) is the load-bearing novelty in both A1 and A2, derived independently with the same repaired chain: Prop 6.7 supplies positive microsteps, the raw St 6.2 computation makes the charged direction searrow, and Prop 6.8 supplies both the next-vertex bridge and the same-branch pole (`c3d6ff92...`, sweep r2 `581219e0...`). Its no-pole-budget character is what makes Theorem MP m-uniform.
