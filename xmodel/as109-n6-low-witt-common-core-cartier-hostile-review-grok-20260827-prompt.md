...
Work in `/Users/dc/code/math/jc2`.  Do not edit any file.  Do not use or inspect
`jc2-lean`.  No web is needed.  No heavy local computation; the registered
...
Return one self-contained Markdown review in stdout, with a headline verdict
from `CONFIRMED`, `CONFIRMED_WITH_REPAIR`, `GAP`, or `REFUTED`; a numbered
claim table; independent derivation; replay/hashes; novelty adjudication;
licensed conclusion; and explicit scope firewall.  Do not write files.
