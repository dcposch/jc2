...
verify these SHA-256 hashes first, MECHANICALLY: the receipt `xmodel/xu71-offset-one-sol56-r2-20260906.run.v2` carries `charged_input_<i>_sha256=`/`_basename=` lines; build the manifest with awk and run `sha256sum -c`; stop only on a real content mismatch:
