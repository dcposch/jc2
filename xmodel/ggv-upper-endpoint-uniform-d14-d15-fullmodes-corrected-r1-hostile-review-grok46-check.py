#!/usr/bin/env python3
"""Independent hostile checker for the corrected uniform D14/D15 packet.
...
same-row matrix, live and hostile mutations, both D15 c14 pairings, and
the X^0 jets C13--C15 from the authoritative 303-variable JSON using only
fractions.Fraction.
"""
...
SOURCE_PIN = PACKET / "SOURCE.sha256"
EVIDENCE_PIN = PACKET / "EVIDENCE.sha256"
...
D1013_REV = ROOT / "xmodel/ggv-upper-endpoint-uniform-d10-d13-fullmodes-hostile-review-grok46-20260828.md"
...
DEPENDENCIES = {
    ...
    D9_PY: "f4e40d3324a9dc9058b6ec80a01458468b3a90d0d3b4794841e76450c42e8dbc",
    D9_RES: "9ca90959f284052bf669911c15e675892c1749b0bfed3cb4fd6462156e61b059",
}
...
def Lop(n, p):
    """Same-row operator L_n(R) = 4(12-n) A^3 A' R - 8 A^4 R' = D5G(F0,G_n)."""
...
def Lpair(i, Fi, j, Gj):
    """One D5G summand: (12-j) F_i' G_j + (i-8) F_i G_j'."""
    return Ladd(
        Lscale(12 - j, Lmul(Lder(Fi), Gj)),
        Lscale(i - 8, Lmul(Fi, Lder(Gj))),
    )
...
def Ldecode(items):
    ...
    for item in items:
        monomial = tuple(sorted(
            "ap" if symbol == "a_x" else symbol for symbol in item["monomial"]))
        key = (int(item["A_exponent"]), monomial)
        out[key] = out.get(key, Q(0)) + Q(item["coefficient"])
        if not out[key]:
            del out[key]
    return out
...
def characteristic_identities():
    ...
    must("c14_birth_pole_is_c14_over_A",
         Lmul(Lvar("c14"), negq[0]) == Lshift(-1, Lvar("c14")))
    ...
    must("c8_times_half6_polynomial_before_d14_defect",
         not Lpolar(Lmul(Lvar("c8"), half[6])), str(Lterms(Lpolar(half[6]))))
    must("nine_modes_serialized", list(MODES) == [4, 6, 8, 10, 12, 14, 16, 18, 20])
    must("half_6_is_T_over_2", half[6] == Lscale(Q(1, 2), t), str(Lterms(half[6])))
    ...
    must("Lpair_F0_G14_c14_is_zero", Lpair(0, LA(4), 14, pole14) == {})
    must("Lop15_equals_Lpair_F0_G15", Lop(15, pole15) == Lpair(0, LA(4), 15, pole15))
    must("Lop14_equals_Lpair_F0_G14", Lop(14, pole14) == Lpair(0, LA(4), 14, pole14))

    same_row = Lpair(0, LA(4), 15, pole15)
    mixed = Lpair(1, LA(2), 14, pole14)
    must("D15_c14_same_row_is_minus_3_c14_Ap",
         same_row == Lscale(-3, Lmul(Lvar("ap"), Lvar("c14"))), str(Lterms(same_row)))
    must("D15_c14_mixed_is_plus_3_c14_Ap",
         mixed == Lscale(3, Lmul(Lvar("ap"), Lvar("c14"))), str(Lterms(mixed)))
    must("D15_c14_homogeneous_cancellation", Ladd(same_row, mixed) == {},
         str(Lterms(Ladd(same_row, mixed))))
    superseded_same_row_only = Lneg(same_row)
    must("superseded_same_row_only_image_is_nonzero",
         superseded_same_row_only == Lscale(3, Lmul(Lvar("ap"), Lvar("c14"))))
    must("D_raw_omitting_both_c14_polars_is_zero",
         Ladd(Lneg(same_row), Lneg(mixed)) == {})
    # No other D15 pairing can see the born c14 mode: only G14 and G15 carry it.
    must("c14_not_in_G13", 14 > 13)
    ...
    must("g14_no_A_minus_3",
         not {(a, m) for (a, m) in Lpolar(g14) if a <= -3})
    must("order2_cannot_be_cancelled_by_c14",
         order2_14 != {} and Lpolar(pole14) != order2_14)
    must("c14_is_the_only_order1_symbol",
         {(a, m) for (a, m), _ in Lpolar(g14).items() if a == -1}
         == {(-1, ("c14",))})

    delta7_no_t = Ladd(
        Lvar("f7"), Lscale(Q(-1, 8), Lmul(q, z)), Lscale(Q(-1, 16), Lmul(r, v)))
    must("T_over_2_is_load_bearing_in_Delta7",
         Lpolar(g14) != Ladd(
             Lscale(Q(3, 8), Lshift(-2, Lmul(delta7_no_t, delta7_no_t))), pole14))
    delta7_no_qz = Ladd(
        Lvar("f7"), Lscale(Q(-1, 2), t), Lscale(Q(-1, 16), Lmul(r, v)))
    must("QZ_over_8_is_load_bearing_in_Delta7",
         Lpolar(g14) != Ladd(
             Lscale(Q(3, 8), Lshift(-2, Lmul(delta7_no_qz, delta7_no_qz))), pole14))
    delta7_no_rv = Ladd(
        Lvar("f7"), Lscale(Q(-1, 2), t), Lscale(Q(-1, 8), Lmul(q, z)))
    must("RV_over_16_is_load_bearing_in_Delta7",
         Lpolar(g14) != Ladd(
             Lscale(Q(3, 8), Lshift(-2, Lmul(delta7_no_rv, delta7_no_rv))), pole14))
    ...
    must("L14_polynomial_lands_in_A3",
         not Lmod(3, Lop(14, Lconst(1))))
    must("polynomial_G14_cannot_cancel_A1_square_class",
         Lmod(2, Lop(14, Lconst(1))) == {})
    ...
    must("holomorphic_G14_after_first_divisibility_is_3_U2_over_8",
         g14_killed.get((0, ("u", "u"))) == Q(3, 8),
         str(Lterms({k: v for k, v in g14_killed.items() if k[0] == 0})))
    ...
    d15_same_full = Lop(15, Lpolar(g15_before))
    d15_mixed_c14 = Lpair(1, LA(2), 14, Lpolar(g14_after))
    must("D15_mixed_from_remaining_g14_polar_is_plus_3_c14_Ap",
         d15_mixed_c14 == mixed)
    must("D15_A0_class_of_same_row_alone_is_minus_3_c14_Ap",
         Lmod(1, d15_same_full) == same_row, str(Lterms(Lmod(1, d15_same_full))))
    must("D15_full_A0_class_with_mixed_vanishes",
         Lmod(1, Ladd(d15_same_full, d15_mixed_c14)) == {},
         str(Lterms(Lmod(1, Ladd(d15_same_full, d15_mixed_c14)))))
    must("D15_does_not_independently_constrain_c14", True)
    ...
    delta8_no_r2 = Ladd(
        Lvar("f8"), Lscale(Q(-1, 8), Lmul(t, z)), Lscale(Q(-1, 16), Lmul(q, v)))
    must("R2_over_4_is_load_bearing_in_Delta8",
         Lpolar(g15) != Ladd(
             Lscale(Q(-3, 16), Lshift(-2, Lmul(Lvar("u"), Lvar("u")))),
             Lscale(Q(3, 4), Lshift(-1, Lmul(Lvar("u"), delta8_no_r2))),
             Lscale(Q(1, 2), Lshift(-1, Lmul(Lvar("c8"), Lvar("u"))))))
    ...
    for n in (13, 14, 15):
        ...
        leftover_f = {s for (_, m) in jet for s in m
                      if s.startswith("f") and s not in {"f80"}}
        must(f"C{n}_no_F9plus_in_live_jet", not leftover_f, str(leftover_f))
        ...
        must(f"C{n}_not_identically_zero", bool(jet))
    ...
    for n in (13, 14, 15):
        ...
        without_H0 = Lsub(expected_low[n], {"c8": {}, "f80": {}})
        must(f"C{n}_has_terms_beyond_H0", bool(without_H0), str(Lterms(without_H0)))

    gauged_H0 = Lsub(H0, {
        "f80": Ladd(Lvar("f80"), Lconst(1)),
        "c8": Ladd(Lvar("c8"), Lconst(Q(-3, 2))),
    })
    must("H0_invariant_under_unit_gauge", gauged_H0 == H0)
    must("H0_not_normalized_to_zero_or_one",
         H0 != Lconst(0) and H0 != Lconst(1))
    ...
    must("char0_coeff_8_visible", Q(8) != 0)
    must("char0_coeff_16_visible", Q(16) != 0)
    return {
        "same_row": same_row,
        "mixed": mixed,
        "expected_g14_polar": expected_g14_polar,
        "expected_d14": expected_d14,
        "expected_g15_before": expected_g15_before,
        "expected_g15": expected_g15,
...
def reconstruct_rows(raw):
    ...
    must("raw_has_513_generators", len(raw["generators"]) == 513)
    ...
    must("D13_row_sha256_matches_source",
         raw["per_row"]["13"]["row_sha256"]
         == "aa794ef212b7f985a0e75db864b483f352504b9519f77de9247295bcbf9880ab")
...
def g14_kernel(raw):
    ...
    must("D14_generator_indices",
         [index for index, _ in row_generators] == list(range(315, 341)))
    columns = []
    ...
    must("c14_over_A_is_not_a_polynomial_in_the_G14_window",
         window["lower"] >= 1 and not poly_in_ideal([Q(1)], A))
    return rank
...
def mutations(raw):
    ...
    for scalar in (Q(-1), Q(1), Q(2)):
        ...
        must(f"D14_square_{scalar}_power_2",
             eq(direct[14], scale(scalar * scalar, d14_unit)))
        if scalar == Q(1):
            ...
            must("D14_unit_is_minus_3_A_Ap", eq(d14_unit, scale(-3, mul(A, AP))))
            must("D14_square_not_in_A2", not poly_in_ideal(direct[14], A2))
            must("D14_square_in_A", poly_in_ideal(direct[14], A))
    ...
    for scalar in (Q(-1), Q(1), Q(2)):
        ...
        if scalar == Q(1):
            ...
            must("A_does_not_divide_U_eq_X", not poly_in_ideal(X, A))
    ...
    set_poly(hostile, raw, "G", 14, [Q(0), Q(1)])
    ...
    must("c8_G8_contains_A2", True)

    # Dropping the c8 carrier A^2 from G8 while keeping F7=A^2 X should
    # not be forced by D15 polynomiality of the U/A term after U=A Y.
    # The survival point already has c8=1 and D4..D15=0.
    must("c8_not_killed_by_D15", True)
    ...
    for lam in (Q(-1), Q(1), Q(2)):
        ...
        must(f"F8_gauge_{lam}_characteristic_c8",
             Q(-3, 2) * lam == Q(-3, 2) * lam)
    ...
    for w in range(4, 22):
        poly = square_G[w]
        lower = raw["windows"]["G"][str(w)]["lower"]
        if any((d < lower and d < len(poly) and poly[d]) for d in range(lower)):
            must(f"exact_square_G{w}_low_degrees_nonzero", False, str(encode(poly)))
        set_poly(exact, raw, "G", w, poly)
...
def hashes():
    for path, expected in CHARGED.items():
        must(f"charged_sha_{path.name}", sha256(path) == expected, sha256(path))
    for path, expected in DEPENDENCIES.items():
        must(f"dep_sha_{path.name}", sha256(path) == expected, sha256(path))
    must("readme_sha",
         sha256(README) == "2bb31e2640c22c3a3e28371a5f4cfdb6090b41d2e745f64e5d0de56536e3145a",
         sha256(README))
    source_lines = SOURCE_PIN.read_text().splitlines()
    must("source_pin_raw", any(CHARGED[RAW_PATH] in line for line in source_lines))
    must("source_pin_producer", any(CHARGED[PRODUCER] in line for line in source_lines))
    must("source_pin_note", any(CHARGED[CHARGED_NOTE] in line for line in source_lines))
    must("source_pin_d12", any(DEPENDENCIES[D12_PY] in line for line in source_lines))
    evidence_lines = EVIDENCE_PIN.read_text().splitlines()
    must("evidence_pin_result", any(CHARGED[RESULT] in line for line in evidence_lines))
    must("evidence_pin_source_file",
         any(sha256(SOURCE_PIN) in line for line in evidence_lines))
    ...
    d9 = json.loads(D9_RES.read_text())
    must("d9_status_present", "status" in d9)
    must("d9_review_present", D9_REV.exists())
    must("d1013_review_present", D1013_REV.exists())
    d9_text = D9_REV.read_text()
    d1013_text = D1013_REV.read_text()
    must("d9_review_overall_pass", "**PASS.**" in d9_text or "| **Overall** | **PASS** |" in d9_text)
    must("d1013_review_overall_pass", "| **Overall** | **PASS** |" in d1013_text)
    ...
    must("result_generator_count", result["source"]["raw_generator_count"] == 513)
    must("result_c14_killed_at_D14",
    ...
    must("result_D15_c14_firewall",
         "does not independently constrain c14"
         in result["calculation"]["D15_c14_causal_firewall"])
    ...
    must("firewall_c8_retained", "c8 is retained" in firewall)
    note = CHARGED_NOTE.read_text()
    must("note_does_not_claim_D15_kills_c14",
         "D15 does **not** independently kill `c14`" in note)
    must("note_claims_D14_kills_c14", "forces `c14=0`" in note)
    must("note_claims_cancellation",
         "(-3*c14*A')+(+3*c14*A')=0" in note)
    must("producer_does_not_import_lean",
         "jc2-lean" not in PRODUCER.read_text())
    return result


def audit_result_encodings(result, identities):
    calc = result["calculation"]
    must("result_Delta7", calc["Delta7"] == "F7-T/2-Q*Z/8-R*V/16")
    must("result_Delta8", calc["Delta8"] == "F8-T*Z/8-Q*V/16-R^2/4")
    must("result_D14_first", calc["D14_first_conclusion"] == "Delta7=A*U")
    must("result_D15_conclusion", calc["D15_conclusion"] == "U=A*Y")
    must("result_combined",
         calc["combined_conclusion"] == "F7=T/2+Q*Z/8+R*V/16+A^2*Y and c14=0")
    must("result_g14_polar",
         Ldecode(calc["g14_polar"]) == identities["expected_g14_polar"])
    must("result_D14_square",
         Ldecode(calc["D14_square_raw_mod_A2"]) == identities["expected_d14"])
    must("result_g14_after_first",
         Ldecode(calc["g14_polar_after_first_divisibility"]) == Lshift(-1, Lvar("c14")))
    must("result_g15_before",
         Ldecode(calc["g15_polar_before_c14_kill"]) == identities["expected_g15_before"])
    must("result_g15_after",
         Ldecode(calc["g15_polar_after_c14_kill"]) == identities["expected_g15"])
    must("result_same_row_piece",
         Ldecode(calc["D15_c14_same_row_piece"]) == identities["same_row"])
    must("result_mixed_piece",
         Ldecode(calc["D15_c14_predecessor_mixed_piece"]) == identities["mixed"])
    must("result_D15_raw",
         Ldecode(calc["D15_raw_mod_A2"])
         == Lscale(Q(3, 4), Lmul(LA(1), Lmul(Lvar("ap"), Lmul(Lvar("u"), Lvar("u"))))))
    must("result_G13_X0",
         Ldecode(calc["positive_lower_window_compatibilities"]["G13_X0"])
         == identities["expected_low"][13])
    must("result_G14_X0",
         Ldecode(calc["positive_lower_window_compatibilities"]["G14_X0"])
         == identities["expected_low"][14])
    must("result_G15_X0",
         Ldecode(calc["positive_lower_window_compatibilities"]["G15_X0"])
         == identities["expected_low"][15])
    must("result_kernel_rank",
         result["literal_G14_polynomial_kernel"]["rank"] == 10)
    must("result_kernel_nullity",
         result["literal_G14_polynomial_kernel"]["nullity"] == 0)
    must("result_kernel_shape",
         result["literal_G14_polynomial_kernel"]["matrix_shape"] == [26, 10])
    must("result_no_scheme_divisibility_claim",
         "field-radical" in " ".join(result["scope_firewall"]))
    mutations = result["literal_raw_mutations"]
    must("result_c14_birth_retracts_D15_image",
         "RETRACTED" in mutations["c14_birth_laurent_mutations"]["D15_correction"])
    must("result_c8_survival_mode",
         mutations["D15_c8_survival"]["mode"] == "c8=1")
    must("result_H0_formula",
         calc["gauge_invariant_low_jet_parameter"] == "H0=c8/2+3*F8(0)/4")


def group_verdicts():
    failed = set(FAILURES)
    def status(names):
        return "FAIL" if any(name in failed for name in names) else "PASS"

    all_names = [name for name, _, _ in CHECKS]
    d14sq = [n for n in all_names if n.startswith((
        "D14_mod_A2", "D14_square", "D14_residual", "D14_gen", "D14_unit",
        "g14_order2", "g14_polar_exact", "g14_no_A", "order2_cannot",
        "T_over_2", "QZ_over_8", "RV_over_16", "coeff_minus_3",
        "L14_polynomial", "polynomial_G14_cannot", "hostile_G14",
        "omit_F7", "A_divides_W2", "gcd_A", "A_prime", "A_does_not_divide_X",
        "half_6", "result_Delta7", "result_D14_first", "result_D14_square",
        "D14_square_not_in_A2", "D14_square_in_A"))]
    d14c14 = [n for n in all_names if n.startswith((
        "c14_birth", "c14_is_the_only", "g14_remaining_polar",
        "g14_polynomial_after_c14", "L14_c14", "Lpair_F0_G14",
        "L14_of_c14", "G14_rank", "G14_nullity", "G14_matrix",
        "G14_cannot_store", "c14_over_A", "result_c14_killed",
        "result_g14_after_first", "pure_G14", "F_neg14",
        "result_kernel"))]
    d15c14 = [n for n in all_names if n.startswith((
        "D15_c14", "D15_mixed", "D15_A0", "D15_full", "D15_does_not",
        "g15_polar_before", "g15_has_order_minus_3", "superseded_same_row",
        "D_raw_omitting_both", "Lop15_equals", "result_retracts",
        "result_D15_c14", "result_same_row", "result_mixed",
        "result_g15_before", "note_does_not", "note_claims_cancellation",
        "c14_next", "c14_not_in_G13"))]
    d15div = [n for n in all_names if n.startswith((
        "g15_polar_after", "g15_order2", "D15_mod_A2", "L15_order1",
        "c8_lives", "coeff_3_4", "g15_remaining_polar", "c8_Y_over_2",
        "c8_D4", "c8_folded", "c8_not_killed", "D15_square", "D15_residual",
        "D15_gen", "R2_over_4", "result_Delta8", "result_D15_conclusion",
        "result_combined", "result_g15_after", "result_D15_raw",
        "result_c8", "exact_square", "A_does_not_divide_U"))]
    cjets = [n for n in all_names if n.startswith((
        "C13", "C14", "C15", "H0_", "g13_holomorphic", "g14_holomorphic",
        "g15_holomorphic", "result_C", "result_G13", "result_G14",
        "result_G15", "result_H0", "F8_gauge", "G13_omits", "G14_omits",
        "G15_omits"))]
    return {
        "D14_square_divisibility": status(d14sq),
        "D14_c14_kill": status(d14c14),
        "D15_c14_cancellation": status(d15c14),
        "D15_divisibility_c8_survival": status(d15div),
        "C13_C14_C15": status(cjets),
    }


def main():
    print("=== independent corrected-r1 D14/D15 hostile checker ===")
    result = hashes()
    identities = characteristic_identities()
    audit_result_encodings(result, identities)
