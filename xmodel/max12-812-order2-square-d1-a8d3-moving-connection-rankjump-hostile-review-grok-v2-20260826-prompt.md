# Hostile review charge: D1 a=8,d=3 moving-connection rank jump (v2)

The first Claude launch failed before review because that provider's API
usage limit was exhausted; it produced no report and no mathematical
verdict.  You are the independent replacement reviewer.

Read the repository evidence and write exactly one report:

`xmodel/max12-812-order2-square-d1-a8d3-moving-connection-rankjump-hostile-review-grok-v2-20260826.md`
...
2. Independently derive the moving-inverse-root correction.  Starting with
   ...
   is exact, including signs, factors, and translation to rows 4,5,7.
   Decide whether it vindicates the canonical literal tails and falsifies
   only the fixed-z bridge.
3. Independently extract family-15 `k6A^2` from the frozen inventory.
   Audit why it has no analogous inverse-root correction at this order
   (`q_A` has no negative part).  Verify rows 5,7 are
   `(-3/16,+3p/64)`.
4. Verify corrected `k6RC` rows 5,7 are
   `(-3/16,-3p/64)`, forced alpha values are `+1/4,-1/4`, determinant is
   `9p/512`, and

   `e7=(32/(3p))*col(k6A^2)-(32/(3p))*col(k6RC)`.

5. Charge scope aggressively.  Confirm that this proves only failure of a
   normalized coefficientwise universal odd functional on `D(p)`.  It is
   not an a=8,d=3 cell verdict, nonlinear solution, point, or
   existence/counterexample result.  State the legitimate successor:
   saturation/chamber splitting by the two source products in the complete
   earlier zero locus.
6. Audit the Singular qring noncanonical-assignment hazard.  State whether
   any conclusion depends on qring `==0`, `subst`, or `diff` without
   explicit reduction.  Do not import unrelated campaign results.

If any formula, hash, identity, or firewall fails, identify the smallest
exact defect and reject or condition accordingly.
