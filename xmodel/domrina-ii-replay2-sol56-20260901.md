...
excludes incidence of \(\widetilde g_1\) with a right arm.  Direct transfer
also supplies the compressed formula
...
are unchanged.  In the essential \(\widetilde g_1\)-side row the actual edge
equation is
...
\(t=\det\widetilde L_b^3\), give \(D_1=t-4\) and \(D_1=t-2\).  This is the
missing companion check.  **Lemma 6.1: REPAIRED | Both.**
...
has the same endpoint index as the degree-three route, so [4], Lemma 5 gives
...
| Lemma 6.10, p. 27 | row 7 is the degree-three version of Lemma 5.5; rows 8 and 10 are the two placements of Lemma 5.8.  The corrected \(g_2\)-version of Lemma 2.6 gives the first sign contradiction; the others give all \(\mu_i=1\) or a unit linear branch contradicting determinant two | REPAIRED | Both |
...
| Lemma 6.11(2a) | (2.6) with \(D_1=\det U,\mu_1=1\), determinant \(-2\det U\), later \(\mu_i=2\), gives \(\det U=4\); then \(2\mid\det(ah)\) violates branch coprimality with \(\det U\) | REPLAYED-SOUND | Both |
| Lemma 6.11(2b) | the mapped (2.11) calculation gives \(4-(\det U)^2=0\), hence \(\det U=2\), and the same coprimality contradiction | REPAIRED | Both |
...
\[
 \det\widetilde\Delta_{a_1}=\lambda d_0d_2H,
 \qquad
 \det\widetilde\Delta_{b_2}=\lambda d_2.              \tag{7.3}
\]
...
All cancelled factors are positive determinants, so
...
The three root-free branches listed in (7.9) are units.  Formula (E) for
...
\(\delta=\det\varnothing=1\), not zero.  **Lemma 7.8: REPAIRED | Both.**
...
**Lemma 7.9: REPLAYED-SOUND | Both.**  Notice that the printed Fig. 14
...
There is no fourth case: \(t=s=1\) is precisely the inessential row, and
both cannot exceed one under the rooted branch rule.  This replaces the p. 32
sentence “Successive consideration of all cases ...”.  **Lemma 7.10:
REPAIRED | Both**, also consuming REPLAY-1's repaired Lemma 2.11.
...
\(d_4\)-audit.  **Case 4a transfer: REPAIRED.**
...
7.8–7.10.  Section 7 introduces no new intrinsic GAP.
...
The report closes all requested second-half checks; no
`OPEN[REPLAY2-CONT]` continuation is needed.
