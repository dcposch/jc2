...
obstruction in the plane Jacobian-conjecture campaign.  Work from the exact
files and rederive the argument; do not inherit the producer verdict.
...
2. Recompute every term weight under
   ...
   inequalities hold.  Audit least-versus-greatest initial convention and
   rational rescaling.
3. Check the logical step `W in J => Lambda^20 in in_w(J)` and the torus
   localization consequence.  State the precise valued-arc/leading-
   coefficient lemma needed to turn this into exclusion of a control-2 arc.
4. Audit whether setting the Rees parameter `s=1` permits reweighting at all
   rational slopes in the fixed-source ideal, or whether saturation,
   homogenization, a denominator, or a discovery-order artifact is missing.
5. Trace the control-2 mapping: why `alpha=15/2`, `5<beta<6`, positivity of
   ...
   another coefficient/layer allowed by the prior correction firewall but
   absent from this fixed support.
...
algebra, or another computational replay locally.  This is a source/text
audit; request an AWS replay if a substantive computation is needed.

Write the review only to
`xmodel/max12-912-order3-d1-control2-slope-uniform-review-claude-20260826.md`.
...
`REPAIR`, or `REJECTED`.  Do not edit the target, evidence, or top-level files.
