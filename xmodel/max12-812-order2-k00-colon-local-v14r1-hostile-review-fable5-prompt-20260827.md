# Hostile review request: K00 V14R1 exact local membership
...
2. Independently parse or reconstruct `r1,...,r7,h,u1,...,u6` over exact
   ...
   the signed identity
   `-h*r7+sum_i ui*ri=0`; check that substitution `d0=...=d5=0` gives
   `h(0)=20`.  State the first nonzero coefficient if replay fails.
...
5. Independently check the algebraic inference: a polynomial identity
   ...
   global nonmembership for local nonmembership or conversely.
...
`xmodel/max12-812-order2-k00-colon-local-v14r1-hostile-review-fable5-20260827.md`.
