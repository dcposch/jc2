...
def _chain_edge_data(cd, log):
    ...
    if cd.final.b != zdeg:
        ...
        # root alpha with gamma := m_alpha/m at the 1/m level.  Exactness of
        # gamma is the FAMILY-BRANCH DATUM, not a theorem: GGV5 Remark `bala`
        # defines A_(gamma) from the chosen root and the enumeration branches
        # over all gamma in Gamma(A,A'); GGV5 Prop 2.5(3)/(4) only bound the
        # multiplicities (REDUCE4-CUT-REVIEW F1 rider).  The remaining
        # z-degree zdeg-gamma sits in other roots, all nonzero and distinct
        # from alpha (st(P) = m*A0' exact => no zero root).  The e_K(alpha)
        # cut is support-exact regardless of how zdeg-gamma splits.
        gamma = cd.final.b
        ...
        log.append(f"chain edge {stE}->{enE}: MULTI-ROOT form x^c y^d "
                   ...
                   f"generated at gamma={gamma} < zdeg={zdeg}: branch datum, "
                   "GGV5 Remark `bala` + Prop 2.5(3)/(4) bounds); alpha and "
                   "all beta_i != 0 since st(P) = m*A0' exactly; support "
                   "after e_K(alpha) is partition-independent")
