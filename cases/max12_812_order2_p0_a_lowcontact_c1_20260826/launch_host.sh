...
package=cases/max12_812_order2_p0_a_lowcontact_c1_20260826
tag="max12_812_order2_p0_a_c1_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"; remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t p0-a-c1.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
tar -czf "$tarball" \
  "$package" \
  cases/max12_812_order2_p0_cusp_raw_g12_cech_certificate_v2_20260826 \
  cases/max12_812_order2_p0_cusp_raw_g12_cech_certificate_20260826 \
...
ssh "${ssh_options[@]}" "$ssh_target" "cd '$remote_root' && tar -xzf source.tar.gz && mkdir -p '$remote_job' && nohup bash '$package/run_aws.sh' '$remote_root' '$remote_job' '$tag' 16777216 '$characteristic' 600 1800 > '$remote_job.outer.stdout' 2> '$remote_job.outer.stderr' & echo \$!"
printf 'host=%s\nremote_job=%s\ntag=%s\n' "$host" "$remote_job" "$tag"
