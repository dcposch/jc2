#!/usr/bin/env python3
"""AWS-only custody supervisor for the D43 `K0` field certificate, R1.
...
``R1_SOURCE_READY_AWS_NOT_AUTHORIZED``.
...
ROUTE = "D43-K0-FIELD-CERT-R1"
AUTH_SCHEMA = "d43-k0-field-certificate-launch-auth-r1"
MARKER_SCHEMA = "d43-k0-field-certificate-job-marker-r1"
TERMINAL_SCHEMA = "d43-k0-field-certificate-aws-terminal-r1"
PREREG_SCHEMA = "d43-k0-field-certificate-prereg-r1"
...
ALLOWED_INSTANCE_TYPES = ("r6i.large", "r6a.large", "r6g.large",
                          "r6id.large", "r6in.large", "r6idn.large")
...
def live_identity(timeout=2.0):
    try:
        ...
        tag = _imds("meta-data/tags/instance/" + AUTHORIZATION_TAG_KEY, token,
                    timeout)
        job_tag = _imds("meta-data/tags/instance/jc2-d43-k0-field-cert-job",
                        token, timeout)
        host_label = _imds("meta-data/tags/instance/jc2-host-label", token,
                           timeout)
    ...
    if document.get("instanceType") not in ALLOWED_INSTANCE_TYPES:
        fault("INSTANCE_TYPE",
              "instance type %r is not in the registered allowlist %s"
...
def load_authorization(path, live_tag):
    ...
    for key in ("schema", "route", "status", "host", "source_archive_sha256",
                "packet_manifest_sha256", "gp_binary_sha256", "gp_binary_path",
                "run_path", "coordinator_go", "reviewer_pass"):
        if key not in record:
            fault("AUTHORIZATION_SHAPE", "field %s absent" % key)
    if record["schema"] != AUTH_SCHEMA:
        fault("AUTHORIZATION_SCHEMA", record["schema"])
    if record["route"] != ROUTE:
        fault("AUTHORIZATION_ROUTE", record["route"])
    if record["status"] != "AUTHORIZED":
        fault("AUTHORIZATION_STATUS", record["status"])
    if record["coordinator_go"] is not True:
        fault("NO_COORDINATOR_GO", "coordinator_go is not true")
    reviewer = record["reviewer_pass"]
    if not isinstance(reviewer, dict) or not SHA_RE.match(
            str(reviewer.get("report_sha256", ""))):
        fault("NO_REVIEWER_PASS",
              "a different-model source PASS report hash is required")
    if reviewer.get("model") in (None, "", "opus5", "Opus 5"):
...
def publish_terminal(run_dir, core, runner_terminal=None, error=None):
    ...
    if error is not None:
        ...
        payload["K0_UNCONDITIONAL_THEOREM"] = "NO_VERDICT"
        payload["E_CONDITIONAL_RATIO_THEOREM"] = "NO_VERDICT"
    else:
        ...
        payload["K0_UNCONDITIONAL_THEOREM"] = \
            runner_terminal["K0_UNCONDITIONAL_THEOREM"]["status"]
        payload["E_CONDITIONAL_RATIO_THEOREM"] = \
            runner_terminal["E_CONDITIONAL_RATIO_THEOREM"]["status"]
...
def run_job(authorization_path):
    ...
    if host.get("host_label") != EXPECTED_HOST_LABEL:
        fault("HOST_BINDING", "authorization host_label %r" % host.get("host_label"))
    if host.get("gp_version") != EXPECTED_GP_VERSION:
        fault("HOST_BINDING", "authorization gp_version %r" % host.get("gp_version"))
    try:
    ...
    try:
        ...
        pins_path = packet / "execution_pins_r1.json"
        ...
        argv = [sys.executable, "-I", "-B", str(packet / "runner_r1.py"),
...
def main(argv=None):
    ...
    if not args.i_understand_this_is_aws_only:
        sys.stderr.write(
            ...
            "R1_SOURCE_READY_AWS_NOT_AUTHORIZED; a different-model source "
