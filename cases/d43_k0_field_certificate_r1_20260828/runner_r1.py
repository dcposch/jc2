#!/usr/bin/env python3
"""Execution runner for the D43 `K0` field certificate, R1.
...
publishes exactly one atomic terminal that separates

    K0_UNCONDITIONAL   the coefficient-field theorem (layers L0-L4)
    E_CONDITIONAL      the ratio theorem, conditional on the displayed E (L5)

Nothing here decides whether it is allowed to run: ``aws_supervisor_r1.py``
...
SCHEMA = "d43-k0-field-certificate-runner-r1"
ROUTE = "D43-K0-FIELD-CERT-R1"
GP_CENSUS_EXPECTED = 79
GP_OBSERVABLE_COUNT = 21
...
def emit_immutable(path, blob):
    """Write once, fsync, then drop to read-only for every principal."""
    path = Path(path)
    if path.exists():
...
TERMINAL_LINE = re.compile(
    r"^K0_UNCONDITIONAL (?P<k0>[01])\s+E_CONDITIONAL (?P<e>[01])$")
    ...
    proc = subprocess.run(
        ...
        cwd=str(workdir), env={k: v for k, v in os.environ.items()
                               if k in ("PATH", "HOME", "LANG", "LC_ALL", "TZ",
                                        "GP_DATA_DIR") or k in THREAD_VARIABLES})
...
def main(argv=None):
    ...
    ap.add_argument("--execution-root", required=True)
    ...
    for name in ("terminal.json", "result.json", "mutations.json"):
        if (run_dir / name).exists():
            fault("RUN_DIR_REUSE", "%s already exists" % name)
    ...
    gp_out = run_gp(args.gp, packet / "k0_field_cert_r1.gp",
                    pins["gp_script_sha256"], pins.get("gp_binary_sha256"),
                    run_dir)

    banks = CK.read_pointbanks(
        [(int(p), str(Path(args.source_root) / rel), sha)
         for p, rel, sha in pins["pointbanks"]])
    result = CK.run(CK.Spec(label="AWS_R1"), args.source_root, pins["sources"],
    ...
    with tarfile.open(str(archive), "r") as tar:
        ...
        for m in tar.getmembers():
            ...
            if sha256_bytes(blob) != (artifacts.get(m.name)
                                      or artifacts_manifest_sha):
    ...
    k0_ok = bool(result["unconditional_k0_theorem"]
                 and gp_out["terminal"]["k0_unconditional"] == 1
                 and battery["battery_pass"])
    e_ok = bool(result["e_conditional_ratio_theorem"]
                and gp_out["terminal"]["e_conditional"] == 1
                and battery["battery_pass"])
    ...
    terminal = {
        ...
        "engines": ["PARI/GP 2.15.4 (k0_field_cert_r1.gp)",
                    "stdlib Python (k0_field_checker_r1.py)"],
        "engine_agreement": "exact on all %d shared integer observables"
                            % GP_OBSERVABLE_COUNT,
        "mutation_battery": {
        ...
        "K0_UNCONDITIONAL_THEOREM": {
            "status": "PASS" if k0_ok else "FAIL",
        ...
        "E_CONDITIONAL_RATIO_THEOREM": {
            ...
            "depends_on_E": True,
            "conditional_input": "the displayed E from the reviewed bridge; "
                                 "not re-derived in this packet"},
    ...
    sys.stdout.write("TERMINAL %s K0_UNCONDITIONAL %s E_CONDITIONAL %s\n"
                     % (terminal_sha, terminal["K0_UNCONDITIONAL_THEOREM"]
                        ["status"],
                        terminal["E_CONDITIONAL_RATIO_THEOREM"]["status"]))
    return 0 if (k0_ok and e_ok) else 1
