...
set +e
timeout "$compile_timeout" python3 "$package/compile_t_rs0_discovery.py" \
...
input="$aws_job/compiled/t_rs0_discovery_${label}.sing"
...
set +e
python3 "$package/validate_t_rs0_discovery.py" \
  --stdout "$stdout" \
  --compiler-result "$aws_job/compiled/result.json" \
  --input "$input" \
  --output "$aws_job/DISCOVERY.json" \
...
if [[ "$validator_rc" -ne 0 ]]; then
  printf 'validator=FAIL_DISCOVERY_PARSER\n' >> "$aws_job/validator.validation"
  exit "$validator_rc"
fi
printf 'validator=PASS_T_RS0_DISCOVERY_NAVIGATION_ONLY\n' >> "$aws_job/validator.validation"
sha256sum "$aws_job/DISCOVERY.json" "$stdout" "$aws_job/run/$tag.stderr" \
  "$aws_job/run/$tag.meta" > "$aws_job/EVIDENCE.sha256"
