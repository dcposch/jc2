...
def omega_histogram(source_data, h):
    ...
    rank, particular, kernel = solve_affine(matrix_rows, constant)
    if particular is None:
        return rank, [0, 0, 0], None, None
    ...
    return rank, histogram, projected_particular, first_zero_q
...
for base_index, svalues in enumerate(
    ...
    for fvalues in itertools.product(range(3), repeat=len(frob)):
        for xvalues in affine_solutions(pivots, work, fvalues):
            ...
            rank, hist, projected_particular, first_zero_q = omega_histogram(
...
result = {
    ...
    "q9_rank_histogram": {str(key): value
                          for key, value in sorted(q9_rank_histogram.items())},
    "per_base": per_base,
