# Preregistration: reducer-safe quotient rank certificates r5

Date: 2026-08-28

R4 stopped in the tiny backend fixture before any matrix algebra because
Singular reserves the exact identifier `NF`.  R5 makes the sole adapter
change `matrix NF` to `matrix REDUCED`; the mathematical compiler
`build_nf_rank.py`, its input hashes, its reduction algorithm, and its rank
contract are byte-identical to r4.  The custodian transcript is redirected
outside the evidence tree so it cannot mutate an internal evidence member
after final hashing.
...
R5 recomputes these six strata from the original frozen 106 by 105 matrix in
