...
mkdir -p "$lane"
python3 "$case_dir/generate_clean.py" --w-value "$w_value" \
...
meta=$lane/run.meta
{
  ...
  echo "route=corrected original-ideal fixed-fibre stdfglm lex shape"
  echo "virtual_memory_limit_kib=134217728"
  echo "timeout_seconds=7200"
  Singular --version | head -1 | sed 's/^/singular=/'
  sha256sum "$0" "$case_dir/generate_clean.py" "$case_dir/generate.py" \
