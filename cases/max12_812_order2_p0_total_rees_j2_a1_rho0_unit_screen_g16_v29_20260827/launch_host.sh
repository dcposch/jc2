...
host=$1; stamp=$2; suffix=$3; characteristic=$4; cap=$5; compile_wall=$6; engine_wall=$7
...
tag="max12_812_order2_p0_total_rees_j2_a1_rho0_unit_g16_v29_${stamp}_${suffix}"
root="/home/ubuntu/jobs/$tag/source"; job="$root/$package/aws_${suffix}"
tarball=$(mktemp -t j2-a1-rho0-unit-g16-v29.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
...
ssh "${opt[@]}" "$target" "cd '$root' && test \"\$(sha256sum source.tar.gz|awk '{print \$1}')\" = '$sha' && tar -xzf source.tar.gz && mkdir -p '$job' && setsid -f bash '$package/run_aws.sh' '$root' '$job' '$tag' '$characteristic' '$cap' '$compile_wall' '$engine_wall' '$sha' </dev/null > '$job.outer.stdout' 2> '$job.outer.stderr'"
