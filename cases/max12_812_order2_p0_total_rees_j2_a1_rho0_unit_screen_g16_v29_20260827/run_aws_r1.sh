...
if [[ "$characteristic" -ne 0 ]]; then exit 126; fi
mkdir -p "$job/run"
printf 'tag=%s\nhost=%s\nstart_utc=%s\ncharacteristic=%s\nsource_archive_sha256=%s\nrepair=R1_BUNDLED_V28_PATHS\n' "$tag" "$(hostname)" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$characteristic" "$archive_sha" > "$job/launch_registration.txt"
cd "$root"
sha256sum -c "$package/FREEZE_R1.sha256" > "$job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap"
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_compiler" timeout "$compile_wall" python3 "$package/compile_unit_screen_g16_v29r1.py" "$job/compiled" --characteristic "$characteristic"
grep -Fx 'V29_R1_BUNDLED_REMAPS=7' "$job/run/${tag}_compiler.stdout" >/dev/null
