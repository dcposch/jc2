...
if [[ "$(uname -s)" != Linux ]]; then
  echo "REFUSED: V84S launcher requires Linux" >&2
  exit 97
fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: V84S launcher requires Amazon EC2" >&2
  exit 96
fi
...
tag="td6_v84r2_k2_spencer_reverse_${label}_${stamp}"
printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=8388608\n' \
...
(
  ...
  /usr/bin/time -v timeout 21600 bash run_v84s.sh \
