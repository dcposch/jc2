# TRIPLE02 node-1 closed-successor resume: AWS preregistration R4
...
packet licenses is a hostile source review by a different model; R4 itself
...
items and the R3 review's O2-B1 regression) and a separate coordinator
`GO` are both mandatory before any pilot.
...
`JOB_TAG = ggv_triple02_closed_successor_resume_r4_<STAMP>_<NONCE>` (exact
...
## Decision record and late authority (C1; R3 O-B1 binding; R4 O2-B1 single-open)
...
terminal authority then authenticates the **frozen archive bytes through a
single open descriptor** (R3 O-B1, R4 O2-B1): it opens the installed
archive path **exactly once** (`O_NOFOLLOW`/`O_NONBLOCK`, regular-file
`fstat` on the descriptor), streams the outer SHA-256 from that descriptor
against the charged digest, rewinds, and hands the **same open object** to
`tarfile`; no archive-path reopen exists anywhere on the decision path, so
a concurrent same-uid `rename()` of the path cannot redirect any decision
read, and an in-place mutation/truncation of the inode during the read is
refused by a pre/post `fstat` identity/size/mtime/ctime stability gate.
From those held bytes it re-runs the complete member-safety,
embedded-manifest, and directory censuses, requires the fresh extraction
to be byte-identical member-for-member (a same-uid edit, deletion,
replacement, symlink, or archive/extraction mix-and-match after the freeze
fails closed), and reads every decision input from the archive bytes held
in memory: it re-derives the candidate from the archived decision record
plus the archived production and classifier verdicts and replays every
recorded equality (lease — also against the live lease file, identities,
summary and verdict hashes, artifact manifest, Singular digest,
five-property runtime limits, containment mode, integer-typed fault
latches — refusing any regression against the live flags and any worker-rc
disagreement).  The live candidate file is never read at decision time;
the supervisor's decision command has no live candidate argument at all.
The public `TERMINAL.marker` is published with a no-replace hard link and
contains three lines: the classification, the terminal-archive SHA-256
(the digest **actually computed from the descriptor**, not the CLI
string), and the decision-record SHA-256 (or `NONE` where no
archive/decision exists); a mathematical first line can appear only if the
decision re-authenticated exactly that archive digest.  The post-freeze
`FINALIZATION_LATCHES.txt` binds the job tag and archive digest and
carries a `FINALIZATION_LATCHES.sha256` sidecar, but — stated honestly
(R3 review O2-N3) — this is a **post-freeze consistency sidecar**, not an
independent authentication: both files are same-uid-writable and outside
the archive, and the archive digest inside is a public value.  The
load-bearing latch state is the in-archive `FAULT_LATCHES_PRE_ARCHIVE.json`.
All custody failures remain `CUSTODY_OR_ADAPTER_FAILURE_NO_VERDICT` with
sticky latches.
...
trees and injected records.  The R4 concurrent-rename and in-place-mutation
regressions use real OS threads and real `os.rename`/append against real
frozen archives, but still exercise only `derive_late_decision` and the
contract functions — no fixture executes the launcher, supervisor, stage
runner, systemd, or live `/proc` custody paths, and none claims to.  That
live behavior is AWS rehearsal debt, discharged only by the rehearsal
below (step 12 replays the O2-B1 race on a live extraction).
...
and, added by the R2 and R3 hostile reviews and mandatory here:
...
11. **runner-log discipline**: capture real `*.runner.stdout.txt` /
    ...
    live shape);
12. **O2-B1 concurrent-rename regression** (added by the R3 hostile
    review): while `containment_contract.py terminal` is running on the
    live `.terminal_archive_replay.*` extraction, start a same-uid helper
    that `rename()`s a self-consistent forged terminal archive over
    `$terminal_archive`, and separately truncate/append to the archive
    inode in place; confirm the terminal refuses (`DECISION_ARCHIVE_*` or
    `DECISION_FRESH_ROOT_*`) and publishes only a custody no-verdict, and
    that the published `TERMINAL_ARCHIVE_SHA256` (when a verdict is
    reached) is the digest of the descriptor actually read.
...
R3 (O-N5), retained in R4: `LAUNCH_MANIFEST.sha256` does not list the
producer report, so the R4 report tables the launch manifest's own hash
and the coordinator charges BOTH values independently: the launch-manifest
hash (from the report) and the report hash (from `REPORT_BINDING.sha256`,
written after the report).  No nearby branch, open-route, or whole-stratum
job may be launched from this packet.
