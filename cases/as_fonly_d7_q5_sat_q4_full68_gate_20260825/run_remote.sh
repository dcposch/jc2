...
source_dir="$JC2_ROOT/cases/as_fonly_d7_q5_sat_q4_full68_gate_20260825"
program="$source_dir/solve_full68.py"
...
/usr/bin/time -v timeout --signal=TERM --kill-after=30s "$TIMEOUT_SECONDS" \
  env JC2_ROOT="$JC2_ROOT" MODEL_OUTPUT="$MODEL_OUTPUT" \
      PARENT_OUTPUT_JSON="$JOB_DIR/q4_only_parent.json" \
      OUTPUT_JSON="$JOB_DIR/full68.json" \
