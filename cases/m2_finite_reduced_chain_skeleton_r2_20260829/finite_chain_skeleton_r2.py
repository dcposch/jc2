...
SCHEMA = "m2-finite-reduced-chain-skeleton-r2"
...
def close_reduced_skeleton(w0: Fraction, M0: int, budget: int) -> dict[str, object]:
    ...
    best_seen: dict[tuple[Fraction, int], int] = {(w0, M0): 0}
    ...
    while queue:
        ...
        key = (w, M)
        if key in distance or spent != best_seen.get(key):
            continue
        distance[key] = spent
        ...
        for step in menu:
            ...
            if child_spent < best_seen.get(child, child_spent + 1):
                best_seen[child] = child_spent
                predecessor[child] = {
