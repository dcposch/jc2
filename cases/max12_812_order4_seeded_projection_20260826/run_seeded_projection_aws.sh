...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "AWS-only seeded projection client refused non-Linux host" >&2
  exit 125
fi
vendor=$(tr -d '\n' < /sys/class/dmi/id/sys_vendor 2>/dev/null || true)
if [[ "$vendor" != "Amazon EC2" ]]; then
  echo "AWS-only seeded projection client refused vendor=${vendor:-unknown}" >&2
  exit 125
fi
if [[ $# -ne 5 ]]; then
  echo "usage: run_seeded_projection_aws.sh AWS_ROOT AWS_RUN TAG INPUT CAP_KIB" >&2
  exit 125
fi
