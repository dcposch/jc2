...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "AWS-only seeded launcher refused non-Linux host" >&2
  exit 125
fi
vendor=$(tr -d '\n' < /sys/class/dmi/id/sys_vendor 2>/dev/null || true)
if [[ "$vendor" != "Amazon EC2" ]]; then
  echo "AWS-only seeded launcher refused vendor=${vendor:-unknown}" >&2
  exit 125
fi
if [[ $# -ne 5 ]]; then
  echo "usage: launch_remote.sh AWS_ROOT AWS_JOB TAG MODE CAP_KIB" >&2
  exit 125
fi
...
cd "$aws_root"
sha256sum -c cases/max12_812_order4_seeded_projection_20260826/FREEZE.sha256 \
  > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$lane_tag"
python3 cases/max12_812_order4_seeded_projection_20260826/compile_seeded_projection_v1.py \
  ...
  "$aws_job/compiled/seeded_projection_v1.sing" --mode "$mode" \
  > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
sha256sum "$aws_job/compiled/seeded_projection_v1.sing" \
  > "$aws_job/compiled_input.sha256"
exec cases/max12_812_order4_seeded_projection_20260826/run_seeded_projection_aws.sh \
  "$aws_root" "$aws_job/run" "$lane_tag" \
  "$aws_job/compiled/seeded_projection_v1.sing" "$cap_kib"
