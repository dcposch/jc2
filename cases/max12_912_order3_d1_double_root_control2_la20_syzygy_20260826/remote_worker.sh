...
case "$tag" in
  max12_912_order3_d1_double_root_control2_la20_syzygy_20260826T*) ;;
...
(
  ulimit -v 2097152
  nice -n 10 timeout 300 python3 -u compile_la20_syzygy.py
) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
sha256sum la20_syzygy_lpdp.sing > compiled.sha256 2>/dev/null || true
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_CONTROL2_LA20_SYZYGY_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then
...
(
  ...
  nice -n 10 timeout --signal=TERM --kill-after=300 "$solve_timeout" \
    /usr/bin/time -v -o singular.time /usr/bin/Singular -q la20_syzygy_lpdp.sing
...
sha256sum compiler.stdout compiler.stderr compiler.rc la20_syzygy_lpdp.sing \
