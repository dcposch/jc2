...
if [[ "$(uname -s)" != Linux ]]; then
  echo "REFUSED: V82QSS fleet requires Linux" >&2
  exit 97
fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: V82QSS fleet requires Amazon EC2" >&2
  exit 96
fi
actual=$(sha256sum "$archive" | cut -d' ' -f1)
[[ "$actual" == "$expected" ]] || { echo "REFUSED: archive SHA mismatch" >&2; exit 95; }
available_kib=$(awk '/MemAvailable:/ {print $2}' /proc/meminfo)
(( available_kib >= 16777216 )) || { echo "REFUSED: less than 16 GiB available" >&2; exit 94; }
...
(
  set +e
  for exponent in 11 16; do
    lane="$root/q${exponent}"
    mkdir -p "$lane/output"
    tag="td6_v82qss3_symbol_${label}_q${exponent}_${stamp}"
    printf 'hostname=%s\nlabel=%s\ntag=%s\nq_exponent=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=4194304\ntimeout_seconds=10800\n' \
      "$(hostname)" "$label" "$tag" "$exponent" "$(date -u +%FT%TZ)" "$actual" \
      > "$lane/launch.meta"
    (
      set +e
      ulimit -v 4194304
      ...
      /usr/bin/time -v timeout 10800 bash run_v82qss_symbol.sh \
