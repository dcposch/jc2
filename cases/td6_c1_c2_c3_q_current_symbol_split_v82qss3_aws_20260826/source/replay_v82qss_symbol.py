...
def map_denominator(coordinates):
    if coordinates:
        return p.denominator_for(coordinates.values())
    # `tri.CTX` is the FLINT context object, not a polynomial constructor.
    # Empty obstruction maps have the unit common denominator in this ring.
    denominator = p.tri.ONE
    assert denominator == p.tri.CTX.constant(1)
...
def main():
    print("producer=TD6-V82QSS3-Q-CURRENT-SYMBOL-SPLIT")
    ...
    assert map_denominator({}) == p.tri.ONE
    print("empty_map_unit_denominator_positive_control=true", flush=True)
    ...
    # `p.U` is an fmpq_mpoly center coordinate.  E3 accepts the source
    # rational-function wrapper Rat3, not a bare FLINT polynomial.
    U6 = p.E3(p.Rat3(p.U**6))
    print("u6_typed_constructor_boundary=Rat3(fmpq_mpoly)->E3")
