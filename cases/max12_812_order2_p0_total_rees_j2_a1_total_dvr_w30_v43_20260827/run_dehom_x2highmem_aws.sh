...
if [[ "$order" != dp || "$method" != tracked ]]; then
  exit 126
fi
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_total_dvr_w30_v43_dehom_dp_x2highmem_* ]]; then
  exit 127
fi
# This additive runner changes only the registered resource ceiling relative
# to run_dehom_highmem_aws.sh.  Compiler, Singular input, replay and
# homogenizer remain byte-identical.
if [[ "$memory_cap_kib" -ne 943718400 || "$total_wall_seconds" -ne 21600 ]]; then
  exit 128
fi
...
printf 'tag=%s\nhost=%s\nstart_utc=%s\norder=%s\nmethod=%s\nmemory_cap_kib=%s\ntotal_wall_seconds=%s\nsource_manifest_sha256=%s\nsource_label=%s\n' \
  ...
  "$method" "$memory_cap_kib" "$total_wall_seconds" "$source_manifest_sha256" \
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "$compiler_lane" timeout "$(remaining)" \
  python3 "$package/compile_cascade_dehom_v43.py" "$job/compiled" \
  --order "$order" --method "$method"
grep -Fx 'PASS-A1-TOTAL-DVR-W30-V43-DEHOM-COMPILER' \
...
grep -Fx 'V43_DEHOM_LIFT_REPLAY=1' "$job/run/${singular_lane}.stdout" >/dev/null
...
printf 'end_utc=%s\nelapsed_seconds=%s\noutcome=exact-rho0-homogenized-a1-power\n' \
