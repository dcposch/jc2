...
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_total_dvr_w30_v43_rho0dual_modp_* ]]; then
  exit 126
fi
if [[ "$prime" -ne 65519 || "$memory_cap_kib" -ne 201326592 \
      || "$total_wall_seconds" -ne 21600 ]]; then
  exit 127
fi
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "$build_lane" timeout "$(remaining)" \
  g++ -O3 -DNDEBUG -fopenmp "$package/rho0_dual_linbox_modp_v43.cpp" \
  -o "$job/rho0_dual_linbox_modp_v43" "${linbox_flags[@]}"
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "$solver_lane" timeout "$(remaining)" \
  "$job/rho0_dual_linbox_modp_v43" "$matrix" "$rhs" "$prime" \
  "$job/rho0_dual_modp.solution"
validator_lane="${tag}_validator"
bash ops/aws_exact_lane.sh "$root" "$job/run" "$validator_lane" timeout "$(remaining)" \
  python3 "$package/validate_rho0_dual_linbox_modp_v43.py" \
  --compiler-result "$compiler_result" --solution "$job/rho0_dual_modp.solution" \
  --output "$job/validated"
grep -Fx 'PASS-A1-TOTAL-DVR-W30-V43-RHO0-DUAL-LINBOX-MODP-REPLAY' \
...
printf 'end_utc=%s\nelapsed_seconds=%s\noutcome=finite-field-nonmembership-screen-only\n' \
