...
case "$mode:$phase:$prime" in
  exact:eliminate:65521|exact:lift:65521|modp:eliminate:65519) ;;
  *) exit 126 ;;
esac
...
printf 'tag=%s\nhost=%s\nstart_utc=%s\nmode=%s\nphase=%s\nprime=%s\nmemory_cap_kib=%s\ntotal_wall_seconds=%s\nsource_archive_sha256=%s\nsource_label=%s\n' \
  ...
  "$prime" "$memory_cap_kib" "$total_wall_seconds" "$source_archive_sha256" \
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "$compiler_lane" timeout "$(remaining)" \
  python3 "$package/compile_total_dehom_eliminant_v43.py" "$job/compiled" \
...
stderr="$job/run/${singular_lane}.stderr"
