...
int main(int argc, char **argv) {
    if (argc != 5) {
        std::cerr << "usage: rho0_dual_linbox_modp_v43 MATRIX RHS PRIME OUTPUT\n";
        return 125;
    }
    ...
    output << "V43_RHO0_DUAL_LINBOX_MODP\n" << prime << '\n'
           << solution.size() << '\n';
    for (const auto &entry : solution) field.write(output, entry) << '\n';
    output << "PASS_A1_TOTAL_DVR_W30_V43_RHO0_DUAL_LINBOX_MODP\n";
    return output ? 0 : 132;
}
