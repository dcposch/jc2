...
package=cases/max12_812_order2_p0_total_rees_t_rs0_rowshard_v8_20260826
...
tag="max12_812_order2_p0_total_rees_t_rs0_rowshard_v8_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"
remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t p0-total-rees-t-rs0-rowshard-v8.XXXXXX.tar.gz)
...
ssh "${ssh_options[@]}" "$ssh_target" \
  "cd '$remote_root' && test \"\$(sha256sum source.tar.gz | awk '{print \$1}')\" = '$archive_sha' && tar -xzf source.tar.gz && mkdir -p '$remote_job' && setsid -f bash '$package/run_aws.sh' '$remote_root' '$remote_job' '$tag' '$characteristic' 100663296 900 7200 '$archive_sha' </dev/null > '$remote_job.outer.stdout' 2> '$remote_job.outer.stderr'"
