#!/usr/bin/env python3
"""Validate V8 row shards and emit the ordinary-ring Delta certificate."""
...
def load_v6_prepare():
    actual = digest(V6_PREPARE)
    if actual != V6_PREPARE_SHA256:
        fail(("V6 prepare hash mismatch", actual, V6_PREPARE_SHA256))
    spec = importlib.util.spec_from_file_location("t_rs0_v6_prepare_frozen", V6_PREPARE)
    if spec is None or spec.loader is None:
        fail("cannot import frozen V6 prepare validator")
...
def main() -> None:
    ...
    if not isinstance(tag, str) or not tag.startswith("max12_812_order2_p0_total_rees_t_rs0_rowshard_v8_"):
        fail("bad registered V8 row-shard tag")
    ...
    for row in range(1, 8):
        ...
        if "qring " in program or "quit(" in program:
            fail(("forbidden qring or numeric quit in V8 input", row))
        ...
        expected_values = {
            ...
            "T_RS0_IMPLEMENTATION": IMPLEMENTATION,
    ...
    if not discovered or not nonzero_grades:
        fail(("empty discovered manifest or all-zero grades", discovered, nonzero_grades))
    ...
    for key, path in expected_keep_paths.items():
        ...
        keep_text[key] = helper.polynomial_text(path)
    ...
    cert = [
        ...
        'print("T_RS0_CERTIFICATE_PREFIX_ALGEBRA=PARENT_RING_NO_QRING");',
    ]
    for key in ("KeepT10_2", "KeepF10_2", "KeepT10_3", "KeepF10_3", "KeepT12_6", "KeepF12_6"):
    ...
    cert += [
        ...
        'if (frozenCert*frozenOmit*deltaSpecial*deltaEven*deltaRho2*deltaMultiplyBack!=1) { print("T_RS0_FAIL=CERTIFICATE_RECOMBINATION"); quit; }',
        'print("T_RS0_CERTIFICATE_ENDPOINT=PASS_NAVIGATION_ONLY");',
        "quit;",
    ]
    args.certificate_input.write_text("\n".join(cert) + "\n")
    result = {
        "status": "PASS-T-RS0-ROW-SHARDS-V8-PREPARED-CERTIFICATE",
        ...
        "v6_prepare_helper_sha256": V6_PREPARE_SHA256,
    }
