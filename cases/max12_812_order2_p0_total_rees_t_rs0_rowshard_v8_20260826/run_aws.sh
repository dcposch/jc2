...
v7_package=cases/max12_812_order2_p0_total_rees_t_rs0_rowshard_v7_20260826
v6_package=cases/max12_812_order2_p0_total_rees_t_rs0_rowshard_v6_20260826
...
mkdir -p "$aws_job/run"
{
  ...
  printf 'parallel_row_processes=7\nprefix_algebra=parent_ring_explicit_normal_form\nqring_used=0\nsingular_version=%s\n' \
    "$(Singular --version 2>&1 | head -1)"
} > "$aws_job/launch_registration.txt"
printf '%s\n' "$$" > "$aws_job/launcher.pid"
cd "$aws_root"
sha256sum -c "$package/FREEZE.sha256" > "$aws_job/freeze_check.stdout"
sha256sum -c "$v7_package/FREEZE.sha256" > "$aws_job/v7_freeze_check.stdout"
sha256sum -c "$v6_package/FREEZE.sha256" > "$aws_job/v6_freeze_check.stdout"
sha256sum -c "$stream_package/FREEZE.sha256" > "$aws_job/stream_freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"

set +e
timeout "$compile_timeout" python3 "$package/compile_t_rs0_rowshard_v8.py" \
...
set +e
python3 "$package/prepare_certificate_v8.py" \
...
set +e
python3 "$package/finalize_discovery_v8.py" \
...
printf 'validator=PASS_T_RS0_ROW_SHARD_INCREMENTAL_NF_V8_NAVIGATION_ONLY\n' >> "$aws_job/validator.validation"
