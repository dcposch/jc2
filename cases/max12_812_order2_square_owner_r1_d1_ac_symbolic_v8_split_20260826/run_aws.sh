...
cd "$aws_root"; sha256sum -c cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v8_split_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v8_split_20260826/compile_v8_split.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
r1_input="$aws_job/compiled/square_r1_v8_${label}.sing"; d1_input="$aws_job/compiled/square_d1_ac_v8_${label}.sing"
...
for required in \
 'R1_SOURCE_HASHES=PASS' 'R1_SOURCE_DIVISIBLE=1' 'R1_SOURCE_QUOTIENT_IDENTITIES=1' 'R1_SOURCE_FORBIDDEN=1' \
 'R1_ANALYTIC_DIVISIBLE=1' 'R1_ROW_IDENTITIES_13=1' 'R1_DENOMINATOR_RECURRENCE=1' 'R1_NUMERATOR_IDENTITY=1' \
 'R1_LOCALIZED_REDUCED_SUPPORT_ZERO=1' 'R1_ENDPOINT=PASS_R1_LEADING_SECTION_ZERO_ON_DPK'; do
...
for required in \
 'D1AC_SOURCE_HASHES=PASS' 'D1AC_SOURCE_DIVISIBLE=1' 'D1AC_SOURCE_QUOTIENT_IDENTITIES=1' 'D1AC_SOURCE_FORBIDDEN=1' \
 'D1AC_ANALYTIC_DIVISIBLE=1' 'D1AC_ROW_IDENTITIES_15=1' 'D1AC_ROW_IDENTITIES_16=1' \
 'D1AC_SYMBOLIC_SHIFT_G15=1' 'D1AC_SYMBOLIC_SHIFT_G16_THREE_MODULES=1' \
 'D1AC_DENOMINATOR_RECURRENCE_G15=1' 'D1AC_DENOMINATOR_RECURRENCE_G16=1' \
 'D1AC_BOTH_ROOT_ORIENTATIONS=1' 'D1AC_UNMATCHED_DOUBLE_POLE=1' \
 'D1AC_ENDPOINT=PASS_SYMBOLIC_D1_UNIQUE_AC_DOUBLE_POLE_GATE' 'R1_D1_AC_PACKAGE_ENDPOINT=PASS'; do
 if [[ "$(grep -Fxc "$required" "$d1_stdout" || true)" -ne 1 ]]; then printf 'validator=FAIL_D1_MISSING_OR_NONUNIQUE:%s\n' "$required" >> "$validation"; exit 90; fi
done
if grep -Fq '=FAIL' "$r1_stdout" || grep -Eq '^\? ' "$r1_stdout" || grep -Fq '=FAIL' "$d1_stdout" || grep -Eq '^\? ' "$d1_stdout"; then
 printf 'validator=FAIL_SENTINEL_OR_DIAGNOSTIC\n' >> "$validation"; exit 90
fi
printf 'validator=PASS_R1_D1_AC_SYMBOLIC_V8_SPLIT\n' >> "$validation"
