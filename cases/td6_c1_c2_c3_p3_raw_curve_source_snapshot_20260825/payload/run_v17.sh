...
if [ "$#" -ne 5 ]; then
  echo "usage: run_v17.sh ARCHIVE RUN_DIR PYTHON LANE EXPECTED_ARCHIVE_SHA256" >&2
  exit 64
fi
...
case "$lane" in
  ascending) lane_args=() ;;
  reverse-first) lane_args=(--reverse-first) ;;
...
payload="$run_dir/td6-aws-handoff-20260824-v17"
