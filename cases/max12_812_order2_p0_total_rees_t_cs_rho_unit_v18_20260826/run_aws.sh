...
set +e
timeout "$compiler_wall" python3 "$package/compile_t_cs_rho_unit_v18.py" "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
set +e
python3 "$package/validate_t_cs_rho_unit_v18.py" --compiler-result "$aws_job/compiled/result.json" --stdout "$stdout" --stderr "$stderr" --characteristic "$characteristic" --output "$aws_job/RESULT.json" > "$aws_job/validator.stdout" 2> "$aws_job/validator.stderr"
validator_rc=$?
set -e
printf 'validator_rc=%s\n' "$validator_rc" > "$aws_job/validator.validation"; if [[ "$validator_rc" -ne 0 ]]; then exit "$validator_rc"; fi
printf 'validator=PASS_T_CS_RHO_UNIT_V18\n' >> "$aws_job/validator.validation"
