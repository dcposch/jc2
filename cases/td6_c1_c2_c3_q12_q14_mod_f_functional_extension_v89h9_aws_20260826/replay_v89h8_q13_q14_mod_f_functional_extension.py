#!/usr/bin/env python3
"""Exact q13 extension gate for the V89H7 q14 quotient functional."""
...
spec = importlib.util.spec_from_file_location("td6_v89h8_h6_parent", H6_PATH)
...
ALLOWED_Q = (13, 14) + tuple(range(16, 25))
ZERO_LOW_Q = tuple(range(2, 13))
...
def main():
    ...
    assert os.environ.get("AWS_RUN_TAG", "").startswith(
        "td6_v89h8_q13_q14_mod_f_functional_extension_"
    )
    print("producer=TD6-V89H8-Q13-Q14-MOD-F-FUNCTIONAL-EXTENSION")
    ...
    scc_sha = write_scc(outdir / "Q13_EXTENSION_SCC_DETERMINANTS.tsv", records)
    print(f"q13_extension_FIRST_graph_edges={edges}")
    print(f"q13_extension_FIRST_components={components!r}")
    print(f"q13_extension_FIRST_cyclic_components={cyclic!r}")
    print(f"q13_extension_SCC_determinants_sha256={scc_sha}")
    print(f"q13_extension_all_SCC_determinants_one={str(unit).lower()}", flush=True)

    if not unit:
        result_sha = write_result(
            outdir / "Q13_FUNCTIONAL_EXTENSION_RESULT.txt",
            [
                f"h7_result_sha256={H7_RESULT_SHA256}",
                f"scc_determinants_sha256={scc_sha}",
                "q13_polynomial_functional_extension=false",
        ...
        print(f"q13_extension_result_sha256={result_sha}")
        print("q13_extension_fail_closed_before_P12=true")
        print("TD6-V89H8-Q13-Q14-MOD-F-FUNCTIONAL-EXTENSION NONUNIT-PIVOT-PASS")
    ...
    print("q13_extension_full_q_pivot_inverse_two_sided=true")
    print("q13_extension_normalized_original_FIRST_replay=true", flush=True)
    ...
    max_q13_degree = max((monomial.count(13) for monomial in support), default=0)
    mixed_q13_q14 = [
        monomial for monomial in support if 13 in monomial and 14 in monomial
    ]
    remainder_sha = h5.write_polynomial(
        outdir / "Q13_EXTENSION_CANONICAL_REMAINDER.tsv", remainder
    )
    support_lines = ["q_monomial"] + [repr(monomial) for monomial in support]
    support_text = "\n".join(support_lines) + "\n"
    (outdir / "Q13_EXTENSION_Q_SUPPORT.tsv").write_text(support_text)
    support_sha = sha256(support_text.encode()).hexdigest()
    print(f"q13_extension_canonical_remainder_sha256={remainder_sha}")
    print(f"q13_extension_q_support_sha256={support_sha}")
    print(f"q13_extension_q_support_count={len(support)}")
    print(f"q13_extension_max_total_q_degree={max_q_degree}")
    print(f"q13_extension_max_q13_degree={max_q13_degree}")
    print(f"q13_q14_mixed_support_count={len(mixed_q13_q14)}")
    print("q13_extended_functional_equals_H7_exact=true")
    print("q13_extended_functional_nonzero=true", flush=True)
    ...
    result_sha = write_result(
        ...
        [
            ...
            f"max_q13_degree={max_q13_degree}",
            f"mixed_q13_q14_support_count={len(mixed_q13_q14)}",
            "q13_polynomial_functional_extension=true",
    ...
    print("TD6-V89H8-Q13-Q14-MOD-F-FUNCTIONAL-EXTENSION PASS")
    v85.restore_base_qd_state()
