...
cleanup() {
  ...
  if [[ -n "$worker_pid" && -f "$job_dir/records/GROUP.tsv" ]]; then
    python3 "$job_dir/source/runner/process_group_guard.py" stop \
      --registry "$job_dir/records/GROUP.tsv" --grace-seconds 20 \
...
setsid timeout --foreground --signal=TERM --kill-after=30s 3700s \
...
sha256sum "$job_dir/output/probe.stdout" "$job_dir/output/probe.stderr" \
  "$job_dir/output/probe.time" "$job_dir/output/TANGENT_CERTIFICATE.json" \
