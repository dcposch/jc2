...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_affine_faber_a_ift_lift_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
python3 cases/max12_812_order2_affine_faber_a_ift_lift_20260826/compile_a_ift.py \
  "$compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
...
for required in \
  'A_IFT_TCOEFF_CONTROL=1' \
  'A_IFT_EXACT_DIVISIONS=1' \
  'A_IFT_POINT_CONTROL=1' \
  'A_IFT_JAC_CONTROL=1' \
  'A_IFT_ENDPOINT=PASS_RATIONAL_J_POINT_AND_FORMAL_IFT' \
  'A_IFT_DONE=1' \
  'A_IFT_SCOPE=NORMALIZED_ORDINARY_FABER_ONLY_NO_TOTAL_REES_TERMINAL_TAYLOR_ORDER2_OR_JC2_VERDICT'
do
  if [[ "$(grep -Fxc "$required" "$aws_job/run/$tag.stdout" || true)" -ne 1 ]]; then
    printf 'validator=FAIL_MISSING_OR_NONUNIQUE:%s\n' "$required" >> "$aws_job/run/$tag.validation"; exit 92
  fi
done
printf 'validator=PASS_A_IFT\n' >> "$aws_job/run/$tag.validation"
