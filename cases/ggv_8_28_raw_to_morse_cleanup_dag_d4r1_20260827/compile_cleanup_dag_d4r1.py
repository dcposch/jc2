#!/usr/bin/env python3
"""Exact DEP-D3 factor-local raw-to-Morse cleanup DAG D4R1 compiler.
...
PINS = {
    "preregistration": (
        "cases/ggv_8_28_raw_to_morse_cleanup_dag_d4r1_20260827/PREREGISTRATION.md",
        "325d1ceeb1958e9062b6d8d9825f6dc6da7fa9c9268ea750fac12c6301443fe9",
    ),
    "quarantined_d4_freeze": (
...
def evaluate_dag(dag, raw_assignment):
    values = []
    for node in dag.nodes:
        if node["op"] == "ConstA":
            value = tuple(Q(x) for x in node["value"])
        elif node["op"] == "Raw":
            value = raw_assignment.get(node["slot"], AZERO)
        elif node["op"] == "Add":
            value = AZERO
            for child in node["args"]:
                value = aadd(value, values[child])
        elif node["op"] == "Mul":
            value = AONE
            for child in node["args"]:
                value = amul(value, values[child])
        else:
...
def discriminator(source, dag, critical, morse_U):
    Htx = {(0, 0): Q(-1), (0, 8): Q(1)}
    u_pre = sparse_tx_add(Htx, {(6, 1): Q(1), (8, 0): Q(-1, 2)})
    R_pre = {(14, 1): Q(1), (16, 0): Q(-1, 4)}
    F = sparse_tx_add(sparse_tx_mul(u_pre, u_pre), R_pre)
    ...
    bad_sign_F = sparse_tx_add(sparse_tx_mul(bad_u, bad_u), R_pre)
    assert bad_sign_F != expected
    bad_R = {(14, 1): Q(1), (16, 0): Q(1, 4)}
    bad_u16_F = sparse_tx_add(sparse_tx_mul(u_pre, u_pre), bad_R)
    assert any(t == 16 for t, _ in bad_u16_F)
    assert not any(slot["weight"] == 16 for slot in source["raw_slots_through_weight_22"]["F"])

    # This is the load-bearing type check: evaluate the already constructed
    # generic cleanup DAG, rather than treating X in R_pre as a local scalar.
    assignment_q = {row["slot"]: Q(row["coefficient"]) for row in rows}
    assignment_a = {slot: ascale(AONE, coeff) for slot, coeff in assignment_q.items()}
    evaluated = evaluate_dag(dag, assignment_a)
    evaluated_U = [evaluated[node] for node in morse_U]
    evaluated_critical = [evaluated[node] for node in critical]
    assert evaluated_U[14] == cpow(1), (evaluated_U[14], cpow(1))
    nonzero_U = [
        {"weight": n, "value_in_A_basis_1_c_through_c7": aencode(value)}
        for n, value in enumerate(evaluated_U)
        if value != AZERO
    ]
    nonzero_critical = [
        {"weight": n, "value_in_A_basis_1_c_through_c7": aencode(value)}
        for n, value in enumerate(evaluated_critical)
        if value != AZERO
    ]
    ...
    return {
        ...
        "identity": {
            ...
            "R_pre": "t^14*X-(1/4)*t^16 (X-dependent before recentering; not the Morse U(t))",
            "F_equals_u_pre2_plus_R_pre": encode_tx(F),
            ...
            "local_unit": "(u_pre)_X=H'(X)+t^6, whose closed-fibre value H'(c) is a unit",
        },
        "exact_cleanup_dag_evaluation": {
            "raw_assignment": {slot: qstr(coeff) for slot, coeff in sorted(assignment_q.items())},
            "nonzero_critical_shift_coefficients": nonzero_critical,
            "nonzero_Morse_U_coefficients": nonzero_U,
            "required_discriminator": "U14=c",
            "U14_value_in_A_basis_1_c_through_c7": aencode(evaluated_U[14]),
            "status": "PASS-EXACT-DAG-EVALUATION",
        },
        "consequence": "raw F14 support alone cannot exclude local U14=X",
        "mutations": {
            "flip_minus_half_in_u": "REJECTED: sparse identity changes",
            "flip_minus_quarter_in_R_pre": "REJECTED: leaves a t^16/2 term where the raw F16 slice is empty",
...
def build_result(source_bytes, source):
    ...
    result = {
        "status": "PASS-DEP-D3-D4R1-FACTOR-LOCAL-CLEANUP-DAG-W22",
        "dependency": "DEP-D3",
        ...
        "first_discriminator": discriminator(source, dag, critical, U),
        "dag": {
            ...
            "storage": "certificate-sized digest manifest; compile_cleanup_dag_d4r1.py deterministically reconstructs every node before checking this result",
        },
        "firewall": {
...
def main():
    ...
    if len(sys.argv) == 4 and sys.argv[1] == "--check":
        ...
        print("PASS DEP-D3 D4R1 exact cleanup DAG replay")
        return
    raise SystemExit("usage: compile_cleanup_dag_d4r1.py --print-result D3_RAW | --check D3_RAW RESULT")
