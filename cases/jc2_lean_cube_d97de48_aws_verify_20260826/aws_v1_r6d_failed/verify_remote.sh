...
if [[ $# -ne 5 ]]; then
  echo "usage: verify_remote.sh SOURCE_ROOT OUT_DIR EXPECTED_ARCHIVE_SHA EXPECTED_SOLUTION_SHA EXPECTED_COMMIT" >&2
  exit 64
fi
...
export LEAN_NUM_THREADS="${AWS_LEAN_JOBS:-4}"
...
for theorem_name in \
  ...
  GCD369CubeC1NoCommonRootOnVZeroChart \
  GCD369CubeC1NoCommonRoot; do
  grep -F "$theorem_name" "$out_dir/axioms.stdout" >/dev/null
done
