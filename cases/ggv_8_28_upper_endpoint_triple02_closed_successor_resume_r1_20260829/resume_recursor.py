...
search ends the run as a bounded no-verdict; any other irregularity is an
adapter failure.  Exactly one summary classification is written.
"""
...
class Recursor:
    def __init__(self, args, r5, gate):
        ...
        self.job_tag = os.environ.get("JOB_TAG")
        self.pgid = os.environ.get("JOB_CONTAINMENT_PGID")
        self.sid = os.environ.get("JOB_CONTAINMENT_SID")
        if not self.job_tag or not self.pgid or not self.sid:
    ...
    def run_stage(self, label: str, script: Path, cwd: Path, cap: int,
        ...
        command = [
            ...
            "--cap-seconds", str(cap),
            "--expected-pgid", str(self.pgid),
            "--expected-sid", str(self.sid),
            "--job-tag", str(self.job_tag),
        ]
