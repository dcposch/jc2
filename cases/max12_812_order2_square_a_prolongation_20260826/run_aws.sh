...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_a_prolongation_20260826/FREEZE.sha256 \
  > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 \
  cases/max12_812_order2_square_a_prolongation_20260826/compile_square_a_prolongation.py \
...
input="$aws_job/compiled/square_a_prolongation_${label}.sing"
...
for required in \
  'SQUARE_APROL_SOURCE_HASHES=PASS' \
  'SQUARE_APROL_SIGMA14_DIVISIBLE=1' \
  'SQUARE_APROL_GRADE14_IDENTITY=1' \
  'SQUARE_APROL_GRADE15_IDENTITY=1' \
  'SQUARE_APROL_GRADE14_FORBIDDEN=1' \
  'SQUARE_APROL_GRADE15_FORBIDDEN=1' \
  'SQUARE_APROL_GRADE14_CANCELLATION=1' \
  'SQUARE_APROL_A0_IN_RADICAL=1' \
  'SQUARE_APROL_A1_IN_RADICAL=1' \
  'SQUARE_APROL_ENDPOINT=PASS_HIGH_CONTACT_A_KILLED'
do
...
printf 'validator=PASS_SQUARE_A_PROLONGATION\n' >> "$validation"
