...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only K00 g-open launcher refused host" >&2
  exit 125
fi
if [[ $# -ne 7 ]]; then
  echo "usage: launch_remote_gopen.sh AWS_ROOT AWS_JOB TAG CAP_KIB TIMEOUT_SECONDS CHARACTERISTIC FREEZE" >&2
  exit 125
fi
