...
mkdir -p "$lane"
python3 "$case_dir/generate.py" --candidate "$candidate" --mode "$mode" \
...
meta=$lane/run.meta
{
  ...
  echo "route=pure-Singular candidate plane-curve genus mode=$mode"
  echo "timeout_seconds=$timeout_seconds"
  echo "virtual_memory_limit_kib=$vm_kib"
  Singular --version | head -1 | sed 's/^/singular=/'
  sha256sum "$0" "$case_dir/generate.py" "$candidate"
