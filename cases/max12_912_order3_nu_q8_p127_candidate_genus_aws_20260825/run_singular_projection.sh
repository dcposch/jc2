...
mkdir -p "$lane"
python3 "$case_dir/generate_singular_projection.py" --candidate "$candidate" \
...
meta=$lane/run.meta
{
  ...
  echo "route=fail-closed lex elimination of H singular locus to w"
  echo "timeout_seconds=$timeout_seconds"
  echo "virtual_memory_limit_kib=$vm_kib"
  Singular --version | head -1 | sed 's/^/singular=/'
  sha256sum "$0" "$case_dir/generate_singular_projection.py" "$case_dir/generate_v2.py" "$candidate"
...
if [ "$rc" -eq 0 ] && ! grep -qx 'singular_projection_end' "$lane/result.out"; then rc=93; fi
{
