...
if [[ "$(uname -s)" != Linux ]]; then
  echo "REFUSED: V84R2 is AWS/Linux only" >&2
  exit 97
fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: V84R2 requires Amazon EC2" >&2
  exit 96
fi
if [[ -z "${AWS_RUN_TAG:-}" || "${AWS_RUN_TAG}" != td6_v84r2_k2_* ]]; then
  echo "REFUSED: registered td6_v84r2_k2_ tag required" >&2
  exit 95
fi
...
sha256sum -c V84R2_SOURCE.sha256
"$PYTHON" \
  payload/jc2/cases/td6_c1_c2_c3_qdead_previous_pole_k2_shard_v84r2_20260826/replay.py
