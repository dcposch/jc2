...
printf 'hostname=%s\nlabel=%s\narchive_sha256=%s\nstart_utc=%s\ncap_kib=268435456\ntimeout_seconds=43200\nscope=high-q-total-F-K-repair\n' \
  "$(hostname)" "$label" "$expected" "$(date -u +%FT%TZ)" > "$root/registration.meta"
(
  set +e
  lane="$root/highqtf"
  mkdir -p "$lane/output"
  tag="td6_v89h4_high_q_total_f_k_repair_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=43200\nscope=high-q-total-F-K-repair\n' \
  ...
  (
    ...
    export TD6_Q_SCOPE=high-q16-q24
    export TD6_OUTPUT_DIR="$lane/output"
    /usr/bin/time -v timeout 43200 bash run_v89h4_high_q_total_f_k_repair.sh \
