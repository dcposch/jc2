...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a9_source_support_census_v3_parenthesized_loads_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_a9_source_support_census_v3_parenthesized_loads_20260826/compile_v3_parenthesized_loads.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_a9_recursive_source_grades_v3_${label}.sing"
sha256sum "$input" "$aws_job/compiled/result.json" > "$aws_job/compiled.sha256"
for required in 'A9V3_LOAD_WEIGHT_DISTRIBUTIVITY_K10=190' \
  'A9V3_LOAD_WEIGHT_DISTRIBUTIVITY_K6=72' 'A9V3_LOAD_WEIGHT_DISTRIBUTIVITY_K2=27'; do
...
for required in 'A9V2_SOURCE_HASHES=PASS' 'A9V2_RECURSIVE_QUOTIENT_IDENTITIES=1' \
  'A9V2_RECURSIVE_SOURCE_CENSUS_ENDPOINT=PASS_NAVIGATION_ONLY'; do
...
printf 'validator=PASS_A9_V3_PARENTHESIZED_LOAD_SOURCE_CENSUS\n' >> "$validation"
