...
job_tag=$2
lane=$3
case_dir="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_lambda0_endpoint_kernel_strata_r2_20260828"
...
setsid timeout --foreground --signal=TERM --kill-after=40s 7200s \
  ...
  "$case_dir/aws_worker.sh" "$job_dir" "$lane" \
...
printf 'GGV_ENDPOINT_STRATA_%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$lane" \
