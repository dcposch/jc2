...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_halfweight_kuranishi_20260826/FREEZE.sha256 \
  > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 \
  cases/max12_812_order2_square_halfweight_kuranishi_20260826/compile_square_halfweight.py \
...
input="$aws_job/compiled/square_halfweight_${label}.sing"
...
for required in \
  'SQUARE_HALFWEIGHT_SOURCE_HASHES=PASS' \
  'SQUARE_HALFWEIGHT_SIGMA10_DIVISIBLE=1' \
  'SQUARE_HALFWEIGHT_DIVISION_IDENTITY=1' \
  'SQUARE_HALFWEIGHT_FORBIDDEN=1' \
  'SQUARE_HALFWEIGHT_RAD_IN_EXPECTED=1' \
  'SQUARE_HALFWEIGHT_EXPECTED_IN_RAD=1' \
  'SQUARE_HALFWEIGHT_ENDPOINT=PASS_CORRECTION_AWARE_NORMALIZED_RAY'
do
...
printf 'validator=PASS_SQUARE_HALFWEIGHT\n' >> "$validation"
