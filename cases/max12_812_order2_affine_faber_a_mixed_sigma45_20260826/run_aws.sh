...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_affine_faber_a_mixed_sigma45_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
python3 cases/max12_812_order2_affine_faber_a_mixed_sigma45_20260826/compile_sigma45.py "$compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
...
for required in 'A_SIGMA45_TCOEFF_CONTROL=1' 'A_SIGMA45_H3_LOWER_ZERO=1' 'A_SIGMA45_CERT=1' 'A_SIGMA45_ENDPOINT=PASS_COMPLETE_SOURCE_H3_MINUS_M3_OVER_16' 'A_SIGMA45_DONE=1' 'A_SIGMA45_SCOPE=DELAYED_LOAD_REPEATED_A_TOWER_THROUGH_SIGMA45_ONLY_NO_TERMINAL_TAYLOR_ORDER2_OR_JC2_VERDICT'; do
...
printf 'validator=PASS_A_SIGMA45_IDENTITY\n' >> "$aws_job/run/$tag.validation"
