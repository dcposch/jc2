#!/usr/bin/env python3
"""Validate the recombination certificate and freeze V6 discovery JSON."""
...
def load_v2_validator():
    if digest(V2_VALIDATOR) != V2_VALIDATOR_SHA256:
        fail("V2 validator hash mismatch")
...
def unique(values: dict[str, list[str]], key: str, expected: str) -> None:
    if values.get(key) != [expected]:
        fail(("missing, duplicate, or wrong certificate sentinel", key, values.get(key), expected))
...
def main() -> None:
    ...
    if pre.get("status") != "PASS-T-RS0-ROW-SHARDS-PREPARED-CERTIFICATE":
        fail("pre-certificate status mismatch")
    if pre.get("implementation") != "ROW_SHARD_DIFFSCAN_V6" or compiler.get("implementation") != "ROW_SHARD_DIFFSCAN_V6":
        fail("implementation mismatch")
    if pre.get("characteristic") != args.characteristic or compiler.get("characteristic") != args.characteristic:
        fail("characteristic mismatch")
    ...
    if meta.get("stdout_sha256") != digest(args.certificate_stdout):
        fail("certificate stdout not bound by AWS metadata")
    if meta.get("stderr_sha256") != digest(args.certificate_stderr):
        fail("certificate stderr not bound by AWS metadata")
    ...
    result = {
        "status": "PASS-T-RS0-ROW-SHARD-DIFFSCAN-V6-NAVIGATION-ONLY",
        "scope": pre["scope"],
