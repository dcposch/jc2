...
: "${MEMORY_KIB:=67108864}"
case_dir="$JC2_ROOT/cases/as_fonly_d7_q3_state_complete_bv_20260825"
mkdir -p "$JOB_DIR"
hostname > "$JOB_DIR/hostname"
date -u +%FT%TZ > "$JOB_DIR/start_utc"
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/emit_formula.py" \
  "$case_dir/replay_model.py" "$0" "$MODEL_OUTPUT" > "$JOB_DIR/INPUT.sha256"
ulimit -v "$MEMORY_KIB"
env JC2_ROOT="$JC2_ROOT" MODEL_OUTPUT="$MODEL_OUTPUT" \
  ...
  python3 "$case_dir/emit_formula.py" \
...
case "$rc" in
  10)
    env JC2_ROOT="$JC2_ROOT" SOLVER_MODEL="$JOB_DIR/boolector.stdout" \
      PARENT_OUTPUT_JSON="$JOB_DIR/q3_parent_replay.json" \
      OUTPUT_JSON="$JOB_DIR/direct_replay.json" \
      python3 "$case_dir/replay_model.py" \
