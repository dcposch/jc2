...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V16 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 7 ]]; then
  echo "usage: run_syzygy_projection_v16_aws.sh AWS_ROOT AWS_JOB TAG FIELD CAP_KIB TIMEOUT_SECONDS FREEZE" >&2
  exit 125
fi
aws_root=$1
aws_job=$2
...
if [[ "$field" != "Q" && "$field" != "65521" ]]; then
  echo "unsupported V16 field" >&2
  exit 125
fi
rel=cases/max12_812_order2_u2_62_k00_syzygy_constant_projection_v16_20260827
...
python3 "$rel/compile_syzygy_projection_v16.py" "$aws_job/compiled" --field "$field" \
  > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
input="$aws_job/compiled/k00_syzygy_projection_v16_${field}.sing"
...
python3 "$rel/build_serialized_replay_v16.py" "$prelude" "$aws_job/run/artifacts" \
...
python3 "$rel/validate_syzygy_projection_v16.py" \
...
printf 'validator=PASS_K00_SYZPROJ_V16\n' >> "$aws_job/run/FINAL.validation"
sha256sum "$aws_job/run/RESULT.json" "$aws_job/run/artifacts"/BASE_SYZYGY_*.txt \
  "$aws_job/run/artifacts"/FREEDOM_SYZYGY_*.txt \
  "$aws_job/run/serialized_replay/replay.sing" "$aws_job/run/serialized_replay/replay.stdout" \
  "$aws_job/run/serialized_replay/artifacts/BASE_RESIDUAL.txt" \
  "$aws_job/run/serialized_replay/artifacts/FREEDOM_RESIDUAL.txt" \
