...
cleanup() {
  ...
  if [[ -n "$worker_pid" && -f "$job_dir/records/GROUP.tsv" ]]; then
    python3 "$guard" stop --registry "$job_dir/records/GROUP.tsv" --grace-seconds 20 >>"$job_dir/records/TRAP_STOP.jsonl" 2>&1 || true
  fi
  return "$rc"
}
...
setsid timeout --foreground --signal=TERM --kill-after=30s 900s \
  env AWS_RUN_TAG="$job_tag" AWS_RUN_LANE=p bash "$case_dir/aws_worker.sh" "$job_dir" \
...
printf 'GGV_ENDPOINT_REPLAY_DIAG_R1\t%s\t%s\t%s\t%s\t%s\t%s\n' "$worker_pid" "$pgid" "$sid" "$starttime" "$job_dir" "$source_sha" >"$job_dir/records/GROUP.tsv"
touch "$job_dir/records/REGISTERED"
set +e
python3 "$guard" monitor --registry "$job_dir/records/GROUP.tsv" --grace-seconds 20 >"$job_dir/records/MONITOR.jsonl" 2>"$job_dir/records/MONITOR.stderr"
monitor_rc=$?
wait "$worker_pid"
worker_rc=$?
set -e
...
find "$job_dir/output" "$job_dir/records" -type f ! -name EVIDENCE.sha256 -print0 | sort -z | xargs -0 sha256sum >"$job_dir/EVIDENCE.sha256"
