...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only weighted K00 launcher refused host" >&2
  exit 125
fi
if [[ $# -ne 7 ]]; then
  echo "usage: launch_remote_weighted.sh AWS_ROOT AWS_JOB TAG CAP_KIB TIMEOUT_SECONDS CHARACTERISTIC FREEZE" >&2
  exit 125
fi
aws_root=$1
aws_job=$2
...
characteristic=$6
freeze=$7
v3_rel=cases/max12_812_order2_u2_62_k00_closure_weighted_v3_20260827
mkdir -p "$aws_job/run"
{
  printf 'tag=%s\n' "$lane_tag"
  printf 'host=%s\n' "$(hostname)"
