...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only weighted K00 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 6 ]]; then
  echo "usage: run_weighted_aws.sh AWS_ROOT AWS_RUN TAG INPUT CAP_KIB TIMEOUT_SECONDS" >&2
  exit 125
fi
aws_root=$1
aws_run=$2
...
python3 "$aws_root/$v3_rel/promote_weighted_result.py" \
