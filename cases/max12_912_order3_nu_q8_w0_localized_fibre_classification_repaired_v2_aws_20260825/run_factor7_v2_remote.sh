...
if [[ $# -ne 4 ]]; then
  echo "usage: run_factor7_v2_remote.sh VM_KIB REPO OUT TAG" >&2
  exit 64
fi
...
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/find_irreducible_prime.py" \
  "$case_dir/generate_factor7_v2.py" "$case_dir/run_factor7_v2_remote.sh" \
  "$case_dir/PREREGISTRATION.manifest.sha256" > "$out/source.sha256"
python3 "$case_dir/generate_factor7_v2.py" > "$out/input.sing" 2> "$out/generator.stderr"
...
for expected in factor_count=1 degree_sum=8 degree8_count=1 \
