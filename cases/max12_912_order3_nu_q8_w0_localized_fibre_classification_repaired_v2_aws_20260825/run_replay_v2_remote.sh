...
if [[ $# -ne 2 ]]; then
  echo "usage: run_replay_v2_remote.sh REPO OUTDIR" >&2
  exit 64
fi
repo=$1; out=$2
...
test ! -e "$out"
mkdir -p "$out"
sha256sum "$case_dir/replay.py" "$case_dir/run_replay_v2_remote.sh" \
...
test "$(grep -c '^Q8_W0_LOCALIZED_FIBRE_REPAIRED_V2_REPLAY_PASS$' "$out/stdout" || true)" = 1
