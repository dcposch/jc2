...
if [[ $# -ne 5 ]]; then
  echo "usage: launch_remote_v2.sh MODE AWS_REPO_ROOT AWS_RUN_DIRECTORY REGISTERED_TAG INPUT_OR_DASH" >&2
  exit 125
fi
...
case "$mode" in
  compile)
    ...
    runner="$aws_root/cases/max12_812_order4_mu4_nonzero_curve_20260825/run_compile_v2_aws.sh"
    ;;
  geometry)
