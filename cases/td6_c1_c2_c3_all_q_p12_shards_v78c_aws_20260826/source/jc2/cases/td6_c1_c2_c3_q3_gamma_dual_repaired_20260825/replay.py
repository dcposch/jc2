...
V32_PATH = HERE.parent / "td6_c1_c2_c3_q2_beta_dual_20260825" / "replay.py"
...
spec = importlib.util.spec_from_file_location("td6_v77r_v32_base", V32_PATH)
...
def source_vector_gamma(key, include_q3=True):
    base = r.t.source_vector(key)
    derivative = (
        ONE_VECTOR
        if include_q3 and key == ("g", "X", 0, 3)
        else ZERO_VECTOR
    )
...
def propagate_gamma(rows, records, include_q3=True):
    ...
    for source_row, record in zip(rows, records):
        ...
        base, derivative = source_vector_gamma(key, include_q3)
...
def configure_qd_gamma(
    direct_qprime=True, stale_B=False, leak_q2_qprime=False
):
    qd = r.qd
    qd.Dual = EDual
    qd.B = EDual(0, 1) if stale_B else EDual(0)
    ...
    if not stale_B:
        assert not qd.B.value and not qd.B.derivative
...
def exact_source_replay(polynomial, remainder, relations, first_rows):
    ...
    omitted = {}
    for index, (relation, (_, row, rhs)) in enumerate(
        zip(relations, first_rows)
    ):
        if index != nonzero:
            omitted = add(
...
def main():
    print("producer=TD6-V77R-PURE-Q3-SCALAR-AUDIT")
    print(f"V32_base_source_sha256={V32_SHA256}")
    ...
    assert sum(row[0] == ('g', 'X', 0, 3) for row in ordered) == 1
    assert sum(row[0] == ('g', 'X', 0, 2) for row in ordered) == 1
    ...
    omitted_pivot_rhs, omitted_transport_compatibility = propagate_gamma(
        ordered, records, include_q3=False
    )
    assert not transport_compatibility
    assert not omitted_transport_compatibility
    ...
    assert not any(
        any(derivative) for _, derivative in omitted_pivot_rhs.values()
    )
    ...
    print("transport_q3_source_omission_negative_control=true", flush=True)
    ...
    configure_qd_gamma(direct_qprime=True, stale_B=True)
    first_rows_stale_B = r.qd.pack(
        'X-2', r.qd.first_band_polynomials(
            bands[('f', 1)], bands[('g', 1)]
        )
    )
    assert first_rows_stale_B == first_rows
    configure_qd_gamma(direct_qprime=True, leak_q2_qprime=True)
    first_rows_q2_qprime_leak = r.qd.pack(
        'X-2', r.qd.first_band_polynomials(
            bands[('f', 1)], bands[('g', 1)]
        )
    )
    assert first_rows_q2_qprime_leak != first_rows
    configure_qd_gamma(direct_qprime=True)
    assert not r.qd.B.value and not r.qd.B.derivative
    ...
    print("legacy_qd_B_state_is_semantically_inert=true", flush=True)
    print("q2_Q_PRIME_leakage_negative_control=true", flush=True)
    print("pure_q3_B_value_zero=true")
    print("pure_q3_B_derivative_zero=true", flush=True)
    ...
    print("first_minor_gamma_is_exact_zero=true")
    print("zero_scalar_digest_semantics_asserted=true")
    ...
    print(
        "gamma_first_stage_remainder_nonzero="
        + str(bool(remainder_gamma)).lower()
    )
    ...
    print("TD6-V77R-PURE-Q3-SCALAR-AUDIT PASS")
