...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V13 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 6 ]]; then
  echo "usage: run_local_membership_v13_aws.sh AWS_ROOT AWS_JOB TAG CAP_KIB TIMEOUT_SECONDS FREEZE" >&2
  exit 125
fi
aws_root=$1
aws_job=$2
...
timeout_seconds=$5
freeze=$6
rel=cases/max12_812_order2_u2_62_k00_local_membership_v13_20260827
v11rel=cases/max12_812_order2_u2_62_k00_local_membership_v11_20260827
...
mkdir -p "$aws_job/run/artifacts"
{
  ...
  printf 'mode=exact_q_ds_v13\n'
} > "$aws_job/launch_registration.txt"
cd "$aws_root"
sha256sum -c "$freeze" > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$lane_tag"
python3 "$rel/compile_local_membership_v13.py" "$aws_job/compiled" \
  > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
input="$aws_job/compiled/k00_local_membership_v13_q.sing"
...
python3 "$v11rel/validate_local_membership.py" \
...
printf 'validator=PASS_LOCAL_MEMBERSHIP_V13\n' >> "$aws_job/run/FINAL.validation"
