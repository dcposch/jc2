#!/usr/bin/env bash
# External launcher for the D43 v4 exact a00pp conditional pipeline.
#
...
# (held on fd 9 for the whole job).  The supervisor is started under
# `setsid --wait` in BOTH containment modes, so a real job-scoped process
# group (leader = supervisor) exists in both.  After the boundary exits the
# launcher reaps and censuses; a POSITIVE terminal paired with an unclean
# post-mortem census is voided by an immutable TERMINAL_POSTMORTEM_FAULT.json
# and a non-zero launcher exit (the terminal itself is never overwritten).
# If no terminal object exists the launcher publishes the single NO_VERDICT
# authority.  The launcher never overwrites TERMINAL.json.
set -uo pipefail

EXPECTED_SOURCE_ARCHIVE_SHA256="9051a3dbfc4d6d8714a6ee3ca53e84517bcadddfab4fc49baf480d65f0dd31ba"
CASE_REL="cases/d43_exact_sparse_rows_v4_20260829"
ARCHIVE_MANIFEST_REL="jc2/$CASE_REL/SOURCE_ARCHIVE_MANIFEST_V4.sha256"

usage() {
  echo "usage: aws_launch_v4.sh --job-root DIR --archive FILE \
--registration FILE" >&2
  exit 64
}
...
fault_object="$job_root/TERMINAL_POSTMORTEM_FAULT.json"
supervisor_pid=0
supervisor_start=0
containment_mode=none
scope_unit="jc2d43v4-${JOB_TAG//[^A-Za-z0-9_.-]/-}"
...
publish_no_verdict() {
  ...
  printf '{\n "schema": "jc2.d43.a00pp-exact-source-rows.v4.terminal",\n "status": "NO_VERDICT_%s",\n "terminal_authority": true,\n "positive": false,\n "job_tag": "%s",\n "utc": "%s"\n}\n' \
    "$reason" "$JOB_TAG" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    >"$temporary" || return 0
  mv "$temporary" "$terminal" 2>/dev/null || true
}

supervisor_leader_pid() {
  # The session/PGID leader is the supervisor started by setsid, not $!
  # (which is the setsid or systemd-run wrapper); read it from the
  # supervisor's own containment identity record.
  python3 - "$job_root/custody/containment_identity.json" <<'EOF' 2>/dev/null
import json, sys
try:
    value = json.load(open(sys.argv[1]))["supervisor_pid"]
    print(int(value))
except Exception:
    pass
EOF
...
kill_boundary() {
  ...
  local leader
  leader=$(supervisor_leader_pid)
  if [[ "$leader" =~ ^[0-9]+$ && "$leader" -gt 1 ]]; then
    kill -KILL -- "-$leader" 2>/dev/null || true
  fi
  if [[ "$supervisor_pid" -gt 1 ]]; then
...
postmortem() {
  ...
  local leader
  leader=$(supervisor_leader_pid)
  [[ "$leader" =~ ^[0-9]+$ && "$leader" -gt 1 ]] || leader=0
  python3 -B "$contract" cleanup --pgid "$leader" --job-tag "$JOB_TAG" \
    --exclude-pid $$ \
  ...
  printf '{"schema":"jc2.d43.a00pp-exact-source-rows.v4.launcher-postmortem","cleanup_rc":%s,"cleanup_pgid":%s,"containment_mode":"%s","scope_state":"%s","terminal_exists":%s,"utc":"%s","terminal_authority":false}\n' \
    "$postmortem_rc" "$leader" "$containment_mode" "$cgroup_state" \
...
host_lock="${JC2_D43_V4_LOCK_PATH:-$HOME/.jc2_d43_a00pp_v4_host.lock}"
...
contract="$job_root/source/jc2/$CASE_REL/job_contract_v4.py"
...
manifest_json="$job_root/source/jc2/$CASE_REL/PIPELINE_MANIFEST_V4.json"
read_contract_value() {
...
# setsid is required in BOTH modes: the supervisor must be a session/PGID
# leader so the final process-group census parameter is real, never vacuous.
...
printf '{"schema":"jc2.d43.a00pp-exact-source-rows.v4.launch","containment_mode":"%s","scope_unit":"%s","archive_sha256":"%s","launcher":%s,"utc":"%s","terminal_authority":false}\n' \
...
supervisor="$job_root/source/jc2/$CASE_REL/run_conditional_pipeline_aws_v4.sh"
...
kill_boundary
postmortem
# The post-mortem is authority-bearing: a POSITIVE terminal paired with a
# non-zero census rc or non-empty survivor census is voided by an immutable
# fault object and a non-zero launcher exit.  The terminal is never touched.
postmortem_gate_rc=0
if [[ -e "$terminal" ]]; then
  if [[ -n "$contract" && -f "$contract" ]]; then
    python3 -B "$contract" postmortem-gate --terminal "$terminal" \
      --postmortem-rc "$postmortem_rc" \
      --pgid-census "$job_root/records/launcher_postmortem_pgid.json" \
      --tag-census "$job_root/records/launcher_postmortem_tag.json" \
      --fault-output "$fault_object" \
      >"$job_root/records/launcher_postmortem_gate.txt" 2>&1
    postmortem_gate_rc=$?
  else
    postmortem_gate_rc=125
  fi
fi
if [[ ! -e "$terminal" ]]; then
...
[[ -e "$terminal" ]] || exit 77
if [[ "$postmortem_gate_rc" != 0 ]]; then
  echo "TERMINAL_POSTMORTEM_FAULT=1 (immutable fault object: $fault_object;" \
    "any positive in $terminal is VOID)" >&2
  exit 78
fi
exit 0
