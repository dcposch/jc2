...
spec = importlib.util.spec_from_file_location(
    "d43_exact_sparse_rows_v4", HERE / "selected_rows_v4.py")
...
MANIFEST_PATH = HERE / "PIPELINE_MANIFEST_V4.json"


class ExactA00ppRowsV4Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_manifest_binds_v4_contract_and_status(self):
        self.assertEqual(self.manifest["schema"],
                         "jc2.d43.a00pp-exact-source-manifest.v4")
        self.assertEqual(self.manifest["status"],
                         "V4_REPAIRED_LAUNCH_NOT_AUTHORIZED")
        ...
        self.assertEqual(self.manifest["terminal"]["single_authority"],
                         "TERMINAL.json")
        provenance = self.manifest["revision_provenance"]
        self.assertEqual(
            provenance["v2_hostile_review_sha256"],
            "4f6f6c90e0526ab07591e2ec6a97f475e477b534cb6991085764cd693a99020d")
        # v4 answers the Opus 5 v3 hostile review and pins its digests plus
        # the v3 seal and the v3 repair report.
        self.assertEqual(
            provenance["v3_packet_source_seal_sha256"],
            "5423bd88328c338c54072b056d3f64251dc57de6f5112646f9d157f991ab5271")
        self.assertEqual(
            provenance["v3_hostile_review_opus5_sha256"],
            "6e01e5d7e7852d094559ab57cd7a256c41390fca99e38a6c55fbea8f12209bd9")
        self.assertEqual(
            provenance["v3_hostile_review_opus5_body_sha256"],
            "1b90552abddb14bf5946a2e85f0a5ac26dae05d3049ef578db507c766862bd82")
        self.assertEqual(
            provenance["v3_repair_report_fable5_sha256"],
            "5759cfe81e21e7e4d3967b96152eaed5e789ed0498b039804b17c30853616b75")
        self.assertIn("TERMINAL_POSTMORTEM_FAULT.json",
                      self.manifest["terminal"]
                      ["post_boundary_fault_object"])
    ...
    def test_producer_writes_no_early_positive_authority(self):
        source = (HERE / "selected_rows_v4.py").read_text()
        ...
        supervisor = (HERE / "run_conditional_pipeline_aws_v4.sh").read_text()
    ...
    def test_no_production_asserts_survive(self):
        import ast
        for name in ("selected_rows_v4.py", "aws_preflight_v4.py",
                     "job_contract_v4.py"):
    ...
    def test_single_terminal_writer_census(self):
        ...
        supervisor = (HERE / "run_conditional_pipeline_aws_v4.sh").read_text()
        launcher = (HERE / "aws_launch_v4.sh").read_text()
    ...
    def test_v4_repair_wiring_census(self):
        """Static census of the five Opus 5 v3-review repairs in the shell
        lanes: real pgid in both modes, NOT_APPLICABLE cgroup gate in the
        fallback, terminal binding arguments, setsid in both containment
        branches, and the authority-bearing launcher post-mortem gate."""
        supervisor = (HERE / "run_conditional_pipeline_aws_v4.sh").read_text()
        launcher = (HERE / "aws_launch_v4.sh").read_text()
        # R4: the supervisor census pgid is $$ unconditionally (no
        # vacuity-inducing 0 remains), and the fallback cgroup gate is
        # NOT_APPLICABLE, never a copy of another gate.
        self.assertIn("cleanup_pgid=$$", supervisor)
        self.assertNotIn("cleanup_pgid=0", supervisor)
        self.assertNotIn("cgroup_final_gate=$cleanup_gate", supervisor)
        self.assertIn("cgroup_final_gate=NOT_APPLICABLE", supervisor)
        self.assertIn('"$session_leader" == 1', supervisor)
        # R1/R2: the terminal decision receives every artifact it must
        # rehash plus the census provenance parameters.
        for argument in ('--manifest "$manifest"',
                         '--preflight-receipt "$records/PREFLIGHT.json"',
                         '--pilot-gate "$output/PILOT_GATE.json"',
                         '--merge-receipt "$output/MERGE_RECEIPT.json"',
                         '--merge-payload '
                         '"$output/d43_a00pp_exact_184_raw_J_rows.pkl"',
                         '--boundary-mode "$CONTAINMENT_MODE"',
                         '--cleanup-pgid "$cleanup_pgid"'):
            self.assertIn(argument, supervisor, argument)
        # Both launcher containment branches start the supervisor under
        # setsid, so the supervisor is a session/PGID leader in both modes.
        self.assertEqual(launcher.count('setsid --wait bash "$supervisor"'),
                         2)
        # R5: the launcher gates on the post-mortem census and emits the
        # immutable fault object without ever overwriting the terminal.
        self.assertIn("postmortem-gate", launcher)
        self.assertIn("TERMINAL_POSTMORTEM_FAULT.json", launcher)
        self.assertIn('postmortem_gate_rc=$?', launcher)
        self.assertIn('[[ "$postmortem_gate_rc" != 0 ]]', launcher)
        self.assertIn("exit 78", launcher)

    def test_v2_v3_v4_math_function_diff_closed(self):
        """Close the byte-diff of every shared mathematical function that
        the Opus 5 v3 review disclosed as UNADJUDICATED: the pinned diff
        tool must prove zero v3->v4 function drift and the exact reviewed
        v2->v4 decomposition, and the sealed report must regenerate
        byte-identically."""
        diff_spec = importlib.util.spec_from_file_location(
            "d43_math_diff_v4", HERE / "math_function_diff_v4.py")
        ...
        sources = diff_tool.check_all()
        import tempfile
        with tempfile.TemporaryDirectory() as tmp:
            replay = Path(tmp) / "report.md"
            diff_tool.write_report(replay, sources)
            self.assertEqual(
                replay.read_bytes(),
                (HERE / "V2_V4_MATH_FUNCTION_DIFF.md").read_bytes(),
                "sealed diff report is not the deterministic regeneration")
        # The math core named by the tool is byte-identical v2->v4.
        v2f, _ = sources["v2"]
        v4f, _ = sources["v4"]
        for name in sorted(diff_tool.MATH_CORE):
            self.assertEqual(v2f[name], v4f[name], name)

    def test_preflight_census_is_job_tag_aware(self):
        source = (HERE / "aws_preflight_v4.py").read_text()
        self.assertIn("selected_rows_v4", source)
        self.assertIn("run_conditional_pipeline_aws", source)
        self.assertIn("job_contract_v4", source)
        self.assertIn("aws_launch_v4", source)
