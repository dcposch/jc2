#!/usr/bin/env bash
# Supervisor for the D43 v4 exact a00pp conditional pipeline.
#
...
#
# The mathematical flow is revision 2 unchanged (via the reviewed v3 lane): independent f and g side
...
case_dir="$source_root/cases/d43_exact_sparse_rows_v4_20260829"
manifest="$case_dir/PIPELINE_MANIFEST_V4.json"
contract="$case_dir/job_contract_v4.py"
producer="$case_dir/selected_rows_v4.py"
preflight_py="$case_dir/aws_preflight_v4.py"
...
emergency_containment() {
  ...
  # The launcher starts this supervisor under setsid in BOTH containment
  # modes, so $$ is the real session/process-group leader in both; the
  # containment identity gate verifies that before any heavy stage.
  local cleanup_pgid=$$
...
# The launcher wraps this supervisor in `setsid --wait` in BOTH modes, so a
# real job-scoped process group (leader = this supervisor) exists in both;
# session/PGID leadership is therefore a mandatory identity input in both,
# which makes the final PGID census parameter real, never vacuous.
supervisor_sid=$(ps -o sid= -p $$ | tr -d ' ')
supervisor_pgid=$(ps -o pgid= -p $$ | tr -d ' ')
session_leader=0
[[ "$supervisor_sid" == "$$" && "$supervisor_pgid" == "$$" ]] \
  && session_leader=1
if [[ "$CONTAINMENT_MODE" == systemd_scope ]]; then
  ...
  if [[ "$observed_swap_max" == 0 \
        ...
        && "$observed_pids_max" == "$whole_job_pids_max" \
        && "$session_leader" == 1 ]]; then
...
printf '{"mode":"%s","cgroup_dir":"%s","memory_swap_max":"%s","memory_max":"%s","pids_max":"%s","gate":%s,"supervisor_pid":%s,"supervisor_sid":"%s","supervisor_pgid":"%s","session_leader":%s}\n' \
  "$CONTAINMENT_MODE" "$cgroup_dir" "$observed_swap_max" \
  "$observed_memory_max" "$observed_pids_max" \
  "$containment_identity_gate" "$$" \
  "$supervisor_sid" "$supervisor_pgid" "$session_leader" \
...
if [[ "$containment_identity_gate" == 1 && "$monitor_started" == 1 ]]; then
  ...
  if [[ "$stage_failed" == 0 ]]; then
    ...
    (cd "$source_root" && sha256sum -c \
      "cases/d43_exact_sparse_rows_v4_20260829/OPERATIONAL_SOURCE_V4.sha256") \
...
# Real process-group census parameter in BOTH modes: the launcher's setsid
# wrapper makes this supervisor the session/PGID leader in both (verified by
# the containment identity gate), so the terminal's PGID census is never
# taken with a vacuity-inducing pgid of 0.
cleanup_pgid=$$
...
if [[ "$CONTAINMENT_MODE" == systemd_scope && -n "$cgroup_dir" ]]; then
  ...
  # No cgroup exists in the pgid fallback.  The gate is reported as
  # NOT_APPLICABLE with provenance in the terminal - never as a copy of
  # another gate's value; the real orphan terminal inputs in this mode are
  # the exact-PGID and JOB_TAG censuses above.
  printf '{"status": "NOT_APPLICABLE_NO_CGROUP_IN_SETSID_PGID_FALLBACK"}\n' \
    >"$custody/cgroup_final_census.json"
  cgroup_final_gate=NOT_APPLICABLE
fi
...
py "$contract" terminal \
  ...
  --merge-payload "$output/d43_a00pp_exact_184_raw_J_rows.pkl" \
