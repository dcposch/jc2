...
job_tag=$2
lane=$3
case_dir="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_lambda0_rank_jump_fitting_census_r1_20260828"
...
setsid timeout --foreground --signal=TERM --kill-after=40s 7200s \
  ...
  bash "$case_dir/aws_worker.sh" "$job_dir" "$lane" \
...
printf 'GGV_FITTING_CENSUS_R1_%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$lane" \
