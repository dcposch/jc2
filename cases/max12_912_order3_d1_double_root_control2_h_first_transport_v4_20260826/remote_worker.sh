...
case "$tag" in max12_912_order3_d1_double_root_control2_h_first_transport_v4_20260826T*) ;; *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;; esac
...
( ulimit -v 4194304; nice -n 10 timeout 900 /usr/bin/time -v -o compiler.time \
  python3 -u compile_h_first_transport_v4.py --order "$order" --output h_transport.json \
...
if test "$rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_H_FIRST_TRANSPORT_V4$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_OR_DIAGNOSTIC_FAILURE > FAILED; exit 96
fi
sha256sum h_transport.json compiler.stdout compiler.stderr compiler.rc compiler.time > result.sha256
touch DONE
