#!/usr/bin/env python3
"""Exact generic three-center TD6 q2 dual-number discriminator.

The base is the source-typed section

    y=s^-1, x=C*s+V*s^2+U*s^3+t*s^4,
    p=t^15, q_beta=t+beta*t^2+t^25.

Arithmetic is over E(C,V,U)[eps]/eps^2 at beta=eps.  The homogeneous
transport matrix is beta-independent, but its affine right hand side is
not.  The first-J matrix and the genuine current P12 both vary through
q_beta'=1+2*beta*t+25*t^24.  Elimination is performed over the dual ring
...
This is a first-order discriminator on D(U*(C-3U^2)*B3), not a beta-family
or neighborhood theorem.
"""
...
spec = importlib.util.spec_from_file_location("td6_a3_beta_tri_source", TRI_PATH)
...
class EDual:
    """E(C,V,U)[eps]/eps^2, with derivative taken in beta."""
...
def source_vector_beta(key):
    base = r.t.source_vector(key)
    derivative = (
        ONE_VECTOR if key == ("g", "X", 0, 2) else ZERO_VECTOR
    )
...
def propagate_beta(rows, records):
    ...
    for source_row, record in zip(rows, records):
        ...
        base, derivative = source_vector_beta(key)
...
def configure_qd_beta(direct_qprime=True):
    qd = r.qd
    qd.Dual = EDual
    qd.B = EDual(0, 1)
...
def main():
    ...
    print("q2_orbit_class=reviewed_fixed-chart-transverse")
    print("target_shear_absorption=false_by_frozen_orbit_audit")
    print("scope_open=D(U*(C-3U^2)*B3)")
    print("B3_divisor_name_reserved_not_beta=true", flush=True)
    ...
    pivot_rhs, transport_compatibility = propagate_beta(ordered, records)
    ...
    assert len(free) == 132
    transport_beta_rhs_pivots = sum(
        any(derivative) for _, derivative in pivot_rhs.values()
    )
    assert transport_beta_rhs_pivots
    print(f"transport_rank={len(transport_pivots)}/{nf+ng}")
    print(f"transport_event_count={len(events)}")
    print(f"transport_beta_rhs_pivot_count={transport_beta_rhs_pivots}")
    print("transport_matrix_beta_derivative_zero=true")
    print("transport_rhs_beta_derivative_retained=true", flush=True)
    ...
    for owner, imax, jmax, offset in (
        ...
        for exponent in (1, 2, 3):
            ...
            for degree in range(16 if owner == 'f' else 26):
                ...
                forms.append(restrict_row_beta(
                    row, transport_pivots, pivot_rhs, free_parameter
                ))
            bands[(owner, exponent)] = forms
    band_beta_constant_count = sum(
        bool(constant.derivative)
        for forms in bands.values() for constant, _ in forms
    )
    assert band_beta_constant_count
    print(f"transport_section_beta_constant_count={band_beta_constant_count}")

    configure_qd_beta(direct_qprime=True)
    first_rows = r.qd.pack(
        'X-2', r.qd.first_band_polynomials(
            bands[('f', 1)], bands[('g', 1)]
        )
    )
    first_beta_coefficient_count = sum(
        bool(coefficient.derivative)
        for _, row, rhs in first_rows
        for coefficient in list(row.values()) + [rhs]
    )
    assert first_beta_coefficient_count

    configure_qd_beta(direct_qprime=False)
    first_rows_no_direct_qprime = r.qd.pack(
        'X-2', r.qd.first_band_polynomials(
            bands[('f', 1)], bands[('g', 1)]
        )
    )
    first_beta_no_direct = sum(
        bool(coefficient.derivative)
        for _, row, rhs in first_rows_no_direct_qprime
        for coefficient in list(row.values()) + [rhs]
    )
    assert first_rows_no_direct_qprime != first_rows
    assert first_beta_no_direct < first_beta_coefficient_count
    configure_qd_beta(direct_qprime=True)
    print(f"first_beta_coefficient_count={first_beta_coefficient_count}")
    print(f"first_beta_without_direct_qprime_count={first_beta_no_direct}")
    ...
    print(f"first_minor_beta_sha256={e3_digest(first_minor.derivative)}")
    ...
    raw_beta = split_polynomial(polynomial, True)
    ...
    print(f"raw_beta_terms={len(raw_beta)}")
    print(f"raw_base_sha256={polynomial_digest(raw_base)}")
    print(f"raw_beta_sha256={polynomial_digest(raw_beta)}", flush=True)
    ...
    remainder_beta = split_polynomial(remainder, True)
    ...
    print(f"remainder_beta_terms={len(remainder_beta)}")
    print(f"remainder_beta_degree={max(map(len, remainder_beta), default=-1)}")
    print(f"remainder_base_sha256={polynomial_digest(remainder_base)}")
    print(f"remainder_beta_sha256={polynomial_digest(remainder_beta)}")
    ...
    for relation, (_, row, rhs) in zip(relations, first_rows):
        ...
        relation_beta = split_polynomial(relation, True)
        row_base = split_polynomial(source_row)
        row_beta = split_polynomial(source_row, True)
        if relation_base:
        ...
        lambda0_bprime = r.add(
            lambda0_bprime,
            r.multiply(relation_base, row_beta),
        )
        lambdaprime_b0 = r.add(
            lambdaprime_b0,
            r.multiply(relation_beta, row_base),
        )
        ...
        for multiplier in relation.values():
            for source_value in source_row.values():
                ...
                termwise_slots += 2
    derivative_target = r.add(raw_beta, remainder_beta, E3(-1))
