...
`qd.B` must have exact value and derivative zero.  A stale nonzero `qd.B`
must be proven semantically inert after `Q_PRIME` is replaced, while an
actual q2 leak through `Q_PRIME[1]` must change the rows.  Omission of the q3
transport source, omission of direct q-prime, lambda-prime omission, and
original-source-row omission must all be detected.
