...
if [ "$#" -ne 5 ]; then
  echo "usage: run_v27.sh ARCHIVE RUN_DIR PYTHON LANE EXPECTED_ARCHIVE_SHA256" >&2
  exit 64
fi
...
case "$lane" in
  ascending) lane_args=("--stratum=b3-param") ;;
  reverse-first) lane_args=("--stratum=b3-param" --reverse-first) ;;
...
payload="$run_dir/td6-aws-handoff-20260824-v27"
...
script="$payload/jc2/cases/td6_c1_c3_two_center_cover_20260824/c1_c2_c3_trivariate.py"
start_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)
{
  echo "lane=$lane"
  echo "stratum=b3-param"
  echo "host=$(hostname)"
  echo "supervisor_pid=$$"
