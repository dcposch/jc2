...
case "$tag" in
  max12_912_order3_d1_double_root_control2_la20_syzygy_v2_20260826T*) ;;
...
if ! sha256sum -c SOURCE_CLOSURE.sha256 > source.check 2>&1; then
  echo SOURCE_HASH_FAILURE > COMPILE_FAILED; exit 95
fi
export JC2_AWS_TAG="$tag"
(
  ulimit -v 2097152
  nice -n 10 timeout 300 python3 -u compile_la20_syzygy_v2.py
) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
sha256sum la20_syzygy_v2_lpdp.sing > compiled.sha256 2>/dev/null || true
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_CONTROL2_LA20_SYZYGY_V2_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^source_diff=ONE_TAG_REPLACEMENT_PLUS_FOUR_GCD_TO_CDSTD_TOKEN_REPLACEMENTS$' compiler.stdout 2>/dev/null)" -ne 1; then
...
(
  ...
  nice -n 10 timeout --signal=TERM --kill-after=300 "$solve_timeout" \
    /usr/bin/time -v -o singular.time /usr/bin/Singular -q la20_syzygy_v2_lpdp.sing
) > singular.stdout 2> singular.stderr
solver_rc=$?
echo "$solver_rc" > singular.rc
date -u +%FT%TZ > solve_finished_utc.txt
free -h > free.postsolve.txt
grep -En '(^// \*\*|^[[:space:]]*\?|^halt [0-9]+$|error occurred|wrong type|skipping text|outdated identifier)' \
  singular.stdout > singular.stdout.diagnostics || true
if test "$solver_rc" -ne 0 || test -s singular.stderr || \
   test -s singular.stdout.diagnostics || \
   test "$(grep -c '^PASS_CONTROL2_LA20_SYZYGY_LPDP$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_TOY_SATURATION_AND_LIFT$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_SAT_WITH_EXP_DIRECT_EQUALITY$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_H_LIFT$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_USED_C_LIFT$' singular.stdout 2>/dev/null)" -ne 1 || \
...
sha256sum compiler.stdout compiler.stderr compiler.rc la20_syzygy_v2_lpdp.sing \
  singular.stdout singular.stderr singular.stdout.diagnostics singular.time singular.rc > result.sha256
touch SOLVE_DONE
