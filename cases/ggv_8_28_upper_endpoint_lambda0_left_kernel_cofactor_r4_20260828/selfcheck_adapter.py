...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="left-cofactor-selfcheck-") as raw:
        ...
        tail_text = (case_dir / "strata_tail.sing").read_text()
        for fragment in (
            "matrix RAW_MATRIX=matrix(LEFT_MINORS)",
            "raw_slot_count=ncols(RAW_MATRIX)",
            'nonzero_indices!="81,93"',
            "RAW_RANK_DROP_CONDITION=M81_ZERO_AND_M93_ZERO",
            "SET_THEORETIC_DECOMPOSITION=V(RAW_CONTENT)_UNION_V(RESIDUAL_81,RESIDUAL_93)",
        ):
            if fragment not in tail_text:
                raise SystemExit(f"STRATA_ADAPTER_FRAGMENT_MISSING {fragment}")
        if "size(LEFT_MINORS)" in tail_text:
            raise SystemExit("STRATA_IDEAL_SIZE_SEMANTIC_REGRESSION")
        ...
        toy_strata = temp / "toy_strata.sing"
        toy_entries = ["0"] * 106
        toy_entries[80] = "q0*q1"
        toy_entries[92] = "q0*q2"
        toy_strata.write_text(
            "ring P=0,(q0,q1,q2,c4,c6,c8),dp;\n"
            "ideal LEFT_MINORS=" + ",".join(toy_entries) + ";\n" + tail_text
        )
        toy_stdout = run_singular(toy_strata, temp)
        for marker in (
            "RAW_SLOT_COUNT=106",
            "RAW_NONZERO_INDICES=81,93",
            "RAW_NONZERO_INDEX_SET_PASS=1",
            "RAW_CONTENT_UNIT=0",
            "CONTENT_RECOMPOSITION_PASS=1",
            "STRATA_GCD_PASS=1",
        ):
            if marker not in toy_stdout:
                raise SystemExit(f"TINY_STRATA_CONTAINER_FAILURE {marker} {toy_stdout!r}")
    ...
    print("TINY_106_SLOT_STRATA_PASS")
