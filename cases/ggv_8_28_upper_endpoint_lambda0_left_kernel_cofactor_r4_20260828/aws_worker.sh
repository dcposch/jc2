...
case_dir="$source_dir/jc2/cases/ggv_8_28_upper_endpoint_lambda0_left_kernel_cofactor_r4_20260828"
...
for marker in LEFT_KERNEL_REPLAY_ZERO=1 RAW_PARAMETER_DENOMINATORS_ABSENT=1 OMITTED_CALIBRATION=1 ENDPOINT_DUAL_REPLAY_ZERO=1 LEFT_KERNEL_REPLAY_QPARAMS_ZERO=1 ENDPOINT_DUAL_REPLAY_QPARAMS_ZERO=1 OMITTED_CALIBRATION_QPARAMS=1 RAW_ZERO_PATTERN_PRESERVED=1 RAW_SIGNED_MINORS_COUNT=106; do
  grep -q "^${marker}$" "$run_dir/left.stdout" || {
    printf '%s\n' NO_VERDICT_EXACT_REPLAY_DISAGREEMENT >"$output_dir/TERMINAL"
    exit 40
  }
done
[[ -s LEFT_KERNEL_RAW.sing && -s LEFT_KERNEL_RAW.tsv && -s ENDPOINT_DUALS_RAW.tsv && -s LEFT_KERNEL_ZERO_PATTERN.txt ]] || {
  printf '%s\n' NO_VERDICT_MISSING_RAW_CERTIFICATE >"$output_dir/TERMINAL"
  exit 41
}
sha256sum -c "$case_dir/R3_RAW_REFERENCE.sha256" >"$run_dir/r3_raw_replay.stdout" 2>"$run_dir/r3_raw_replay.stderr" || {
  printf '%s\n' NO_VERDICT_R3_RAW_BYTE_DRIFT >"$output_dir/TERMINAL"
  exit 43
}
printf '%s\n' R3_RAW_BYTES_REPLAY_PASS >"$run_dir/R3_RAW_BYTES_REPLAY_PASS"
...
for marker in RAW_NONZERO_INDEX_SET_PASS=1 CONTENT_RECOMPOSITION_PASS=1 STRATA_GCD_PASS=1; do
  grep -q "^${marker}$" "$run_dir/strata.stdout" || {
    printf '%s\n' NO_VERDICT_STRATA_OR_CAP >"$output_dir/TERMINAL"
    exit 42
  }
done
[[ -s COFACTOR_CONTENT.txt && -s FITTING_STRATA_RAW.sing ]] || {
  printf '%s\n' NO_VERDICT_STRATA_OR_CAP >"$output_dir/TERMINAL"
  exit 42
}

sha256sum LEFT_KERNEL_RAW.sing LEFT_KERNEL_RAW.tsv ENDPOINT_DUALS_RAW.tsv \
  LEFT_KERNEL_ZERO_PATTERN.txt \
  R3_RAW_BYTES_REPLAY_PASS COFACTOR_CONTENT.txt FITTING_STRATA_RAW.sing \
  STRATA_GCD_RUN.sing >CERTIFICATE_FILES.sha256
