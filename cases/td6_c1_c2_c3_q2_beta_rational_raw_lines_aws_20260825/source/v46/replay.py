#!/usr/bin/env python3
"""Canonical exact first-band incompatibility on V=0, C=-U^2.
...
def e3_exact(value):
    """Canonical E(C,V,U) coordinates; never use object-address repr."""
...
def canonical(value):
    if isinstance(value, E3):
        return ("E3", e3_exact(value))
    if isinstance(value, BetaPoly):
        return ("BetaPoly", tuple(e3_exact(entry) for entry in value.coefficients))
...
# Canonically serialize every imported stage reporter before calculation.
n.digest = canonical_digest
...
def main():
    ...
    print("certificate_serializer=canonical_E3_coordinate_v1")
    print("legacy_object_address_digest_used=false", flush=True)

    h_center = center_c - 3*center_u**2
    b3_center = (
    ...
    b3_v_zero = 4*C**2*U**2 + 24*C*U**4 + 20*U**6
    assert b3_v_zero == 4*U**2*(C + U**2)*(C + 5*U**2)
    if STRATUM == "v-h-zero":
        assert not center_v and not h_center
    elif STRATUM in {"v-cplus1-zero", "v-cplus5-zero"}:
        assert not center_v and not b3_center
    print("rational_stratum_source_typing_exact=true")
    print("B3_at_V_zero_factorization=4*U^2*(C+U^2)*(C+5*U^2)")
    print("U_zero_component_dependency=separate_frozen_global_empty_theorem", flush=True)
    ...
    assert len(first_pivots) == 37
    bad = [record for record in first_dependent if record[2]]
    assert len(bad) == 1
    residuals, gcd, bezout = n.polynomial_ideal_gcd(
        [residual for _, _, residual, _ in bad]
    )
    assert gcd == BetaPoly(1)
    assert sum(
        (weight*residual for weight, residual in zip(bezout, residuals)),
        BetaPoly(),
    ) == gcd
    assert n.emit_stage_incompatibilities(
        first_dependent, "first_full_beta_P12"
    )
    certificate_values = list(residuals) + list(bezout)
    for _, _, _, combination in bad:
        certificate_values.extend(combination.values())
    certificate_denominator = n.denominator_lcm_beta(certificate_values)
    _, certificate_factors = certificate_denominator.factor()
    assert all(factor == U for factor, _ in certificate_factors)
    print(
        "first_full_beta_P12_unit_certificate_denominator="
        f"({certificate_denominator})"
    )
    print(
        "first_full_beta_P12_unit_certificate_denominator_factor="
        f"{certificate_denominator.factor()}"
    )
    print("first_full_beta_P12_only_U_exception=true")
    print("rational_raw_stratum_first_band_empty=true")
    print("U_zero_dependency=separate_frozen_global_empty_theorem")
    print("full_A3_beta_family_killed=false")
    print("whole_TD6_killed=false")
    print("SP2_killed=false")
    print("JC2_resolved=false")
    print("TD6-A3-Q2-CPLUS1-FIRST-UNIT-INCOMPATIBILITY-CANONICAL PASS")
    return
    ...
    print("SP2_killed=false")
    print("JC2_resolved=false")
    print("TD6-A3-Q2-FULL-P12-N13-UNIT-RATIONAL-RAW-CANONICAL PASS")
