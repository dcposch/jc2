...
for shard_index in $(seq 0 $((SHARD_COUNT - 1))); do
  ...
  python3 "$JC2_ROOT/cases/as_fonly_d7_q9_canonical_signature_census_v3_20260825/compile_shard_v3.py" \
...
pass_count=$(grep -l 'PASS-AS-Q9-CANONICAL-SIGNATURE-V3-SHARD' \
  "$RESULTS_DIR"/shard_*.stdout | wc -l | tr -d ' ')
test "$pass_count" = "$SHARD_COUNT"

python3 "$JC2_ROOT/cases/as_fonly_d7_q9_canonical_signature_census_v3_20260825/aggregate_v3.py" \
  >"$RESULTS_DIR/aggregate.stdout" 2>"$RESULTS_DIR/aggregate.stderr"
test ! -s "$RESULTS_DIR/aggregate.stderr"
grep -q 'PASS-AS-Q9-CANONICAL-SIGNATURE-V3-AGGREGATE' \
  "$RESULTS_DIR/aggregate.stdout"
