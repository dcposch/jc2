...
if [[ "$(uname -s)" != Linux ]]; then
  echo "REFUSED: V84S3 is AWS/Linux only" >&2
  exit 97
fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: V84S3 requires Amazon EC2" >&2
  exit 96
fi
if [[ -z "${AWS_RUN_TAG:-}" || "${AWS_RUN_TAG}" != td6_v84s3_reverse_* ]]; then
  echo "REFUSED: registered V84S3 tag required" >&2
  exit 95
fi
...
sha256sum -c V84S3_SOURCE.sha256
"$PYTHON" replay_v84s3.py
