...
def configure_qd_gamma(direct_qprime=True, leak_q2=False):
    qd = r.qd
    qd.Dual = EDual
    qd.B = EDual(0, 1) if leak_q2 else EDual(0)
...
def main():
    ...
    configure_qd_gamma(direct_qprime=True, leak_q2=True)
    first_rows_q2_leak = r.qd.pack(
        'X-2', r.qd.first_band_polynomials(
            bands[('f', 1)], bands[('g', 1)]
        )
    )
    assert first_rows_q2_leak != first_rows
    configure_qd_gamma(direct_qprime=True)
    ...
    print("q2_B_leakage_negative_control=true", flush=True)
