...
V18 uses the same exact producer for the three corrected B3-local resultant
components.  Select one of

```text
--component=b16   : C=11U^2, V^2=16U^3;
--component=bneg2 : C=-4U^2, V^2=-2U^3;
--component=bq    : C=(Y-11/3)U^2, V^2=YU^3,
                    9Y^2-96Y-128=0.
```

These are substitutions into the exact raw source family, not orbit or
scaling identifications.  For the two rational-Y modes the implementation
audits that every source and certificate coefficient descends out of its
unused auxiliary quadratic constant extension.  Each component meets U=0
only at the already separate origin, but the producer still hard-codes the
whole-curve claim false pending combination with that frozen raw certificate.
