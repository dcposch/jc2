#!/usr/bin/env python3
"""Bounded local preflight for the D43 `K0` field certificate, R2.
...
Runs everything that can honestly be run without AWS and without PARI:
  ...
  P6   the census and observable hashes pinned in execution_pins_r2.json
...
SCHEMA = "d43-k0-field-certificate-preflight-r2"
PACKET_DIRNAME = "d43_k0_field_certificate_r2_20260829"
SEALED_MODULES = ("k0_algebra_r2", "k0_field_checker_r2", "k0_mutations_r2",
                  "runner_r2", "aws_supervisor_r2")
...
CK = sys.modules["k0_field_checker_r2"]
MB = sys.modules["k0_mutations_r2"]
KA = sys.modules["k0_algebra_r2"]
RUN = sys.modules["runner_r2"]
SUP = sys.modules["aws_supervisor_r2"]
...
GP_LABEL_WIDTH_CAP = 80          # runner_r2.GATE_LINE allows \S.{0,80}
...
def _smoke_copy(tmp, mutate):
    ...
    dst = Path(tmp) / "packet"
    dst.mkdir()
    for name in ("PAYLOAD.sha256", "k0_algebra_r2.py",
                 "k0_field_checker_r2.py", "k0_mutations_r2.py"):
...
def isolated_import_controls():
    out = {}
    runner = str(PACKET / "runner_r2.py")
    ...
    r = _run([sys.executable, "-I", "-B", "-c",
              "import k0_field_checker_r2"], cwd=str(PACKET))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        def drop(dst):
            ...
            keep = [l for l in path.read_text().splitlines()
                    if "k0_field_checker_r2.py" not in l]
            path.write_text("\n".join(keep) + "\n")
...
def authority_controls():
    ...
    template = json.loads((PACKET / "authorization_template_r2.json").read_bytes())
    try:
    ...
    # run-directory reuse: exactly the two supervisor files, nothing else
    with tempfile.TemporaryDirectory() as tmp:
        run_dir = Path(tmp) / "run"
        run_dir.mkdir()
        (run_dir / "job_marker.json").write_text("{}")
        (run_dir / "execution_pins_live.json").write_text("{}")
        clean = RUN.check_run_dir_clean(run_dir)
        (run_dir / "terminal.json").write_text("{}")
        try:
            RUN.check_run_dir_clean(run_dir)
...
def gate_surface_controls(root, pins, banks):
    ...
    text = (PACKET / "k0_field_cert_r2.gp").read_text()
...
def main(argv=None):
    ...
    root = Path(args.root)
    pins = json.loads((PACKET / "execution_pins_r2.json").read_bytes())
    ...
    report = {"schema": SCHEMA, "packet": PACKET.name,
              ...
              "gp_not_executed_reason":
                  ...
                  "host by runner_r2.py.  No claim is made that the GP "
                  "syntax or API has run.",
              "aws_executed": False,
              "aws_not_executed_reason":
                  "status is R2_SOURCE_READY_AWS_NOT_AUTHORIZED"}
    ...
    gp_text = (PACKET / "k0_field_cert_r2.gp").read_text()
    ...
    report["P6_pinned_hashes"] = {
        ...
        "gp_script_sha256_matches_pin":
            sha256_path(PACKET / "k0_field_cert_r2.gp")
