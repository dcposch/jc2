#!/usr/bin/env python3
"""Execution runner for the D43 `K0` field certificate, R2.
...
Nothing here decides whether it is allowed to run: ``aws_supervisor_r2.py``
...
exactly that loader and nothing else, which is what ``preflight_r2.py``
...
SCHEMA = "d43-k0-field-certificate-runner-r2"
ROUTE = "D43-K0-FIELD-CERT-R2"
EXPECTED_PACKET_DIRNAME = "d43_k0_field_certificate_r2_20260829"
PACKET_MANIFEST_NAME = "PAYLOAD.sha256"
SEALED_MODULES = ("k0_algebra_r2", "k0_field_checker_r2", "k0_mutations_r2")
GP_SCRIPT_NAME = "k0_field_cert_r2.gp"
...
def load_sealed_siblings(pkt, expect_manifest_sha=None):
    ...
    for name in SEALED_MODULES:
        ...
        # ``import k0_algebra_r2`` resolves out of sys.modules and never
    ...
    globals()["CK"] = sys.modules["k0_field_checker_r2"]
    globals()["MB"] = sys.modules["k0_mutations_r2"]
...
def check_run_dir_clean(run_dir):
    """R2 repair D11: the whole pre-run directory content is pinned.

    R1 listed three artifact names and asked whether each existed, so any
    fourth stale member was invisible.  Here the directory may hold exactly
    the two files the supervisor put there and nothing else.
    """
    present = set(os.listdir(str(run_dir)))
    if not present <= PRE_RUN_FILES:
        fault("RUN_DIR_REUSE",
              "the run directory already holds %s; only %s may be present"
              % (sorted(present - PRE_RUN_FILES), sorted(PRE_RUN_FILES)))
...
def main(argv=None):
    ...
    terminal = {
        ...
        "K0_UNCONDITIONAL_THEOREM": {
            ...
            "engines": ["stdlib Python (k0_field_checker_r2.py)",
                        "PARI/GP 2.15.4 (k0_field_cert_r2.gp)"],
        ...
        "E_CONDITIONAL_RATIO_THEOREM": {
            ...
            "single_engine_reason":
                "k0_field_cert_r2.gp carries no cube-root layer, so it cannot "
