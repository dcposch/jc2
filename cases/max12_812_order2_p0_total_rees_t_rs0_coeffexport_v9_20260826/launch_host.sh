...
v7=cases/max12_812_order2_p0_total_rees_t_rs0_rowshard_v7_20260826
v6=cases/max12_812_order2_p0_total_rees_t_rs0_rowshard_v6_20260826
stream=cases/max12_812_order2_p0_total_rees_t_rs0_stream_v2_20260826
old=cases/max12_812_order2_p0_total_rees_t_rs0_discovery_20260826
tag="max12_812_order2_p0_total_rees_t_rs0_coeffexport_v9_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"
remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t p0-total-rees-t-rs0-coeffexport-v9.XXXXXX.tar.gz)
trap 'rm -f "$tarball"' EXIT
tar -czf "$tarball" "$package" "$v8" "$v7" "$v6" "$stream" \
  "$old/compile_t_rs0_discovery.py" \
...
ssh "${ssh_options[@]}" "$ssh_target" "cd '$remote_root' && test \"\$(sha256sum source.tar.gz | awk '{print \$1}')\" = '$archive_sha' && tar -xzf source.tar.gz && mkdir -p '$remote_job' && setsid -f bash '$package/run_aws.sh' '$remote_root' '$remote_job' '$tag' '$characteristic' 100663296 900 7200 '$archive_sha' </dev/null > '$remote_job.outer.stdout' 2> '$remote_job.outer.stderr'"
