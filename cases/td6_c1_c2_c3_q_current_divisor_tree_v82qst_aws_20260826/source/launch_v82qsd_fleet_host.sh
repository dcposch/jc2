...
shift 5
(( $# > 0 )) || { echo "REFUSED: no exponents" >&2; exit 92; }

if [[ "$(uname -s)" != Linux ]]; then
  echo "REFUSED: V82QST0 fleet requires Linux" >&2
  exit 97
fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: V82QST0 fleet requires Amazon EC2" >&2
  exit 96
fi
...
(
  ...
  for exponent in "$@"; do
    ...
    mkdir -p "$lane/output"
    tag="td6_v82qst0_current_${label}_q${exponent}_${stamp}"
    printf 'hostname=%s\nlabel=%s\ntag=%s\nq_exponent=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=2097152\n' \
    ...
    (
      ...
      /usr/bin/time -v timeout 7200 bash run_v82qsd_current.sh \
