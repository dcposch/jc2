...
v13=cases/max12_812_order2_p0_total_rees_t_rs_rho_unit_v13_20260826
...
tag="max12_812_order2_p0_total_rees_t_rs_rho_unit_v16_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"; remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t p0-total-rees-t-rs-rho-unit-v16.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
COPYFILE_DISABLE=1 tar -czf "$tarball" "$package" "$v14" "$v13" "$v12" "$v9" ops/aws_exact_lane.sh
