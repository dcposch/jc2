...
package=cases/max12_812_order2_p0_total_rees_t_rs_rho_unit_v16_20260826
v14=cases/max12_812_order2_p0_total_rees_t_rs_rho_unit_v14_20260826
mkdir -p "$aws_job/run"
{
  printf 'tag=%s\n' "$tag"; printf 'host=%s\n' "$(hostname)"
...
(cd "$aws_root/$package" && sha256sum -c FREEZE.sha256) > "$aws_job/v16_freeze_check.stdout"
(cd "$aws_root/$v14" && sha256sum -c FREEZE.sha256) > "$aws_job/v14_freeze_check.stdout"
cd "$aws_root"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
set +e
timeout 300 python3 "$package/compile_rho_unit_v16.py" "$aws_job/compiled" --characteristic "$characteristic" --algorithm "$algorithm" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
set +e
python3 "$package/validate_rho_unit_v16.py" --compiler-result "$aws_job/compiled/result.json" --stdout "$stdout" --stderr "$stderr" --characteristic "$characteristic" --algorithm "$algorithm" --output "$aws_job/RESULT.json" > "$aws_job/validator.stdout" 2> "$aws_job/validator.stderr"
validator_rc=$?
set -e
printf 'validator_rc=%s\n' "$validator_rc" > "$aws_job/validator.validation"; if [[ "$validator_rc" -ne 0 ]]; then exit "$validator_rc"; fi
printf 'validator=PASS_T_RS_RHO_UNIT_V16\n' >> "$aws_job/validator.validation"
