...
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/audit_degree1.py" \
  "$case_dir/run_remote.sh" "$MODEL_OUTPUT" > "$JOB_DIR/INPUT.sha256"
ulimit -v "$MEMORY_KIB"
set +e
/usr/bin/time -v timeout --signal=TERM --kill-after=60s "$TIMEOUT_SECONDS" \
  ...
  python3 "$case_dir/audit_degree1.py" \
