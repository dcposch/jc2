...
/usr/bin/time -v timeout 3600 env JC2_ROOT="${JC2_ROOT}" \
  ...
  PARENT_ANF_OUTPUT="${PARENT_ANF_OUTPUT}" \
  PARENT_SMT_OUTPUT="${PARENT_SMT_OUTPUT}" SMT2_OUTPUT="${SMT2_OUTPUT}" \
  BOOLECTOR_SMT_OUTPUT="${BOOLECTOR_SMT_OUTPUT}" \
  python3 "${JC2_ROOT}/cases/as_b9_max12_leading_proportional_3p11_20260825/emit_leading_proportional.py" \
