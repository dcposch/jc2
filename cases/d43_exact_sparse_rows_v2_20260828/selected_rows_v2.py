#!/usr/bin/env python3
"""Exact a00pp D43 source-row producer, revision 2.

This producer collapses HW1=h*W1 and HW2=h*W2 into the 432-term radical
quotient before any expensive jet multiplication.  The B orbits are replaced
by their exact EB-free block before joining the A-side jets.  It emits rows
only; it never solves them.
"""
...
SCHEMA = "jc2.d43.a00pp-exact-source-rows.v2"
MANIFEST_SCHEMA = "jc2.d43.a00pp-exact-source-manifest.v2"
...
def finalize_run(manifest_path: Path, preflight_path: Path,
                 output_dir: Path):
    manifest, manifest_hash = load_manifest(manifest_path)
    assert_operational_registration(manifest)
    verify_operational_sources(manifest)
    _preflight, preflight_hash = validate_preflight_receipt(
        preflight_path, manifest, manifest_hash)
    merge_path = output_dir / "MERGE_RECEIPT.json"
    merge = json.loads(merge_path.read_text())
    assert merge["status"] == "EXACT_A00PP_184_RAW_J_ROWS_MERGED_NO_SOLVE"
    assert merge["manifest_sha256"] == manifest_hash
    assert merge["preflight_receipt_sha256"] == preflight_hash
    gate_path = output_dir / "PILOT_GATE.json"
    gate = json.loads(gate_path.read_text())
    assert gate["status"] == "PASS_CONTINUE_SAME_MANIFEST_ALL184"
    atomic_text(output_dir / "VERDICT",
                "EXACT_A00PP_184_RAW_J_ROWS_EMITTED_NO_SOLVE\n")
    atomic_text(output_dir / "TERMINAL",
                "D43_A00PP_EXACT_ROW_PIPELINE_COMPLETE\n")
    atomic_text(output_dir / "CURRENT_STAGE", "COMPLETE\n")
    terminal = {
        "schema": SCHEMA + ".terminal",
        "status": "COMPLETE_EXACT_EMISSION_NO_SOLVE",
        "manifest_sha256": manifest_hash,
        "preflight_receipt_sha256": preflight_hash,
        "pilot_gate_sha256": sha256_path(gate_path),
        "merge_receipt_sha256": sha256_path(merge_path),
        "verdict": (output_dir / "VERDICT").read_text().strip(),
        "terminal": (output_dir / "TERMINAL").read_text().strip(),
    }
    atomic_json(output_dir / "FINAL_RECEIPT.json", terminal)
    excluded = {"ARTIFACTS.json"}
    entries = {}
    for path in sorted(output_dir.rglob("*")):
        if path.is_file() and path.name not in excluded:
            entries[str(path.relative_to(output_dir))] = sha256_path(path)
    artifact_manifest = {
        "schema": SCHEMA + ".artifacts",
        "status": "ATOMIC_FINAL_INVENTORY_AFTER_TERMINAL",
        "manifest_sha256": manifest_hash,
        "files": entries,
        "root_semantic_sha256": sha256_bytes(canonical_json(entries)),
    }
    atomic_json(output_dir / "ARTIFACTS.json", artifact_manifest)
