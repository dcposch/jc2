...
if [[ $# -ne 3 ]]; then
  echo "usage: run_compile_v2_aws.sh AWS_REPO_ROOT AWS_RUN_DIRECTORY REGISTERED_TAG" >&2
  exit 125
fi
...
exec "$aws_root/ops/aws_exact_lane.sh" \
  ...
  "$aws_root/cases/max12_812_order2_u2_62_strict_rees_20260825/compile_rees_v2.py" \
  "$aws_run/output/compiled_v2"
