...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_disc_triple_k3_receiver_v3_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_disc_triple_k3_receiver_v3_20260826/compile_k3_v3.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
printf 'compiler_rc=%s\n' "$compiler_rc" > "$aws_job/compiler.validation"
if [[ "$compiler_rc" -ne 0 ]]; then exit "$compiler_rc"; fi
input="$aws_job/compiled/disc_triple_k3_v3_p${characteristic}.sing"
...
for required in \
  ...
  'DISC_TRIPLE_K3_V3_SYNTAX_API_REPAIR=1' \
...
printf 'validator=PASS_DISC_TRIPLE_K3_RECEIVER_V3\n' >> "$validation"
