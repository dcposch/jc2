...
job_dir=$1
mode=$2
case_dir="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_q1_fixed_a_full_raw_lambda0_resume_r3_20260828"
worker_pid=""
registered_pid=""
...
setsid timeout --foreground --signal=TERM --kill-after=40s 7200s \
  ...
  "$case_dir/aws_job_worker.sh" "$job_dir" "$mode" \
...
printf 'GGV_Q1_SPLIT_%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
  "$mode" "$worker_pid" "$observed_pgid" "$observed_sid" "$observed_starttime" \
