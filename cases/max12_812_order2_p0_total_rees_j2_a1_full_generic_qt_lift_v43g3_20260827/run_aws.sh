...
package=cases/max12_812_order2_p0_total_rees_j2_a1_full_generic_qt_lift_v43g3_20260827
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_full_generic_qt_lift_v43g3_*_exact_lift_* ]]; then
  exit 126
fi
if [[ "$memory_cap_kib" -ne 402653184 || "$wall_seconds" -ne 7200 ]]; then
  exit 127
fi
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "$compiler_lane" timeout "$(remaining)" \
  python3 "$package/compile_generic_qt_lift_v43g3.py" "$job/compiled"
...
grep -Fx 'V43G3_LIFTSTD_BASIS_REPLAY=1' "$stdout" >/dev/null
grep -Fx 'V43G3_GENERIC_UNIT_REPLAY=1' "$stdout" >/dev/null
grep -Fx 'PASS_A1_GENERIC_QT_LIFT_V43G3' "$stdout" >/dev/null
...
printf 'end_utc=%s\nelapsed_seconds=%s\noutcome=%s\n' \
  ...
  'exact-full-generic-unit-with-replayed-58-row-bezout' \
