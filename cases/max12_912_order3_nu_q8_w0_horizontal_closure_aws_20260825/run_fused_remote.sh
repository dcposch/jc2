...
if [[ $# -ne 7 ]]; then
  echo "usage: run_fused_remote.sh VM_KIB REPO OUT TAG KIND ENGINE ORDER" >&2
  exit 64
fi
...
mkdir -p "$out"
ulimit -v "$vm_kib"
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/SEMANTIC_ERRATUM.md" \
  "$case_dir/README_V2.md" "$case_dir/generate.py" "$case_dir/generate_fused.py" \
  "$case_dir/run_fused_remote.sh" "$case_dir/PREREGISTRATION.manifest.sha256" \
...
python3 "$case_dir/generate_fused.py" --kind "$kind" --engine "$engine" --order "$order" \
...
sha256sum "$out/input.sing" >> "$out/source.sha256"
{
  ...
  echo "virtual_memory_limit_kib=$vm_kib"; echo "kind=$kind"; echo "engine=$engine"; echo "order=$order";
...
test "$(rg -c '^Q8_W0_HORIZONTAL_CLOSURE_FUSED_PASS$' "$out/result.out" || true)" = 1
test "$(rg -c '^STRATUM_BEGIN=' "$out/result.out" || true)" = 4
test "$(rg -c '^source_remainder_zero=1$' "$out/result.out" || true)" = 4
test "$(rg -c '^branch_remainder_zero=1$' "$out/result.out" || true)" = 4
