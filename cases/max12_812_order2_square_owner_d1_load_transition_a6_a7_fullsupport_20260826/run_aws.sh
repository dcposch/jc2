...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_load_transition_a6_a7_fullsupport_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_load_transition_a6_a7_fullsupport_20260826/compile_load_transition.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_load_transition_a6_a7_${label}.sing"
...
printf 'validator=PASS_D1_LOAD_TRANSITION_A6_A7_FULLSUPPORT\n' >> "$validation"
