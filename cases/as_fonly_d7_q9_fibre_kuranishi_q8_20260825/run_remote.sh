...
(
  ...
  /usr/bin/time -v -o "$result_dir/replay.time" \
    ...
    env JC2_ROOT="$root" OUTPUT_JSON="$result_dir/kuranishi_system.json" \
    python3 "$case_dir/compile_fibre_kuranishi.py"
) > "$result_dir/replay.stdout" 2> "$result_dir/replay.stderr"
rc=$?
set -e
printf 'rc=%s\nend_utc=%s\n' "$rc" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  >> "$result_dir/run.meta"
for output in replay.stdout replay.stderr replay.time run.meta \
  kuranishi_system.json; do
  if [ -f "$result_dir/$output" ]; then
    sha256sum "$result_dir/$output"
  fi
done > "$result_dir/OUTPUTS.sha256"
exit "$rc"
