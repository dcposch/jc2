...
printf 'hostname=%s\nlabel=%s\narchive_sha256=%s\nstart_utc=%s\ncap_kib=268435456\ntimeout_seconds=21600\nscope=q14-mod-F-minimal-dual-functional\n' \
  "$(hostname)" "$label" "$expected" "$(date -u +%FT%TZ)" > "$root/registration.meta"
(
  set +e
  lane="$root/dualfunctional"
  mkdir -p "$lane/output"
  tag="td6_v89h7_q14_mod_f_dual_functional_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=21600\nscope=q14-mod-F-minimal-dual-functional\n' \
  ...
  (
    ...
    export TD6_PRIME_SENTINEL=p1000033-C15-V4-U1
    export TD6_FUNCTIONAL=q14-empty-parameter-e3-coordinate0
    export TD6_OUTPUT_DIR="$lane/output"
    /usr/bin/time -v timeout 21600 bash run_v89h7_q14_mod_f_dual_functional.sh \
