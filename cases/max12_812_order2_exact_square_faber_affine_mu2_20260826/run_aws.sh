...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_exact_square_faber_affine_mu2_20260826/FREEZE.sha256 \
  > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
set +e
python3 cases/max12_812_order2_exact_square_faber_affine_mu2_20260826/compile_faber_affine_mu2.py \
...
for required in \
  'FABER_AFFINE_CONNECTIONS=1' \
  'FABER_AFFINE_EXPECTED_SOLUTIONS=1' \
  'FABER_AFFINE_RAD_IN_EXPECTED=1' \
  'FABER_AFFINE_EXPECTED_IN_RAD=1' \
  'FABER_AFFINE_OFF_C_UNIT=1' \
  'FABER_AFFINE_MINASS_COUNT=2' \
  'FABER_AFFINE_ENDPOINT=PASS_SQUARE_UNION_AFFINE_GRAPH' \
  'FABER_AFFINE_PROBE_DONE=1' \
  'FABER_AFFINE_SCOPE=EXACT_SQUARE_ORDINARY_FABER_AFFINE_TARGET_ONLY_NO_REES_ACCESSIBILITY_CORRECTION_TERMINAL_TAYLOR_ORDER2_OR_JC2_VERDICT'
do
...
printf 'validator=PASS_FABER_AFFINE_MU2_PROBE\n' >> "$aws_job/run/$tag.validation"
