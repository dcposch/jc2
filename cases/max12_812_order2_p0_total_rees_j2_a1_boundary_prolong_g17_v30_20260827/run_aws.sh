...
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_compiler" timeout "$compile_wall" python3 "$package/prolong_boundary_g17_v30.py" "$job/compiled" --characteristic "$characteristic"
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_validator" python3 "$package/validate_boundary_g17_v30.py" --result "$job/compiled/result.json" --stdout "$job/run/${tag}_singular.stdout" --stderr "$job/run/${tag}_singular.stderr" --characteristic "$characteristic" --output "$job/RESULT.json"
grep -Fx 'PASS-A1-BOUNDARY-PROLONG-G17-V30-VALIDATOR' "$job/run/${tag}_validator.stdout" >/dev/null
