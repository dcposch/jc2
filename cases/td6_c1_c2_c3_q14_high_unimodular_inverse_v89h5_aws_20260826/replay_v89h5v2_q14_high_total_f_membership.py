#!/usr/bin/env python3
"""Exact q14-positive remainder membership in (F) and total-F lift."""
...
def divide_qpolynomial_by_f(polynomial, inventory_path):
    """Divide coefficientwise in the field and audit localized-ring membership."""
    divisor = Rat3(F)
    quotient = {}
    failures = []
    lines = [
        "parameter_monomial\tq_monomial\tcoordinate\t"
        "original_numerator\toriginal_denominator\t"
        "quotient_numerator\tquotient_denominator\t"
        "quotient_denominator_factor\tF_in_quotient_denominator\t"
        "quotient_denominator_registered"
    ]
    coordinate_count = 0
    for parameter_monomial, value in sorted(polynomial.items()):
        q_coefficients = {}
        for q_monomial, coefficient in sorted(value.coefficients.items()):
            ...
            for coordinate_index, coordinate in enumerate(
                m.r.scalar_coordinates(E3.coerce(coefficient))
            ):
                divided = coordinate / divisor
                assert divided*divisor == coordinate
                denominator = divided.denominator
                f_in_denominator = denominator.gcd(F).total_degree() > 0
                registered = (
                    v85.factors_allowed(denominator)
                    and not f_in_denominator
                )
                lines.append(
                    f"{parameter_monomial!r}\t{q_monomial!r}\t"
                    f"{coordinate_index}\t{coordinate.numerator}\t"
                    f"{coordinate.denominator}\t{divided.numerator}\t"
                    f"{denominator}\t{denominator.factor()!r}\t"
                    f"{str(f_in_denominator).lower()}\t"
                ...
                coordinate_count += 1
            divided_coefficient = m.from_vector(tuple(coordinates))
            if divided_coefficient:
                q_coefficients[q_monomial] = divided_coefficient
        value_quotient = QPoly(0, q_coefficients)
        if value_quotient:
            quotient[parameter_monomial] = value_quotient
    assert v87.scale(
        quotient, QPoly(E3(Rat3(F)))
    ) == v87.clean(polynomial)
    text = "\n".join(lines) + "\n"
...
def main():
    ...
    assert os.environ.get("AWS_RUN_TAG", "").startswith(
        "td6_v89h5v2_q14_high_total_f_membership_"
    )
    print("producer=TD6-V89H5V2-Q14-HIGH-TOTAL-F-MEMBERSHIP")
