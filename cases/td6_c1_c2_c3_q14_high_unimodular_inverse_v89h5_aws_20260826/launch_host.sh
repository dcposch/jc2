...
printf 'hostname=%s\nlabel=%s\narchive_sha256=%s\nstart_utc=%s\ncap_kib=268435456\ntimeout_seconds=43200\nscope=q14-high-unimodular-total-F\n' \
  "$(hostname)" "$label" "$expected" "$(date -u +%FT%TZ)" > "$root/registration.meta"
(
  set +e
  lane="$root/q14unit"
  mkdir -p "$lane/output"
  tag="td6_v89h5_q14_high_unimodular_total_f_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=43200\nscope=q14-high-unimodular-total-F\n' \
  ...
  (
    ...
    export TD6_INVERSE_POLICY=adjugate-scc-condensation
    export TD6_OUTPUT_DIR="$lane/output"
    /usr/bin/time -v timeout 43200 bash run_v89h5_q14_high_unimodular_total_f.sh \
