...
/usr/bin/time -v timeout 1800 Singular verify_degree14.sing \
...
grep -q '^PASS_DEGREE14$' "$outdir/verifier.stdout"
grep -q '^OLD_LAMBDA_V_FORMULA_NONZERO$' "$outdir/verifier.stdout"

printf 'PASS squarefree degree-14 Poisson gate\n' > "$outdir/status.txt"
