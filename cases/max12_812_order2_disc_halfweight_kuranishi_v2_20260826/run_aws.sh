...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_disc_halfweight_kuranishi_v2_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_disc_halfweight_kuranishi_v2_20260826/compile_disc_halfweight_v2.py \
...
input="$aws_job/compiled/disc_halfweight_v2_p${characteristic}.sing"
...
for required in \
  ...
  'DISC_HALF_V2_PER_GENERATOR_CHECK=1' \
...
printf 'validator=PASS_DISC_HALFWEIGHT_V2_SOURCE_CERTIFICATE\n' >> "$validation"
