...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_disc_halfweight_support_v2_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_disc_halfweight_support_v2_20260826/compile_support_v2.py \
...
input="$aws_job/compiled/disc_halfweight_support_v2_p${characteristic}.sing"
...
for required in \
  'DISC_HALF_SUPPORT_V2_SOURCE_HASH=PASS' \
  'DISC_HALF_SUPPORT_V2_K_NONZERO_UNIT=1' \
  'DISC_HALF_SUPPORT_V2_K0_E_NONZERO_UNIT=1' \
  'DISC_HALF_SUPPORT_V2_RADICAL_IN_EXPECTED=1' \
  'DISC_HALF_SUPPORT_V2_EXPECTED_IN_RADICAL=1' \
  'DISC_HALF_SUPPORT_V2_ENDPOINT=PASS_DIRECT_M_SUPPORT_AND_SCHEME'
do
...
printf 'validator=PASS_DISC_HALFWEIGHT_SUPPORT_V2\n' >> "$validation"
