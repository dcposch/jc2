...
/usr/bin/time -v timeout 600 python3 verify_centralizer.py \
  > "$outdir/centralizer.stdout" 2> "$outdir/centralizer.stderr"
grep -q '^PASS_CENTRALIZER$' "$outdir/centralizer.stdout"

/usr/bin/time -v timeout 1800 Singular verify_identities.sing \
  > "$outdir/identities.stdout" 2> "$outdir/identities.stderr"
if grep -Eq '(^[[:space:]]*\?|error at token|skipping text)' \
    "$outdir/identities.stdout" "$outdir/identities.stderr"; then
  echo "REFUSE_SINGULAR_DIAGNOSTIC" >&2
  exit 96
fi
grep -q '^PASS_IDENTITIES$' "$outdir/identities.stdout"

printf 'PASS squarefree Poisson recurrence\n' > "$outdir/status.txt"
