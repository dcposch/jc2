...
sha256sum "$JC2_ROOT/cases/as_fonly_d7_global_q5_h6_pointwise_20260825/audit_q5_h6.py" \
  > "$RESULT_DIR/SOURCE.sha256"
date -u +%Y-%m-%dT%H:%M:%SZ > "$RESULT_DIR/start_utc"
hostname > "$RESULT_DIR/hostname"
set +e
/usr/bin/time -v env JC2_ROOT="$JC2_ROOT" MODEL_OUTPUT="$MODEL_OUTPUT" \
  RESULT_DIR="$RESULT_DIR" OUTPUT_JSON="$RESULT_DIR/result.json" \
  python3 "$JC2_ROOT/cases/as_fonly_d7_global_q5_h6_pointwise_20260825/audit_q5_h6.py" \
