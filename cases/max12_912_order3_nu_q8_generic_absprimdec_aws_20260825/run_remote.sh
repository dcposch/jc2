...
case_dir=$repo_root/cases/max12_912_order3_nu_q8_generic_absprimdec_aws_20260825
lane=$out_root/$tag
if [ -e "$lane" ]; then echo "duplicate lane refused" >&2; exit 3; fi
mkdir -p "$lane"
python3 "$case_dir/generate.py" --engine "$engine" > "$lane/input.sing" 2> "$lane/generator.stderr"
meta=$lane/run.meta
{
  echo "tag=$tag"
  echo "host=$(hostname)"
  ...
  echo "engine=$engine"
  echo "route=direct absolute primary decomposition over Q(w)"
...
set +e
(ulimit -v 268435456; timeout --signal=TERM --kill-after=300 86400 nice -n 9 \
