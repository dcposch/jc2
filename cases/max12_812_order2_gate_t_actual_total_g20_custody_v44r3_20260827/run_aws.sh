...
wall=$6
archive_sha=$7
package=cases/max12_812_order2_gate_t_actual_total_g20_custody_v44r3_20260827
mkdir -p "$job/run"
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_${lane}_validator" \
  timeout "$wall" python3 "$package/validate_frozen_v44r1_v44r3.py" \
  --lane "$lane" --output "$job/RESULT_${lane}.json"
grep -Fx "PASS-ACT-TOT-G20-CUSTODY-V44R3-${lane^^}" "$job/run/${tag}_${lane}_validator.stdout" >/dev/null
if [[ "$mode" == qcross ]]; then
  export JC2_V44R3_CROSS_OUTPUT="$job/RESULT_CROSS.json"
  bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_cross_validator" \
    timeout "$wall" python3 "$package/validate_crosslane_v44r3.py"
  grep -Fx 'PASS-ACT-TOT-G20-CUSTODY-V44R3-CROSSLANE' "$job/run/${tag}_cross_validator.stdout" >/dev/null
fi
