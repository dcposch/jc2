...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only unloaded V8 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 7 ]]; then
  echo "usage: run_unloaded_membership_v8_aws.sh AWS_ROOT AWS_RUN TAG INPUT CAP_KIB TIMEOUT_SECONDS MODE" >&2
  exit 125
fi
aws_root=$1
aws_run=$2
...
v8_rel=cases/max12_812_order2_u2_62_k00_unloaded_membership_v8_20260827
...
python3 "$aws_root/$v8_rel/validate_unloaded_membership_v8.py" \
...
if [[ -f "$artifact_dir/MEMBERSHIP_LIFT.txt" ]]; then
  ...
  sha256sum "$artifact_dir/NONMEMBERSHIP_RESIDUAL.txt" \
    "$artifact_dir/LEADING_TRANSVERSE_CLASS.txt" >> "$aws_run/ENDPOINT_EVIDENCE.sha256"
fi
