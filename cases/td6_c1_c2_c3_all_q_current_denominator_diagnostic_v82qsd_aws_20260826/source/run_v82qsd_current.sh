...
if [[ "$(uname -s)" != Linux ]]; then
  echo "REFUSED: V82QSD requires Linux" >&2
  exit 97
fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: V82QSD requires Amazon EC2" >&2
  exit 96
fi
if [[ -z "${AWS_RUN_TAG:-}" || "${AWS_RUN_TAG}" != td6_v82qsd_current_* ]]; then
  echo "REFUSED: registered V82QSD tag required" >&2
  exit 95
fi
...
sha256sum -c V82QSD_SOURCE.sha256
