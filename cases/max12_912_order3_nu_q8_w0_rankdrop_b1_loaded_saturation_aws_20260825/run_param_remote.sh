...
python3 "$CASE/generate_param.py" --engine "$ENGINE" --order "$ORDER" > "$OUT/input.sing" 2> "$OUT/generator.stderr"
...
sha256sum "$REPO/$CASE/PREREGISTRATION.md" "$REPO/$CASE/PARAM_PREREGISTRATION.md" "$REPO/$CASE/generate_param.py" "$REPO/$CASE/run_param_remote.sh" "$REPO/cases/max12_912_order3_nu_q8_global_quotient_probe_20260824/quotient_compiler.py" "$REPO/cases/max12_912_order3_fibre_20260824/order3_fibre.py" "$OUT/input.sing" > "$OUT/source.sha256"
if [ "$RC" -ne 0 ]; then exit "$RC"; fi
test "$(grep -c Q8_W0_RANKDROP_B1_PARAMETER_SATURATION_PASS "$OUT/stdout")" -eq 1
