...
: "${MEMORY_KIB:=33554432}"
case_dir="$JC2_ROOT/cases/as_fonly_d7_q3_two_level_q2_q1_v3_branches_20260825"
mkdir -p "$JOB_DIR"
hostname > "$JOB_DIR/hostname"
date -u +%FT%TZ > "$JOB_DIR/start_utc"
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/classify_branches.py" \
...
/usr/bin/time -v timeout --signal=TERM --kill-after=30s "$TIMEOUT_SECONDS" \
  env JC2_ROOT="$JC2_ROOT" MODEL_OUTPUT="$MODEL_OUTPUT" \
      ...
      OUTPUT_JSON="$JOB_DIR/branches.json" \
  python3 "$case_dir/classify_branches.py" \
