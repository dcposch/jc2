...
case "$tag" in max12_812_order2_square_d1_a8_d2_loadfirst_dual_*) ;; *) exit 125 ;; esac
...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a8_d2_loadfirst_dual_functional_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
"$aws_root/ops/aws_exact_lane.sh" "$aws_root" "$aws_job/run" "$tag" timeout "$timeout_seconds" \
  bash -lc 'python3 cases/max12_812_order2_square_owner_d1_a8_d2_loadfirst_dual_functional_20260826/compile_a8_d2.py "$1" --characteristic "$2" > "$3" 2> "$4" && for f in "$1"/*.sing; do Singular -q "$f" || exit $?; done' _ \
...
for required in \
  'A8D2_PRIMITIVE_COLUMNS=6' \
  'A8D2_DK60_LOAD_FIRST=1' \
  'A8D2_DK60_EXACT_C_CHART_UNITS=1' \
  'A8D2_VK60_FIRST_TIE=1' \
  'A8D2_VK60_RANK2_OFF_DELTA=1' \
  'A8D2_VK60_DUAL_TARGET_PAIR=1' \
  'A8D2_VK60_EXACT_C_CHART_UNITS=1' \
  'A8D2_ROW2_TARGET_RETAINED=1' \
  'A8D2_ENDPOINT=PASS_EMPTY_A8_D2_CLOSED_R_TAIL'; do
...
printf 'validator=PASS_D1_A8_D2_LOADFIRST_DUAL_EMPTY\n' >> "$validation"
