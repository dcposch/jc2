...
case "$tag" in max12_812_order2_square_r1_d1_ac_symbolic_v2_*) ;; *) exit 125 ;; esac
...
sha256sum -c cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v2_20260826/FREEZE.sha256 > "$base/prelaunch_freeze.stdout"
nohup bash cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v2_20260826/run_aws.sh "$source_root" "$evidence" "$tag" 25165824 "$characteristic" 600 3600 > "$base/outer.stdout" 2> "$base/outer.stderr" < /dev/null &
