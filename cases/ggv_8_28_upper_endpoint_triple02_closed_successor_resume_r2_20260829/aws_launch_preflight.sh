#!/usr/bin/env bash
# Archive-external binding for the closed-successor R2 source packet.
# Frozen (with the archive sidecar) in LAUNCH_MANIFEST.sha256; kept outside
# the source archive to avoid hash self-reference.  R2: requires a nonempty
# schema-checked nonce, verifies its own bytes against the charged launch
# manifest, installs a copy of itself into the job root, and builds the
# atomic content-addressed lease that every later stage and the decision
# record bind to.
set -euo pipefail
...
expected_archive_name=ggv_triple02_closed_successor_resume_r2_SOURCE.tar.gz
expected_archive_sha=183ab5e0139cf6b68482f58ed72e63262f3423c6ffa3f3b5568a014c44a65987
...
allowed = (
    "jc2/cases/ggv_8_28_upper_endpoint_triple02_closed_successor_resume_r2_20260829/",
