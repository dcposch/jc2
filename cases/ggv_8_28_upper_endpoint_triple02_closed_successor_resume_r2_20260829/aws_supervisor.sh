...
# Verify the worker scope actually carries the independent runtime deadline
# and kill semantics; record the exact shown values for the decision layer.
if [[ "$containment_mode" == systemd_scope ]]; then
  ...
  if [[ "$identity_gate" == 1 ]] \
      && systemctl --user show "$scope_unit.scope" \
        ...
        > "$custody/systemd_runtime_limits.txt" 2>&1; then
    if python3 - "$custody/systemd_runtime_limits.txt" \
        "$custody/systemd_runtime_limits.json" <<'PY'
import json
import sys
...
record["verified"] = (
    record["killmode"] == "control-group"
    and bool(record["runtime_max_usec"])
    and record["runtime_max_usec"] != "infinity"
    and record["memory_swap_max"] == "0")
...
printf 'swap_violation=%s\nwhole_timeout=%s\ncontainment_preflight_failure=%s\nlauncher_reap_failure=%s\nsystemd_final_fault=%s\nsystemd_runtime_fault=%s\nmanifest_generator_rc=%s\nmanifest_replay_rc=%s\narchive_replay_rc=%s\narchive_ready=%s\ncontainment_mode=%s\n' \
  ...
  "$archive_ready" "$containment_mode" > "$JOB_ROOT/FINALIZATION_LATCHES.txt"

# C1: the late terminal authority reads only the fresh extraction of the
# frozen archive (still on disk in $archive_replay_dir), re-derives the
# candidate from the archived decision record plus the archived production
# and classifier verdicts, verifies every recorded equality, and binds the
# public marker to the archive digest and the decision-record digest.  The
# live candidate file is never read here.
