#!/usr/bin/env python3
"""Fail-closed independent classifier for the closed-successor recursion (R2).
...
class Classifier:
    ...
    def verify_node_sb(self, node_dir: Path, node_index: int,
        ...
        """The node standard basis is appended once per completed stage."""
        ...
        if had_timeout:
            ...
            return
        unit, count = self.recursor.identical_copies(path.read_bytes())
        if count % header_stage_count != 0 or not unit:
            raise RuntimeError(
                f"NODE_SB_APPEND_CENSUS:{node_index}:{count}:"
                f"{header_stage_count}")
