# TRIPLE02 node-1 closed-successor resume R2: mathematical preregistration

Date: 2026-08-29 (packet authored 2026-08-28)

Status: `SOURCE_READY_AWS_NOT_AUTHORIZED`

R2 keeps the R1 mathematical recursion fixed and repairs only custody: the
witness-replay standard-basis comparison (see the appended-copy law below),
the exact frozen-archive member censuses, and nothing else in this document
changes the mathematics reviewed in R1.
...
**Appended-copy law (R2 repair).**  Singular's `write()` appends, and the
archived node-1 directory ran exactly three stages (reduce, rank_size_6,
saturation) whose shared `node_header` each wrote the standard basis, so the
archived `NODE_001_STANDARD_BASIS.txt` (SHA-256 `bd95508c...640d`, 14,883
bytes) is exactly **three identical concatenated copies** of the
single-stage write (4,961 bytes).  The witness replay produces one copy;
the R1 rule `produced == archived` was therefore unsatisfiable on the live
host, and R2 requires exactly `archived == produced * 3`.  The pivot and
residual TSVs have stage-unique filenames and remain byte-exact
comparisons.  Node standard-basis files of the resumed recursion are
likewise verified as exact whole-number stacks of one repeated unit
(one copy per completed node stage).
