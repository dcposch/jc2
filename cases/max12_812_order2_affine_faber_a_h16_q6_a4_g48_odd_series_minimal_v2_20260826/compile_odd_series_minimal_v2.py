#!/usr/bin/env python3
"""Emit only the grade-48 moving odd connection from rows 1, 3, and 7."""
...
def require_aws() -> str:
    if platform.system() != "Linux":
        fail("AWS-only minimal odd-series compiler refused non-Linux host")
    vendor = Path("/sys/class/dmi/id/sys_vendor")
    if not vendor.is_file() or vendor.read_text().strip() != "Amazon EC2":
        fail("AWS-only minimal odd-series compiler refused non-Amazon host")
...
def main() -> None:
    ...
    target = output / f"affine_faber_a_h16_q6_a4_g48_odd_series_minimal_v2_{label}.sing"
    ...
    if source.count(anchor) != 1:
        fail({"anchor_count": source.count(anchor)})
    ...
    tail = [
        ...
        'print("A_H16Q6A4_ODD_MIN_V2_TCOEFF_CONTROL="+string(tcControl));',
        'print("A_H16Q6A4_ODD_MIN_V2_48="+string(ODD48));',
        'print("A_H16Q6A4_ODD_MIN_V2_ENDPOINT=GRADE48_EMITTED");',
        'print("A_H16Q6A4_ODD_MIN_V2_DONE=1");',
        'print("A_H16Q6A4_ODD_MIN_V2_SCOPE=ROWS1_3_7_COMPLETE_SOURCE_RAW_FUNCTIONAL_NO_PREDECESSOR_REDUCTION_RATIONAL_REGRADING_REES_ORDER2_MAX12_OR_JC2_VERDICT");',
        "quit;",
    ]
    target.write_text("\n".join(kept + tail) + "\n")
    ...
    if set(controls.values()) != {1}:
        fail({"retained_row_counts": controls})
    payload = {
        "status": "PASS-A-H16-Q6-A4-G48-ODD-SERIES-MINIMAL-V2-COMPILER",
        "registered_aws_lane": tag,
        "characteristic": args.characteristic,
        "parent_sha256": digest(PARENT),
        "base_sha256": digest(parent.BASE),
        "tails_sha256": digest(base.TAILS),
        "retained_row_counts": controls,
    }
