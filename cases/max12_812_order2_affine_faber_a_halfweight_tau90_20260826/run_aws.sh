...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_affine_faber_a_halfweight_tau90_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
python3 cases/max12_812_order2_affine_faber_a_halfweight_tau90_20260826/compile_halfweight_tau90.py "$compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
...
printf 'validator=PASS_A_HALFWEIGHT_TAU90_COMPLETE_SOURCE_UNIT\n' >> "$aws_job/run/$tag.validation"
