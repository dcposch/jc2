...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only D9 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 7 ]]; then
  echo "usage: run_D9_aws.sh AWS_ROOT AWS_JOB TAG FIELD CAP_KIB TIMEOUT_SECONDS FREEZE" >&2
  exit 125
fi
aws_root=$1
aws_job=$2
...
timeout_seconds=$6
freeze=$7
rel=cases/max12_812_order2_u2_62_k00_filtered_macaulay_d9_v15_20260827
...
ulimit -v "$cap_kib"
python3 "$rel/emit_D9.py" "$aws_job/output/emitted" \
...
"$aws_root/ops/aws_exact_lane.sh" \
  ...
  "$aws_job/output/emitted/macaulay_D9.tsv" "$aws_job/output/solution_D9.tsv"
rc=$?
set -e
...
python3 "$rel/validate_D9.py" "$aws_job/run/RESULT.json" \
  "$aws_job/output/emitted/SOURCE_AUDIT.json" "$aws_job/output/emitted/macaulay_D9.tsv" \
  "$aws_job/output/solution_D9.tsv" --field "$field" \
  > "$aws_job/run/validator.stdout" 2> "$aws_job/run/validator.stderr"
printf 'validator=PASS_FILTERED_D9\n' >> "$aws_job/run/FINAL.validation"
