...
package=cases/max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_rebased_factor_v43c4_20260827
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_rebased_factor_v43c4_* ]]; then exit 126; fi
...
lane="${tag}_exact_derivation"
(
  ulimit -v "$memory_cap_kib"
  bash ops/aws_exact_lane.sh "$root" "$job/run" "$lane" timeout "$wall_seconds" \
    python3 "$package/replay_constructive_circuit_rebased_factor_v43c4.py" "$job/output"
)
stdout="$job/run/${lane}.stdout"
grep -Fx 'PASS-A1-RHO0-RAW-CONSTRUCTIVE-CIRCUIT-REBASED-FACTOR-A1-104-V43C4' "$stdout" >/dev/null
...
printf 'end_utc=%s\noutcome=exact-rebased-factor-a1-104-derivation-DAG-replayed\n' \
