...
case_dir="$JC2_ROOT/cases/as_fonly_d7_global_q5_q4_cartier_fitting_v3_20260825"
mkdir -p "$JOB_DIR"
hostname > "$JOB_DIR/hostname"
date -u +%FT%TZ > "$JOB_DIR/start_utc"
sha256sum "$case_dir/PREREGISTRATION.md" \
  "$case_dir/analyze_cartier_fitting_v3.py" "$case_dir/run_remote.sh" \
  "$JC2_ROOT/cases/as_fonly_d7_global_q5_q4_cartier_fitting_20260825/analyze_cartier_fitting.py" \
  "$JC2_ROOT/cases/as_fonly_d7_global_q5_q4_cartier_20260825/solve_global_q5_q4_cartier.py" \
  "$JC2_ROOT/cases/as_fonly_d7_global_q5_q4_cartier_20260825/compatible_bases.tsv" \
...
/usr/bin/time -v timeout --signal=TERM --kill-after=60s "$TIMEOUT_SECONDS" \
  ...
  python3 "$case_dir/analyze_cartier_fitting_v3.py" \
