#!/usr/bin/env python3
"""AWS-only dependency-degree acceleration of frozen row-streaming T-rs-0."""
...
def require_aws() -> str:
    ...
    if (
        ...
        or not tag.startswith("max12_812_order2_p0_total_rees_t_rs0_degscan_v3_")
    ):
        fail("degscan compiler requires a registered AWS EC2 lane")
    return tag
...
def main() -> None:
    ...
    for name in names:
        ...
        if count != 7:
            fail(("dependency statement count", name, count, 7))
        text = text.replace(before, after)
        replacement_count += count
    if replacement_count != 581 or "if (subst(TPhi," in text:
        fail(("dependency rewrite census", replacement_count))
