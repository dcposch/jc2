...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a9_k60_zero_grade28_29_collision_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_a9_k60_zero_grade28_29_collision_20260826/compile_a9_k60_zero_collision.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_a9_k60_zero_grade28_29_${label}.sing"
sha256sum "$input" "$aws_job/compiled/result.json" "$aws_job/compiled/parent_v2_result.json" "$aws_job/compiled/parent_v1_result.json" "$aws_job/compiled/parent_v3_result.json" > "$aws_job/compiled.sha256"
for required in \
  ...
  'D1A9_VK60_LITERAL_TARGET_J_COUNT=1'; do
...
printf 'validator=PASS_D1_A9_VK60_GRADE28_29_TWO_STRATA\n' >> "$validation"
