...
case "$tag" in max12_812_order2_square_d1_highcontact_c2_shadow_*) ;; *) exit 125 ;; esac
...
sha256sum -c cases/max12_812_order2_square_owner_d1_highcontact_c2_target_shadow_20260826/FREEZE.sha256 > "$base/prelaunch_freeze.stdout"
nohup bash cases/max12_812_order2_square_owner_d1_highcontact_c2_target_shadow_20260826/run_aws.sh "$source_root" "$evidence" "$tag" 33554432 "$characteristic" 1200 3600 > "$base/outer.stdout" 2> "$base/outer.stderr" < /dev/null &
