...
case "$tag" in max12_912_order3_d1_triple_root_weighted_two_chart_v10_20260826T*) ;; *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;; esac
case "$order" in forward|reverse) ;; *) echo REFUSE_ORDER > REFUSED; exit 94 ;; esac
{
  echo "pid=$$"; echo "tag=$tag"; echo "order=$order"
  echo "memory_kib=8388608"; echo "timeout=1800"; echo WAITING_GO
} > worker.metadata
...
( ulimit -v 8388608; nice -n 10 timeout 1800 /usr/bin/time -v -o compiler.time \
  python3 -u compile_weighted_two_chart_v10.py --order "$order" --output weighted_two_chart.json \
...
if test "$rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -Ec '^PASS_WEIGHTED_TWO_CHART_V10$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_OR_DIAGNOSTIC_FAILURE > FAILED; exit 96
fi
sha256sum weighted_two_chart.json compiler.stdout compiler.stderr compiler.rc compiler.time > result.sha256
touch DONE
