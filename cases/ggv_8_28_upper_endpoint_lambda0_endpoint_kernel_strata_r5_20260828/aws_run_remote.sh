...
job_tag=$2
lane=$3
case_dir="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_lambda0_endpoint_kernel_strata_r5_20260828"
...
runtime_compiler="$job_dir/runtime/compiler_r5_case/build_strata_scripts.py"
runtime_worker="$job_dir/runtime/aws_worker_r5.sh"
worker_pid=""
registered_pid=""
...
python3 -B "$case_dir/build_compiler_r5.py" \
...
grep -q '^R5_COMPILER_QRING_SYMBOL_FIX_PASS$' "$job_dir/records/COMPILER_BUILD.stdout"
sha256sum "$runtime_compiler" >"$job_dir/records/RUNTIME_COMPILER.sha256"
grep -q '^575f0f34065bf06b76311a51fa71622d42d2e9a75fbb85979f79d9da97f3c06e ' \
  "$job_dir/records/RUNTIME_COMPILER.sha256"
python3 -B "$case_dir/build_worker_r5.py" --r2-worker "$r2_case/aws_worker.sh" \
  --output "$runtime_worker" >"$job_dir/records/WORKER_BUILD.stdout" \
  2>"$job_dir/records/WORKER_BUILD.stderr"
grep -q '^R5_WORKER_NARROW_TRANSFORM_PASS$' "$job_dir/records/WORKER_BUILD.stdout"
sha256sum "$runtime_worker" >"$job_dir/records/RUNTIME_WORKER.sha256"
grep -q '^8acf14af1b27476ff8a9134f9ad9e40d3a1a260963cae24835fe33632ac35256 ' \
...
printf 'GGV_ENDPOINT_STRATA_R5_%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$lane" \
