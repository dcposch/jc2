...
meta=$lane/run.meta
set +e
python3 "$case_dir/generate_fixed.py" --samples "$samples" --order "$order" \
...
set -e
{
  ...
  echo "method=fixed-base-jacobian"
  echo "generator_rc=$generator_rc"
  Singular --version | head -1 | sed 's/^/singular=/'
  sha256sum "$0" "$case_dir/generate_fixed.py" "$case_dir/generate.py" "$samples" \
...
set +e
(ulimit -v 67108864; timeout --signal=TERM --kill-after=180 7200 nice -n 5 \
