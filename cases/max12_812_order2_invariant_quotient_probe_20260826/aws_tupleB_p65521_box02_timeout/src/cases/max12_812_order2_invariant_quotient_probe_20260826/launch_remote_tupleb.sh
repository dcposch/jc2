...
mkdir -p "$aws_job/run"
{
  ...
  printf 'characteristic=65521\n'
  printf 'loads=17,19,23,29,31,37\n'
} > "$aws_job/launch_registration.txt"
printf '%s\n' "$$" > "$aws_job/launcher.pid"
cd "$aws_root"
sha256sum -c "$freeze" > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
python3 cases/max12_812_order2_invariant_quotient_probe_20260826/compile_probe_tupleb.py \
  "$aws_job/compiled" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
input="$aws_job/compiled/order2_invariant_quotient_p65521.sing"
