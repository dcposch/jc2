...
job_tag=$2
lane=$3
case_dir="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_lambda0_endpoint_kernel_strata_r6_20260828"
r2_case="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_lambda0_endpoint_kernel_strata_r2_20260828"
...
runtime_compiler="$job_dir/runtime/compiler_r6_case/build_strata_scripts.py"
runtime_worker="$job_dir/runtime/aws_worker_r6.sh"
worker_pid=""
registered_pid=""
...
mkdir -p "$job_dir/output" "$job_dir/records" "$(dirname "$runtime_compiler")"
python3 -B "$case_dir/build_compiler_r6.py" \
  --r2-compiler "$r2_case/build_strata_scripts.py" --output "$runtime_compiler" \
  >"$job_dir/records/COMPILER_BUILD.stdout" 2>"$job_dir/records/COMPILER_BUILD.stderr"
grep -q '^R6_COMPILER_AMBIENT_QIDEAL_GUARD_PASS$' "$job_dir/records/COMPILER_BUILD.stdout"
sha256sum "$runtime_compiler" >"$job_dir/records/RUNTIME_COMPILER.sha256"
grep -q '^a5587996f3d9de7049fe5de790d9bd16a53fa389732016bc4642e2c02a7d0d11 ' \
  "$job_dir/records/RUNTIME_COMPILER.sha256"
python3 -B "$case_dir/build_worker_r6.py" --r2-worker "$r2_case/aws_worker.sh" \
  --output "$runtime_worker" >"$job_dir/records/WORKER_BUILD.stdout" \
  2>"$job_dir/records/WORKER_BUILD.stderr"
grep -q '^R6_WORKER_NARROW_TRANSFORM_PASS$' "$job_dir/records/WORKER_BUILD.stdout"
sha256sum "$runtime_worker" >"$job_dir/records/RUNTIME_WORKER.sha256"
grep -q '^e6f3ee2772bb3f6eb3d362c134769fc179414b2d7833f9174e1ad41949e31706 ' \
  "$job_dir/records/RUNTIME_WORKER.sha256"
...
setsid timeout --foreground --signal=TERM --kill-after=40s 7200s \
  env AWS_RUN_TAG="$job_tag" AWS_RUN_LANE="$lane" \
  bash "$runtime_worker" "$job_dir" "$lane" \
...
printf 'GGV_ENDPOINT_STRATA_R6_%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$lane" \
...
python3 "$guard" snapshot --registry "$job_dir/records/GROUP.tsv" \
...
find "$job_dir/output" "$job_dir/records" "$job_dir/runtime" \
  -type f ! -name EVIDENCE.sha256 -print0 | sort -z | xargs -0 sha256sum \
