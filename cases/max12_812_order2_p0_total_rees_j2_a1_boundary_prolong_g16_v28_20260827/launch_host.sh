...
tag="max12_812_order2_p0_total_rees_j2_a1_boundary_prolong_g16_v28_${stamp}_${suffix}"
root="/home/ubuntu/jobs/$tag/source"; job="$root/$package/aws_${suffix}"
tarball=$(mktemp -t j2-a1-boundary-prolong-g16-v28.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
COPYFILE_DISABLE=1 tar -czf "$tarball" "$package" "$v20/export_allrows_g13_g14_v20.py" "$v23/census_j2_typed_v23.py" "$v23/output_r1" "$replay" "$tails" ops/aws_exact_lane.sh
