...
package=cases/max12_812_order2_p0_total_rees_j2_a1_boundary_prolong_g16_v28_20260827
mkdir -p "$job/run"
...
ulimit -v "$cap"
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_compiler" timeout "$compile_wall" python3 "$package/prolong_boundary_g16_v28.py" "$job/compiled" --characteristic "$characteristic"
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_validator" python3 "$package/validate_boundary_g16_v28.py" --result "$job/compiled/result.json" --stdout "$job/run/${tag}_singular.stdout" --stderr "$job/run/${tag}_singular.stderr" --characteristic "$characteristic" --output "$job/RESULT.json"
grep -Fx 'PASS-A1-BOUNDARY-PROLONG-G16-V28-VALIDATOR' "$job/run/${tag}_validator.stdout" >/dev/null
