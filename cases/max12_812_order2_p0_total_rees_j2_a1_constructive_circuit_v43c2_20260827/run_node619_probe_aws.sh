...
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_v43c2_*_node619_probe_* ]]; then exit 126; fi
...
lane="${tag}_exact_factor_probe"
(
  ulimit -v "$memory_cap_kib"
  bash ops/aws_exact_lane.sh "$root" "$job/run" "$lane" timeout "$wall_seconds" \
    python3 "$package/probe_node619_v43c2.py" "$job/output"
)
stdout="$job/run/${lane}.stdout"
grep -Fx 'PASS-V43C2-NODE619-EXACT-NONZERO-DIAGNOSTIC' "$stdout" >/dev/null
if grep -ERq 'Traceback|FAIL_|Killed|out of memory|error occurred' "$job/run"; then exit 91; fi
cp "$job/output/node619_probe.json" "$job/RESULT.json"
printf 'end_utc=%s\noutcome=exact-node619-nonzero-obstruction\n' \
