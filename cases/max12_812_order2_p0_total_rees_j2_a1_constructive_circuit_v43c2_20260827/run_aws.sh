...
package=cases/max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_v43c2_20260827
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_v43c2_* ]]; then exit 126; fi
if [[ "$memory_cap_kib" -ne 16777216 || "$wall_seconds" -ne 3600 ]]; then exit 127; fi
...
(
  ...
  bash ops/aws_exact_lane.sh "$root" "$job/run" "$lane" timeout "$wall_seconds" \
    python3 "$package/replay_constructive_circuit_v43c2.py" "$job/output"
)
stdout="$job/run/${lane}.stdout"
grep -Fx 'PASS-A1-RHO0-RAW-CONSTRUCTIVE-CIRCUIT-A1-104-V43C2' "$stdout" >/dev/null
...
printf 'end_utc=%s\noutcome=exact-a1-104-derivation-DAG-replayed\n' \
