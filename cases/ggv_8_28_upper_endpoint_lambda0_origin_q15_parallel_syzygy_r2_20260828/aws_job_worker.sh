...
case_dir="$source_dir/jc2/cases/ggv_8_28_upper_endpoint_lambda0_origin_q15_parallel_syzygy_r2_20260828"
...
run_bounded() {
  ...
  if [[ "$bounded_rc" -ne 0 && "$bounded_rc" -ne 124 ]]; then
    printf '%s\n' ADAPTER_FAILURE_BOUNDED_SOLVER_ERROR >"$output_dir/VERDICT"
    printf '%s\n' NO_VERDICT_BOUNDED_SOLVER_ERROR >"$output_dir/TERMINAL"
    exit 37
  fi
}
...
case "$job_type" in
  syzygy_g15_target_d7_p65521)
    run_bounded SPARSE_SYZYGY_G15_D7_P65521 1200 \
      ...
      python3 -B search_sparse_syzygy.py --dedup "$output_dir/compiled/DEDUP.json" \
        ...
        --output "$output_dir/run/CERTIFICATE.json"
  ...
  syzygy_full_active_d7_p65519)
    run_bounded SPARSE_SYZYGY_FULL_D7_P65519 2400 \
      "$output_dir/run/time.txt" "$output_dir/run/stdout.txt" "$output_dir/run/stderr.txt" \
      python3 -B search_sparse_syzygy.py --dedup "$output_dir/compiled/DEDUP.json" \
  ...
  singular_g15_core_block_std_p65497)
    ...
    run_bounded G15_CORE_BLOCK_STD_P65497 1800 "$output_dir/run/time.txt" "$output_dir/run/stdout.txt" "$output_dir/run/stderr.txt" \
      Singular -q "$output_dir/compiled/${stem}.sing"
  ...
  singular_full_core_block_slimgb_p65521)
    ...
    stage PARSE_SELECTED_ORDER 60 "$output_dir/run/parse.time" "$output_dir/run/parse.stdout" "$output_dir/run/parse.stderr" \
      Singular -q "$output_dir/compiled/${stem}_parse.sing" || {
        printf '%s\n' NO_VERDICT_ORDER_PARSE >"$output_dir/TERMINAL"; exit 36; }
    run_bounded FULL_CORE_BLOCK_SLIMGB_P65521 1800 "$output_dir/run/time.txt" "$output_dir/run/stdout.txt" "$output_dir/run/stderr.txt" \
      Singular -q "$output_dir/compiled/${stem}.sing"
    grep -q 'TARGET_NF_ZERO=1' "$output_dir/run/stdout.txt" && verdict=MODULAR_TARGET_MEMBERSHIP_SIGNAL
    ;;
  singular_full_reverse_dp_slimgb_p65519)
    ...
    stage PARSE_SELECTED_ORDER 60 "$output_dir/run/parse.time" "$output_dir/run/parse.stdout" "$output_dir/run/parse.stderr" \
      Singular -q "$output_dir/compiled/${stem}_parse.sing" || {
        printf '%s\n' NO_VERDICT_ORDER_PARSE >"$output_dir/TERMINAL"; exit 36; }
    run_bounded FULL_REVERSE_DP_SLIMGB_P65519 1800 "$output_dir/run/time.txt" "$output_dir/run/stdout.txt" "$output_dir/run/stderr.txt" \
      Singular -q "$output_dir/compiled/${stem}.sing"
    grep -q 'TARGET_NF_ZERO=1' "$output_dir/run/stdout.txt" && verdict=MODULAR_TARGET_MEMBERSHIP_SIGNAL
    ;;
  msolve_full_endpoint_p65497)
    run_bounded MSOLVE_FULL_ENDPOINT_P65497 1800 "$output_dir/run/time.txt" "$output_dir/run/stdout.txt" "$output_dir/run/stderr.txt" \
      msolve -f "$output_dir/compiled/msolve_full_endpoint_p65497.in" \
        -o "$output_dir/run/msolve.out" -t 1 -v 1 -g 1
