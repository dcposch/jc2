#!/usr/bin/env python3
"""Bounded total-degree modular endpoint-syzygy search with exact replay."""
...
def degree(poly):
    assert poly
    return max(len(monomial) for monomial in poly)
...
def monomials_upto(variables, degree_value):
    for current_degree in range(degree_value + 1):
        yield from monomials(variables, current_degree)
...
def main():
    ...
    target_max_degree = degree(target)
    assert target_max_degree <= args.total_degree
    ...
    for generator_index, item in enumerate(generators):
        ...
        multiplier_max_degree = args.total_degree - degree(poly)
        if multiplier_max_degree < 0:
            continue
        planned_column_count += math.comb(
            len(variables) + multiplier_max_degree, multiplier_max_degree
        )
        generator_data.append((generator_index, item, poly, multiplier_max_degree))
    if planned_column_count > args.max_columns:
        raise RuntimeError(
            f"column cap exceeded: {planned_column_count} > {args.max_columns}"
        )
    if args.plan_only:
        plan = {
            "status": "TOTAL_DEGREE_PLAN_PASS",
            "subset": args.subset,
            "prime": args.prime,
            "total_degree": args.total_degree,
            "target_max_degree": target_max_degree,
            "target_term_degrees": sorted({len(monomial) for monomial in target}),
            "multiplier_variable_profile": args.multiplier_vars,
            "multiplier_variable_count": len(variables),
            "generator_max_degrees": [degree(poly) for _, _, poly, _ in generator_data],
    ...
    columns = []
    column_polys = []
    for generator_index, item, poly, multiplier_max_degree in generator_data:
        for monomial in monomials_upto(variables, multiplier_max_degree):
            columns.append({
    ...
    assert len(columns) == planned_column_count
    ...
    result = {
        ...
        "scope": {
            ...
            "inhomogeneous_total_degree_macaulay_space": True,
