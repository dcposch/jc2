...
def load_parser():
    ...
    spec = importlib.util.spec_from_file_location("v44r1_validator_parser", PARSER)
...
def main() -> None:
    ...
    cli.add_argument("--compiler-sha256", required=True)
    ...
    if (
        ...
        or result.get("compiler_sha256") != args.compiler_sha256
    ...
    if set(result.get("terminal_source_variables", [])) != expected_terminal:
        raise RuntimeError("terminal source variables")
    receiver_rows = result.get("terminal_receiver_rows", {})
    if set(receiver_rows) != expected_terminal or any(receiver_rows.values()):
        raise RuntimeError(("terminal receiver-free census", receiver_rows))
    ...
    print("PASS-ACT-TOT-G20-CUSTODY-V44R1-VALIDATOR")
