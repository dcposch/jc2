...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_disc_halfweight_support_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_disc_halfweight_support_20260826/compile_support.py \
...
input="$aws_job/compiled/disc_halfweight_support_p${characteristic}.sing"
...
for required in \
  'DISC_HALF_SUPPORT_SOURCE_HASH=PASS' \
  'DISC_HALF_SUPPORT_COMB6_ZERO=1' \
  'DISC_HALF_SUPPORT_COMB7_ZERO=1' \
  'DISC_HALF_SUPPORT_K0_E_NONZERO_UNIT=1' \
  'DISC_HALF_SUPPORT_ENDPOINT=PASS_NAVIGATION_ONLY'
do
...
printf 'validator=PASS_DISC_HALFWEIGHT_SUPPORT_NAVIGATION\n' >> "$validation"
