...
case "$order" in 256|512|1024) ;; *) exit 2 ;; esac
case_dir=$repo_root/cases/max12_912_order3_nu_q8_p127_hensel_high_order_aws_20260825
low_dir=$repo_root/cases/max12_912_order3_nu_q8_p127_hensel_coordinate_lift_aws_20260825
lane=$out_root/$tag
if [ -e "$lane" ]; then echo "duplicate lane refused" >&2; exit 3; fi
mkdir -p "$lane"
meta=$lane/run.meta
set +e
python3 "$case_dir/generate_high.py" --samples "$samples" --order "$order" \
...
set -e
{
  ...
  echo "method=coefficient-wise-moving-v-basis-high-order-wrapper"
  ...
  sha256sum "$0" "$case_dir/generate_high.py" \
    "$low_dir/generate_moving.py" "$low_dir/generate.py" "$samples" \
...
set +e
(ulimit -v 67108864; timeout --signal=TERM --kill-after=300 14400 nice -n 5 \
