...
if [[ $# -ne 6 ]]; then
  echo "usage: run_v2_remote.sh VM_KIB REPO OUT TAG ENGINE ORDER" >&2
  exit 64
fi
...
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/generate.py" \
  "$case_dir/generate_v2.py" "$case_dir/run_v2_remote.sh" \
  "$case_dir/V2.manifest.sha256" \
...
python3 "$case_dir/generate_v2.py" --engine "$engine" --order "$order" \
...
for marker in L24_F24_ideal_identity=1 F24_root_ideal_identity=1 F24_vdim=2 \
  branch0_linear_ideal_identity=1 branch0_L7=0 \
