#!/usr/bin/env python3
"""AWS-only custody supervisor for the D43 `K0` field certificate, R4.
...
* D8  ``reviewer_pass.verdict`` must be exactly the string ``"PASS"``, and the
  ...
  :func:`validate_authorization_record`, which ``preflight_r4.py`` exercises
...
* the per-run working directory is created by root (``ExecStartPre=+`` running
  ``mint_claim_r4.sh``) and chowned to the service user, so the unprivileged
...
* minting is one-shot outside this process: ``mint_claim_r4.sh`` creates
...
non-root user.  ``preflight_r4.py`` P11 pins the production construction
...
AUTH_SCHEMA = "d43-k0-field-certificate-launch-auth-r4"
MARKER_SCHEMA = "d43-k0-field-certificate-job-marker-r4"
TERMINAL_SCHEMA = "d43-k0-field-certificate-aws-terminal-r4"
...
class CustodyPolicy:
    ...
    with the literal custodian uid ``0``; ``preflight_r4.py`` P11 pins that
...
def validate_authorization_record(record):
    ...
    Raises :class:`CustodyFault`.  ``preflight_r4.py`` calls this directly on a
...
def require_service_owned_run_dir(policy, run_path):
    ...
    if st is None:
        fault("RUN_DIR_ABSENT",
              ...
              "(ExecStartPre=+ mint_claim_r4.sh); the service never creates "
...
def verify_claim_token(policy, run_dir, auth_sha, record):
    ...
    enforced by ``mint_claim_r4.sh``, which runs as root outside this process.
    """
...
def verify_custody_preconditions(policy, record, auth_sha):
    ...
    nothing.  ``custody_selftest_r4.py --on-host-dry-run`` calls exactly this
...
def stage_packet(policy, authorization, run_dir):
    ...
    Factored out of :func:`run_job` in R3 so that ``preflight_r4.py`` P11 can
...
def run_job(authorization_path):
    ...
    try:
        ...
        pins_path = packet / "execution_pins_r4.json"
        ...
        argv = [sys.executable, "-I", "-B", str(packet / "runner_r4.py"),
...
def main(argv=None):
    ...
    if not args.i_understand_this_is_aws_only:
        sys.stderr.write(
            ...
            "R4_SOURCE_READY_AWS_NOT_AUTHORIZED; a different-model source "
