#!/usr/bin/env python3
"""Deterministic sealer for the D43 `K0` field certificate packet, R4.
...
R2 added three external members to the sealed archive: the hashed E5/E6 bridge
that ``E_LITERAL_PIN`` parses, the Grok 4.6 theorem review the unconditional
layer's prose steps rest on, and the Fable 5 R1 source review that packet
repaired.  R3 added a fourth, the Fable 5 R2 source review whose REPAIR-1 that
packet answered, and R4 adds a fifth, the Grok 4.6 R3 source review whose
REPAIR-1 (the mint/dry-run/start choreography) this packet answers.  All five
are pinned in ``execution_pins_r4.json`` and checked by ``SRC_FILE_HASHES``.
"""
...
PACKET_REL = "cases/d43_k0_field_certificate_r4_20260829"
EXTERNAL_MEMBERS = (
