#!/bin/bash
# Root-side retirement for the D43 K0 field certificate, R4.
#
#   retire_claim_r4.sh <run-directory>
#
# Invoked by systemd as
#   ExecStopPost=+<packet>/retire_claim_r4.sh <run dir>
...
# by mint_claim_r4.sh is what makes a re-run impossible, and this script never
...
if (( $# != 1 )); then
  printf 'usage: retire_claim_r4.sh <run-directory>\n' >&2
  exit 64
fi
