...
case "$tag" in max12_912_order3_d1_double_root_control2_axis_covariance_v9_20260826T*) ;; *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;; esac
...
( ulimit -v 8388608; nice -n 10 timeout 1800 /usr/bin/time -v -o compiler.time \
  python3 -u compile_axis_covariance_v9.py --order "$order" --output axis_covariance.json \
...
if test "$rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -Ec '^PASS_AXIS_COVARIANCE_V9$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_OR_DIAGNOSTIC_FAILURE > FAILED; exit 96
fi
sha256sum axis_covariance.json compiler.stdout compiler.stderr compiler.rc compiler.time > result.sha256
touch DONE
