...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "AWS-only seeded V2 launcher refused non-Linux host" >&2
  exit 125
fi
vendor=$(tr -d '\n' < /sys/class/dmi/id/sys_vendor 2>/dev/null || true)
if [[ "$vendor" != "Amazon EC2" ]]; then
  echo "AWS-only seeded V2 launcher refused vendor=${vendor:-unknown}" >&2
  exit 125
fi
if [[ $# -ne 5 ]]; then
  echo "usage: launch_remote_v2.sh AWS_ROOT AWS_JOB TAG MODE CAP_KIB" >&2
  exit 125
fi
...
python3 cases/max12_812_order4_seeded_projection_v2_20260826/compile_seeded_projection_v2.py \
  ...
  "$aws_job/compiled/seeded_projection_v2.sing" --mode "$mode" \
...
exec cases/max12_812_order4_seeded_projection_v2_20260826/run_seeded_projection_aws_v2.sh \
  "$aws_root" "$aws_job/run" "$lane_tag" "$mode" \
  "$aws_job/compiled/seeded_projection_v2.sing" "$cap_kib"
