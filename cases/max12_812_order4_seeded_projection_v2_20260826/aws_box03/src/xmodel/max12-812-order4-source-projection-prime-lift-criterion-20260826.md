...
There is one further, logically separate dimension charge.  The exact
characteristic-zero computation must print

```text
Q_SAT_DIM=1.
```

Equivalently, once the two initial ideals have been proved identical, this
follows from the one-dimensional special fibre because a monic Groebner
model has the same standard-monomial Hilbert data in the generic and special
fibres.  We require the direct exact-`Q` value as an explicit sentinel.  In
particular, primeness plus a one-dimensional `(s,t)` *image* is not being
silently used to assert that the source itself is a curve.
...
allowed power of `t`.  Section 2 together with the explicit
`Q_SAT_DIM=1` sentinel gives a source domain of transcendence degree one,
and Section 3 gives a nonconstant residual-plane image.  Therefore there is
...
Promotion requires all exact source-equivalence sentinels, the exact
characteristic-zero `Q_SAT_DIM=1` sentinel, the two-sided ideal and
initial-ideal comparisons, equality with the unique modular prime, the
one-dimensional special projection, the two-sided `a6` saturation, clean
