#!/usr/bin/env python3
"""AWS-only compiler for fail-closed seeded order-four projection V2."""
...
ideal GQraw=std(JQ);
ideal GQ=GQraw;
int seed_nonzero_leads=1;
int seed_i;
for (seed_i=1; seed_i<=size(GQ); seed_i++)
{{
  number seed_lc=leadcoef(GQ[seed_i]);
  if (seed_lc==0)
  {{
    seed_nonzero_leads=0;
  }}
  else
  {{
    GQ[seed_i]=GQ[seed_i]/seed_lc;
  }}
}}
int seed_monic=1;
for (seed_i=1; seed_i<=size(GQ); seed_i++)
...
int norm_to_raw=ideal_is_zero(reduce(GQ,GQraw));
...
print("SEEDED_Q_NONZERO_LEADS="+string(seed_nonzero_leads));
print("SEEDED_Q_MONIC="+string(seed_monic));
print("SEEDED_Q_RAW_TO_NORM="+string(raw_to_norm));
print("SEEDED_Q_NORM_TO_RAW="+string(norm_to_raw));
...
if ((seed_nonzero_leads==0)||(seed_monic==0)||(raw_to_norm==0)||
    (norm_to_raw==0)||(seed_self==0)||(source_to_seed==0)||
