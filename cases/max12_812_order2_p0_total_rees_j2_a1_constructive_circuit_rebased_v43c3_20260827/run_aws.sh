...
package=cases/max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_rebased_v43c3_20260827
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_rebased_v43c3_* ]]; then exit 126; fi
...
(
  ...
  bash ops/aws_exact_lane.sh "$root" "$job/run" "$lane" timeout "$wall_seconds" \
    python3 "$package/replay_constructive_circuit_rebased_v43c3.py" "$job/output"
)
stdout="$job/run/${lane}.stdout"
grep -Fx 'PASS-A1-RHO0-RAW-CONSTRUCTIVE-CIRCUIT-REBASED-A1-104-V43C3' "$stdout" >/dev/null
...
printf 'end_utc=%s\noutcome=exact-rebased-a1-104-derivation-DAG-replayed\n' \
