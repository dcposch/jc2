#!/usr/bin/env python3
"""Execution runner for the D43 `K0` field certificate, R3.
...
Nothing here decides whether it is allowed to run: ``aws_supervisor_r3.py``
...
exactly that loader and nothing else, which is what ``preflight_r3.py``
...
SCHEMA = "d43-k0-field-certificate-runner-r3"
ROUTE = "D43-K0-FIELD-CERT-R3"
EXPECTED_PACKET_DIRNAME = "d43_k0_field_certificate_r3_20260829"
PACKET_MANIFEST_NAME = "PAYLOAD.sha256"
SEALED_MODULES = ("k0_algebra_r3", "k0_field_checker_r3", "k0_mutations_r3")
GP_SCRIPT_NAME = "k0_field_cert_r3.gp"
...
def load_sealed_siblings(pkt, expect_manifest_sha=None):
    ...
    for name in SEALED_MODULES:
        ...
        # ``import k0_algebra_r3`` resolves out of sys.modules and never
    ...
    globals()["CK"] = sys.modules["k0_field_checker_r3"]
    globals()["MB"] = sys.modules["k0_mutations_r3"]
...
def main(argv=None):
    ...
    terminal = {
        ...
        "K0_UNCONDITIONAL_THEOREM": {
            ...
            "engines": ["stdlib Python (k0_field_checker_r3.py)",
                        "PARI/GP 2.15.4 (k0_field_cert_r3.gp)"],
        ...
        "E_CONDITIONAL_RATIO_THEOREM": {
            ...
            "single_engine_reason":
                "k0_field_cert_r3.gp carries no cube-root layer, so it cannot "
