#!/usr/bin/env python3
"""Bounded local preflight for the D43 `K0` field certificate, R3.
...
Runs everything that can honestly be run without AWS and without PARI:
  ...
  P6   the census and observable hashes pinned in execution_pins_r3.json
  ...
  P11  the custody-boundary controls: the R3 privilege boundary executed
       against real directories, real uid/gid values and real POSIX
       ...
       identity contract, and a hostile source-mutation battery that shows
       every one of those controls is load-bearing
...
SCHEMA = "d43-k0-field-certificate-preflight-r3"
PACKET_DIRNAME = "d43_k0_field_certificate_r3_20260829"
SEALED_MODULES = ("k0_algebra_r3", "k0_field_checker_r3", "k0_mutations_r3",
                  "runner_r3", "aws_supervisor_r3", "custody_selftest_r3")
...
CK = sys.modules["k0_field_checker_r3"]
MB = sys.modules["k0_mutations_r3"]
KA = sys.modules["k0_algebra_r3"]
RUN = sys.modules["runner_r3"]
SUP = sys.modules["aws_supervisor_r3"]
CS = sys.modules["custody_selftest_r3"]
...
GP_LABEL_WIDTH_CAP = 80          # runner_r3.GATE_LINE allows \S.{0,80}
...
def _smoke_copy(tmp, mutate):
    ...
    dst = Path(tmp) / "packet"
    dst.mkdir()
    for name in ("PAYLOAD.sha256", "k0_algebra_r3.py",
                 "k0_field_checker_r3.py", "k0_mutations_r3.py"):
...
def isolated_import_controls():
    out = {}
    runner = str(PACKET / "runner_r3.py")
    ...
    r = _run([sys.executable, "-I", "-B", "-c",
              "import k0_field_checker_r3"], cwd=str(PACKET))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        def drop(dst):
            ...
            keep = [l for l in path.read_text().splitlines()
                    if "k0_field_checker_r3.py" not in l]
            path.write_text("\n".join(keep) + "\n")
...
def authority_controls():
    ...
    template = json.loads((PACKET / "authorization_template_r3.json").read_bytes())
    try:
...
def gate_surface_controls(root, pins, banks):
    ...
    record("D5_r3_exponent_duplication", "D5",
    ...
    text = (PACKET / "k0_field_cert_r3.gp").read_text()
...
def custody_controls():
    ...
    sup_text = (PACKET / "aws_supervisor_r3.py").read_text()
    run_text = (PACKET / "runner_r3.py").read_text()
    entries = []
    for (name, module, kind, old, new, controls, why) in CUSTODY_MUTATIONS:
        base_text = sup_text if module == "supervisor" else run_text
        ...
        with tempfile.TemporaryDirectory() as tmp:
            if kind == "static":
                (Path(tmp) / "aws_supervisor_r3.py").write_text(patched)
                ...
                sup_mut = _load_mutated(name, "aws_supervisor_r3", patched, tmp)
                ...
                run_mut = _load_mutated(name, "runner_r3", patched, tmp)
                root = Path(tmp) / "fixtures"
                root.mkdir()
...
def main(argv=None):
    ...
    root = Path(args.root)
    pins = json.loads((PACKET / "execution_pins_r3.json").read_bytes())
    ...
    report = {"schema": SCHEMA, "packet": PACKET.name,
              ...
              "gp_not_executed_reason":
                  ...
                  "host by runner_r3.py.  No claim is made that the GP "
                  "syntax or API has run.",
              "aws_executed": False,
              "aws_not_executed_reason":
                  "status is R3_SOURCE_READY_AWS_NOT_AUTHORIZED"}
    ...
    gp_text = (PACKET / "k0_field_cert_r3.gp").read_text()
    ...
    report["P6_pinned_hashes"] = {
        ...
        "gp_script_sha256_matches_pin":
            sha256_path(PACKET / "k0_field_cert_r3.gp")
