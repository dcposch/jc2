...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_affine_faber_k_moving_discriminant_sigma15_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
python3 cases/max12_812_order2_affine_faber_k_moving_discriminant_sigma15_20260826/compile_sigma15.py "$compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
...
for required in 'K_SIGMA15_TCOEFF_CONTROL=1' 'K_SIGMA15_LOWER_ZERO=1' 'K_SIGMA15_ENDPOINT=PASS_COMPLETE_EMISSION_AND_STANDARD_BASIS' 'K_SIGMA15_DONE=1' 'K_SIGMA15_SCOPE=EXACT_MOVING_DOUBLE_ROOT_CHART_ONLY_NO_ROOT_SPLITTING_TOTAL_FAN_TERMINAL_TAYLOR_ORDER2_OR_JC2_VERDICT'; do
...
printf 'validator=PASS_K_SIGMA15_EMISSION_STD\n' >> "$aws_job/run/$tag.validation"
