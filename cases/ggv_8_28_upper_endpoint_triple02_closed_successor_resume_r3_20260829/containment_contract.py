#!/usr/bin/env python3
"""Fail-closed Linux process-custody and terminal-promotion helpers (R3).

R3 repairs relative to the reviewed R2 packet (hostile review O-B1/O-N3/
O-N4/O-N8v):

  O-B1  The late terminal authority authenticates the FROZEN ARCHIVE BYTES,
        not the extraction directory.  At decision time it re-hashes the
        installed terminal archive against the charged digest, re-runs the
        complete member-safety, embedded-manifest, and directory census
        from the archive bytes alone, requires the fresh extraction to be
        byte-identical member-for-member (no symlink, no extra, no missing,
        no substitution anywhere), and then reads every decision input from
        the archive bytes held in memory.  A same-uid edit, deletion,
        replacement, symlink, or archive/fresh-root mix-and-match after the
        freeze therefore fails closed.  The live candidate file is never
        read at decision time.
  O-N3  The systemd runtime read-back verifies all five charged properties
        (KillMode=control-group, RuntimeMaxUSec exactly 21600 s,
        MemoryMax=274877906944, TasksMax=512, MemorySwapMax=0); an
        unexpected rendering downgrades, never promotes.
  O-N4  The sticky supervisor fault latches are banked inside the archive
        (custody/FAULT_LATCHES_PRE_ARCHIVE.json) before the freeze and the
        late authority refuses any fault regression against them.
  O-N8v The job-tag stamp shape (YYYYMMDDTHHMMSSZ) is enforced by
        require_job_tag itself, so direct supervisor entry cannot use a
        malformed stamp.

R2 architecture retained: one content-addressed decision record with a
derived candidate; systemd-only success with a NO_VERDICT-only PGID
fallback; schema-checked nonce; atomic lease; no-replace hard-link
publication; archive verification exact over regular files AND directories
...
LEASE_SCHEMA = "triple02_closed_successor_r3_lease_v1"
DECISION_SCHEMA = "triple02_closed_successor_r3_decision_v1"
FAULT_LATCH_SCHEMA = "triple02_closed_successor_r3_fault_latches_v1"
JOB_TAG_PREFIX = "ggv_triple02_closed_successor_resume_r3_"
...
# ---- frozen-archive authentication for the late authority (O-B1) ----------
...
def authenticate_frozen_archive(terminal_archive: Path,
    ...
    Re-hashes the archive against the charged digest; re-runs the complete
    member-safety, embedded-manifest, and directory censuses from the archive
    bytes; requires the fresh root's file/directory sets to equal the archive
    member sets exactly and every fresh file to be byte-identical to its
    archive member; retains the decision-input members in memory so nothing
    later is read from the mutable extraction."""
    require_digest(terminal_archive_sha256, "terminal-archive")
    if terminal_archive.is_symlink() or not terminal_archive.is_file():
        raise RuntimeError("DECISION_ARCHIVE_NOT_REGULAR_FILE")
    if sha256_path(terminal_archive) != terminal_archive_sha256:
    ...
    regular: set[str] = set()
    directories: set[str] = set()
    with tarfile.open(terminal_archive, "r:gz") as archive:
        members = archive.getmembers()
        names: set[str] = set()
        for member in members:
            name = safe_relative(member.name).as_posix()
            if name in names or not archive_member_allowed(name, includes):
                raise RuntimeError(
                    "DECISION_ARCHIVE_UNSAFE_DUPLICATE_OR_EXTRA:" + name)
            names.add(name)
            if member.isfile():
                regular.add(name)
            elif member.isdir():
                ...
                raise RuntimeError("DECISION_ARCHIVE_NONREGULAR_MEMBER:" + name)
        for required_name, label in ((manifest_name, "MANIFEST"),
                                     (dirs_name, "DIRECTORY_LIST")):
            if required_name not in regular:
                raise RuntimeError(f"DECISION_ARCHIVE_{label}_MISSING")
        if fs_files != regular or fs_dirs != directories:
            missing = sorted((regular - fs_files) | (directories - fs_dirs))
            extra = sorted((fs_files - regular) | (fs_dirs - directories))
            raise RuntimeError(
        ...
        for member in members:
            ...
            source = archive.extractfile(member)
            if source is None:
                raise RuntimeError("DECISION_ARCHIVE_MEMBER_UNREADABLE:" + name)
            digest = hashlib.sha256()
            keep = bytearray() if name in retain else None
            try:
                with (fresh_root / safe_relative(name)).open("rb") as live:
                    while True:
                        ...
                        if live.read(len(block) or 1) != block:
                            raise RuntimeError(
                                "DECISION_FRESH_ROOT_BYTES_DRIFT:" + name)
                        if not block:
                            break
    ...
    for number, line in enumerate(manifest_text.splitlines(), 1):
        match = SHA_LINE.fullmatch(line)
        if not match:
            raise RuntimeError(f"DECISION_ARCHIVE_MANIFEST_MALFORMED_{number}")
        digest_value, value = match.groups()
        relative = safe_relative(value).as_posix()
        if relative in entries:
            raise RuntimeError("DECISION_ARCHIVE_MANIFEST_DUPLICATE:" + relative)
    ...
    listed_dirs = [line for line in
                   retained[dirs_name].decode("utf-8").splitlines() if line]
...
def derive_late_decision(fresh_root: Path, terminal_archive: Path,
    ...
    """Late authority (O-B1 repair): authenticate the frozen archive bytes
    against the charged digest, bind the fresh extraction to them
    member-for-member, and derive the candidate exclusively from the archive
    bytes held in memory while verifying every decision-record equality."""
    require_nonce(nonce)
    require_job_tag(job_tag, nonce)
