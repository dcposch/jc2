# Preregistration: K00 filtered first-load obstruction V18R1

Date: 2026-08-27

Status: **PARSER-ONLY REPAIR FROZEN AFTER V18 FAILED BEFORE ALGEBRA; ALL
FILTERED-RANK OUTCOMES REMAIN OPEN.**

V18 verified its freeze and then failed before emitting any matrix because
its monomial-only parser rejected the frozen prelude's parenthesized factored
syntax.  V18R1 replaces only that parser with a whitelisted AST evaluator for
integer/rational constants, `d0,...,d5`, addition, subtraction,
multiplication, constant division, and nonnegative integer powers.  Every
other invariant, source, generator order, cutoff, rank engine, validator, and
both-outcome rule below is unchanged.  Original V18 is nonpromotable; see
`../max12_812_order2_u2_62_k00_filtered_load_obstruction_v18_20260827/FAILURE.md`.
...
At the first incompatible cutoff it must serialize a nonzero left functional
annihilating every column and pairing nontrivially with the target, over the
lane's field.  The highest compatible lift automatically replays every lower
...
`p=65521` is navigation/software control only, even with a replayed modular
dual.  Exact-Q conclusions require the exact V17 lane and an exact rational
certificate replay.
