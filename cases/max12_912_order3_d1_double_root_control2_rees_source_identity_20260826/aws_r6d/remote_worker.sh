#!/usr/bin/env bash
set -u

job=$1
tag=$2
cd "$job" || exit 91
hostname > hostname.txt
date -u +%FT%TZ > worker_started_utc.txt
uname -a > uname.txt
nproc > nproc.txt
free -h > free.prelaunch.txt
/usr/bin/Singular --version > singular.version.txt 2>&1

if test "$(uname -s)" != Linux; then echo REFUSE_NON_LINUX > REFUSED; exit 92; fi
if test "$(cat /sys/class/dmi/id/sys_vendor 2>/dev/null)" != "Amazon EC2"; then
  echo REFUSE_NON_AWS_EC2 > REFUSED; exit 93
fi
case "$tag" in
  max12_912_order3_d1_double_root_control2_rees_source_identity_20260826T*_r6d) ;;
  *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;;
esac

echo "pid=$$" > worker.metadata
echo "tag=$tag" >> worker.metadata
echo "compiler_memory_kib=2097152" >> worker.metadata
echo "compiler_timeout=120" >> worker.metadata
echo "solver_memory_kib=4194304" >> worker.metadata
echo "solver_timeout=300" >> worker.metadata
echo WAITING_GO >> worker.metadata
while test ! -f GO; do sleep 1; done

if ! sha256sum -c source.sha256 > source.check 2>&1; then
  echo SOURCE_HASH_FAILURE > FAILED; exit 95
fi
export JC2_AWS_TAG="$tag"
(
  ulimit -v 2097152
  nice -n 10 timeout 120 python3 -u compile_source_identity.py
) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_CONTROL2_REES_SOURCE_IDENTITY_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_FAILURE > FAILED; exit 96
fi
sha256sum source_identity.sing > input.sha256
(
  ulimit -v 4194304
  nice -n 10 timeout --signal=TERM --kill-after=10 300 \
    /usr/bin/time -v -o singular.time /usr/bin/Singular -q source_identity.sing
) > singular.stdout 2> singular.stderr
solver_rc=$?
echo "$solver_rc" > singular.rc
if test "$solver_rc" -ne 0 || test -s singular.stderr || \
   test "$(grep -c '^PASS_CONTROL2_REES_SOURCE_IDENTITY$' singular.stdout 2>/dev/null)" -ne 1 || \
   grep -q '^FAIL_' singular.stdout; then
  echo SOLVER_FAILURE > FAILED; exit 97
fi
sha256sum compiler.stdout compiler.stderr compiler.rc source_identity.sing \
  singular.stdout singular.stderr singular.time singular.rc > result.sha256
free -h > free.postsolve.txt
touch DONE
echo PASS_REMOTE_WORKER >> worker.metadata
