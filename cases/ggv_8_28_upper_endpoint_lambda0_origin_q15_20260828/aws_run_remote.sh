...
setsid timeout --foreground --signal=TERM --kill-after=40s 7200s \
  ...
  "$job_dir/source/runner/aws_job_worker.sh" \
    "$job_dir" "$system_mode" "$host_alias" \
...
printf 'GGV_ORIGIN_Q15\t%s\t%s\t%s\t%s\t%s\t%s\n' \
  "$worker_pid" "$observed_pgid" "$observed_sid" "$observed_starttime" \
  "$job_dir" "$source_manifest_sha" \
  >"$job_dir/records/GROUP.tsv"
printf '%s\n' "$system_mode" >"$job_dir/records/SYSTEM_MODE"
