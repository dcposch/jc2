#!/usr/bin/env python3
"""Sparse-AST affinity proof with the complete symbolic predecessor state."""
...
# V4 correctly discovered that the coefficient matrix has a larger base state
# than structural+Frobenius.  Preserve every exact dependency and suppress the
# invalid 79*3^6 specialization rather than projecting it away.
replace_once(
    '''assert matrix_dependencies <= matrix_control_z3_names, sorted(
    matrix_dependencies - matrix_control_z3_names)
''',
    '''matrix_extra_dependencies = sorted(
    matrix_dependencies - matrix_control_z3_names)
matrix_full_dependency_names = sorted(matrix_dependencies)
''')
replace_once(
    '''control_var_order = tuple(f"pred_{name}" for name in matrix_control_names)
''',
    '''control_var_order = tuple(matrix_full_dependency_names)
''')
replace_once(
...
''',
    '''# Symbolic checkpoint only: do not compile/evaluate entries on an
# incomplete 13-coordinate base.
entry_codes = []
''')
replace_once(
    '''bases = parse_bases()
...
''',
    '''assert len(parsed_bases) == 79 and state_count == 0
''')
...
right_dependencies = sorted(set().union(*[
    variables(entry) for entry in right_column]))
matrix_row_dependency_signatures = [sorted(set().union(*[
    variables(entry) for entry in row])) for row in matrix]
dependency_incidence_payload = json.dumps({
    "matrix_dependencies": matrix_full_dependency_names,
    "matrix_extra_dependencies": matrix_extra_dependencies,
    "matrix_row_dependency_signatures": matrix_row_dependency_signatures,
    "right_column_dependencies": right_dependencies,
...
''',
    ...
    "ast_unsupported_dependent_node_count": len(
        ast_unsupported_dependent_nodes),
    "symbolic_full_state_checkpoint": True,
    "false_structural_frobenius_only_specialization_refused": True,
    "matrix_extra_dependencies": matrix_extra_dependencies,
    "matrix_full_dependency_count": len(matrix_full_dependency_names),
    "right_column_dependencies": right_dependencies,
    "dependency_incidence_sha256": hashlib.sha256(
        dependency_incidence_payload).hexdigest(),
''')
...
''',
    '''    "matrix_control_names": list(control_var_order),
    "unique_matrix_entry_expression_count": len(unique_entries),
    "compatible_structural_base_count": len(parsed_bases),
''')
...
(OUT / "dependency_incidence.json").write_bytes(
    dependency_incidence_payload + b"\\n")
encoded = (json.dumps(result, sort_keys=True, separators=(",", ":"))
''')
...
print("full_matrix_dependencies", len(matrix_full_dependency_names),
      "extra", len(matrix_extra_dependencies))
print("symbolic_only_no_rank_specialization", True)
print("result_sha256", hashlib.sha256(encoded).hexdigest())
''')

namespace = {"__file__": str(PARENT), "__name__": "__cartier_fitting_v5__"}
