...
def build_chart_script(remaining: list[str], generators: list[str], node: int,
    ...
    bordered_plant_expression = "+".join(
        f"R[1,{col}]*Y[{col},1]" for col in range(1, 11))
    lines += [
        ...
        f"poly BORDERED_PLANT_RAW=({bordered_plant_expression})+DELTA; poly BORDERED_PLANT_NF=reduce(BORDERED_PLANT_RAW,ACTIVE_SB);",
        f'write("NODE_{node:03d}_BORDERED_NONZERO_PLANT.txt","normal_form|"+string(BORDERED_PLANT_NF)+"|expected_delta|"+string(DELTA));',
        'print("BORDERED_NONZERO_PLANT_NF_NONZERO="+string(BORDERED_PLANT_NF!=0)); print("BORDERED_NONZERO_PLANT_EQUALS_DELTA="+string(BORDERED_PLANT_NF==DELTA));',
        'if(BORDERED_PLANT_NF==0 || BORDERED_PLANT_NF!=DELTA){ print("FATAL_BORDERED_NONZERO_PLANT"); quit; }',
