# Preregistration: reducer-safe endpoint chart-complement recursion R5
...
R3 retained the unchanged mathematical payload and added these fail-closed
...
remained blocked until one R3 pilot node passed the complete saturation,
rank-witness, adjugate, original-row, endpoint, and complement contract.

R3 Q1P03 passed that contract.  R3 TRIPLE03 then exposed a distinct adapter
disagreement: the chart script recomputed the 95-pivot transform after passing
from `Q[params]/J` to its saturated quotient.  Although both pivot sequences
had 95 rational-unit pivots, the second sequence chose a different residual
basis, so the node witness minor did not equal the same-index determinant in
the recomputed residual.  R3 correctly stopped at
`SCANNED_DELTA_REPLAY_EQUAL=0`; TRIPLE03 is frozen as
`ADAPTER_FAILURE_NO_VERDICT` with no endpoint or rank-locus inference.

R4 made one mathematical-adapter correction.  It reconstructs the exact
node transform using the pinned node reducer `std(J)`, then base-changes every
entry of its 11x10 residual and full 105x105 right transformation by normal
form against the certified saturated reducer.  It never repivots under the
saturated reducer.  The base-change census is exactly 11*10 + 105*105 = 11135
normal forms; the selected determinant must then replay the node witness after
base change, and all bordered identities, 106 original rows, and endpoint
coefficients are checked as before.  A source mutation replacing the node
pivot reducer by the saturated reducer must be rejected before launch.  The
failed R3 TRIPLE03 packet is the frozen negative control.  R4 replayed
TRIPLE03 and obtained an exact finite geometric chart cover for the literal
root-chart complement.  That result is banked but not promoted because the R4
chart payload did not include a planted nonzero endpoint semantic control.

R5 leaves every ideal, rank scan, pivot, kernel, endpoint expression, and
classification unchanged.  It adds two fail-closed semantic controls to every
nonempty production chart.  First, for the first diagonal pulled-back endpoint
coefficient `e`, it requires both `NF(e+1) != 0` and the exact affine replay
`NF(NF(e+1)-NF(e)-1)=0`.  Second, it adds the certified nonzero determinant
`Delta` to the first bordered kernel identity, and requires the resulting
normal form to be nonzero and literally equal to `Delta`.  Both plant outputs
are frozen.  A source mutation deleting the endpoint plant marker must be
rejected before Singular starts.  The positive adapter fixture independently
checks both plants.  R5 first retrospectively replays TRIPLE03; the R4 result
remains unpromoted unless the replay agrees exactly.  Other live jobs are not
altered or stopped.
