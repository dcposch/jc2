...
job_tag=$2
lane=$3
case_dir="$job_dir/source/jc2/cases/ggv_8_28_upper_endpoint_lambda0_endpoint_kernel_strata_r3_20260828"
...
runtime_worker="$job_dir/runtime/aws_worker_r3.sh"
worker_pid=""
registered_pid=""
...
python3 -B "$case_dir/build_worker_r3.py" --r2-worker "$r2_case/aws_worker.sh" \
  --output "$runtime_worker" >"$job_dir/records/WORKER_BUILD.stdout" \
  2>"$job_dir/records/WORKER_BUILD.stderr"
grep -q '^R3_WORKER_NARROW_TRANSFORM_PASS$' "$job_dir/records/WORKER_BUILD.stdout"
sha256sum "$runtime_worker" >"$job_dir/records/RUNTIME_WORKER.sha256"
grep -q '^8c5705b1124efead1649724d3db7eb03f33e8ff44311ce6114d6a3f8efaf90da ' \
...
printf 'GGV_ENDPOINT_STRATA_R3_%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$lane" \
