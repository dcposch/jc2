...
if [[ "$(uname -s)" != Linux ]]; then echo "REFUSED: Linux required" >&2; exit 97; fi
if ! grep -q 'Amazon EC2' /sys/devices/virtual/dmi/id/sys_vendor; then
  echo "REFUSED: Amazon EC2 required" >&2; exit 96
fi
...
tag="td6_v84s2_reverse_${label}_${stamp}"
printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=8388608\ntimeout_seconds=21600\n' \
  "$(hostname)" "$label" "$tag" "$(date -u +%FT%TZ)" "$actual" > "$root/registration.meta"
(
  set +e
  ulimit -v 8388608
  ...
  /usr/bin/time -v timeout 21600 bash run_v84s2.sh > "$root/stdout" 2> "$root/stderr"
  rc=$?
  printf '%s\n' "$rc" > "$root/rc"
