...
case "$tag" in max12_812_order2_square_d1_age10_j38_r3_*) ;; *) exit 125 ;; esac
...
sha256sum -c cases/max12_812_order2_square_owner_d1_age10_j38_odd_recurrence3_20260826/FREEZE.sha256 > "$base/prelaunch_freeze.stdout"
nohup bash cases/max12_812_order2_square_owner_d1_age10_j38_odd_recurrence3_20260826/run_aws.sh "$source_root" "$evidence" "$tag" 67108864 "$characteristic" 3600 7200 > "$base/outer.stdout" 2> "$base/outer.stderr" < /dev/null &
