...
source_dir="$JC2_ROOT/cases/as_fonly_d7_q5_sat_complete_mod243_20260825"
program="$source_dir/complete_lower_rows.py"
parent="$JC2_ROOT/cases/as_fonly_d7_q5_sat_full_q4_restore_20260825/replay_full_q4_restore.py"
mkdir -p "$JOB_DIR"
...
/usr/bin/time -v timeout --signal=TERM --kill-after=30s "$TIMEOUT_SECONDS" \
  env JC2_ROOT="$JC2_ROOT" MODEL_OUTPUT="$MODEL_OUTPUT" \
      PARENT_OUTPUT_JSON="$JOB_DIR/full_q4_parent.json" \
      OUTPUT_JSON="$JOB_DIR/complete_mod243.json" \
