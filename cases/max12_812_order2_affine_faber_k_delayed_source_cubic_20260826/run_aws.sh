...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_affine_faber_k_delayed_source_cubic_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
python3 cases/max12_812_order2_affine_faber_k_delayed_source_cubic_20260826/compile_k_source_cubic.py "$compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
...
for required in 'K_SOURCE_TCOEFF_CONTROL=1' 'K_SOURCE_LOWER_ZERO=1' 'K_SOURCE_CUBIC_CONTROL=1' 'K_SOURCE_ENDPOINT=PASS_REPEATED_ROOT_CORRECTION_INDEPENDENT_CUBIC_UNIT' 'K_SOURCE_DONE=1' 'K_SOURCE_SCOPE=DELAYED_LOAD_REPEATED_ROOT_K_FACE_NO_TOTAL_FAN_TERMINAL_TAYLOR_ORDER2_OR_JC2_VERDICT'; do
...
printf 'validator=PASS_K_SOURCE_CUBIC\n' >> "$aws_job/run/$tag.validation"
