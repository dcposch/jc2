...
set +e
timeout "$compile_timeout" python3 "$package/compile_t_rs0_stream.py" \
...
input="$aws_job/compiled/t_rs0_stream_${label}.sing"
...
set +e
python3 "$package/validate_t_rs0_stream.py" \
...
printf 'validator=PASS_T_RS0_STREAM_V2_NAVIGATION_ONLY\n' >> "$aws_job/validator.validation"
