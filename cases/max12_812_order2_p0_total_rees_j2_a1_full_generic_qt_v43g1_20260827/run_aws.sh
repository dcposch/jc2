...
package=cases/max12_812_order2_p0_total_rees_j2_a1_full_generic_qt_v43g1_20260827
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_full_generic_qt_v43g1_*_exact_${phase}_* ]]; then
  exit 126
fi
case "$phase" in decision|lift) ;; *) exit 126 ;; esac
...
printf 'tag=%s\nhost=%s\nstart_utc=%s\nphase=%s\nmemory_cap_kib=%s\nwall_seconds=%s\nsource_manifest_sha256=%s\n' \
  "$tag" "$(hostname)" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$phase" \
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "$compiler_lane" timeout "$(remaining)" \
  python3 "$package/compile_full_generic_qt_v43g1.py" "$job/compiled" --phase "$phase"
...
grep -Fx 'V43G1_SPECIAL_FIBRE_UNIT=1' "$stdout" >/dev/null
if grep -Fx 'V43G1_GENERIC_OUTCOME=nonunit' "$stdout" >/dev/null; then
  outcome=exact-full-generic-nonunit
elif grep -Fx 'V43G1_GENERIC_OUTCOME=unit' "$stdout" >/dev/null; then
  if [[ "$phase" == decision ]]; then
    grep -Fx 'V43G1_UNIT_STATUS=requires-tracked-lift' "$stdout" >/dev/null
    outcome=exact-full-generic-unit-requires-tracked-lift
  else
    grep -Fx 'V43G1_LIFTSTD_BASIS_REPLAY=1' "$stdout" >/dev/null
    grep -Fx 'V43G1_GENERIC_UNIT_REPLAY=1' "$stdout" >/dev/null
    outcome=exact-full-generic-unit-replayed-reduced-58-row
  fi
else
  exit 90
fi
grep -Fx 'PASS_A1_FULL_GENERIC_QT_V43G1' "$stdout" >/dev/null
