...
printf 'hostname=%s\nlabel=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=43200\nscope=tail-q14-q16-q24-triangular-integrability\n' \
...
(
  ...
  lane="$root/tailq14"
  mkdir -p "$lane/output"
  tag="td6_v89h2_tail_q14_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=43200\nscope=tail-q14-q16-q24-triangular-integrability\n' \
  ...
  (
    ...
    /usr/bin/time -v timeout 43200 bash run_v89h2_tail_q14_triangular.sh \
