...
Status: **fail-closed successor design; no unit-q result claimed.**
...
The second statement is not yet a finite unit-q coordinate action.  It is,
however, the highest-value integrability test.  Before opening nine separate
high-q charts, seek one exact triangular row transformation

```text
T(q16,...,q24) M(q_low,q_high) = M(q_low,0)
```

with polynomial two-sided inverse, or the weaker identity modulo `F`, over
the registered open.  Require replay on the literal sources.  If it exists,
all nine high jets are source-ideal coordinates and the unit atlas reduces to
the 13 low strata.  If it fails, record the first nonzero quadratic
integrability obstruction and use it to partition the high block.  The V87
quotients `h_16,...,h_24` have only 358--391 parameter terms each, versus
3,126--3,453 in the low block, so this test is much cheaper than a generic
22-variable solve.
