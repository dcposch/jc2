...
cd "$aws_root"; sha256sum -c cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v4_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v4_20260826/compile_v4.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_r1_d1_ac_v4_${label}.sing"; sha256sum "$input" "$aws_job/compiled/result.json" > "$aws_job/compiled.sha256"
set +e
...
printf 'validator=PASS_R1_D1_AC_SYMBOLIC_V4\n' >> "$validation"
