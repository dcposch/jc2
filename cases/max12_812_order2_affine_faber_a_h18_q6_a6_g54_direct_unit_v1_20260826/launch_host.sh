...
package=cases/max12_812_order2_affine_faber_a_h18_q6_a6_g54_direct_unit_v1_20260826
...
tag="max12_812_order2_affine_faber_a_h18_q6_a6_g54_direct_unit_v1_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"
remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t affine-faber-a-h18-q6-g54-v1.XXXXXX.tar.gz)
...
ssh "${ssh_options[@]}" "$ssh_target" "cd '$remote_root' && tar -xzf source.tar.gz && mkdir -p '$remote_job' && nohup bash '$package/run_aws.sh' '$remote_root' '$remote_job' '$tag' '$characteristic' 8388608 1200 > '$remote_job.outer.stdout' 2> '$remote_job.outer.stderr' & echo \$!"
printf 'host=%s\nremote_job=%s\ntag=%s\n' "$host" "$remote_job" "$tag"
