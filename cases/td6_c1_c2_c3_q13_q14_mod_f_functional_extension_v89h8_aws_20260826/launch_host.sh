...
printf 'hostname=%s\nlabel=%s\narchive_sha256=%s\nstart_utc=%s\ncap_kib=268435456\ntimeout_seconds=21600\nscope=q13-q14-mod-F-functional-extension\n' \
  "$(hostname)" "$label" "$expected" "$(date -u +%FT%TZ)" > "$root/registration.meta"
(
  set +e
  lane="$root/q13extension"
  mkdir -p "$lane/output"
  tag="td6_v89h8_q13_q14_mod_f_functional_extension_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=21600\nscope=q13-q14-mod-F-functional-extension\n' \
  ...
  (
    ...
    export TD6_Q_SCOPE=q13-q14-q16-q24
    export TD6_F_SPECIALIZATION=exact-C-equals-V2-minus-U3-over-U
    export TD6_OUTPUT_DIR="$lane/output"
    /usr/bin/time -v timeout 21600 bash run_v89h8_q13_q14_mod_f_functional_extension.sh \
