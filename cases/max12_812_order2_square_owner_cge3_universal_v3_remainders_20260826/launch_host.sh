...
case "$tag" in max12_812_order2_square_cge3_universal_v3_*) ;; *) exit 125 ;; esac
...
sha256sum -c cases/max12_812_order2_square_owner_cge3_universal_v3_remainders_20260826/FREEZE.sha256 > "$base/prelaunch_freeze.stdout"
nohup bash cases/max12_812_order2_square_owner_cge3_universal_v3_remainders_20260826/run_aws.sh "$source_root" "$evidence" "$tag" 33554432 "$characteristic" 600 7200 > "$base/outer.stdout" 2> "$base/outer.stderr" < /dev/null &
