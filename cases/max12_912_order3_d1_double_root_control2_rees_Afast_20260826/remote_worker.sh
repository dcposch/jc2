...
case "$tag" in
  max12_912_order3_d1_double_root_control2_rees_Afast_*) ;;
  *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;;
esac
echo "pid=$$" > worker.metadata
echo "tag=$tag" >> worker.metadata
echo "memory_kib=536870912" >> worker.metadata
echo "timeout=21600" >> worker.metadata
...
(
  ...
  nice -n 10 timeout 600 python3 -u \
    repo/cases/max12_912_order3_d1_double_root_control2_rees_Afast_20260826/compile_Afast.py
) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
sha256sum repo/cases/max12_912_order3_d1_double_root_control2_rees_Afast_20260826/compiled_Afast.sing > input.sha256 2>/dev/null || true
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_CONTROL2_REES_AFAST_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_VALIDATION_FAILURE > FAILED; exit 96
fi
date -u +%FT%TZ > solve_started_utc.txt
(
  ulimit -v 536870912
  nice -n 10 timeout --signal=TERM --kill-after=300 21600 \
    /usr/bin/time -v -o singular.time /usr/bin/Singular -q \
    repo/cases/max12_912_order3_d1_double_root_control2_rees_Afast_20260826/compiled_Afast.sing
...
if test "$singular_rc" -ne 0 || test -s singular.stderr || \
   test "$(grep -c '^PASS_D1_DOUBLE_ROOT_CONTROL2_REES_AFAST$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_D1_DOUBLE_ROOT_CONTROL2_REES_V2_A$' singular.stdout 2>/dev/null)" -ne 1 || \
