...
aws_root=$1; aws_job=$2; tag=$3; characteristic=$4; cap_kib=$5; compile_wall=$6; engine_wall=$7
package=cases/max12_812_order2_p0_total_rees_t_cs_row5_g14_export_v17_20260826
mkdir -p "$aws_job/run"
{
  printf 'tag=%s\nhost=%s\npid=%s\nstart_utc=%s\n' "$tag" "$(hostname)" "$$" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf 'characteristic=%s\ncap_kib=%s\ncompile_wall=%s\nengine_wall=%s\n' "$characteristic" "$cap_kib" "$compile_wall" "$engine_wall"
...
set +e
/usr/bin/time -v timeout "$compile_wall" python3 "$package/export_row5_g14_v17.py" "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
set +e
python3 "$package/validate_export_v17.py" --compiler-result "$aws_job/compiled/result.json" --stdout "$stdout" --stderr "$stderr" --characteristic "$characteristic" --output "$aws_job/RESULT.json" > "$aws_job/validator.stdout" 2> "$aws_job/validator.stderr"
validator_rc=$?
set -e
printf 'validator_rc=%s\n' "$validator_rc" > "$aws_job/validator.validation"; if [[ "$validator_rc" -ne 0 ]]; then exit "$validator_rc"; fi
printf 'validator=PASS_T_CS_ROW5_G14_EXPORT_V17\n' >> "$aws_job/validator.validation"
