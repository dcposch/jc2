...
tag="max12_812_order2_p0_total_rees_t_cs_row5_g14_export_v17_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"; remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t p0-total-rees-t-cs-row5-g14-v17.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
COPYFILE_DISABLE=1 tar -czf "$tarball" "$package" "$replay/replay_row5_grade14.py" "$tails" "$v9/aws_q_v9" "$v9/aws_p65521_v9" ops/aws_exact_lane.sh
