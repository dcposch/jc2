...
case "$tag" in
  max12_912_order3_d1_double_root_toric_*) ;;
  *) echo REFUSE_UNREGISTERED_AWS_TAG > REFUSED; exit 94 ;;
esac
case "$encoding" in
  A) input=compiled/toric_A_factored_sat_dp.sing; marker=PASS_D1_DOUBLE_ROOT_TORIC_A ;;
  B) input=compiled/toric_B_expanded_inverse_lpdp.sing; marker=PASS_D1_DOUBLE_ROOT_TORIC_B ;;
...
(
  ...
  nice -n 10 timeout 3600 python3 -u \
    repo/cases/max12_912_order3_d1_double_root_toric_blowup_20260825/compile_toric_blowup.py \
    --output-dir compiled
...
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_D1_DOUBLE_ROOT_TORIC_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then
