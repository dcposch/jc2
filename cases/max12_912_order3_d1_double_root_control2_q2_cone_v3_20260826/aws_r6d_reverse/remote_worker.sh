...
case "$tag" in max12_912_order3_d1_double_root_control2_q2_cone_v3_20260826T*) ;; *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;; esac
case "$order" in forward|reverse) ;; *) echo REFUSE_ORDER > REFUSED; exit 94 ;; esac
{
  echo "pid=$$"; echo "tag=$tag"; echo "order=$order"
  echo "memory_kib=2097152"; echo "timeout=600"; echo WAITING_GO
} > worker.metadata
...
( ulimit -v 2097152; nice -n 10 timeout 600 /usr/bin/time -v -o compiler.time \
  python3 -u compile_q2_cone_v3.py --order "$order" --output cone_certificate.json \
...
if test "$rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_Q2_CONE_V3$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_OR_DIAGNOSTIC_FAILURE > FAILED; exit 96
fi
sha256sum cone_certificate.json compiler.stdout compiler.stderr compiler.rc compiler.time > result.sha256
touch DONE
