...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_third_tail_sharp_20260826/FREEZE.sha256 \
  > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 \
  cases/max12_812_order2_square_third_tail_sharp_20260826/compile_square_third_tail_sharp.py \
...
input="$aws_job/compiled/square_third_sharp_${config}_${label}.sing"
...
if [[ "$config" == "p0moving" ]]; then
  for required in \
    'SQUARE_THIRD_SHARP_P0_CONSTANT_MINUS_BETA3=1' \
    'SQUARE_THIRD_SHARP_P0_Z3_MINUS_ALPHA3=1'
  do
    if [[ "$(grep -Fxc "$required" "$stdout" || true)" -ne 1 ]]; then
      printf 'validator=FAIL_MISSING_OR_NONUNIQUE:%s\n' "$required" >> "$validation"
      exit 90
...
printf 'validator=PASS_SQUARE_THIRD_SHARP\n' >> "$validation"
