...
cd "$aws_root"; sha256sum -c cases/max12_812_order2_square_owner_cge3_universal_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_cge3_universal_20260826/compile_cge3_universal.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_cge3_universal_${label}.sing"; sha256sum "$input" "$aws_job/compiled/result.json" > "$aws_job/compiled.sha256"
set +e
...
for required in 'SQUARE_CGE3_SOURCE_HASHES=PASS' 'SQUARE_CGE3_DIVISIBLE=1' 'SQUARE_CGE3_QUOTIENT_IDENTITIES=1' 'SQUARE_CGE3_FORBIDDEN=1' 'SQUARE_CGE3_ANALYTIC_DIVISIBLE=1' 'SQUARE_CGE3_G13_ROW_IDENTITIES=1' 'SQUARE_CGE3_G14_ROW_IDENTITIES=1' 'SQUARE_CGE3_G15_ROW_IDENTITIES=1' 'SQUARE_CGE3_NUMERATOR_DIVISIBLE=1' 'SQUARE_CGE3_MOVING_DIVISION_RECURRENCE=1' 'SQUARE_CGE3_G15_MOD_L_EQUALS_MINUS_A_CUBED=1' 'SQUARE_CGE3_ENDPOINT=PASS_UNIVERSAL_CGE3_RGE1_A_ZERO_GATE'; do
...
printf 'validator=PASS_SQUARE_CGE3_UNIVERSAL\n' >> "$validation"
