...
cd "$aws_root"; sha256sum -c cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v9_subst_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_r1_d1_ac_symbolic_v9_subst_20260826/compile_v9_subst.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
r1_input="$aws_job/compiled/square_r1_v9_${label}.sing"; d1_input="$aws_job/compiled/square_d1_ac_v9_${label}.sing"
...
printf 'validator=PASS_R1_D1_AC_SYMBOLIC_V9_SUBST\n' >> "$validation"
