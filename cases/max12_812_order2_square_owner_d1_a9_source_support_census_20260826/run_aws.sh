...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a9_source_support_census_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_a9_source_support_census_20260826/compile_a9_census.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_a9_source_support_census_${label}.sing"
...
for required in 'A9_SOURCE_HASHES=PASS' 'A9_SOURCE_DIVISIBLE=1' \
  'A9_SOURCE_QUOTIENT_IDENTITIES=1' 'A9_SOURCE_SUPPORT_CENSUS_ENDPOINT=PASS_NAVIGATION_ONLY'; do
  if [[ "$(grep -Fxc "$required" "$stdout" || true)" -ne 1 ]]; then
    printf 'validator=FAIL_MISSING_OR_NONUNIQUE:%s\n' "$required" >> "$validation"; exit 90
  fi
done
if [[ "$(grep -Ec '^A9_ROW_(27|28|29|30|31)_[1-7]_BEGIN$' "$stdout" || true)" -ne 35 ]] \
   || [[ "$(grep -Ec '^A9_ROW_(27|28|29|30|31)_[1-7]_END$' "$stdout" || true)" -ne 35 ]]; then
  printf 'validator=FAIL_ROW_BLOCK_COUNT\n' >> "$validation"; exit 90
fi
...
printf 'validator=PASS_A9_LITERAL_FABER_SOURCE_SUPPORT_CENSUS\n' >> "$validation"
