...
case "$tag" in max12_812_order2_square_d1_a9_source_support_census_*) ;; *) exit 125 ;; esac
...
sha256sum -c cases/max12_812_order2_square_owner_d1_a9_source_support_census_20260826/FREEZE.sha256 > "$base/prelaunch_freeze.stdout"
nohup bash cases/max12_812_order2_square_owner_d1_a9_source_support_census_20260826/run_aws.sh "$source_root" "$evidence" "$tag" 16777216 "$characteristic" 600 1800 > "$base/outer.stdout" 2> "$base/outer.stderr" < /dev/null &
