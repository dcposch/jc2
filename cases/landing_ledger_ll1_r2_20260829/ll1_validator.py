#!/usr/bin/env python3
"""Fail-closed validator for the LL1-R2 book.
...
UNCOVERED.  No `assert` statements: all checks raise/collect explicitly, so
...
def validate(book, summary=None):
...
def main(argv):
    ...
    if fails:
        ...
        for m in fails:
            print("  - " + m)
        return 1
    print("LL1-R2 validation OK (all record arithmetic re-derived; "
          "UNCOVERED discipline enforced; summary recomputed)")
    return 0
