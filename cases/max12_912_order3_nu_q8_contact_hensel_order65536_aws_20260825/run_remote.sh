...
case_dir=$repo/cases/max12_912_order3_nu_q8_contact_hensel_order65536_aws_20260825
...
mkdir -p "$lane"
set +e
python3 "$case_dir/generate_order65536.py" --factor linear58 --order 65536 > "$lane/input.sing" 2> "$lane/generator.stderr"
generator_rc=$?
set -e
{
  echo "tag=$tag"
  ...
  echo "factor=linear58"
  echo "root=58"
  echo "order=65536"
  echo "generator_rc=$generator_rc"
  echo "timeout_seconds=28800"
  echo "virtual_memory_limit_kib=536870912"
  echo "method=full-8x8-Newton-doubling-corrected-Q8-contact"
  Singular --version | head -1 | sed 's/^/singular=/'
  sha256sum "$0" "$case_dir/generate_order65536.py" "$base_dir/generate.py" \
...
(
  ulimit -v 536870912
  timeout --signal=TERM --kill-after=180 28800 /usr/bin/time -v Singular -q < "$lane/input.sing"
) > "$lane/result.out" 2> "$lane/stderr.log"
rc=$?
set -e
endpoint=FAIL
if [ "$rc" -eq 0 ] \
  && grep -qx 'factor=linear58' "$lane/result.out" \
  && grep -qx 'base_fail=0' "$lane/result.out" \
  && grep -qx 'gcd_detJ_factor_degree=0' "$lane/result.out" \
  && grep -qx 'order=65536' "$lane/result.out" \
  && grep -qx 'moving_vdim=65536' "$lane/result.out" \
  ...
  && grep -qx 'h_contact_at_least=65536' "$lane/result.out" \
