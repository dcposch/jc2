...
/usr/bin/time -v -o "$custody/overall.time" \
  ...
  bash "$JOB_ROOT/source/jc2/cases/ggv_8_28_upper_endpoint_quotient_nf_endpoint_charts_r1_20260828/aws_job_worker.sh" &
worker_pid=$!
set -e
worker_start=$(awk '{print $22}' "/proc/$worker_pid/stat")
printf '{"pid":%s,"parent_supervisor":%s,"pgid":%s,"sid":%s,"starttime":%s}\n' \
  "$worker_pid" "$supervisor_pid" "$supervisor_pgid" "$supervisor_sid" \
  "$worker_start" > "$custody/worker_identity.json"
