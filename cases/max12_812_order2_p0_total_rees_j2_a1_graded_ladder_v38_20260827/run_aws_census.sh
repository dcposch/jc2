...
case "$tag" in max12_812_order2_p0_total_rees_j2_a1_graded_ladder_v38_*_census_"$host_label") ;; *) exit 127 ;; esac
mkdir -p "$job/run"
printf 'tag=%s\nhost_label=%s\nhost=%s\nstart_utc=%s\nsource_archive_sha256=%s\n' \
  "$tag" "$host_label" "$(hostname)" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$archive_sha" > "$job/launch_registration.txt"
cd "$root"
sha256sum -c "$package/FREEZE_CENSUS.sha256" > "$job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap"
python=/home/ubuntu/venvs/td6/bin/python
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_compiler" timeout "$wall" \
  "$python" "$package/census_graded_ladder_v38.py" "$job/compiled" --registered-lane "$tag"
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_validator" timeout "$wall" \
  "$python" "$package/validate_census_v38.py" --result "$job/compiled/census.json" \
...
grep -Fx 'PASS-A1-GRADED-LADDER-V38-CENSUS' "$job/run/${tag}_validator.stdout" >/dev/null
