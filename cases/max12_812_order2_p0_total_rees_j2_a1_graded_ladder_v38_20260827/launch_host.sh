...
tarball=$(mktemp -t j2-a1-graded-v38.XXXXXX.tar.gz)
COPYFILE_DISABLE=1 tar -czf "$tarball" \
  "$package/PREREGISTRATION.md" "$package/solve_graded_ladder_v38.py" \
  "$package/validate_graded_ladder_v38.py" "$package/run_aws.sh" "$package/launch_host.sh" "$package/FREEZE.sha256" \
...
ssh "${opt[@]}" "$target" "cd '$root' && test \"\$(sha256sum source.tar.gz | awk '{print \$1}')\" = '$sha' && tar -xzf source.tar.gz"
...
for target_label in "${labels[@]}"; do
  ...
  ssh "${opt[@]}" "$target" "mkdir -p '$job' && setsid -f bash '$package/run_aws.sh' '$root' '$job' '$tag' '$prime' '$cap' 21600 '$sha' '$host_label' '$target_label' </dev/null > '$job.outer.stdout' 2> '$job.outer.stderr'"
