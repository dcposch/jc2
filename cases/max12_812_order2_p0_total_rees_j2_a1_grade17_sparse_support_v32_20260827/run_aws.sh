...
package=cases/max12_812_order2_p0_total_rees_j2_a1_grade17_sparse_support_v32_20260827
mkdir -p "$job/run"
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_compiler" timeout "$compile_wall" python3 "$package/compile_sparse_support_v32.py" "$job/compiled" --characteristic "$characteristic"
...
bash ops/aws_exact_lane.sh "$root" "$job/run" "${tag}_validator" python3 "$package/validate_sparse_support_v32.py" --compile-result "$job/compiled/compile_result.json" --stdout "$job/run/${tag}_singular.stdout" --stderr "$job/run/${tag}_singular.stderr" --characteristic "$characteristic" --basis "$job/compiled/BASIS.txt" --nf "$job/compiled/NF.txt" --output "$job/RESULT.json"
grep -Fx 'PASS-A1-GRADE17-SPARSE-SUPPORT-V32-VALIDATOR' "$job/run/${tag}_validator.stdout" >/dev/null
