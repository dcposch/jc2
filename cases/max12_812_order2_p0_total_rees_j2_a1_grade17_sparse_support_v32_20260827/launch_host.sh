...
tag="max12_812_order2_p0_total_rees_j2_a1_grade17_sparse_support_v32_${stamp}_${suffix}"
root="/home/ubuntu/jobs/$tag/source"; job="$root/$package/aws_${suffix}"
tarball=$(mktemp -t j2-a1-grade17-sparse-v32.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
...
ssh "${opt[@]}" "$target" "cd '$root' && test \"\$(sha256sum source.tar.gz | awk '{print \$1}')\" = '$sha' && tar -xzf source.tar.gz && mkdir -p '$job' && setsid -f bash '$package/run_aws.sh' '$root' '$job' '$tag' '$characteristic' '$cap' 900 3600 '$sha' </dev/null > '$job.outer.stdout' 2> '$job.outer.stderr'"
