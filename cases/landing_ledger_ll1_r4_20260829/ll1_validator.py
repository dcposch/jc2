#!/usr/bin/env python3
"""Fail-closed validator for the LL1-R4 full-actual-floor book.
...
re-runs the engine parity checks.  R4 additionally verifies the carrier
theorem/review body seals, explicit per-direction types, lower-floor-only
metadata, exact four-cell reprice, and 13-to-7 inventory delta.  No `assert`
statements: all checks raise/collect explicitly, so behaviour is identical
under `python -O`.
...
def validate(book, summary=None, readers=None, evidence_readers=None):
    ...
    try:
        ...
        epins, ebodies = C.verify_evidence_pins(
            C.load_evidence(evidence_readers))
        need(prov.get("evidence_pins") == epins,
             "book evidence_pins drift against re-verified evidence")
        need(prov.get("evidence_body_pins") == ebodies,
             "book evidence-body pins drift against re-verified seals")
    ...
    need(prov.get("rederivation", {}).get("decision_changes") == [],
         "rederivation.decision_changes must be the empty list at R4 "
         "(any entry is a finding that blocks SOURCE_READY)")

    theorem = prov.get("carrier_theorem", {})
    need(theorem.get("claim") == "PROVED_FULL_TWO_POLE_ATTACHMENT"
         and theorem.get("carrier") == C.FULL_ACTUAL_FIRST_SEPARATION
         and theorem.get("canonical_alias") == C.FULL_ACTUAL_EXIT
         and theorem.get("semantics") == C.LOWER_FLOOR_ONLY
         and theorem.get("attainment") is False,
         "carrier theorem provenance/type/semantics drift")

    policy = book.get("pricing_policy", {})
    need(policy.get("nonzero_carrier") ==
         C.FULL_ACTUAL_FIRST_SEPARATION
         and policy.get("representative_carrier") == C.REPRESENTATIVE
         and policy.get("canonical_alias") == C.FULL_ACTUAL_EXIT
         and policy.get("semantics") == C.LOWER_FLOOR_ONLY
         and policy.get("attainment") is False,
         "R4 pricing policy drift")
    try:
        audit = C.repricing_audit()
        need(C.canonical_json(book.get("repricing_audit")) ==
             C.canonical_json(audit),
             "repricing audit drift against exact representative/full replay")
    except C.LLError as e:
        fails.append("repricing audit fail-closed: %s" % e)
    ...
    rterms, rsteps, _ = C.residue_enumeration(
        6, nonzero_carrier=C.FULL_ACTUAL_FIRST_SEPARATION)
    ...
    for row in steps:
        cell = row.get("cell")
        if cell is None:
            pure = row.get("pricing")
            if pure is not None:
                need(pure.get("direction") == "EPSILON_ZERO"
                     and pure.get("carrier") == C.REPRESENTATIVE
                     and pure.get("rule") == "legacy-P0-pure-epsilon"
                     and pure.get("semantics") == C.LOWER_FLOOR_ONLY
                     and pure.get("attainment") is False,
                     "pure-epsilon row was silently repriced")
            continue
        pricing = cell.get("pricing", {})
        need(pricing.get("nonzero_carrier") ==
             C.FULL_ACTUAL_FIRST_SEPARATION,
             "cell nonzero carrier is not FULL_ACTUAL_FIRST_SEPARATION")
        need(pricing.get("semantics") == C.LOWER_FLOOR_ONLY
             and pricing.get("attainment") is False,
             "cell price lacks lower-floor/no-attainment semantics")
        need(int(cell.get("lam")) == pricing.get("applied_total")
             and int(cell.get("lam_representative")) ==
             pricing.get("representative_total"),
             "cell applied/representative total drift")
        for comp in pricing.get("nonzero", []):
            need(comp.get("carrier") ==
                 C.FULL_ACTUAL_FIRST_SEPARATION
                 and comp.get("certificate") ==
                 "FULL-ACTUAL-FIRST-SEPARATION"
                 and comp.get("attainment") is False,
                 "nonzero component carrier/attainment drift")
        eps = pricing.get("epsilon_zero")
        if eps is not None:
            need(eps.get("carrier") == C.REPRESENTATIVE
                 and eps.get("rule") == "legacy-AF2-epsilon-zero",
                 "epsilon/zero component was silently repriced")
    for t in terms:
        wstr, M, lam = t["state"]
        w = _fr(wstr)
...
def main(argv):
    ...
    if fails:
        ...
        for m in fails:
            print("  - " + m)
        return 1
    print("LL1-R4 validation OK (all record arithmetic re-derived; carrier "
          "and lower-floor semantics checked; exact 4-cell / 13->7 replay; "
          "%d canonical + %d evidence pins re-verified)" %
          (len(C.SOURCE_PINS), len(C.EVIDENCE_PINS)))
    return 0
