...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_finite_band_a2_a5_fullsupport_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_finite_band_a2_a5_fullsupport_20260826/compile_finite_band.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_finite_band_a2_a5_${label}.sing"
...
for a in 2 3 4 5; do
  g=$((11+2*a)); gp=$((g+1))
  for required in \
    "A${a}_SOURCE_HASHES=PASS" \
    "A${a}_FIXED_CONTACT=${a}" \
    "A${a}_GRADE_PAIR=${g},${gp}" \
    "A${a}_SOURCE_DIVISIBLE=1" \
    "A${a}_SOURCE_QUOTIENT_IDENTITIES=1" \
    "A${a}_SOURCE_FORBIDDEN=1" \
    "A${a}_ANALYTIC_DIVISIBLE=1" \
    "A${a}_ROW_IDENTITIES_${g}=1" \
    "A${a}_ROW_IDENTITIES_${gp}=1" \
    "A${a}_DENOMINATOR_RECURRENCE_FIRST=1" \
    "A${a}_DENOMINATOR_RECURRENCE_NEXT=1" \
    "A${a}_COMPLETE_SOURCE_BRIDGE=1" \
    "A${a}_BOTH_ROOT_ALLOCATIONS=1" \
    "A${a}_UNMATCHED_C2_RESIDUE=1" \
    "A${a}_ENDPOINT=PASS_FIXED_D1_CONTACT_A${a}"; do
...
if [[ "$(grep -Fxc 'D1_FINITE_BAND_A2_A5_ENDPOINT=PASS' "$stdout" || true)" -ne 1 ]]; then
...
printf 'validator=PASS_D1_FINITE_BAND_A2_A5_FULLSUPPORT\n' >> "$validation"
