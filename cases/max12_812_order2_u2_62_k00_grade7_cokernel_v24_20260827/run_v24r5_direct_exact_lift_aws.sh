...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V24R5 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 6 ]]; then
...
mkdir -p "$k00_output_root/run"
{
  ...
  printf 'design=v24r5_exact_Q_direct_target_lift_slimgb\n'
  printf 'workers=1\n'
...
/usr/bin/time -v timeout "$k00_timeout_seconds" \
  python3 "$k00_case_rel/compile_v24r5_direct_exact_lift.py" \
...
if [[ $k00_rc -eq 0 ]] && grep -q '^K00_V24R5=' "$k00_output_root/run/compiler.stdout"; then
