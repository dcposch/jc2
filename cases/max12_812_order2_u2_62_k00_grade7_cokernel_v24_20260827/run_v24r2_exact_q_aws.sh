...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V24R2 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 6 ]]; then
...
mkdir -p "$aws_job/run"
{
  ...
  printf 'design=v24r2_tracked_exact_Q_properness_and_compatibility\n'
  printf 'scope=no_Wzero_no_later_grades_no_full_jet_no_arc_no_closure_no_JC2\n'
...
/usr/bin/time -v timeout "$timeout_seconds" \
  python3 "$rel/compile_v24r2_exact_q.py" "$aws_job/output" \
...
grep -q '^K00_V24R2=' "$aws_job/run/compiler.stdout"
