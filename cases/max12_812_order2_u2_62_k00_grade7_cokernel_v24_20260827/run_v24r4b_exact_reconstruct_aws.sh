...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V24R4B runner refused host" >&2
  exit 125
fi
if [[ $# -ne 6 ]]; then
...
mkdir -p "$k00_output_root/run"
{
  ...
  printf 'design=v24r4b_exact_crt_rational_reconstruction\n'
  printf 'workers=1\n'
  printf 'virtual_memory_cap_kib=%s\n' "$k00_vmem_kib"
  printf 'outer_timeout_seconds=%s\n' "$k00_timeout_seconds"
  printf 'scope=exact_Q_D_k10_0_W_prefix_grade6_only_no_arc_no_closure_no_JC2\n'
...
/usr/bin/time -v timeout "$k00_timeout_seconds" \
  python3 "$k00_case_rel/compile_v24r4b_exact_reconstruct.py" \
...
if [[ $k00_rc -eq 0 ]] && grep -q '^K00_V24R4B=' "$k00_output_root/run/compiler.stdout"; then
