...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V24R4A runner refused host" >&2
  exit 125
fi
if [[ $# -ne 6 ]]; then
...
mkdir -p "$output_root/run"
{
  ...
  printf 'design=v24r4a_eight_prime_tracked_unit_certificates\n'
  printf 'workers=8\n'
  printf 'per_process_virtual_memory_cap_kib=%s\n' "$vmem_kib"
  printf 'outer_timeout_seconds=%s\n' "$timeout_seconds"
  printf 'scope=modular_support_discovery_only_no_exact_Q_no_arc_no_closure_no_JC2\n'
...
/usr/bin/time -v timeout "$timeout_seconds" \
  python3 "$case_rel/compile_v24r4a_modcerts.py" "$output_root/output" \
...
if [[ $rc -eq 0 ]] && grep -q '^K00_V24R4A=' "$output_root/run/compiler.stdout"; then
  result_status=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["status"])' \
    "$output_root/output/RESULT.json")
  printf 'engine_rc=0\nvalidator=%s\n' "$result_status" \
    > "$output_root/run/FINAL.validation"
else
  printf 'engine_rc=%s\nvalidator=FAIL\n' "$rc" \
    > "$output_root/run/FINAL.validation"
  exit "$rc"
