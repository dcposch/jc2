...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "AWS-only genus client refused non-Linux host" >&2
  exit 125
fi
vendor=$(tr -d '\n' < /sys/class/dmi/id/sys_vendor 2>/dev/null || true)
if [[ "$vendor" != "Amazon EC2" ]]; then
  echo "AWS-only genus client refused vendor=${vendor:-unknown}" >&2
  exit 125
fi
if [[ $# -ne 4 ]]; then
  echo "usage: run_mod32003_genus_aws.sh AWS_ROOT AWS_RUN TAG INPUT" >&2
  exit 125
fi
