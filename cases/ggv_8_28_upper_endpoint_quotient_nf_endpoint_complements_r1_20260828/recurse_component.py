...
def elimination_lines(label: str, assignment: str, track_transform: bool,
    ...
    return [
        ...
        + c_swap + ' B[step,step]=reduce(B[step,step],REDUCER_PLACEHOLDER); operation_reductions=operation_reductions+1; if(B[step,step]==0 || deg(B[step,step])!=0){ invariant_failures=invariant_failures+1; } inverse_unit=1/leadcoef(B[step,step]); for(i=step+1;i<=106;i=i+1){ multiple=reduce(B[i,step]*inverse_unit,REDUCER_PLACEHOLDER); operation_reductions=operation_reductions+1; for(j=step;j<=105;j=j+1){ B[i,j]=reduce(B[i,j]-multiple*B[step,j],REDUCER_PLACEHOLDER); operation_reductions=operation_reductions+1; } } for(j=step+1;j<=105;j=j+1){ multiple=reduce(B[step,j]*inverse_unit,REDUCER_PLACEHOLDER); operation_reductions=operation_reductions+1; for(i=step;i<=106;i=i+1){ B[i,j]=reduce(B[i,j]-multiple*B[i,step],REDUCER_PLACEHOLDER); operation_reductions=operation_reductions+1; } '
        + c_update + ' } for(i=1;i<=106;i=i+1){ if(i!=step && reduce(B[i,step],REDUCER_PLACEHOLDER)!=0){ invariant_failures=invariant_failures+1; } } for(j=1;j<=105;j=j+1){ if(j!=step && reduce(B[step,j],REDUCER_PLACEHOLDER)!=0){ invariant_failures=invariant_failures+1; } } write(PIVOTFILE,string(step)+"|"+string(pi)+"|"+string(pj)+"|"+string(B[step,step])); pivot_count=pivot_count+1; step=step+1; } }'.replace("REDUCER_PLACEHOLDER", reducer),
...
def build_saturation_script(remaining: list[str], generators: list[str],
    ...
    lines += [
        ...
        "ideal OPEN_SB=std(OPEN_SAT_RESULT[1]); poly OPEN_UNIT_NF=reduce(1,OPEN_SB);",
        f'write("NODE_{node:03d}_OPEN_SAT_STANDARD_BASIS.txt",string(OPEN_SB));',
        'print("OPEN_SATURATION_EXPONENT="+string(OPEN_SAT_RESULT[2])); print("OPEN_CHART_EMPTY="+string(OPEN_UNIT_NF==0));',
...
def build_chart_script(remaining: list[str], generators: list[str], node: int,
    ...
    lines += [
        ...
        "ideal ACTIVE_SB=std(OPEN_SAT_RESULT[1]); poly ACTIVE_UNIT_NF=reduce(1,ACTIVE_SB);",
    ...
    lines += elimination_lines(label, assignment, True, prefix, "ACTIVE_SB")
    if rank == 0:
...
def main() -> int:
    ...
    try:
        for node_index in range(1, args.max_nodes + 1):
            ...
            open_empty = "OPEN_CHART_EMPTY=1" in saturation_text.splitlines()
            next_empty = "NEXT_REMAINDER_EMPTY=1" in saturation_text.splitlines()
            delta = scanned_delta
            if open_empty:
                node_result = {
                    "node": node_index,
