...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_disc_halfweight_kuranishi_v3_row_identity_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_disc_halfweight_kuranishi_v3_row_identity_20260826/compile_disc_halfweight_v3_rows.py \
...
input="$aws_job/compiled/disc_halfweight_v3_rows_p${characteristic}.sing"
...
required=(
  ...
  'DISC_HALF_ROW_TRANSFORM_UNIT_DIAGONAL=1'
  'DISC_HALF_SOURCE_ANALYTIC_IDEAL_EQUALITY_BY_UNITRIANGULAR_ROWS=1'
  'DISC_HALF_V3_DIRECT_ROW_CERTIFICATE=1'
  'DISC_HALF_ENDPOINT=PASS_EXACT_SOURCE_ANALYTIC_ROW_EQUALITY'
)
for row in 1 2 3 4 5 6 7; do required+=("DISC_HALF_ROW_IDENTITY_${row}=1"); done
...
printf 'validator=PASS_DISC_HALFWEIGHT_V3_DIRECT_ROWS\n' >> "$validation"
