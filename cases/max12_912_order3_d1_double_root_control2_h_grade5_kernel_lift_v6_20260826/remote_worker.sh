...
case "$tag" in max12_912_order3_d1_double_root_control2_h_grade5_kernel_lift_v6_20260826T*) ;; *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;; esac
...
( ulimit -v 4194304; nice -n 10 timeout 900 /usr/bin/time -v -o compiler.time \
  python3 -u compile_h_grade5_kernel_lift_v6.py --order "$order" --output h_grade5_lift.json \
...
if test "$rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -Ec '^PASS_H_GRADE5_KERNEL_LIFT_V6_(SOLVABLE|COKERNEL)$' compiler.stdout 2>/dev/null)" -ne 1; then
  echo COMPILER_OR_DIAGNOSTIC_FAILURE > FAILED; exit 96
fi
sha256sum h_grade5_lift.json compiler.stdout compiler.stderr compiler.rc compiler.time > result.sha256
touch DONE
