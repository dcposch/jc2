...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_affine_faber_k_ift_lift_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
python3 cases/max12_812_order2_affine_faber_k_ift_lift_20260826/compile_k_ift.py "$compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
...
for required in 'K_IFT_TCOEFF_CONTROL=1' 'K_IFT_EXACT_DIVISIONS=1' 'K_IFT_POINT_CONTROL=1' 'K_IFT_JAC_CONTROL=1' 'K_IFT_ENDPOINT=PASS_RATIONAL_J_POINT_AND_FORMAL_IFT' 'K_IFT_DONE=1' 'K_IFT_SCOPE=NORMALIZED_ORDINARY_FABER_ONLY_NO_TOTAL_REES_TERMINAL_TAYLOR_ORDER2_OR_JC2_VERDICT'; do
