...
tag="max12_812_order2_p0_total_rees_allrows_g13_g14_export_v20_${stamp}_${suffix}"
remote_root="/home/ubuntu/jobs/$tag/source"; remote_job="$remote_root/$package/aws_${suffix}"
tarball=$(mktemp -t p0-total-rees-allrows-g13-g14-v20.XXXXXX.tar.gz); trap 'rm -f "$tarball"' EXIT
COPYFILE_DISABLE=1 tar -czf "$tarball" "$package" "$replay/replay_row5_grade14.py" "$tails" "$v9/aws_q_v9" "$v9/aws_p65521_v9" "$v17/aws_q/compiled/Tg14_5_q.poly" "$v17/aws_p65521/compiled/Tg14_5_p65521.poly" ops/aws_exact_lane.sh
