...
package=cases/max12_812_order2_p0_total_rees_allrows_g13_g14_export_v20_20260827
mkdir -p "$aws_job/run"
{
...
set +e
/usr/bin/time -v timeout "$compile_wall" python3 "$package/export_allrows_g13_g14_v20.py" "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
set +e
python3 "$package/validate_export_v20.py" --compiler-result "$aws_job/compiled/result.json" --stdout "$stdout" --stderr "$stderr" --characteristic "$characteristic" --output "$aws_job/RESULT.json" > "$aws_job/validator.stdout" 2> "$aws_job/validator.stderr"
validator_rc=$?
set -e
printf 'validator_rc=%s\n' "$validator_rc" > "$aws_job/validator.validation"; if [[ "$validator_rc" -ne 0 ]]; then exit "$validator_rc"; fi
printf 'validator=PASS_TOTAL_REES_ALLROWS_G13_G14_EXPORT_V20\n' >> "$aws_job/validator.validation"
