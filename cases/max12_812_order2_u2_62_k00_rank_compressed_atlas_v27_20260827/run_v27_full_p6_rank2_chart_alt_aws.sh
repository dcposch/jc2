...
if [[ "$(uname -s)" != Linux ]] || \
  ...
  echo "AWS-only alternate V27 full-P6 chart runner refused host" >&2
  exit 125
fi
if [[ $# -ne 10 ]]; then
...
case "$k00_lane_tag" in
  max12_812_order2_u2_62_k00_v27_alt_full_p6_rank2_*_r6a) ;;
  *) echo "malformed alternate V27 full-P6 lane" >&2; exit 125 ;;
esac
if [[ "$k00_cap_kib" != 402653184 ]] || \
   [[ "$k00_outer_timeout" != 21600 ]] || [[ "$k00_inner_timeout" != 21000 ]]; then
  echo "alternate full-P6 resource envelope mismatch" >&2
  exit 125
fi
...
mkdir -p "$k00_job_root/run"
{
  ...
  printf 'target=FULL_P6_PLUS_I3A_SELECTED_I2_CHART_ONE_MINOR_ONLY\n'
  printf 'grade_seven_compatibility_claim=false\nrational_point_claim=false\n'
...
/usr/bin/time -v timeout "$k00_outer_timeout" nice -n 5 python3 -B \
  "$k00_case_rel/run_v27_full_p6_rank2_chart_alt_exact.py" \
  "$k00_source_root" "$k00_job_root/output" --timeout "$k00_inner_timeout" \
  > "$k00_job_root/run/full_p6_rank2_chart.stdout" 2> "$k00_job_root/run/full_p6_rank2_chart.stderr"
k00_rc=$?
set -e
...
k00_max_rss=$(sed -n 's/^[[:space:]]*Maximum resident set size (kbytes):[[:space:]]*//p' "$k00_job_root/run/full_p6_rank2_chart.stderr" | tail -n 1)
k00_swaps=$(sed -n 's/^[[:space:]]*Swaps:[[:space:]]*//p' "$k00_job_root/run/full_p6_rank2_chart.stderr" | tail -n 1)
k00_elapsed=$(sed -n 's/^[[:space:]]*Elapsed (wall clock) time (h:mm:ss or m:ss):[[:space:]]*//p' "$k00_job_root/run/full_p6_rank2_chart.stderr" | tail -n 1)
...
if [[ "$k00_rc" -eq 124 ]] || [[ "$k00_rc" -eq 137 ]]; then
  ...
  k00_exit=124
elif [[ "$k00_rc" -eq 0 ]] && [[ -s "$k00_job_root/run/full_p6_rank2_chart.stdout" ]] && \
  ...
  case "$k00_status" in
    PASS_V27_ALT_FULL_P6_SELECTED_I2_CHART_PROPER_ALGEBRAIC_RANK2_POINT_PRODUCER_UNREVIEWED_ROLLBACK_R1_R2_R3|PASS_V27_ALT_FULL_P6_SELECTED_I2_CHART_UNIT_ONLY_THIS_CHART_EMPTY_PRODUCER_UNREVIEWED_ROLLBACK_R1_R2_R3|RESOURCE_CAP_NO_VERDICT)
