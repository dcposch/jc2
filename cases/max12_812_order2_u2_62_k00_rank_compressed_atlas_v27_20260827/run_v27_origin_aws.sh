...
if [[ "$(uname -s)" != Linux ]] || \
  ...
  echo "AWS-only V27 origin runner refused host" >&2
  exit 125
fi
if [[ $# -ne 10 ]]; then
...
case "$k00_lane_tag" in
  max12_812_order2_u2_62_k00_v27_origin_replay_*_r6a) ;;
  *) echo "malformed registered origin lane" >&2; exit 125 ;;
esac
if [[ "$k00_cap_kib" != 8388608 ]] || [[ "$k00_outer_timeout" != 600 ]] || \
   [[ "$k00_inner_timeout" != 540 ]]; then
  echo "origin resource envelope mismatch" >&2
  exit 125
fi
...
if find "$k00_source_root" -type f -perm /222 -print -quit | grep -q .; then
  echo "source tree writable" >&2
  exit 125
fi
k00_freeze="$k00_source_root/$k00_freeze_rel"
if [[ "$(sha256sum "$k00_freeze" | awk '{print $1}')" != "$k00_expected_freeze_sha" ]] || \
...
/usr/bin/time -v timeout "$k00_outer_timeout" nice -n 5 python3 -B \
  "$k00_case_rel/run_v27_origin_exact_replay.py" \
  "$k00_source_root" "$k00_job_root/output" --timeout "$k00_inner_timeout" \
  > "$k00_job_root/run/origin.stdout" 2> "$k00_job_root/run/origin.stderr"
k00_rc=$?
set -e
printf 'end_utc=%s\nrc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$k00_rc" \
  >> "$k00_job_root/launch_registration.txt"
k00_max_rss=$(sed -n 's/^[[:space:]]*Maximum resident set size (kbytes):[[:space:]]*//p' "$k00_job_root/run/origin.stderr" | tail -n 1)
k00_swaps=$(sed -n 's/^[[:space:]]*Swaps:[[:space:]]*//p' "$k00_job_root/run/origin.stderr" | tail -n 1)
k00_elapsed=$(sed -n 's/^[[:space:]]*Elapsed (wall clock) time (h:mm:ss or m:ss):[[:space:]]*//p' "$k00_job_root/run/origin.stderr" | tail -n 1)
...
if [[ "$k00_rc" -eq 124 ]] || [[ "$k00_rc" -eq 137 ]]; then
  ...
  if [[ "$k00_status" == PASS_V27_AUX_ORIGIN_B_I5_EXACT_TRIVIAL_Q_POINT ]]; then
