...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V27 R2 BASE3 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 10 ]]; then
...
case "$k00_lane_tag" in
  max12_812_order2_u2_62_k00_v27_r2_base3_*_r6a) ;;
  *) echo "malformed registered V27 R2 BASE3 lane" >&2; exit 125 ;;
esac
if [[ "$k00_cap_kib" != "402653184" ]] || \
   [[ "$k00_outer_timeout" != "21600" ]] || \
   [[ "$k00_inner_timeout" != "21000" ]]; then
  echo "V27 R2 BASE3 resource envelope differs from preregistration" >&2
  exit 125
fi
...
if [[ ! -f "$k00_freeze" ]]; then
  echo "source freeze missing" >&2
  exit 125
fi
k00_actual_freeze_sha=$(sha256sum "$k00_freeze" | awk '{print $1}')
k00_actual_archive_sha=$(sha256sum "$k00_source_archive" | awk '{print $1}')
if [[ "$k00_actual_freeze_sha" != "$k00_expected_freeze_sha" ]] || \
   [[ "$k00_actual_archive_sha" != "$k00_expected_archive_sha" ]]; then
...
mkdir -p "$k00_job_root/run"
{
  ...
  printf 'source_freeze_sha256=%s\n' "$k00_actual_freeze_sha"
  printf 'source_archive_sha256=%s\n' "$k00_actual_archive_sha"
  printf 'design=v27_r2_exact_B_plus_I5A_plus_I4A_plus_I3A_rank_recursion\n'
  printf 'lifecycle=PRODUCER_UNREVIEWED_SPECULATIVE_ROLLBACK_BASE4_R1\n'
  printf 'rollback_dependency=BASE4_R1_PRODUCER_UNREVIEWED_SCHEDULING_ONLY\n'
  printf 'placement=r6a_idle_at_live_preflight\n'
  printf 'protected_box02_lanes=R2_R5_NO_SIGNAL_NO_RENICE_NO_MODIFICATION\n'
  printf 'scope=six_variable_base3_only_no_full_P6_no_grade7_compatibility_no_jet_no_arc_no_closure_no_JC2\n'
...
/usr/bin/time -v timeout "$k00_outer_timeout" \
  nice -n 5 python3 -B "$k00_case_rel/run_v27_base3_r2_exact_prepass.py" \
  "$k00_source_root" "$k00_job_root/output" --timeout "$k00_inner_timeout" \
  > "$k00_job_root/run/base3_r2.stdout" \
  2> "$k00_job_root/run/base3_r2.stderr"
k00_rc=$?
set -e
...
k00_max_rss=$(sed -n 's/^[[:space:]]*Maximum resident set size (kbytes):[[:space:]]*//p' "$k00_job_root/run/base3_r2.stderr" | tail -n 1)
k00_swaps=$(sed -n 's/^[[:space:]]*Swaps:[[:space:]]*//p' "$k00_job_root/run/base3_r2.stderr" | tail -n 1)
k00_elapsed=$(sed -n 's/^[[:space:]]*Elapsed (wall clock) time (h:mm:ss or m:ss):[[:space:]]*//p' "$k00_job_root/run/base3_r2.stderr" | tail -n 1)
{
...
if [[ "$k00_rc" -eq 124 ]] || [[ "$k00_rc" -eq 137 ]]; then
  ...
  k00_exit=124
elif [[ "$k00_rc" -eq 0 ]] && [[ -s "$k00_job_root/run/base3_r2.stdout" ]] && \
     [[ -s "$k00_job_root/output/RESULT.json" ]] && \
     [[ "$k00_max_rss" =~ ^[0-9]+$ ]] && [[ "$k00_swaps" == "0" ]] && \
     (( k00_max_rss <= k00_cap_kib )); then
  k00_status=$(python3 -B -c \
    'import json,sys; print(json.load(open(sys.argv[1]))["status"])' \
    "$k00_job_root/output/RESULT.json")
  case "$k00_status" in
    PASS_V27_R2_EXACT_RANK3_PURE_PRODUCER_UNREVIEWED_ROLLBACK_R1|PASS_V27_R2_EXACT_RANK_LE2_SURVIVES_PRODUCER_UNREVIEWED_ROLLBACK_R1|RESOURCE_CAP_NO_VERDICT)
