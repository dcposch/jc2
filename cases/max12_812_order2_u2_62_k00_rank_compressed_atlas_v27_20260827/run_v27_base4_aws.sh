...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "AWS-only V27 BASE4 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 10 ]]; then
...
case "$k00_lane_tag" in
  max12_812_order2_u2_62_k00_v27_base4_*_r6a) ;;
  *) echo "malformed registered V27 BASE4 lane" >&2; exit 125 ;;
esac
if [[ "$k00_cap_kib" != "402653184" ]] || \
   [[ "$k00_outer_timeout" != "21600" ]] || \
   [[ "$k00_inner_timeout" != "21000" ]]; then
  echo "V27 BASE4 resource envelope differs from preregistration" >&2
  exit 125
fi
...
mkdir -p "$k00_job_root/run"
{
  ...
  printf 'design=v27_base4_exact_B_plus_I5A_plus_I4A_rank_purity_prepass\n'
  printf 'lifecycle=PRODUCER_UNREVIEWED_SPECULATIVE\n'
  printf 'placement=r6a_idle_at_live_preflight\n'
  printf 'protected_box02_lanes=R2_R5_NO_SIGNAL_NO_RENICE_NO_MODIFICATION\n'
  printf 'scope=six_variable_base4_only_no_full_P6_no_grade7_compatibility_no_jet_no_arc_no_closure_no_JC2\n'
...
/usr/bin/time -v timeout "$k00_outer_timeout" \
  nice -n 5 python3 -B "$k00_case_rel/run_v27_base4_exact_prepass.py" \
  "$k00_source_root" "$k00_job_root/output" --timeout "$k00_inner_timeout" \
  > "$k00_job_root/run/base4.stdout" \
  2> "$k00_job_root/run/base4.stderr"
k00_rc=$?
set -e
...
k00_max_rss=$(sed -n 's/^[[:space:]]*Maximum resident set size (kbytes):[[:space:]]*//p' "$k00_job_root/run/base4.stderr" | tail -n 1)
k00_swaps=$(sed -n 's/^[[:space:]]*Swaps:[[:space:]]*//p' "$k00_job_root/run/base4.stderr" | tail -n 1)
k00_elapsed=$(sed -n 's/^[[:space:]]*Elapsed (wall clock) time (h:mm:ss or m:ss):[[:space:]]*//p' "$k00_job_root/run/base4.stderr" | tail -n 1)
{
...
if [[ "$k00_rc" -eq 124 ]] || [[ "$k00_rc" -eq 137 ]]; then
  ...
  k00_exit=124
elif [[ "$k00_rc" -eq 0 ]] && [[ -s "$k00_job_root/run/base4.stdout" ]] && \
  ...
  case "$k00_status" in
    PASS_V27_BASE4_EXACT_RANK4_PURE_PRODUCER_UNREVIEWED|PASS_V27_BASE4_EXACT_RANK_LE3_SURVIVES_PRODUCER_UNREVIEWED|RESOURCE_CAP_NO_VERDICT)
