...
if [[ "$(uname -s)" != Linux ]] || \
  ...
  echo "AWS-only V27 R3 BASE2 runner refused host" >&2
  exit 125
fi
if [[ $# -ne 10 ]]; then
...
case "$k00_lane_tag" in
  max12_812_order2_u2_62_k00_v27_r3_base2_*_r6a) ;;
  *) echo "malformed registered V27 R3 BASE2 lane" >&2; exit 125 ;;
esac
if [[ "$k00_cap_kib" != 402653184 ]] || \
   [[ "$k00_outer_timeout" != 21600 ]] || [[ "$k00_inner_timeout" != 21000 ]]; then
  echo "R3 resource envelope mismatch" >&2
  exit 125
fi
...
mkdir -p "$k00_job_root/run"
{
  ...
  printf 'lifecycle=PRODUCER_UNREVIEWED_SPECULATIVE_ROLLBACK_BASE4_R1_BASE3_R2\n'
  printf 'known_origin=UNIT_ENDPOINT_IS_CONTRADICTION\n'
...
/usr/bin/time -v timeout "$k00_outer_timeout" nice -n 5 python3 -B \
  "$k00_case_rel/run_v27_base2_r3_exact_prepass.py" \
  "$k00_source_root" "$k00_job_root/output" --timeout "$k00_inner_timeout" \
  > "$k00_job_root/run/base2_r3.stdout" 2> "$k00_job_root/run/base2_r3.stderr"
k00_rc=$?
set -e
printf 'end_utc=%s\nrc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$k00_rc" \
  >> "$k00_job_root/launch_registration.txt"
k00_max_rss=$(sed -n 's/^[[:space:]]*Maximum resident set size (kbytes):[[:space:]]*//p' "$k00_job_root/run/base2_r3.stderr" | tail -n 1)
k00_swaps=$(sed -n 's/^[[:space:]]*Swaps:[[:space:]]*//p' "$k00_job_root/run/base2_r3.stderr" | tail -n 1)
k00_elapsed=$(sed -n 's/^[[:space:]]*Elapsed (wall clock) time (h:mm:ss or m:ss):[[:space:]]*//p' "$k00_job_root/run/base2_r3.stderr" | tail -n 1)
...
if [[ "$k00_rc" -eq 124 ]] || [[ "$k00_rc" -eq 137 ]]; then
  ...
  k00_exit=124
elif [[ "$k00_rc" -eq 0 ]] && [[ -s "$k00_job_root/run/base2_r3.stdout" ]] && \
  ...
  case "$k00_status" in
    PASS_V27_R3_EXACT_RANK_LE1_SURVIVES_PRODUCER_UNREVIEWED_ROLLBACK_R1_R2|RESOURCE_CAP_NO_VERDICT)
