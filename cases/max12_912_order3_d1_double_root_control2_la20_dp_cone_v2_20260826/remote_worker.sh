...
case "$tag" in
  max12_912_order3_d1_double_root_control2_la20_dp_cone_v2_20260826T*) ;;
...
(
  ulimit -v 2097152
  nice -n 10 timeout 300 python3 -u compile_la20_dp_cone_v2.py
) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
sha256sum la20_dp_cone_v2.sing > compiled.sha256 2>/dev/null || true
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_CONTROL2_LA20_DP_CONE_V2_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1 || \
...
(
  ...
  nice -n 10 timeout --signal=TERM --kill-after=300 "$solve_timeout" \
    /usr/bin/time -v -o singular.time /usr/bin/Singular -q la20_dp_cone_v2.sing
...
sha256sum compiler.stdout compiler.stderr compiler.rc la20_dp_cone_v2.sing \
