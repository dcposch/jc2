...
cd "$aws_root"; sha256sum -c cases/max12_812_order2_square_owner_c3_vertical_thin_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"; ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_c3_vertical_thin_20260826/compile_c3_vertical.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_c3_vertical_thin_${label}.sing"; sha256sum "$input" "$aws_job/compiled/result.json" > "$aws_job/compiled.sha256"
set +e
...
for required in \
 'SQUARE_CGE3_SOURCE_HASHES=PASS' 'SQUARE_CGE3_DIVISIBLE=1' 'SQUARE_CGE3_QUOTIENT_IDENTITIES=1' 'SQUARE_CGE3_FORBIDDEN=1' \
 'SQUARE_CGE3_ANALYTIC_DIVISIBLE=1' 'SQUARE_CGE3_G13_ROW_IDENTITIES=1' 'SQUARE_CGE3_G14_ROW_IDENTITIES=1' 'SQUARE_CGE3_G15_ROW_IDENTITIES=1' \
 'SQUARE_C3_SOURCE_BRIDGE=1' 'SQUARE_C3_OMITTED_CONNECTION_REJECTED=1' 'SQUARE_C3_D1_ROOT_ALLOCATION=1' \
 'SQUARE_C3_D2_MOD_L=1' 'SQUARE_C3_D3_R2_FACTOR=1' 'SQUARE_C3_D4_MOD_COMPLEMENT=1' 'SQUARE_C3_RGE3_SYMBOLIC=1' \
 'SQUARE_C3_DECK_D1=1' 'SQUARE_C3_DECK_D2=1' 'SQUARE_C3_DECK_D4=1' 'SQUARE_C3_WRONG_CUBIC_REJECTED=1' \
 'SQUARE_C3_ENDPOINT=PASS_VERTICAL_C3_RGE2_THIN_DIVISIBILITY'; do
...
printf 'validator=PASS_SQUARE_C3_VERTICAL_THIN\n' >> "$validation"
