#!/usr/bin/env bash
# Hardened AWS-only runner for the `(mu,r,D)=(2,2,6)` invariant-ring pilot.
set -euo pipefail

if (( $# != 11 )); then
  echo "usage: $0 JOB_ROOT LANE_TAG EXPECTED_INSTANCE_ID CHARACTERISTIC JACOBIAN_TARGET ENGINE MEMORY_KIB TIMEOUT_SECONDS EXPECTED_RUNNER_SHA256 EXPECTED_GENERATOR_SHA256 EXPECTED_GIT_BASIS" >&2
  exit 125
fi
...
if [[ ! "$LANE_TAG" =~ ^quartic_inv_mu2_d6_[A-Za-z0-9][A-Za-z0-9_-]{0,95}$ ]]; then
  echo "refusing malformed or unregistered lane tag: $LANE_TAG" >&2
  exit 125
fi
readonly LANE_SUFFIX=${LANE_TAG#quartic_inv_mu2_d6_}
readonly CANONICAL_JOB_ROOT=/home/ubuntu/jobs/quartic_inv_mu2_d6_$LANE_SUFFIX
if [[ "$JOB_ROOT" != "$CANONICAL_JOB_ROOT" ]]; then
  echo "refusing noncanonical job root: $JOB_ROOT" >&2
  exit 125
fi
if [[ ! "$EXPECTED_INSTANCE_ID" =~ ^i-[0-9a-f]+$ ]]; then
  echo "invalid expected EC2 instance id" >&2
  exit 125
fi
if [[ ! "$CHARACTERISTIC" =~ ^(0|[1-9][0-9]{0,9})$ ]]; then
  echo "invalid characteristic" >&2
  exit 125
fi
case "$JACOBIAN_TARGET" in 2|3) ;; *) echo "invalid Jacobian target" >&2; exit 125 ;; esac
...
if [[ ! "$EXPECTED_GIT_BASIS" =~ ^[0-9a-f]{40}$ ]]; then
  echo "invalid expected git basis" >&2
  exit 125
fi
...
readonly SOURCE_GENERATOR=$SCRIPT_DIR/generate_mu2_d6.py
[[ -f "$SOURCE_GENERATOR" ]] || { echo "missing generator" >&2; exit 125; }
...
mkdir -- "$PAYLOAD" "$RUN"
cp "$0" "$PAYLOAD/aws_mu2_d6_run.sh"
cp "$SOURCE_GENERATOR" "$PAYLOAD/generate_mu2_d6.py"
readonly GENERATOR=$PAYLOAD/generate_mu2_d6.py
...
{
  printf 'schema=JC2_QUARTIC_INVARIANT_MU2_D6_AWS_RUN_V1\n'
  printf 'lane=%s\n' "$LANE_TAG"
...
set +e
/usr/bin/timeout --signal=TERM --kill-after=30 300 \
  /usr/bin/python3 "$GENERATOR" \
    --characteristic "$CHARACTERISTIC" \
    --jacobian-target "$JACOBIAN_TARGET" \
    --engine "$ENGINE" \
