...
case "$tag" in
  max12_912_order3_d1_double_root_control2_la20_witness_replay_v1_20260826T*) ;;
...
( ulimit -v 2097152; nice -n 10 timeout 300 python3 -u compile_witness_replay.py ) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
sha256sum witness_replay.sing > compiled.sha256 2>/dev/null || true
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || test "$(grep -c '^PASS_CONTROL2_LA20_WITNESS_REPLAY_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then
...
(
  ...
  nice -n 10 timeout --signal=TERM --kill-after=60 "$solve_timeout" \
    /usr/bin/time -v -o singular.time /usr/bin/Singular -q witness_replay.sing
...
if test "$solver_rc" -ne 0 || test -s singular.stderr || test -s singular.stdout.diagnostics || \
   test "$(grep -c '^PASS_LITERAL_REES_IDENTITY$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_DEHOMOGENIZED_WITNESS$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_PRIMITIVE_INTEGRAL_SCALE$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_UNIQUE_LA20_MINIMUM_AT_REGISTERED_WEIGHT$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -c '^PASS_CONTROL2_LA20_WITNESS_REPLAY_V1$' singular.stdout 2>/dev/null)" -ne 1 || \
   grep -q '^FAIL_' singular.stdout; then
  echo SOLVER_OR_DIAGNOSTIC_FAILURE > SOLVE_FAILED; exit 98
fi
sha256sum compiler.stdout compiler.stderr compiler.rc witness_replay.sing singular.stdout singular.stderr singular.stdout.diagnostics singular.time singular.rc > result.sha256
touch SOLVE_DONE
