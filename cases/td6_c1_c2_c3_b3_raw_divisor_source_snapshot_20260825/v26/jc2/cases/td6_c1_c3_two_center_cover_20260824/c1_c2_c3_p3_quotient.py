#!/usr/bin/env python3
"""Exact TD6 replay on a licensed three-center exceptional curve.
...
Modes include `b3tq` and `b3half`, the first two exact raw factors exposed by
the B3 birational transport/first minors; the older modes include raw
components of the corrected B3-local denominator resultant.  This producer
is a generic-curve discriminator.  It emits every conservative
...
assert len(COMPONENT_ARGUMENTS) <= 1
COMPONENT = COMPONENT_ARGUMENTS[0] if COMPONENT_ARGUMENTS else "p3"
assert COMPONENT in {"p3", "b16", "bneg2", "bq", "b3tq", "b3half"}

if COMPONENT == "p3":
    QUAD_A_VALUE, QUAD_B_VALUE = Fraction(32), Fraction(-128)
    Y_COORDINATES = (Fraction(0), Fraction(1))
    X_COORDINATES = (Fraction(3), Fraction(0))
    TARGET_FIELD_DEGREE = 4
elif COMPONENT == "b16":
    # The unused Z^2=2 constant extension is audited away at the end; all
    # source/certificate coefficients stay in its rational subfield.
    QUAD_A_VALUE, QUAD_B_VALUE = Fraction(0), Fraction(2)
    Y_COORDINATES = (Fraction(16), Fraction(0))
    X_COORDINATES = (Fraction(11), Fraction(0))
    TARGET_FIELD_DEGREE = 2
elif COMPONENT == "bneg2":
    QUAD_A_VALUE, QUAD_B_VALUE = Fraction(0), Fraction(2)
    Y_COORDINATES = (Fraction(-2), Fraction(0))
    X_COORDINATES = (Fraction(-4), Fraction(0))
    TARGET_FIELD_DEGREE = 2
elif COMPONENT == "bq":
    QUAD_A_VALUE, QUAD_B_VALUE = Fraction(32, 3), Fraction(128, 9)
    Y_COORDINATES = (Fraction(0), Fraction(1))
    X_COORDINATES = (Fraction(-11, 3), Fraction(1))
    TARGET_FIELD_DEGREE = 4
elif COMPONENT == "b3tq":
    # Z is the genuine constant t with t^2-4t+2=0.  The outer Curve
    # extension is only an arithmetic adapter and is audited away.
    QUAD_A_VALUE, QUAD_B_VALUE = Fraction(4), Fraction(-2)
    Y_COORDINATES = (Fraction(1), Fraction(0))
    X_COORDINATES = (Fraction(0), Fraction(0))
    TARGET_FIELD_DEGREE = 2
else:
    # t=1/2 is rational.  Both quadratic adapters are audited away.
    QUAD_A_VALUE, QUAD_B_VALUE = Fraction(0), Fraction(2)
    Y_COORDINATES = (Fraction(1), Fraction(0))
    X_COORDINATES = (Fraction(0), Fraction(0))
    TARGET_FIELD_DEGREE = 1
...
class Quad:
    """Q(U)[Z]/(Z^2-aZ-b), with mode-specific exact a,b."""
    ...
    def __mul__(self, other):
        other = Quad.coerce(other)
        # Z^2 = QUAD_A_VALUE*Z + QUAD_B_VALUE.
        return Quad(
            self.a * other.a + QUAD_B_VALUE * self.b * other.b,
            self.a * other.b + self.b * other.a
            + QUAD_A_VALUE * self.b * other.b,
        )
    ...
    def norm(self):
        return (
            self.a**2 + QUAD_A_VALUE*self.a*self.b
            - QUAD_B_VALUE*self.b**2
        )
    ...
    def inverse(self):
        ...
        result = Quad(
            (self.a + QUAD_A_VALUE*self.b)*inverse_norm,
            -self.b*inverse_norm,
        )
...
U_RAT = RatU(U_POLY)
Z_QUAD = Quad(0, 1)
Y_QUAD = Quad(*Y_COORDINATES)
X_QUAD = Quad(*X_COORDINATES)
...
class Curve:
    ...
    def norm_quad(self):
        return self.a**2 - self.b**2 * Y_QUAD * U_RAT**3
...
Y_CURVE = Curve(Y_QUAD)
X_CURVE = Curve(X_QUAD)
...
def rational_constant_subfield(value):
    """No coefficient uses the auxiliary Z direction in rational-y modes."""
    return all(
        not coordinate.a.b and not coordinate.b.b
        for coordinate in efield_curves(value)
    )


def curve_base_subfield(value):
    """No coefficient uses the auxiliary outer Curve direction."""
    return all(not coordinate.b for coordinate in efield_curves(value))
...
def main():
    ...
    assert (
        Z_CURVE**2 - QUAD_A_VALUE*Z_CURVE - QUAD_B_VALUE == Curve()
    )
    assert V_CURVE**2 == Y_CURVE * U_CURVE**3
    if COMPONENT in {"b3tq", "b3half"}:
        t = Z_CURVE if COMPONENT == "b3tq" else Curve(Fraction(1, 2))
        w = U_CURVE
        ...
        if COMPONENT == "b3tq":
            ...
            assert t == Curve(Fraction(1, 2))
        assert center_v / center_u == w
        assert center_c / center_u**2 == x_weight
        assert center_v**2 / center_u**3 == y_weight
        assert y_weight / (x_weight + 5) == t
        b_value = (
            4*x_weight**2 - 4*x_weight*y_weight + 24*x_weight
            + y_weight**2 - 20*y_weight + 20
        )
        raw_b3 = (
        ...
        assert raw_b3 == Curve()
        assert raw_b3 + 1 != Curve()
    else:
        b_value = (
            4*X_CURVE**2 - 4*X_CURVE*Y_CURVE + 24*X_CURVE
            + Y_CURVE**2 - 20*Y_CURVE + 20
        )
    assert b_value == Curve()
    assert b_value + 1 != Curve()
    if COMPONENT == "p3":
        assert Y_CURVE**2 - 32*Y_CURVE + 128 == Curve()
        exceptional_factor = Y_CURVE**2 - 32*Y_CURVE + 128
    elif COMPONENT == "b16":
        exceptional_factor = 4*X_CURVE - 3*Y_CURVE + 4
    elif COMPONENT == "bneg2":
        exceptional_factor = 2*X_CURVE - 3*Y_CURVE + 2
    elif COMPONENT == "bq":
        assert 9*Y_CURVE**2 - 96*Y_CURVE - 128 == Curve()
        exceptional_factor = 3*X_CURVE - 3*Y_CURVE + 11
    elif COMPONENT == "b3tq":
        exceptional_factor = Z_CURVE**2 - 4*Z_CURVE + 2
    else:
        exceptional_factor = 2*Curve(Fraction(1, 2)) - 1
    assert exceptional_factor == Curve()
    print(f"component={COMPONENT}")
    print(
        "constant_tower_relation="
        f"Z^2-({QUAD_A_VALUE})Z-({QUAD_B_VALUE})"
    )
    if COMPONENT in {"b3tq", "b3half"}:
        print("center_weight_X=(-5t^2+20t-4)/(t-2)^2")
        print("square_weight_Y=16t/(t-2)^2")
        print("dummy_outer_square=U^3")
    else:
        print(f"center_weight_X=({X_QUAD})")
        print(f"square_weight_Y=({Y_QUAD})")
        print("V2_equals_YU3=true")
    print("B3_scaled_relation_bXY_zero=true")
    print("B3_scaled_plus_one_negative_control=true")
    if COMPONENT in {"b3tq", "b3half"}:
        print("birational_parameter_w=V0/U0")
        print("raw_curve_w_zero_is_origin=true")
    else:
        print("YU3_U_valuation=3")
        print("YU3_nonsquare_by_odd_U_valuation=true")
    print(f"target_function_field_degree_over_QU={TARGET_FIELD_DEGREE}")
    print("exceptional_denominator_factor_zero=true")
    if COMPONENT in {"b3tq", "b3half"}:
        print(
            "B3_parameter_t="
            + ("root(t^2-4t+2)" if COMPONENT == "b3tq" else "1/2")
        )
        print("B3_parameter_center_identity_zero=true")
        print("B3_parameter_inverse_formulas=true")
        print("B3_parameter_plus_one_negative_control=true")
        print("auxiliary_outer_curve_extension_only=true")
    ...
    center = (
        (center_c, center_v, center_u)
        if COMPONENT in {"b3tq", "b3half"}
        else (X_CURVE * U_CURVE**2, V_CURVE, U_CURVE)
    )
    print(
        (
            (
                "center_stratum=B3 birational t^2-4t+2 raw curve over Q(t)(w)"
                if COMPONENT == "b3tq"
                else "center_stratum=B3 birational t=1/2 raw curve over Q(w)"
            )
            if COMPONENT in {"b3tq", "b3half"}
            else "center_stratum=C=XU^2, V^2=YU^3 over exact component function field"
        )
    )
    r.fb.CENTER = center
    ...
    print("TD6 exceptional-curve quotient transport PASS", flush=True)
    ...
    print("TD6 exceptional-curve quotient first band PASS", flush=True)
    ...
    print("TD6 exceptional-curve quotient genuine P12 PASS", flush=True)
    ...
    rational_descent = True
    if COMPONENT in {"b16", "bneg2", "b3half"}:
        rational_descent = all(
            rational_constant_subfield(value)
            for value in (
                list(polynomial_value.values()) + list(remainder.values())
                + relation_values
                + [
                    coefficient
                    for _, row, rhs in first_rows
        ...
        assert rational_descent
    curve_descent = True
    if COMPONENT in {"b3tq", "b3half"}:
        curve_descent = all(
            curve_base_subfield(value)
            for value in (
                list(polynomial_value.values()) + list(remainder.values())
                + relation_values
                + [
    ...
    print(f"rational_constant_subfield_descent={str(rational_descent).lower()}")
    print(f"auxiliary_curve_subfield_descent={str(curve_descent).lower()}")
    print("component_intersection_U_zero_is_origin=true")
    ...
    print("component_generic_open_killed=true")
    print("component_whole_curve_killed=false")
    print("full_three_center_family_killed=false")
    print("SP2_killed=false")
    print("JC2_resolved=false")
    print("TD6-C1-C2-C3-EXCEPTIONAL-CURVE-QUOTIENT PASS")
