#!/usr/bin/env python3
"""Exact raw TD6 B3-divisor gate via a birational Q(t,w) chart.

The entire matrix-changing transport, first affine band, and genuine P12 are
rebuilt after the exact substitution

    x=C0/U0^2=(-5*t^2+20*t-4)/(t-2)^2,
    y=V0^2/U0^3=16*t/(t-2)^2,
    w=V0/U0.

Here the polynomial variables named C,V,U below are only the independent
chart variables t,w,j; j is an intentionally unused descent sentinel.  This
is not a source scaling or gauge normalization.  Every printed chart divisor
remains a raw specialization obligation.
"""
...
def center_coordinates():
    ...
    if STRATUM == "b3-param":
        # C,V are chart parameters t,w.  U is unused and audited below.
        t, w = Rat3(C), Rat3(V)
        center_u = w**2 * (t-2)**2 / (16*t)
        center_v = w**3 * (t-2)**2 / (16*t)
        center_c = (
            w**4 * (t-2)**2 * (-5*t**2 + 20*t - 4)
            / (256*t**2)
        )
        return (
            center_c, center_v, center_u,
            "B3=0 birational chart over Q(t=C,w=V); U is unused sentinel",
        )
    raise AssertionError(STRATUM)


def audit_b3_parameterization(center_c, center_v, center_u):
    """Executable coverage ledger for the raw B3 birational chart."""
    t, w = Rat3(C), Rat3(V)
    x = (-5*t**2 + 20*t - 4) / (t-2)**2
    y = 16*t / (t-2)**2
    b = 4*x**2 - 4*x*y + 24*x + y**2 - 20*y + 20
    raw_b3 = (
    ...
    assert b + 1 != Rat3()
    assert raw_b3 == Rat3()
    assert raw_b3 + 1 != Rat3()
    assert center_c / center_u**2 == x
    assert center_v**2 / center_u**3 == y
    assert center_v / center_u == w
    assert y / (x+5) == t

    # A line y=t(x+5) through (-5,0) meets b=0 at the stated second
    # point.  The only missed affine base point is (-5,0), on V0=0.
    formal_x = U
    line_y = C*(formal_x+5)
    formal_b_on_line = (
        4*formal_x**2 - 4*formal_x*line_y + 24*formal_x
        + line_y**2 - 20*line_y + 20
    )
    line_factor = (formal_x+5) * (
        (C-2)**2*formal_x + 5*C**2 - 20*C + 4
    )
    assert formal_b_on_line == line_factor
    base_y = V
    assert (
        4*(-5)**2 - 4*(-5)*base_y + 24*(-5)
        + base_y**2 - 20*base_y + 20
    ) == base_y**2
    # t=2 gives only the base point on the affine line section; the other
    # conic intersection is at projective infinity.  In the explicit
    # center map the t=2 divisor collapses to the already raw-closed origin.
    t2_y = 2*(formal_x+5)
    assert (
        4*formal_x**2 - 4*formal_x*t2_y + 24*formal_x
        + t2_y**2 - 20*t2_y + 20
    ) == -16*(formal_x+5)

    print("B3_parameter_b_identity_zero=true")
    print("B3_parameter_plus_one_negative_control=true")
    print("B3_original_center_identity_zero=true")
    print("B3_inverse_w_equals_V0_over_U0=true")
    print("B3_inverse_t_equals_y_over_xplus5=true")
    print("B3_line_intersection_factorization=true")
    print("B3_only_missed_affine_base_point=(-5,0)_lies_on_V0_zero=true")
    print("B3_t_equals_2_only_base_affine_other_intersection_infinity=true")
    print("B3_t_equals_2_center_specialization_is_origin=true")
    print("B3_parameter_formula_denominator=t^2")
    print("B3_parameter_open_requires=t*w*(t-2)!=0")
    print("B3_weighted_scaling_used=false")
    print("B3_unused_sentinel_variable=U")
...
def configure():
    ...
    r.b.REVERSE_PIVOTS = REVERSE_FIRST
...
def e3_rat3_coordinates(value):
    """The 18 Rat3 coordinates of an E3 value, for descent audits."""
...
def denominators_divide(value, common):
    """Every exact Rat3 coordinate denominator divides ``common``."""
    return all(
        not common % coordinate.denominator
        for coordinate in e3_rat3_coordinates(value)
    )


def relation_termwise_clear(relations, first_rows, common):
    """Audit every source-multiplier/source-row coefficient product."""
    slots = 0
    for relation, (_, row, rhs) in zip(relations, first_rows):
...
def main():
    ...
    print(
        ...
        + (
            "B3-local-scalar" if B_LOCAL3
            else ("reverse" if REVERSE_FIRST else "ascending")
        ),
        flush=True,
    )
    ...
    if STRATUM == "b3-param":
        audit_b3_parameterization(center_c, center_v, center_u)
    ...
    if STRATUM in {"generic", "b3-param"}:
    ...
    assert all(
        denominators_divide(value, relations_denominator)
        for value in r.polynomial_values(relations)
    )
    ...
    if STRATUM == "b3-param":
        sentinel_descent = all(
            monomial[2] == 0
            for value in (
                [transport_determinant, first_determinant]
                + list(polynomial_value.values()) + list(remainder.values())
                + list(r.polynomial_values(relations))
                + [
                    coefficient
                    for _, row, rhs in first_rows
                    for coefficient in list(row.values()) + [rhs]
            ...
            for coordinate in e3_rat3_coordinates(value)
            for polynomial_part in (
                coordinate.numerator, coordinate.denominator
            )
            for monomial in polynomial_part.to_dict()
        )
        assert sentinel_descent
        print(f"B3_expected_minus_k_over_50={str(expected_remainder).lower()}")
        print(f"B3_parameterized_open_killed={str(constant_unit).lower()}")
        print("B3_unused_sentinel_descent=true")
        print("B3_whole_divisor_killed=false")
    ...
    print("JC2_resolved=false")
    if STRATUM == "b3-param":
        print("TD6-C1-C2-C3-B3-BIRATIONAL-DISCRIMINATOR PASS")
    else:
        print(
