...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a8_k6_mu2_fullsupport_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_a8_k6_mu2_fullsupport_20260826/compile_a8.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_a8_k6_mu2_${label}.sing"
...
printf 'validator=PASS_D1_A8_K6_MU2_FULLSUPPORT\n' >> "$validation"
