#!/usr/bin/env python3
"""AWS-only exact relative-cone analysis of the H17 direct-unit functional, V9."""
...
def main() -> None:
    ...
    for boundary in (sharp_lower, sharp_upper):
        ...
        for exponent, coefficient in h_q.items():
            if exponent == intrinsic_exp:
                continue
            term_const, term_slope = line(names, exponent)
            if term_const + term_slope * boundary == BASELINE:
                block.append(
                    {
                        "coefficient": str(coefficient),
                        "monomial": monomial_text(names, exponent),
                        "line": f"{term_const}+({term_slope})q",
                    }
                )
        equality_blocks[str(boundary)] = block
    ...
    if {record["line"] for record in equality_blocks[str(sharp_lower)]} != {"34+(4)q"}:
        fail(("lower boundary line mismatch", equality_blocks[str(sharp_lower)]))
    if {record["line"] for record in equality_blocks[str(sharp_upper)]} != {"93+(-6)q"}:
        fail(("upper boundary line mismatch", equality_blocks[str(sharp_upper)]))
    ...
    result = {
        "status": "PASS-A-H17-DIRECT-UNIT-RELATIVE-CONE-V9",
    ...
    print("A_H17_CONE_ENDPOINT=PASS_EXACT_RELATIVE_CONE_V9")
