...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_p0_odd_sheet_receivers_20260826/FREEZE_G13.sha256 \
...
"$aws_root/ops/aws_exact_lane.sh" "$aws_root" "$aws_job/run" "$tag" \
  ...
  cases/max12_812_order2_p0_odd_sheet_receivers_20260826/analyze_grade13.py \
  "$aws_job/compiled" --prime "$prime" --samples 8
rc=$?
set -e
...
for required in \
  P0_ODD_G13_SOURCE_HASHES=PASS \
  P0_ODD_G13_AFFINE_LINEAR=1 \
  P0_ODD_G13_STATUS=PASS_MODULAR_RANK_NAVIGATION
do
...
printf 'validator=PASS_G13_NAVIGATION\n' >> "$validation"
