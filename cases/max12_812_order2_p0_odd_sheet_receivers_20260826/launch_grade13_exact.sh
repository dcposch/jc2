...
tag="max12_812_order2_p0_odd_g13_exact_q_${stamp}_Box03"
remote_root="/home/ubuntu/jobs/$tag/source"
remote_job="$remote_root/$package/aws_Box03_g13_exact_q"
archive=$(mktemp /tmp/jc2-p0-odd-g13-exact.XXXXXX)
trap 'rm -f "$archive"' EXIT
tar -czf "$archive" \
  ops/aws_exact_lane.sh \
  "$package/extract_grade13_exact.py" \
  "$package/REGISTRATION_G13_EXACT.md" \
  "$package/compile_receivers.py" \
  "$package/FREEZE.sha256" \
  "$package/FREEZE_G13_EXACT.sha256" \
  "$package/RESULT.md" \
  "$package/launch_grade13_exact.sh" \
  "$package/run_grade13_exact_aws.sh" \
...
ssh "${ssh_options[@]}" "$ssh_target" \
  "cd '$remote_root' && tar -xzf source.tar.gz && mkdir -p '$remote_job' && nohup bash '$package/run_grade13_exact_aws.sh' '$remote_root' '$remote_job' '$tag' 33554432 1800 > '$remote_job.outer.stdout' 2> '$remote_job.outer.stderr' & echo \$!"
printf 'tag=%s\nremote_job=%s\n' "$tag" "$remote_job"
