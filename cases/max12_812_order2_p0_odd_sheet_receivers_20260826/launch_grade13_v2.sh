...
tag="max12_812_order2_p0_odd_g13_v2_p${prime}_${stamp}_${host}"
remote_root="/home/ubuntu/jobs/$tag/source"
remote_job="$remote_root/$package/aws_${host}_g13_v2_p${prime}"
archive=$(mktemp /tmp/jc2-p0-odd-g13-v2.XXXXXX)
trap 'rm -f "$archive"' EXIT
tar -czf "$archive" \
  ops/aws_exact_lane.sh \
  "$package/analyze_grade13_v2.py" \
  "$package/REGISTRATION_G13_V2.md" \
  "$package/compile_receivers.py" \
  "$package/FREEZE.sha256" \
  "$package/FREEZE_G13_V2.sha256" \
  "$package/GRADE13_V1_DEPLOYMENT_NEGATIVE.md" \
  "$package/RESULT.md" \
  "$package/launch_grade13_v2.sh" \
  "$package/run_grade13_v2_aws.sh" \
...
ssh "${ssh_options[@]}" "$ssh_target" \
  "cd '$remote_root' && tar -xzf source.tar.gz && mkdir -p '$remote_job' && nohup bash '$package/run_grade13_v2_aws.sh' '$remote_root' '$remote_job' '$tag' '$prime' 8388608 1800 > '$remote_job.outer.stdout' 2> '$remote_job.outer.stderr' & echo \$!"
printf 'tag=%s\nremote_job=%s\n' "$tag" "$remote_job"
