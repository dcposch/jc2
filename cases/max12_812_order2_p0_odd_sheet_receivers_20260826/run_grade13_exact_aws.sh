...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_p0_odd_sheet_receivers_20260826/FREEZE_G13_EXACT.sha256 \
...
"$aws_root/ops/aws_exact_lane.sh" "$aws_root" "$aws_job/run" "${tag}_python" \
  ...
  cases/max12_812_order2_p0_odd_sheet_receivers_20260826/extract_grade13_exact.py \
  "$aws_job/compiled"
python_rc=$?
set -e
...
"$aws_root/ops/aws_exact_lane.sh" "$aws_root" "$aws_job/run" "${tag}_singular" \
  timeout "$wall" Singular -q "$aws_job/compiled/grade13_reduce_factor.sing"
singular_rc=$?
set -e
...
printf 'validator=PASS_G13_EXACT_COLLECTION_REDUCTION\n' >> "$validation"
