...
if [[ "$(uname -s)" != "Linux" ]] || \
  ...
  echo "R4-00 R1 exact runner refused non-AWS host" >&2
  exit 125
fi
if [[ $# -ne 9 ]]; then
  echo "usage: $0 AWS_ROOT COMPILED AWS_JOB TAG BRANCH STAGE CAPRUN RSS_BYTES WALL_SECONDS" >&2
  exit 125
fi
aws_root=$1
compiled=$2
...
rss_bytes=$8
wall_seconds=$9
if [[ "$branch" != "PLUS" && "$branch" != "MINUS" ]]; then
  echo "R4-00 R1 branch must be PLUS or MINUS" >&2
  exit 125
fi
if [[ "$stage" != "BASE_LIFT" && "$stage" != "FULL_GRADE19" ]]; then
  echo "R4-00 R1 stage must be BASE_LIFT or FULL_GRADE19" >&2
  exit 125
fi
if [[ -z "$lane_tag" || ! "$lane_tag" =~ ^R400_R1_ || ! "$rss_bytes" =~ ^[1-9][0-9]*$ || ! "$wall_seconds" =~ ^[1-9][0-9]*$ ]]; then
  echo "R4-00 R1 registration malformed" >&2
  exit 125
fi
if [[ -e "$aws_job" ]]; then
  echo "R4-00 R1 job path already exists: $aws_job" >&2
  exit 125
fi
rel=cases/max12_812_order2_u2_62_k00_r400_exact_deciders_20260829
worker="$aws_root/$rel/run_exact_packet.py"
packet="$compiled/R1_${branch}_${stage}.json"
if [[ ! -f "$worker" || ! -f "$packet" || ! -f "$caprun" || ! -f "$compiled/PACKETS.sha256" ]]; then
  echo "R4-00 R1 frozen input missing" >&2
  exit 125
fi
mkdir -p "$aws_job"
