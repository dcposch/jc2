...
ulimit -v "$cap_kib"
set +e
timeout 300 python3 "$package/compile_rho_unit_v13.py" "$aws_job/compiled" \
  --characteristic "$characteristic" --algorithm "$algorithm" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
set +e
python3 "$package/validate_rho_unit_v13.py" --compiler-result "$aws_job/compiled/result.json" \
  --stdout "$stdout" --stderr "$stderr" --characteristic "$characteristic" \
  --algorithm "$algorithm" --output "$aws_job/RESULT.json" > "$aws_job/validator.stdout" 2> "$aws_job/validator.stderr"
validator_rc=$?
set -e
printf 'validator_rc=%s\n' "$validator_rc" > "$aws_job/validator.validation"
if [[ "$validator_rc" -ne 0 ]]; then exit "$validator_rc"; fi
printf 'validator=PASS_T_RS_RHO_UNIT_V13\n' >> "$aws_job/validator.validation"
: > "$aws_job/EVIDENCE.sha256"
for artifact in "$aws_job/RESULT.json" "$aws_job/compiled/result.json" "$script" \
  "$aws_job"/compiled/*.ideal "$aws_job"/compiled/*.polys "$stdout" "$stderr" "$aws_job"/run/*.meta; do
