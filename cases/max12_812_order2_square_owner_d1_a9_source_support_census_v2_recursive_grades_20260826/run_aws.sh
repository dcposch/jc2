...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a9_source_support_census_v2_recursive_grades_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_a9_source_support_census_v2_recursive_grades_20260826/compile_a9_recursive.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_a9_recursive_source_grades_${label}.sing"
...
if ! grep -Eq '^A9V2_LOWER_THAN_27_ROW_COUNT=[1-9][0-9]*$' "$stdout"; then
  printf 'validator=FAIL_EXPECTED_LOWER_CONSTRAINT_ABSENT\n' >> "$validation"; exit 90
fi
...
printf 'validator=PASS_A9_RECURSIVE_LITERAL_FABER_SOURCE_CENSUS\n' >> "$validation"
