...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_disc_triple_k3_receiver_v2_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_disc_triple_k3_receiver_v2_20260826/compile_k3_v2.py \
...
input="$aws_job/compiled/disc_triple_k3_v2_p${characteristic}.sing"
...
for required in \
  ...
  'DISC_TRIPLE_K3_V2_RATIONAL_LITERAL_REPAIR=1' \
...
if grep -Fq '=FAIL' "$stdout" || grep -Eq '^\s*\? ' "$stdout"; then
  printf 'validator=FAIL_SENTINEL_OR_DIAGNOSTIC\n' >> "$validation"
  exit 90
fi
printf 'validator=PASS_DISC_TRIPLE_K3_RECEIVER_V2\n' >> "$validation"
