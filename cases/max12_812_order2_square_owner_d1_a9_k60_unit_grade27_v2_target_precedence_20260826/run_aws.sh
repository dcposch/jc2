...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_owner_d1_a9_k60_unit_grade27_v2_target_precedence_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_owner_d1_a9_k60_unit_grade27_v2_target_precedence_20260826/compile_v2_target_precedence.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_d1_a9_k60_unit_grade27_v2_${label}.sing"
sha256sum "$input" "$aws_job/compiled/result.json" "$aws_job/compiled/parent_v1_result.json" "$aws_job/compiled/parent_v3_result.json" > "$aws_job/compiled.sha256"
for required in \
  ...
  'D1A9_K60_LITERAL_TARGET_J_COUNT=1' \
  'D1A9_K60_V2_TARGET_PRECEDENCE_OLD_COUNT=1' \
  'D1A9_K60_V2_TARGET_PRECEDENCE_NEW_COUNT=1'; do
...
printf 'validator=PASS_D1_A9_K60_UNIT_GRADE27_V2_EMPTY_EXACT_CONTACT\n' >> "$validation"
