#!/usr/bin/env python3
"""Fail-closed validator for the V12 T-rs rho-unit certificate."""
...
def main() -> None:
    ...
    if (
        compiler.get("status") != "PASS-T-RS-RHO-UNIT-V12-COMPILER"
    ...
    required = (
        "V12_QRING_DISABLED=1",
        f"V12_CHARACTERISTIC={args.characteristic}",
        f"V12_ALGORITHM={args.algorithm}",
        "V12_MULTIPLICATION_BACK_COUNT=4",
        "V12_FOUR_TERM_DELTA=1",
        "V12_B_DECOMPOSITION=1",
        "V12_CERTIFICATE_IDENTITY=1",
        "V12_NEGATIVE_CONTROL_COUNT=2",
        "V12_SATURATED_KU_MEMBERSHIP=1",
        "V12_SPECIAL_CONTAINS_K=1",
        "V12_RHO_INVERSE=1",
        "V12_SPECIAL_DK_UNIT=1",
        "V12_ARTIFACT_WRITES=4",
        "V12_SCOPE=T_RS_ACTUAL_CHART_DK_RHO_UNIT_PREFIX_ONLY",
        "PASS_T_RS_RHO_UNIT_V12",
    )
    for token in required:
        if stdout.count(token) != 1:
            raise RuntimeError(("required token", token, stdout.count(token)))
    if args.algorithm == "sat" and "V12_SAT_RETURN_LENGTH=1" not in stdout:
