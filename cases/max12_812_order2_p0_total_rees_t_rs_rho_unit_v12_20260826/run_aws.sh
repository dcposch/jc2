...
engine_timeout=$7
archive_sha=$8
package=cases/max12_812_order2_p0_total_rees_t_rs_rho_unit_v12_20260826
case "$characteristic" in 0|65521) ;; *) exit 66 ;; esac
case "$algorithm" in sat|elim) ;; *) exit 67 ;; esac
mkdir -p "$aws_job/run"
{
  printf 'tag=%s\n' "$tag"
  printf 'host=%s\n' "$(hostname)"
...
ulimit -v "$cap_kib"
set +e
timeout 300 python3 "$package/compile_rho_unit_v12.py" "$aws_job/compiled" \
...
set +e
python3 "$package/validate_rho_unit_v12.py" --job "$aws_job" \
  --compiler-result "$aws_job/compiled/result.json" --stdout "$stdout" \
  --stderr "$stderr" --characteristic "$characteristic" \
...
printf 'validator=PASS_T_RS_RHO_UNIT_V12\n' >> "$aws_job/validator.validation"
: > "$aws_job/EVIDENCE.sha256"
for artifact in "$aws_job/RESULT.json" "$aws_job/compiled/result.json" \
  "$script" "$aws_job"/compiled/*.ideal "$aws_job"/compiled/*.polys \
  "$stdout" "$stderr" "$aws_job"/run/*.meta; do
