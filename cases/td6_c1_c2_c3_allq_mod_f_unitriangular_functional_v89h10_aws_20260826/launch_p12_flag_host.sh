...
printf 'hostname=%s\nlabel=%s\narchive_sha256=%s\nstart_utc=%s\ncap_kib=536870912\ntimeout_seconds=43200\nscope=all-q-literal-p12-flag-functional\n' \
  "$(hostname)" "$label" "$expected" "$(date -u +%FT%TZ)" > "$root/registration.meta"
(
  set +e
  lane="$root/p12flag"
  mkdir -p "$lane/output"
  tag="td6_v89h11_allq_p12_flag_functional_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=536870912\ntimeout_seconds=43200\nscope=all-q-literal-p12-flag-functional\n' \
  ...
  (
    ...
    export TD6_FUNCTIONAL_SCOPE=empty-parameter-allq
    export TD6_OUTPUT_DIR="$lane/output"
    /usr/bin/time -v timeout 43200 bash run_v89h11_allq_p12_flag_functional.sh \
