...
printf 'hostname=%s\nlabel=%s\narchive_sha256=%s\nstart_utc=%s\ncap_kib=268435456\ntimeout_seconds=21600\nscope=all-22-licensed-q-mod-F-unitriangular-functional\n' \
  "$(hostname)" "$label" "$expected" "$(date -u +%FT%TZ)" > "$root/registration.meta"
(
  set +e
  lane="$root/allqunitriangular"
  mkdir -p "$lane/output"
  tag="td6_v89h10v2_allq_mod_f_unitriangular_functional_${label}_${stamp}"
  printf 'hostname=%s\nlabel=%s\ntag=%s\nstart_utc=%s\narchive_sha256=%s\ncap_kib=268435456\ntimeout_seconds=21600\nscope=all-22-licensed-q-mod-F-unitriangular-functional\n' \
  ...
  (
    ...
    /usr/bin/time -v timeout 21600 bash run_v89h10_allq_mod_f_unitriangular_functional.sh \
