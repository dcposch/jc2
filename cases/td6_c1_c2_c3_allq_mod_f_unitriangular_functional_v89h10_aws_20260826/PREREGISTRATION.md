# TD6 V89H10V2 full-q unitriangular FIRST and q14 functional
...
Before P12 division, factor the exact common denominators of the relative
pivot inverse, the full original-FIRST inverse, and every normalized FIRST
coefficient.  Stop unless their radicals are contained in
`U*V*(V^2-4U^3)`, which is exactly the specialized `D(U*H*B3)` firewall.
After division, repeat the audit on all original-FIRST relation coefficients
and the canonical remainder.  No generic coefficient-field inverse alone
licenses a claim on the whole registered open.
