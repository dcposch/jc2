...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "AWS-only K00 closure launcher refused non-Linux host" >&2
  exit 125
fi
vendor=$(tr -d '\n' < /sys/class/dmi/id/sys_vendor 2>/dev/null || true)
if [[ "$vendor" != "Amazon EC2" ]]; then
  echo "AWS-only K00 closure launcher refused vendor=${vendor:-unknown}" >&2
  exit 125
fi
if [[ $# -ne 7 ]]; then
  echo "usage: launch_remote_k00_closure.sh AWS_ROOT AWS_JOB TAG CAP_KIB TIMEOUT_SECONDS CHARACTERISTIC FREEZE" >&2
  exit 125
fi
