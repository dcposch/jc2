...
aws_root=$1; aws_job=$2; tag=$3; cap_kib=$4; characteristic=$5
compile_timeout=$6; engine_timeout=$7
mkdir -p "$aws_job/run"
{
  printf 'tag=%s\n' "$tag"; printf 'host=%s\n' "$(hostname)"
...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_square_a_prolongation_owner_v3_ptangent_20260826/FREEZE.sha256 > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
set +e
timeout "$compile_timeout" python3 cases/max12_812_order2_square_a_prolongation_owner_v3_ptangent_20260826/compile_square_a_ptangent.py "$aws_job/compiled" --characteristic "$characteristic" > "$aws_job/compiler.stdout" 2> "$aws_job/compiler.stderr"
compiler_rc=$?
set -e
...
input="$aws_job/compiled/square_a_ptangent_${label}.sing"
...
for required in \
  'SQUARE_APROL_SIGMA14_DIVISIBLE=1' \
  'SQUARE_APROL_SIGMA15_DIVISIBLE=1' \
  'SQUARE_APROL_QUOTIENT_IDENTITIES=1' \
  'SQUARE_APROL_FORBIDDEN=1' \
  'SQUARE_APROL_PTANGENT_G14_ROW_IDENTITIES=1' \
  'SQUARE_APROL_PTANGENT_G15_ROW_IDENTITIES=1' \
  'SQUARE_APROL_PTANGENT_G14_MOD_L_GATE=1' \
  'SQUARE_APROL_PTANGENT_DELTA_MOD_L=1' \
  'SQUARE_APROL_PTANGENT_DELTA_KILLED_BY_G14=1' \
  'SQUARE_APROL_PTANGENT_MOVING_SEPARATOR=1' \
  'SQUARE_APROL_PTANGENT_ENDPOINT=PASS_MOVING_P_SOURCE_ROW_ADDENDUM'
do
...
printf 'validator=PASS_SQUARE_APROL_PTANGENT\n' >> "$validation"
