...
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_constructive_circuit_rebased_v43c3d_* ]]; then exit 126; fi
...
(
  ...
  bash ops/aws_exact_lane.sh "$root" "$job/run" "$lane" timeout "$wall_seconds" \
    python3 "$package/diagnose_v43c3d.py" "$job/output"
)
...
printf 'end_utc=%s\noutcome=no-expansion-diagnostic-only\n' \
