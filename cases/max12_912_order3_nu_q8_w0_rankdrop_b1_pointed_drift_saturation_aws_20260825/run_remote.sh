...
set -euo pipefail
if [ "$#" -ne 7 ]; then
  echo "usage: $0 VM_KIB REPO OUT_ROOT TAG FIELD ENGINE TIMEOUT_SECONDS" >&2
  exit 64
fi
VM_KIB=$1
REPO=$2
...
ENGINE=$6
TIMEOUT_SECONDS=$7
CASE=cases/max12_912_order3_nu_q8_w0_rankdrop_b1_pointed_drift_saturation_aws_20260825
OUT=$OUT_ROOT/$TAG
mkdir -p "$OUT"
ulimit -v "$VM_KIB"
cd "$REPO"
python3 "$CASE/generate.py" --field "$FIELD" --engine "$ENGINE" > "$OUT/input.sing" 2> "$OUT/generator.stderr"
...
printf '%s\n' "$RC" > "$OUT/runner.rc"
{
  ...
  echo "field=$FIELD"
  echo "engine=$ENGINE"
  echo "scope=pointed-moving-d4-selected-localizer"
  echo "rc=$RC"
} > "$OUT/run.meta"
sha256sum \
...
test "$(grep -c Q8_W0_RANKDROP_B1_POINTED_DRIFT_PASS "$OUT/stdout")" -eq 1
