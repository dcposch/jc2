...
Status: **authoritative computation for the eight preregistered forms only;
...
The frozen V2 enumerator tested all 1,016 nonempty candidate active sets for
the eight preregistered affine weight forms, together with every compatible
zero/positive boundary assignment for `a,r,c`.  On AWS Box02 with
`z3-solver==4.13.3.0` it found:

- 80 feasible active-set/boundary cells;
- 936 candidate active sets reported UNSAT by Z3;
...
lower-load terms absent from the eight-form input: terms arising from the
...
enter later.  Thus the eight forms are now known not to exhaust all source
