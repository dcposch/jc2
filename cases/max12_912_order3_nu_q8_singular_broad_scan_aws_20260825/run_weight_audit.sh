...
meta=$lane/run.meta
{
  ...
  echo "route=exact-support diagonal-weight nullspace"
  sha256sum "$0" "$case_dir/weight_audit.py" \
    "$repo_root/cases/max12_912_order3_nu_q8_global_quotient_probe_20260824/quotient_compiler.py" \
    | sed 's/^/source_sha256=/'
} > "$meta"
set +e
python3 "$case_dir/weight_audit.py" > "$lane/result.json" 2> "$lane/stderr.log"
rc=$?
set -e
{
