# Preregistration v2: exact control-2 Rees initial residue

Date: 2026-08-26  
Execution: registered Amazon EC2 only  
Status: source freeze before compile or solve; v1 is a software control only

V2 separates `/usr/bin/time` telemetry into `singular.time`, requires the
actual CAS stderr to be empty, saturates the special fibre by the product of
all eight nonzero residue coordinates, and emits that torus-special-fibre
basis for independent cross-encoding reconstruction.  V1 used the same file
for timing telemetry and CAS stderr, so its endpoints are not promotable.
...
- Encoding B independently expands every charged row over `Q`, contracts by
  ...
  `(lp(3),dp(8))` block order; the first block contains the two inverse
  variables and the Rees parameter.
...
let `H=C+(s)`.  V2 then forms

```text
Htor=H:(Lambda*tau*rho*q1*q0*r2*r1*r0)^infinity
```

and prints an exact standard basis of `Htor` in each encoding.

- `CONTROL2_REES_RESIDUE_SURVIVES=0` means exactly that `Htor` plus the
  displayed residue maximal ideal is the unit ideal.  Then **this exact
  support/weight/residue has no lift**.
- `CONTROL2_REES_RESIDUE_SURVIVES=1` means exactly that this residue ideal is
  nonunit in both encodings.  Then the displayed residue lies in the
...
- Disagreement, nonzero rc, stderr, a missing/duplicate PASS marker, failed
  ...
  `INCONCLUSIVE_SOFTWARE`.  A source review and a cross-reduction or other
  exact comparison of the two emitted torus bases are required before a
  mathematical endpoint is promoted.
