...
case "$tag" in
  max12_912_order3_d1_double_root_control2_rees_v2_*) ;;
  *) echo REFUSE_UNREGISTERED_AWS_TAG > REFUSED; exit 94 ;;
esac
case "$encoding" in
  A) input=compiled/control2_rees_v2_A_factored_sat_dp.sing; marker=PASS_D1_DOUBLE_ROOT_CONTROL2_REES_V2_A ;;
  B) input=compiled/control2_rees_v2_B_expanded_inverse_lpdp.sing; marker=PASS_D1_DOUBLE_ROOT_CONTROL2_REES_V2_B ;;
...
(
  ...
  nice -n 10 timeout 3600 python3 -u \
    repo/cases/max12_912_order3_d1_double_root_control2_rees_v2_20260826/compile_control2_rees_v2.py \
    --output-dir compiled
...
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || \
   test "$(grep -c '^PASS_D1_DOUBLE_ROOT_CONTROL2_REES_V2_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then
...
if test "$singular_rc" -ne 0 || test -s singular.stderr || \
   ...
   test "$(grep -Ec '^RESIDUE_IDEAL_IS_UNIT=[01]$' singular.stdout 2>/dev/null)" -ne 1 || \
   test "$(grep -Ec '^TORUS_SPECIAL_FIBRE_IS_UNIT=[01]$' singular.stdout 2>/dev/null)" -ne 1 || \
