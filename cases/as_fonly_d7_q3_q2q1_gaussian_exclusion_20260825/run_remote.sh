...
: "${MEMORY_KIB:=33554432}"
case_dir="$JC2_ROOT/cases/as_fonly_d7_q3_q2q1_gaussian_exclusion_20260825"
mkdir -p "$JOB_DIR"
hostname > "$JOB_DIR/hostname"
date -u +%FT%TZ > "$JOB_DIR/start_utc"
sha256sum "$case_dir/PREREGISTRATION.md" "$case_dir/certify_all.py" \
  "$case_dir/verify_certificates.py" "$0" "$MODEL_OUTPUT" \
...
env JC2_ROOT="$JC2_ROOT" MODEL_OUTPUT="$MODEL_OUTPUT" \
  ...
  python3 "$case_dir/certify_all.py" \
  > "$JOB_DIR/producer.stdout" 2> "$JOB_DIR/producer.stderr"
grep -q '^PASS-AS-Q3-Q2Q1-GAUSSIAN-EXCLUSION$' "$JOB_DIR/producer.stdout"
env CERTIFICATE_JSON="$JOB_DIR/certificates.json" \
  python3 "$case_dir/verify_certificates.py" \
  > "$JOB_DIR/verifier.stdout" 2> "$JOB_DIR/verifier.stderr"
grep -q '^PASS-AS-Q3-Q2Q1-INDEPENDENT-CERTIFICATE-VERIFY$' \
...
echo PASS-AS-Q3-Q2Q1-GAUSSIAN-PRODUCER-AND-VERIFY
