...
ORDER=$6
TIMEOUT_SECONDS=$7
CASE=cases/max12_912_order3_nu_q8_w0_rankdrop_b1_slope2_next_order_aws_20260825
OUT=$OUT_ROOT/$TAG
mkdir -p "$OUT"
ulimit -v "$VM_KIB"
cd "$REPO"
...
printf '%s\n' "$RC" > "$OUT/runner.rc"
{
  ...
  echo "engine=$ENGINE"
  echo "order=$ORDER"
  echo "scope=unramified-x5-order-one-finite-next-jet-only"
  echo "rc=$RC"
} > "$OUT/run.meta"
sha256sum \
...
test "$(grep -c Q8_W0_RANKDROP_B1_SLOPE2_NEXT_PASS "$OUT/stdout")" -eq 1
test "$(grep -c leading_zero=1 "$OUT/stdout")" -eq 1
