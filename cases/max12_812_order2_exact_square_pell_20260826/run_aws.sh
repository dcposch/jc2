...
cd "$aws_root"
sha256sum -c cases/max12_812_order2_exact_square_pell_20260826/FREEZE.sha256 \
  > "$aws_job/freeze_check.stdout"
export JC2_REGISTERED_AWS_LANE="$tag"
ulimit -v "$cap_kib"
compiled="$aws_job/compiled"
set +e
python3 cases/max12_812_order2_exact_square_pell_20260826/compile_exact_square_pell.py \
...
for required in \
  'EXACT_SQUARE_PELL_CHEBYSHEV_IDENTITY=1' \
  'EXACT_SQUARE_PELL_PREDICTED_SOLUTIONS=1' \
  'EXACT_SQUARE_PELL_OFF_C_UNIT=1' \
  'EXACT_SQUARE_PELL_RAD_IN_EXPECTED=1' \
  'EXACT_SQUARE_PELL_EXPECTED_IN_RAD=1' \
  'EXACT_SQUARE_PELL_ENDPOINT=PASS_SQUARE_UNION_CHEBYSHEV_REDUCED_SUPPORT' \
  'EXACT_SQUARE_PELL_SCOPE=SEVEN_LAURENT_TAILS_ONLY_NO_REES_TAYLOR_ORDER2_OR_JC2_VERDICT'
do
...
if grep -En '\?|// \*\*|error occurred|EXACT_SQUARE_PELL_FAIL|REPAIR_OR_EXTRA_COMPONENT' \
...
printf 'validator=PASS_EXACT_REDUCED_SUPPORT\n' >> "$aws_job/run/$tag.validation"
