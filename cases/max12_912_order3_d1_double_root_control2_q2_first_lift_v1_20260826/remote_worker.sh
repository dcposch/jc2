...
case "$tag" in max12_912_order3_d1_double_root_control2_q2_first_lift_v1_20260826T*) ;; *) echo REFUSE_UNREGISTERED_TAG > REFUSED; exit 94 ;; esac
case "$encoding" in
  A) input=compiled/q2_first_lift_A_dp.sing; final=PASS_Q2_FIRST_LIFT_A_DP ;;
  B) input=compiled/q2_first_lift_B_lpdp.sing; final=PASS_Q2_FIRST_LIFT_B_LPDP ;;
...
esac
{
  ...
  echo "compiler_memory_kib=4194304"
  echo "compiler_timeout=900"
  echo "solver_memory_kib=$memory_kib"
  echo "solver_timeout=$solve_timeout"
...
( ulimit -v 4194304; nice -n 10 timeout 900 python3 -u compile_q2_first_lift.py --output-dir compiled ) > compiler.stdout 2> compiler.stderr
compiler_rc=$?
echo "$compiler_rc" > compiler.rc
sha256sum compiled/q2_first_lift_A_dp.sing compiled/q2_first_lift_B_lpdp.sing > compiled.sha256 2>/dev/null || true
if test "$compiler_rc" -ne 0 || test -s compiler.stderr || test "$(grep -c '^PASS_Q2_FIRST_LIFT_COMPILER$' compiler.stdout 2>/dev/null)" -ne 1; then echo COMPILER_FAILURE > COMPILE_FAILED; exit 96; fi
touch COMPILE_DONE
...
if test "$solver_rc" -ne 0 || test -s singular.stderr || test -s singular.stdout.diagnostics || \
   ...
   test "$(grep -c '^PASS_Q2_TRANSPORT_IDENTITY$' singular.stdout 2>/dev/null)" -ne 1 || \
