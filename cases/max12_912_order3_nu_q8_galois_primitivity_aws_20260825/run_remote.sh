...
case_dir=$repo_root/cases/max12_912_order3_nu_q8_galois_primitivity_aws_20260825
lane=$out_root/$tag
...
meta=$lane/run.meta
{
  ...
  echo "route=exact bounded finite-field Frobenius certificates"
  sha256sum "$0" "$case_dir/replay.py" | sed 's/^/source_sha256=/'
} > "$meta"
set +e
timeout --signal=TERM --kill-after=5 120 \
  python3 "$case_dir/replay.py" > "$lane/replay.json" 2> "$lane/stderr.log"
rc=$?
set -e
{
  echo "end_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "rc=$rc"
  echo "stdout_sha256=$(sha256sum "$lane/replay.json" | awk '{print $1}')"
