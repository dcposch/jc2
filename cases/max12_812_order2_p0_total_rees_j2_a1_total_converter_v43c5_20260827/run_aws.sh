...
package=cases/max12_812_order2_p0_total_rees_j2_a1_total_converter_v43c5_20260827
if [[ "$tag" != max12_812_order2_p0_total_rees_j2_a1_total_converter_v43c5_* ]]; then exit 126; fi
...
lane="${tag}_exact_total_converter"
(
  ulimit -v "$memory_cap_kib"
  bash ops/aws_exact_lane.sh "$root" "$job/run" "$lane" timeout "$wall_seconds" \
    python3 "$package/replay_total_converter_v43c5.py" "$job/output"
)
stdout="$job/run/${lane}.stdout"
grep -Fx 'PASS-A1-TOTAL-RAW-CIRCUIT-CERTIFICATE-A1-628-V43C5' "$stdout" >/dev/null
...
printf 'end_utc=%s\noutcome=exact-total-a1-628-circuit-certificate-replayed\n' \
