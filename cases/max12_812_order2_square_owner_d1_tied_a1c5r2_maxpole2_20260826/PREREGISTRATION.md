...
Put `L=z^2+p/2` and clear `L^2`.  The Laurent/Faber receiver discards the
ordinary polynomial part, so its reconstructed numerator is the canonical
...
The compiler must reconstruct `N2 = N2_full mod L^2` from the analytic rows,
verify the exact ordinary-part quotient and pole-two recurrence, and identify
`N2(+-lambda)` with the exact Faber functionals
...
2. Verify the common numerator modulo `L^2`, its exact discarded
   ordinary-part quotient, both pole-two root functionals, the four-chart
...
The preregistration's V1 wording incorrectly demanded literal equality of
the negative-tail numerator with (1), rather than equality modulo `L^2`.
Triple-AWS V1 nevertheless passed every root functional, derivative syzygy,
allocation chart, terminal, unit, and negative control; it is quarantined as
no-verdict because that overstrong sentinel failed.  V2 makes only this
Laurent-versus-ordinary normalization correction.
