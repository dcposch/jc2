...
case_dir=$repo/cases/max12_912_order3_nu_q8_projective_fibre_boundary_satlib_aws_20260825
parent_dir=$repo/cases/max12_912_order3_nu_q8_projective_fibre_boundary_saturated_aws_20260825
raw_dir=$repo/cases/max12_912_order3_nu_q8_projective_fibre_boundary_aws_20260825
lane=$outroot/$tag
if [ -e "$lane" ]; then echo "duplicate lane refused" >&2; exit 65; fi
mkdir -p "$lane"
set +e
python3 "$case_dir/generate_satlib.py" --w-value "$wvalue" > "$lane/input.sing" 2> "$lane/generator.stderr"
generator_rc=$?
set -e
{
  echo "tag=$tag"
  ...
  echo "method=pure-Singular-elimlib-sat-global-projective-boundary"
  Singular --version | head -1 | sed 's/^/singular=/'
  sha256sum "$0" "$case_dir/generate_satlib.py" "$parent_dir/generate.py" "$raw_dir/generate.py" \
