...
V32_SHA256 = "dcc7003def824c0d2a88998cbf22a66f022a32714f05754c505e8d825bd91742"
assert sha256(V32_PATH.read_bytes()).hexdigest() == V32_SHA256
spec = importlib.util.spec_from_file_location("td6_v78b_v32_parent", V32_PATH)
...
tri, r = v32.tri, v32.r
Rat3, E3 = v32.Rat3, v32.E3
C, V, U, B3, H = v32.C, v32.V, v32.U, v32.B3, v32.H
qd, nr = r.qd, r.qd.nr
...
ALL_Q_EXPONENTS = tuple(range(2, 15)) + tuple(range(16, 25))
assert len(ALL_Q_EXPONENTS) == 22 and 15 not in ALL_Q_EXPONENTS
...
QST_PIVOT_POLICY = os.environ.get("TD6_PIVOT_POLICY", "ascending")
assert QST_PIVOT_POLICY in ("ascending", "reverse", "sparse")
QST_PIVOT_SCOPE = os.environ.get("TD6_PIVOT_SCOPE", "all-staged")
assert QST_PIVOT_SCOPE in ("all-staged", "current-only")
QST_ACTIVE_STAGE = "none"

# The reviewed trivariate transport is a source-specific ascending echelon
# and stays byte-for-byte/pivot-for-pivot fixed.  This bounded diagnostic
# changes only the staged EJet solvers (FIRST, previous/pole, CURRENT).
# V82QST1R is preserved as a negative control for crossing that boundary.
...
def restrict_row_jet(original_row, pivots, pivot_rhs, free_parameter):
    ...
    base = ZERO_VECTOR
    derivatives = {}
    # The source-specific transport echelon is licensed in ascending variable
    # order and is deliberately not subjected to the staged pivot policy.
...
def configure_qd_jet(omit_direct=()):
    ...
    if 2 in Q_EXPONENTS:
        assert set(qd.B.derivatives) == {2}
        assert qd.B.derivatives[2] == E3(1)
    else:
        assert not qd.B.value and not qd.B.derivatives


def qst_choose_base_pivot(row, base_variables):
    """Choose a base-nonzero EJet pivot under the registered policy."""
    policy = QST_PIVOT_POLICY
    if QST_PIVOT_SCOPE == "current-only" and QST_ACTIVE_STAGE != "current":
        policy = "ascending"
    if policy == "ascending":
        return min(base_variables)
    if policy == "reverse":
        return max(base_variables)
    candidates = []
    for variable in base_variables:
        coefficient = EJet.coerce(row[variable]).value
        scalar_coordinates = [
            coordinate for coordinate in r.scalar_coordinates(coefficient)
            if coordinate
        ]
        if len(scalar_coordinates) != 1:
            continue
        scalar_coordinate = scalar_coordinates[0]
        numerator = scalar_coordinate.numerator
        denominator = scalar_coordinate.denominator
        candidates.append(
            (
                denominator.total_degree(),
                numerator.total_degree(),
                len(str(denominator)) + len(str(numerator)),
                variable,
            )
        )
    assert candidates, "no scalar-coordinate sparse EJet pivot candidate"
...
def solve_cert_jet(rows):
    ...
    for row_index, (key, original_row, original_rhs) in enumerate(source):
        ...
        pivot = qst_choose_base_pivot(row, base_variables)
        lead = row[pivot]
        inverse = lead.inverse()
        pivots[pivot] = (
...
def parameterize(nvariables, rows, stage):
    global QST_ACTIVE_STAGE
    old_stage = QST_ACTIVE_STAGE
    QST_ACTIVE_STAGE = stage
    try:
        pivots, factors, dependent = solve_cert_jet(rows)
    finally:
        QST_ACTIVE_STAGE = old_stage
    ...
    # Each pivot row was reduced against all earlier inserted pivots, hence
    # reverse insertion order is the source-honest substitution DAG for every
    # registered policy.  Reverse variable order was an ascending-only
    # presentation assumption and is preserved as a failed V82QST1 control.
    for variable in reversed(tuple(pivots)):
        row, rhs, _ = pivots[variable]
        form = (rhs, {})
        for other, coefficient in row.items():
            if other == variable:
                continue
            assert forms[other] is not None
...
def divide_jet(polynomial, pivots):
    ...
    # Genuine P12 polynomial reduction retains its reviewed ascending order.
    for pivot in sorted(pivots):
        quotient = {}
        while True:
            targets = sorted(
...
def qst_factor_valuation(polynomial, factor):
    """Return exact divisibility order and the stripped polynomial."""
    order = 0
    stripped = polynomial
    while stripped:
        quotient = stripped // factor
        if quotient * factor != stripped:
            break
        stripped = quotient
        order += 1
...
def qst_remainder(polynomial, factor):
    quotient = polynomial // factor
    return polynomial - quotient*factor


def qst_preassert_coordinate_dump(coordinates, ordered, outdir, label):
    """Serialize every reduced base-field coordinate before fail-closed assert."""
    lines = [
        "compatibility\tkey\tcoordinate\texponent\tefield_coordinate\t"
        "coefficient_sha256\tnumerator\tdenominator\t"
        "numerator_factor\tdenominator_factor\t"
        + "\t".join(
            f"{name}_numerator_order\t{name}_denominator_order\t"
            f"{name}_scaled_numerator_residue\t{name}_scaled_denominator_residue"
            for name, _ in QST_DEBT_FACTORS
        )
    ]
    pole_counts = {name: 0 for name, _ in QST_DEBT_FACTORS}
    coefficient_count = 0
    scalar_coordinate_count = 0
    for key in ordered:
        compatibility, source_key, affine_coordinate = key
        for exponent in Q_EXPONENTS:
            coefficient = coordinates[key].get(exponent, E3())
            if not coefficient:
                continue
            coefficient_count += 1
            for base_index, rational in enumerate(r.scalar_coordinates(coefficient)):
                if not rational:
                    continue
                scalar_coordinate_count += 1
                numerator = rational.numerator
                denominator = rational.denominator
                fields = [
                    str(compatibility),
                    repr(source_key),
                    str(affine_coordinate),
                    str(exponent),
                    str(base_index),
                    e3_digest(coefficient),
                    str(numerator),
                    str(denominator),
                    repr(numerator.factor()),
                    repr(denominator.factor()),
                ]
                for name, factor in QST_DEBT_FACTORS:
                    numerator_order, numerator_stripped = qst_factor_valuation(
                        numerator, factor
                    )
                    denominator_order, denominator_stripped = qst_factor_valuation(
                        denominator, factor
                    )
                    if denominator_order:
                        pole_counts[name] += 1
                        assert numerator_order == 0
                    fields.extend(
                        [
                            str(numerator_order),
                            str(denominator_order),
                            str(qst_remainder(numerator_stripped, factor)),
                            str(qst_remainder(denominator_stripped, factor)),
                        ]
                    )
                lines.append("\t".join(fields))
    text = "\n".join(lines) + "\n"
    path = outdir / f"{label}_PREASSERT_REDUCED_COORDINATES.tsv"
    path.write_text(text)
    print(f"{label}_preassert_reduced_coefficient_count={coefficient_count}")
    print(f"{label}_preassert_base_coordinate_count={scalar_coordinate_count}")
    print(
        f"{label}_preassert_pole_counts="
        + ",".join(f"{name}:{pole_counts[name]}" for name, _ in QST_DEBT_FACTORS)
    )
    print(f"{label}_preassert_reduced_coordinate_path={path}")
    print(
        f"{label}_preassert_reduced_coordinate_sha256="
...
def denominator_for(values):
    values = list(values)
    return v32.denominator_lcm(values) if values else tri.CTX(1)
...
def conormal_table(dependent, outdir, label):
    ...
    allowed = factors_only_allowed(denominator)
    factorization = denominator.factor()
    diagnostic_text = "\n".join(
        [
            f"label={label}",
            f"coordinate_count={len(ordered)}",
            f"rank={len(pivots)}/{len(Q_EXPONENTS)}",
            f"kernel_dimension={len(kernel)}",
            f"denominator_exact={denominator}",
            f"denominator_factor={factorization}",
            f"denominator_radical_subset_U_H_B3={str(allowed).lower()}",
            "assertion_remains_fail_closed=true",
        ]
    ) + "\n"
    diagnostic_path = outdir / f"{label}_DENOMINATOR_DIAGNOSTIC.txt"
    diagnostic_path.write_text(diagnostic_text)
    print(f"{label}_preassert_coordinate_count={len(ordered)}")
    print(f"{label}_preassert_rank={len(pivots)}/{len(Q_EXPONENTS)}")
    print(f"{label}_preassert_kernel_dimension={len(kernel)}")
    print(f"{label}_preassert_denominator_exact={denominator}")
    print(f"{label}_preassert_denominator_factor={factorization}")
    print(
        f"{label}_preassert_denominator_radical_subset_U_H_B3="
        f"{str(allowed).lower()}"
    )
    print(f"{label}_denominator_diagnostic_path={diagnostic_path}")
    print(
        f"{label}_denominator_diagnostic_sha256="
        f"{sha256(diagnostic_text.encode()).hexdigest()}",
        flush=True,
    )
    pole_counts = qst_preassert_coordinate_dump(
        coordinates, ordered, outdir, label
    )
    if label == "CURRENT" and not allowed:
        denominator_factors = tuple(factor for factor, _ in factorization[1])
        assert all(
            any(actual == expected for actual in denominator_factors)
            for _, expected in QST_DEBT_FACTORS[:3]
        )
        assert all(pole_counts[name] > 0 for name in ("F", "G", "L"))
        print("CURRENT_nested_F_G_L_factors_exactly_detected=true", flush=True)
    assert allowed
...
def run_p12(bands, first_rows, first_pivots, first_factors, outdir):
    ...
    if 2 in Q_EXPONENTS:
        assert not first_minor.derivatives.get(2, E3())
        print("first_minor_q2_projection_matches_V32=true")
    if 3 in Q_EXPONENTS:
        assert not first_minor.derivatives.get(3, E3())
        print("first_minor_q3_exact_zero=true")
    print("zero_scalar_digest_semantics_asserted=true", flush=True)
    ...
    print("P12_requested_q_dual_remainder_is_unit_automatic=true", flush=True)
    ...
    print("requested_q_derivative_source_identity_exact=true")
    if 2 in Q_EXPONENTS:
        print("q2_projection_matches_V32=true")
    if 3 in Q_EXPONENTS:
        print("q3_projection_emitted_for_pure_V77R_comparison=true")
    print("requested_q_denominator_radicals_subset_U_H_B3=true")
    print("TD6-A3-ALL-Q-VECTOR-AD-P12-SHARD PASS")
...
def main():
    ...
    print("producer=TD6-A3-ALL-Q-VECTOR-AD-V78C-SHARD")
    print(f"mode={mode}")
    print(f"requested_q_exponent={REQUESTED_Q_EXPONENT}")
    print(f"qst_pivot_policy={QST_PIVOT_POLICY}")
    print(f"qst_pivot_scope={QST_PIVOT_SCOPE}")
    if QST_PIVOT_SCOPE == "current-only":
        assert QST_PIVOT_POLICY == "reverse"
        print("qst_FIRST_and_PREVIOUS_POLE_policy=ascending")
        print("qst_CURRENT_policy=reverse")
    print("qst_reviewed_ascending_transport_preserved=true")
    print("qst_staged_EJet_pivot_policy_changed=true")
    print("qst_staged_pivot_insertion_DAG_source_honest=true")
    print("qst_V82QST1_and_V82QST1R_failures_preserved_negative=true")
    print("qst_reverse_sparse_diagnostic_only=true")
    print("full_q_inventory=" + ",".join(map(str, ALL_Q_EXPONENTS)))
    print("single_coordinate_exact_shard=true")
    ...
    print(f"V32_base_source_sha256={V32_SHA256}", flush=True)
    ...
    print("first_requested_q_full_family_parameterization=true", flush=True)
