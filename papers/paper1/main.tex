...
Proposition 4.3 of \cite{GGHV} reduces this family to two subcases: find $(P,Q)$ with $[P,Q]:=P_xQ_y-P_yQ_x=x^2$, with $N(P)$ and $N(Q)$ in prescribed small polygons and prescribed vertices attained. In subcase (2) both polygons are narrow lattice \emph{strips} along $(1,2)$. Independently replayed exact artifacts exclude that subcase over characteristic zero, conditional on Proposition 4.3 and the transcription bridge (Section \ref{sec:computation}); within our reduction the generic chart itself empties by a short exact substitution chain ending in $-1=0$. This note isolates that mechanism, the \emph{vertex-gap obstruction}, and develops it as far as it provably goes.
...
Annotations. The single theorem-covered row is covered only in part: the $(8,28)$ family reduces in \cite{GGHV} to two subcases, and only subcase (2) is strip-shaped; subcase (1) is pentagon-shaped with $y$-axis support, outside the theory, and was handled computationally by independent artifacts, first by Helali \cite{Helali} (published July 21, 2026) and then by Suzuki \cite{Suzuki}, both of which we replayed exactly in the campaign repository. Subsequent independent replications by Strinz \cite{Strinz} and Ishihara \cite{Ishihara} have not yet been replayed by us. For the $19$ rows with $k=1$, the absence of the mechanism is proved (under (i), $k=1$ forces both bottom corners into column $x=1$, so no gap column exists on either side); this does not discard those families, only that this obstruction cannot explain their failure. The $4$ rows with $k\geq3$ all have $b/a$ integral, so if their corners survive reduction the shape is not a strip and the question is moot; if some subcase were strip-shaped, Theorem \ref{thm:R} would cover its depth-two block at every $d_2\geq2$ (with Theorem \ref{thm:kbig} showing that at $d_2=2$ the block imposes nothing beyond support rigidity). Of the $9$ unknown $k=2$ rows, one (the second reduced branch of $(8,28)$ itself) has $d_2=2$ and would land in the proved cell if it strip-reduces; the other eight are $y$-axis flagged.
...
\noindent\textbf{Pipeline and companion claim.} The campaign behind this note transcribes the Proposition 4.3 polygon data into the full bracket system \eqref{eq:key}, runs the level-ordered unit-pivot elimination and a generic-chart decomposition, and hands each stratum to the Gr\"obner engine msolve \cite{msolve}. The generic chart has an exact one-term certificate: its emitted ideal literally contains $-1$. The archived msolve \texttt{-g} outputs for the other two internal strata, however, are only first-machine-prime traces: in msolve 0.10.1 the characteristic-zero unit-basis path can display $[1]$ and return before rational reconstruction. (Successful non-unit runs continue through reconstruction.) Hence those internal unit outputs, even together with two to three large-prime screens, do not establish a unit ideal over $\mathbb{Q}$. Exact characteristic-zero exclusion of the full subcase instead comes from the independently replayed Helali and Suzuki artifacts described below. That exclusion is conditional on Proposition 4.3 itself and on the faithful normalization/transcription bridge; \cite{GGV2,GGV3,GGV4,GGHV} are arXiv-only and unrefereed (versions pinned in the repository, \cite{GGHV} at v1), while \cite{GGV1} is published with one corollary present only in the arXiv version. The theorems of Sections \ref{sec:gap} and \ref{sec:variants} are unconditional statements about strip pairs; only their application to $(72,108)$ inherits the dependency. Theorem \ref{thm:22} gives the exact generic-chart proof and yields the $\{a_2=0\}\cup\{a_6=0\}$ split under which the remaining internal strata were screened. Subcase (1) is not claimed here. The public computational record of its exclusion begins with Helali's artifact \cite{Helali} and now includes several independent replications \cite{Suzuki,Strinz,Ishihara}; the resulting degree-bound statement, that no plane counterexample exists below degree $125$, belongs to that collective record rather than to this note.
...
\bibitem{Helali} B. Helali, Exact computer-assisted exclusion of the (72,108) frontier in the two-dimensional Jacobian problem, Zenodo, doi:10.5281/zenodo.21479814 (published July 21, 2026).
...
\bibitem{Ishihara} K. Ishihara, A reproducible exact-computational artifact for the planar Jacobian degree pair (72,108), Zenodo, doi:10.5281/zenodo.21757679 (published August 2, 2026).
...
\bibitem{Strinz} W. Strinz, plane-jacobian-72-108: an audited computational case-closure program, Zenodo, doi:10.5281/zenodo.21633408 (published July 24, 2026).

\bibitem{Suzuki} E. Suzuki, Exclusion of the (8,28) Newton-polygon case in the plane Jacobian conjecture, Zenodo, doi:10.5281/zenodo.21483636 (published July 22, 2026).
