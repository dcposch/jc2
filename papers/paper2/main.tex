...
The campaign's Gr\"obner engine, msolve \cite{msolve}, reports emptiness as a reduced basis $[1]$. That is trace-level evidence, not an object a referee can check by hand; and on characteristic-$0$ input the printed basis is computed modulo the first machine prime of the multi-modular run, so even a char-$0$ header is not a lifted result. The campaign explored two ways to do better (artifact \path{CERT-UPGRADE.md}), and reports both outcomes, one of them a clean negative.
...
\emph{Retirement under the last-chance rule.} The char-$0$ lift certificates for two $(72,108)$ strata were a redundancy rider on a record that rests on exact mod-$p$ verdicts at three independent large primes plus satisfiability guards. When a scheduled last-chance check produced no certificate, the lane was retired and the runs killed, favorable as the certificates would have been. Redundancy that cannot be produced on schedule is deleted, not kept as an aspiration.

\emph{The 243/729 re-emission.} Review of a template document found that a cleared row constant was $243=3^5$, not the $729$ an earlier draft had baked into three emitters. The constant sits in a unit-rescalable row, so a scale map carries each stale system onto its corrected twin and no banked verdict flips; the campaign nonetheless re-emitted all 26 affected files (originals kept beside them with a \texttt{.stale729} suffix), re-verified the two-row difference with an independent parser at random points, and re-ran the engine guards. For the empty-verdict class the corrected twins were deliberately not re-screened: the banked controls whose rows are a subset of the full systems are empty with no such row at all, so emptiness is implied for any value of the constant; the audit records this subset argument and a falsification flag (if a corrected twin ever fails to reproduce EMPTY, the argument is void and review reopens). Hygiene re-emissions are cheap; silently inherited constants are not.
...
\emph{The $2^{63}$ clamp.} A second hazard, isolated by micro-test: msolve's integer-token parser clamps at $2^{63}-1$, silently, so a positive-characteristic file carrying unreduced coefficients larger than that is corrupted at parse time (coefficients between $p$ and $2^{63}$ are reduced correctly). A full inventory found 110 mod-$p$ files with unreduced coefficients, splitting at the true boundary into 49 corrupt-class and 61 word-sized; a cross-reference of every accepted campaign verdict against the inventory found the contaminated count to be zero, class by class (the char-$0$ certificates never touch the parser's modular path; the corrupt-class files produced no accepted verdict; the word-sized files parse correctly, re-verified empirically by reduced re-emissions reproducing every banked verdict). The standing emission rule now has four points: expanded sums only; all coefficients reduced into $[0,p)$ before shipping; an independent-parser round-trip at the target prime; and a constant-term satisfiability smoke test. Even word-sized violations are barred, because parser internals are not a trust anchor.
