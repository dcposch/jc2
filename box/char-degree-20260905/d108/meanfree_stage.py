...
def main(stage):
    ...
    start=time.monotonic();maps,res,meta=build(stage,True,normalized=True,strong_front=True)
    ...
    tag=f'd108_meanfree_stage{stage}_strongfront';path=OUT/f'{tag}.input.json';path.write_text(json.dumps(inputs,indent=2)+'\n')
    meta.update({'tag':tag,'source_variant':'supplementary mean-free quadratic-face necessary chart',
      'mean_parameter':'minor_mean','mean_free':True,'arc_unchanged':True,
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--stage',type=int,required=True,choices=range(9));args=ap.parse_args();main(args.stage)
