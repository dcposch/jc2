...
def main(input_path,out_path,timeout):
    ...
    names=['zz','tt']+list(inp['names']);ctx=fmpq_mpoly_ctx.get(tuple(names),ordering='lex')
    ...
    with body_path.open('w') as body:
        ...
        def lift(tab,label,kind='lift'):
            ...
            for (r,z),expression in sorted(tab.items()):
                ...
                name=f'cx{label}_{r}_{z}';newnames.append(name);row(name+'-('+expression+')',kind);result[(r,z)]=name
            return result
        ...
        minR=min(r for r,z in Rtab);minU=min(r for r,z in Utab)
    ...
    allnames=list(inp['names'])+newnames;record['ring_generator_order']=allnames;record['counts']=counts
