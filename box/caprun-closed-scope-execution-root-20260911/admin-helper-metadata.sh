...
for jc2_helper in bash env systemd-run systemctl timeout ps chrt setpriv test id uname tr hostname readlink date sed awk find sort stat file ldd dpkg-query sha256sum cut head grep install mount findmnt mkfifo sync cmp od basename dirname cat sleep mkdir; do
...
dpkg-query -W -f='ADMIN_PACKAGE ${Package} ${Version}\n' bash coreutils systemd systemd-sysv util-linux procps libc6 findutils mawk grep sed file openssh-server
...
done < <(printf '%s\n' "${!jc2_native_files[@]}" "${!jc2_native_aliases[@]}"; sed -n -E 's/^[a-f0-9]{64}  (\/.*)$/\1/p;s/^DIRECTORY [0-9]+ [0-9]+ [0-7]+ (\/.*)$/\1/p;s/^ALIAS (\/[^ ]+) .*$/\1/p' /home/ubuntu/jc2-closedchild-preflight9-20260911a/native.stdout)
