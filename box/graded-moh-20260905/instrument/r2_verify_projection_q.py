...
for n in (77,136):
 d=ROOT/'resume-r2'/'triangular'/str(n);m=json.loads((d/'projection-custody.json').read_text());vv=m['receiver_variable_order'];vi={v:i for i,v in enumerate(vv)};weights=[m['receiver_weights'][v] for v in vv];X=m['degree_bound'];zero=(0,)*len(vv)
  ...
  for raw in TERM.findall(s):
   ...
   for f in raw.lstrip('+-').split('*'):
    ...
    if ma:
     ...
     if source and m['source_variable_map'].get(v)=='0':omit=True;continue
     assert v in vi,(n,v)
     mon[vi[v]]+=e
    else:coef*=Q(f)
 ...
 G=[parse(p) for p in (d/'projection-basis.txt').read_text().strip().split(',')];assert all(G)
 ...
 for line in Path(m['source_rows']).read_text().splitlines()[1:]:
  ...
  if int(ix) not in set(m['source_generator_indices']):continue
  p=parse(expr,True)
 ...
 for i,g in enumerate(G):
  for j in range(i):
   ...
   count+=1
 c=parse('c');nf=normalform(c);saved=parse((d/'projection-nf.txt').read_text().strip());assert nf==saved
