...
out=BASE/'resume-r2/truncation/C_n24m18_Mm15_14_ell1_s3_V1_9_N1/curve-fast'
cust=json.loads((out/'custody.json').read_text());src=Path(cust['original_source_input']);meta=json.loads((src.parent/'manifest.json').read_text());vs=cust['variables'];vi={v:i for i,v in enumerate(vs)};W=cust['weights'];X=cust['combined_degree_bound'];nv=len(vs);zero=set(cust['assignments'][0]);ass=cust['assignments'][0]
assert zero=={v for v,w in meta['weights'].items() if w[0]==0}
assert vs==[v for v in meta['variables'] if meta['weights'][v][0]>0]+['curve_t']
assert W==[meta['weights'][v][1]+cust['L']*meta['weights'][v][0] for v in vs[:-1]]+[1]
assert hashlib.sha256(Path(meta['source_rows']).read_bytes()).hexdigest()==meta['source_sha256']
assert hashlib.sha256(src.read_bytes()).hexdigest()==cust['original_source_input_sha256']
def parse(s, specialize=False):
 ...
 for t in re.findall(r'[+-]?[^+-]+',s.replace(' ','').replace('\n','').strip('()')):
  ...
  for f in t.split('*'):
   ...
   if v in zero and specialize:
    c*=ass[v]**e;m[-1]+=meta['weights'][v][1]*e;continue
   if v in vi:m[vi[v]]+=e
   else:c*=F(f)
  if c:
...
c=parse('c');nf=reduce(c,G);assert nf and nf==parse((out/'normalform-0.txt').read_text().strip())
...
result=dict(coefficient_field='Q',source_rows_checked=len(rows),basis_elements=len(G),basis_homogeneous_in_combined_degree=True,combined_degree_formula='w+42B',truncated_degree=X,spairs_checked=count,input_images_reduce_zero=True,all_required_spairs_reduce_zero=True,c_normalform_terms=len(nf),c_normalform_file=str(out/'normalform-0.txt'),c_nonmembership=True,negative_corrupted_row_detected=True,positive_basis_row_reduces_zero=True,elapsed_seconds=time.monotonic()-start,proof_scope='c is not in the full original ideal by exact bidegree restriction and the monomial-curve specialization v->curve_t^w(v) for every B=0 variable, preserving B and w; no claim about c powers or saturation.',files={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [src,Path(meta['source_rows']),out/'basis-0.txt',Path(__file__)]})
