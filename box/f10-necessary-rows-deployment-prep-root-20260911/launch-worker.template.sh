...
test ! -e box/f10-necessary-rows-deployment-prep-root-20260911/launch-response.json
sha256sum -c box/f10-necessary-rows-deployment-prep-root-20260911/ROOT-READY-INPUTS.sha256
...
/usr/local/bin/aws ec2 run-instances --profile personal --region us-east-1 \
 ...
 --output json > box/f10-necessary-rows-deployment-prep-root-20260911/launch-response.json
test "$(jq 'length' box/f10-necessary-rows-deployment-prep-root-20260911/launch-response.json)" = 1
jc2_r3_instance=$(jq -r '.[0].ID' box/f10-necessary-rows-deployment-prep-root-20260911/launch-response.json)
...
sed -n '1,150p' box/f10-necessary-rows-deployment-prep-root-20260911/launch-response.json
