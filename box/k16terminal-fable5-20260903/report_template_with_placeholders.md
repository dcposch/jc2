...
- **PROVED-HERE, fixed `t`:** characteristic-zero certificates of (8.1),
  ...
  (previously `INCONCLUSIVE_TIMEOUT` / modular only)T78_VERDICT; `t=2,3,4`
...
`t=3,4`, by 5.1 at `t=5,6`) RESZ_MORE, but the axis values carry the pivot
...
`sha256sum -c` from inside the directory).  MANIFEST_LINE
