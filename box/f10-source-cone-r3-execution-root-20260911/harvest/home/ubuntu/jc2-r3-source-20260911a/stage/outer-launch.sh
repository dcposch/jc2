#!/bin/bash
# ROOT exact batch release. Existing OS controls; no science changes.
set -euo pipefail
test "$(id -u)" = 0
...
test 'ROOT_RELEASED_EXACT_R3_BATCH' = ROOT_RELEASED_EXACT_R3_BATCH
...
test "$(date -u +%s)" -lt "$(date -ud '2026-09-11 02:05:00 UTC' +%s)"
jc2_unit=jc2-r3-source-20260911a.service
...
systemd-run --unit=jc2-r3-source-20260911a-term \
 --on-calendar='2026-09-11 03:05:00 UTC' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=TERM "$jc2_unit"
systemd-run --unit=jc2-r3-source-20260911a-kill \
 --on-calendar='2026-09-11 03:05:05 UTC' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=KILL "$jc2_unit"
systemctl is-active jc2-r3-source-20260911a-term.timer jc2-r3-source-20260911a-kill.timer
date -u '+ROOT_OUTER_TIMERS_ARMED_BEFORE_LAUNCH %Y-%m-%d %H:%M:%S.%N UTC'
systemd-run --unit="$jc2_unit" --property=Type=exec \
 --property=WorkingDirectory=/opt/jc2-r3-source-20260911a/wrapper \
 ...
 --property=StandardOutput=file:/run/jc2-r3-source-20260911a/outer.stdout \
 --property=StandardError=file:/run/jc2-r3-source-20260911a/outer.stderr \
 /usr/bin/env -i PATH=/usr/bin:/bin LANG=C LC_ALL=C TZ=UTC \
 /usr/bin/python3.12 -I -S -B \
 /opt/jc2-r3-source-20260911a/wrapper/dispatch.py \
 --registration /opt/jc2-r3-source-20260911a/metadata/ROOT-REGISTRATION.json
