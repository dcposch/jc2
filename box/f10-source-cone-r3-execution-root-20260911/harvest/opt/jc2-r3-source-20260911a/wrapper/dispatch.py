"""Fixed-r3 reconstruction + one mixed-row runtime, STATIC/UNEXECUTED; disabled without fresh ROOT registration.
...
class Batch:
    def __init__(self, name):
        ...
        r = self.r
        need(r.get('schema') == 'f10-source-cone-r3-runtime/v1' and r.get('enabled') is True, 'disabled ROOT registration')
        ...
        need(all(p != self.mount and self.mount not in p.parents for p in
                 list(self.files.values()) + [self.path, self.native_path, self.simple_path]), 'static inputs outside tmpfs')
        for fd in (1, 2):
    ...
    def execute(self):
        reasons = {'status': 'no ROOT registration', 'caps': 'registered limits/source inventory',
                   'inventory': 'registered limits/source inventory', 'source': 'source pin: produce.py', 'hash': 'authority/inventory byte pin'}
        for mutation, reason in reasons.items():
            t, out, err, payload = self.run('refuse-' + mutation, 'probe', 'control')
            self.normal(t, 1)
            need(not payload.exists() and out.read_bytes() == b'' and err.read_text() == 'ValueError: ' + reason + '\n', 'exact actual-authority ValueError')
        ...
        for role in ('produce', 'check'):
            ...
            self.normal(t, 1)
            need(not payload.exists() and out.read_bytes() == b'' and err.read_text() == 'REFUSED: registered CLI contract\n', 'actual entry startup refusal')
        ...
        self.normal(t, 0)
        need(out.read_bytes() == b'FORMED_UNCHECKED_R3_25_SLOTS_NO_SOURCE_OUTCOME\n' and err.read_bytes() == b'', 'producer literal verdict')
        ...
        self.normal(t, 0)
        need(out.read_bytes() == b'CHECKED_R3_25_GRAPH_SLOTS_45_LOW_108_JACOBIAN_NO_SOURCE_OUTCOME\n'
             and err.read_bytes() == b'', 'independent checker literal verdict')
        phase = {'schema': 'f10-r3-positive-phase/v1', 'status': 'POSITIVE_CHECKED_NO_SOURCE_OUTCOME',
                 'root_registration_sha256': self.root_sha, 'input': {'path': str(target),
                 'sha256': digest, 'bytes': target.stat().st_size}, 'source_files': SCIENCE,
        ...
        need(set(receipt) == {'schema', 'status', 'root_registration_sha256', 'input_sha256',
             ...
             and receipt['schema'] == 'f10-r3-mixed-row-mutation/v1'
             and receipt['status'] == 'FORMED_UNCHECKED' and receipt['root_registration_sha256'] == self.root_sha
             and receipt['input_sha256'] == digest and receipt['mutation'] == 'graph.slots[24] += X1^27'
        ...
        self.normal(t, 1)
        need(t['child_signal'] is None and t['child_exit_code'] == 1 and out.read_bytes() == b'', 'normal semantic exit1')
        self.semantic_traceback(err)
        need(tuple(x['label'] for x in self.runs) == LABELS, 'exact thirteen completed commands')
        result = {'status': 'RECONSTRUCTION_AND_MIXED_ROW_REJECTION_CHECKED_NO_SOURCE_OUTCOME',
                  'baseline_sha256': digest, 'fixture_sha256': sha(fixture), 'source_files': SCIENCE,
                  'negative_predicate': 'all25 literal graph rows T', 'other_five_semantic_controls': 'NOT_RUN_SPEC',
    ...
    def semantic_traceback(self, path):
        ...
        frames = [(check, 540, '<module>', 'main()'), (check, 536, 'main', 'print(check_file(path))'),
                  (check, 520, 'check_file', "eq(p, rawrow, 'all25 literal graph rows ' + label)"),
                  (arithmetic, 227, 'require', "raise ValueError('CHECK FAILED: ' + label)")]
        need(lines and lines[0] == 'Traceback (most recent call last):\n', 'exact traceback header')
        i = 1
        for file, number, function, statement in frames:
            wanted = ['  File "' + file + '", line ' + str(number) + ', in ' + function + '\n', '    ' + statement + '\n']
            need(lines[i:i + 2] == wanted, 'ordered exact mathematical traceback frame')
            i += 2
            if i < len(lines) and len(lines[i]) <= 512 and re.fullmatch(r' +[~^]*\^[~^]*\n', lines[i]):
                i += 1
        need(lines[i:] == ['ValueError: CHECK FAILED: all25 literal graph rows T\n'], 'exact final, no chain/extra traceback')
    ...
    def durable_copy(self, result):
        ...
        emit(self.durable / 'CUSTODY.json', {'schema': 'f10-source-cone-r3-runtime-custody/v1', 'result': result,
...
def main():
    ...
    return 0 if result['status'] == 'RECONSTRUCTION_AND_MIXED_ROW_REJECTION_CHECKED_NO_SOURCE_OUTCOME' else 2
