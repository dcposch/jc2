"""FIRST r2 runtime entry, STATIC/UNEXECUTED; disabled without fresh ROOT registration.
...
PROFILES = {'control': [5, 3, 2147483648], 'dummy': [5, 3, 33554432],
            'produce': [300, 239, 2147483648], 'check': [210, 169, 2147483648]}
LABELS = ('refuse-status', 'refuse-caps', 'refuse-inventory', 'refuse-source', 'refuse-hash',
          'valid', 'startup-produce', 'startup-check', 'dummy', 'produce', 'check')
...
class Batch:
    def __init__(self, name):
        ...
        r = self.r
        need(r.get('schema') == 'f10-r2-runtime/v1' and r.get('enabled') is True, 'disabled ROOT registration')
        ...
        need(set(r['commands']) == set(LABELS), 'exact eleven command registrations')
    ...
    def run(self, label, role, profile, input_path=None):
        ...
        need(command == self.r['commands'][label], 'literal ROOT CAPRUN argv mismatch')
    ...
    def execute(self):
        ...
        self.normal(t, 0)
        need(out.read_bytes() == b'FORMED_UNCHECKED_19_ROWS_NO_SOURCE_OUTCOME\n' and err.read_bytes() == b'', 'producer literal verdict')
        need(packet.is_file() and not packet.is_symlink() and 0 < packet.stat().st_size <= PACKET, 'formed packet size')
        digest = sha(packet); target = self.frozen / 'artifact.json'
        need(not target.exists(), 'fresh frozen target')
        os.rename(packet, target); os.chown(target, 0, 0); os.chmod(target, 0o444)
        syncdir(self.writer); syncdir(self.frozen); os.chmod(self.frozen, 0o555)
        need(sha(immutable(target)) == digest, 'move/freeze identity')
        t, out, err, ignored = self.run('check', 'check', 'check', target)
        self.normal(t, 0)
        need(out.read_bytes() == b'CHECKED_STATIC_CONTRACT_19_ROWS_32_RESIDUALS_80_SLOTS_NO_SOURCE_OUTCOME\n'
             and err.read_bytes() == b'', 'independent checker literal verdict')
        result = {'status': 'RECONSTRUCTION_CHECKED_NO_SOURCE_OUTCOME', 'input_sha256': digest,
                  'checker_stdout_sha256': sha(out), 'science_pins': SCIENCE, 'root_registration_sha256': self.root_sha,
                  'joint_wall_seconds': time.monotonic() - self.begin, 'joint_cgroup_cpu_seconds': self.cpu() - self.cpu_start}
        emit(self.mount / 'checker-binding.json', result)
        return result
...
def main():
    ...
    return 0 if result['status'] == 'RECONSTRUCTION_CHECKED_NO_SOURCE_OUTCOME' else 2
