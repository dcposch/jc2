...
task_stop='JC2_QUAL_STOP_PLACEHOLDER'
task_term='JC2_QUAL_TERM_PLACEHOLDER'
task_stage=/home/ubuntu/jc2-mixed-v3-stage-20260913
task_base=/opt/jc2-mixed-20260912
task_prep="$task_base/prep"
task_dispatch_sha=JC2_DISPATCH_SHA_PLACEHOLDER
test "$(id -u)" = 0
...
test "$(sha256sum "$task_stage/native-metadata.actual.sh" | cut -d' ' -f1)" = JC2_COLLECTOR_SHA_PLACEHOLDER
test "$(sha256sum "$task_stage/inspect_native.py" | cut -d' ' -f1)" = JC2_INSPECTOR_SHA_PLACEHOLDER
test "$(sha256sum "$task_stage/inspect_imports.py" | cut -d' ' -f1)" = JC2_IMPORT_SHA_PLACEHOLDER
