...
test "$(date -u +%s)" -lt "$(date -ud 'JC2_ADMIT_CUTOFF_UTC_PLACEHOLDER' +%s)"
...
cd '/home/ubuntu/jc2-closedchild-preflight9-20260912a/stage'
...
systemd-run --unit=jc2-closedchild-preflight9-20260912a-term \
 --on-calendar='JC2_TERM_DEADLINE_UTC_PLACEHOLDER' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=TERM "$jc2_unit"
systemd-run --unit=jc2-closedchild-preflight9-20260912a-kill \
 --on-calendar='JC2_KILL_DEADLINE_UTC_PLACEHOLDER' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=KILL "$jc2_unit"
systemctl is-active jc2-closedchild-preflight9-20260912a-term.timer jc2-closedchild-preflight9-20260912a-kill.timer
...
/usr/bin/timeout 10 systemd-run --unit="$jc2_unit" --property=Type=exec \
 --property=WorkingDirectory=/opt/jc2-closedchild-preflight9-20260912a/wrapper \
 ...
 --property=StandardOutput=file:/run/jc2-closedchild-preflight9-20260912a/outer.stdout \
 --property=StandardError=file:/run/jc2-closedchild-preflight9-20260912a/outer.stderr \
...
while test "$SECONDS" -lt "$jc2_capture_end"; do
 ...
 if [[ "$jc2_main" =~ ^[1-9][0-9]*$ ]] && test -r /proc/"$jc2_main"/cmdline &&
    test "$(head -c 129 /run/jc2-closedchild-preflight9-20260912a/outer.stdout)" = HOLDER_WAITING_FOR_ROOT_RELEASE; then
