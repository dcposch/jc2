...
test "$(sha256sum "$jc2_base/admin/holder.py" | cut -d' ' -f1)" = 'c91d0a3bc1bd803fac86531cc7a6d482b04c964eadbffca0c5cce898b946b352'
...
systemd-run --unit=jc2-closedchild-preflight9-20260911c-term \
 --on-calendar='2026-09-11 16:20:00 UTC' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=TERM "$jc2_unit"
systemd-run --unit=jc2-closedchild-preflight9-20260911c-kill \
 --on-calendar='2026-09-11 16:20:05 UTC' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=KILL "$jc2_unit"
systemctl is-active jc2-closedchild-preflight9-20260911c-term.timer jc2-closedchild-preflight9-20260911c-kill.timer
...
/usr/bin/timeout 10 systemd-run --unit="$jc2_unit" --property=Type=exec \
 --property=WorkingDirectory=/opt/jc2-closedchild-preflight9-20260911c/wrapper \
 ...
 --property=StandardOutput=file:/run/jc2-closedchild-preflight9-20260911c/outer.stdout \
 --property=StandardError=file:/run/jc2-closedchild-preflight9-20260911c/outer.stderr \
...
while test "$SECONDS" -lt "$jc2_capture_end"; do
 ...
 if [[ "$jc2_main" =~ ^[1-9][0-9]*$ ]] && test -r /proc/"$jc2_main"/cmdline &&
    test "$(head -c 129 /run/jc2-closedchild-preflight9-20260911c/outer.stdout)" = HOLDER_WAITING_FOR_ROOT_RELEASE; then
