"""Small prospective metadata boundary, not ROOT authentication or a runner."""
import os
import sys

TAG = 'f10-r2-highest-bezout-v1'
...
def enter(mode):
    ...
    if len(sys.argv) != 17 or tuple(sys.argv[1::2]) != flags or sys.argv[2] != TAG:
        raise SystemExit('REFUSED: exact registered highest-Bezout vector required')
...
def arithmetic(ctx):
    # No mathematical module is loaded until enter() completed all pin barriers.
    import importlib.util
    name = '_highest_producer_arithmetic' if ctx['mode'] == 'produce' else '_highest_checker_arithmetic'
...
def receipt(ctx, status, certificate_sha=None):
    ctx['recheck']()
    data = {'schema': 'f10-r2-highest-bezout-receipt/v1', 'status': status,
            'science_outcome': 'NONE', 'mode': ctx['mode'],
