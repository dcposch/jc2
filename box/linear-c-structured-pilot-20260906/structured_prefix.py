...
def parse(text):
    s=str(text)
    def ev(n):
        if isinstance(n,ast.Constant) and type(n.value)is int:return ctx.constant(n.value)
        ...
        if isinstance(n,ast.BinOp):
            ...
            if isinstance(n.op,ast.Pow):
                require(isinstance(n.right,ast.Constant) and type(n.right.value)is int and n.right.value>=0,"Polynomial power")
            ...
            if isinstance(n.op,ast.Div):
                require(not any(isinstance(x,ast.Name) for x in ast.walk(n.right)),"Parameter denominator")
                require(b.is_constant(),"Nonconstant denominator")
                return a/b
        raise ValueError(ast.dump(n))
    if s not in cache:cache[s]=ev(ast.parse(s.replace("^","**"),mode="eval").body)
    return cache[s]
def add(a,b,c=1):
    out=dict(a)
    for p,v in b.items():
