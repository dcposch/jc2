...
for name,pin in expected.items():
 ...
 entries.append({'path':name,'sha256':h,'bytes':len(b),'read_scope':'whole pinned report; double-chart proof re-audited, total two-pattern conclusion not imported'})
...
r=subprocess.run(['/usr/bin/python3','-I','-B',str(R/'ops/artifact_finalize.py'),'begin','--final',str(R/'xmodel/triple-source-contact-cover-astra-20260909.md'),'--basis','0d39df3c9fd69c939a8420c54d03228b9077777d','--owner','/root/nonemptiness_certificate'],capture_output=True,timeout=25,check=True)
