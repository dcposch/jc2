...
OUT = pathlib.Path("/home/ubuntu/jc2/box/k16terminal-opus-20260903")
...
def build(t: int, b4val=None):
    ...
    if b4val is not None:
        allvars = [v for v in allvars if v is not b4]
    ...
    shift = b4 if b4val is None else sp.Integer(b4val)
    U = sp.expand(U_h.subs(h, s+shift))
    C = sp.expand(C_h.subs(h, s+shift))
    A = sp.expand(s*C)
    ...
    Xprime_h = algebra.reduce(Xprime.subs(s, h-shift))
    ...
    R_h = algebra.reduce(R_s.subs(s, h-shift))
    ...
    record = {
        ...
        "t": t,
        "b4_chart": ("free" if b4val is None else str(b4val)),
        "H": str(algebra.H),
        "d": str(d),
...
def main():
    ...
    parser.add_argument("--b4", default="free")
    args = parser.parse_args()
    b4val = None if args.b4 == "free" else int(args.b4)
    OUT.mkdir(parents=True, exist_ok=True)
    for t in args.t:
        record = build(t, b4val)
        path = OUT/("chart_t%d_b4_%s.json" % (t, args.b4))
        path.write_text(json.dumps(record, indent=2, sort_keys=True)+"\n",
                        encoding="utf-8")
        print(json.dumps({
            "t": t,
        "b4_chart": ("free" if b4val is None else str(b4val)), "pivots": len(record["high_pivots"])+2,
