...
work=BASE/'fleet'/host/(stem+'_'+mode+'_retry64GiB');work.mkdir(parents=True,exist_ok=True)
...
status={'stem':stem,'mode':mode,'host':host,'state':'EMITTING','vm_limit_KiB':67108864,'watchdog_seconds':900,'started_utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())}
...
status.update(state='SOLVE_RUNNING',input_sha256=hashlib.sha256(ms.read_bytes()).hexdigest());save()
run(SSH+[remote,'mkdir -p '+shlex.quote(str(work))]);run(SCP+[str(ms),remote+':'+str(ms)])
opts='-l 44 -m 100 -s 12 -u 1'if mode=='circuit'else '-l 44 -m 1000'
cmd='export PATH='+shlex.quote(str(BASE.parent/'ops/bin'))+':$PATH; ulimit -v 67108864; bash '+shlex.quote(str(BASE/'ops/run_msolve.sh'))+' '+shlex.quote(str(ms))+' 900 12 '+opts+' > '+shlex.quote(str(work/'solve.driver.log'))+' 2>&1; true'
run(SSH+[remote,cmd]);base=str(ms)[:-3]
