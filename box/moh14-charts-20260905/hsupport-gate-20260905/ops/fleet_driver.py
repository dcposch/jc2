...
BASE=BUNDLE/'hsupport-gate-20260905'
...
def main():
 ...
 cmd='cd '+shlex.quote(str(src))+' && /usr/bin/time -v timeout --signal=TERM --kill-after=30s '+str(a.build_seconds)+'s Singular --cpus=1 --threads=1 --flint-threads=1 -q --no-rc '+shlex.quote(str(builder))+' > '+shlex.quote(str(work/'builder.stdout'))+' 2> '+shlex.quote(str(work/'builder.stderr'))+'; rc=$?; printf "%s\\n" "$rc" > '+shlex.quote(str(work/'builder.rc'))
 run(SSH+[remote,cmd])
 ...
 cmd='export PATH='+shlex.quote(str(BASE/'ops/bin'))+':$PATH; ulimit -v '+str(a.vm_kib)+'; bash '+shlex.quote(str(runner))+' '+shlex.quote(str(ms))+' '+str(a.solve_seconds)+' '+str(a.threads)+' -l 44 -m 1000 > '+shlex.quote(str(work/'solve.driver.log'))+' 2>&1; true'
 run(SSH+[remote,cmd])
 base=str(ms)[:-3]
