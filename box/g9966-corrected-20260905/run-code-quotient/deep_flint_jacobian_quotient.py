...
def jacobian_bands(h,C2,C3,outer,cutoff,source,first_band=0,progress=None,quotient=None):
    ...
    if quotient:
        qd,qe=sp.Symbol(quotient['d']),sp.Symbol(quotient['e'])
        variables=sorted(set(variables)|{qd,qe},key=str)
    variables=[w]+variables
    index={v:i for i,v in enumerate(variables)}
    ctx=fmpq_mpoly_ctx.get(tuple(map(str,variables)),'lex')
    zero=ctx.constant(0)
    if quotient:
        di,ei=index[qd],index[qe]
        qa=sp.Rational(quotient['a']);qb=sp.Rational(quotient['b'])
        qA_native=fmpq(int(qa.p),int(qa.q));qB_native=fmpq(int(qb.p),int(qb.q))
        assert sp.expand(sp.sympify(quotient['polynomial'])-(qd**2-qa*qd*qe**2+qb*qe**4))==0
        recurrence=[(fmpq(0),fmpq(1)),(fmpq(1),fmpq(0))]
    def normalize(poly):
        if not quotient or poly.is_zero():return poly
        out={}
        for exponent,value in poly.to_dict().items():
            power=exponent[di]
            if power<2:
                out[exponent]=out.get(exponent,fmpq(0))+value
                continue
            while len(recurrence)<=power:
                left,right=recurrence[-1]
                recurrence.append((qA_native*left+right,-qB_native*left))
            left,right=recurrence[power]
            if left:
                target=list(exponent);target[di]=1;target[ei]+=2*power-2;target=tuple(target)
                out[target]=out.get(target,fmpq(0))+value*left
            if right:
                target=list(exponent);target[di]=0;target[ei]+=2*power;target=tuple(target)
                out[target]=out.get(target,fmpq(0))+value*right
    ...
    def encode(poly):
        ...
        return {r:p for r,row in rows.items() if not (p:=normalize(ctx.from_dict(row))).is_zero()}
    def add(*series):
        out={}
        for poly in series:
    ...
    def mul(a,b):
        ...
        for r,left in a.items():
            for s,right in b.items():
                if r+s<=cutoff:out[r+s]=out.get(r+s,zero)+normalize(left*right)
        return {r:p for r,p in out.items() if not p.is_zero()}
    def bracket(a,b,D,E):
        ...
        for r,left in a.items():
            for s,right in b.items():
                if r+s<=cutoff:
                    value=normalize((D-r)*left*db[s]-(E-s)*da[r]*right)
    ...
    result={};total_terms=0
    for band_power in range(first_band,cutoff+1):
        final=J.get(band_power,zero);total_terms+=len(final.to_dict())
        ...
        result[band_power]={k:sp.Add(*terms) for k,terms in grouped.items()}
    if progress:progress(f'FLINT exact products complete; bands {first_band}..{cutoff} have {total_terms} terms; {time.monotonic()-tick:.2f}s')
    return result,{'coefficient_field':'Q','backend':'python-flint fmpq_mpoly',
        'active_computation_generators':list(map(str,variables)),'band_terms':total_terms,
        'quotient':quotient,'normal_form_rule':'retain the combined polynomial A+B*d; every reduction difference is a multiple of the monic q',
        'first_band':first_band,'t_cutoff':cutoff,'full_chart_coordinates_removed':False,'elapsed_seconds':time.monotonic()-tick,
...
def control(code):
    ...
    assert not zero
    cached,_=jacobian_bands(h,c2,c3,outer,6,S,first_band=0)
    triangular={b:a**2+sp.Rational(2,3)}
    image=lambda poly:{key:sp.expand(value.xreplace(triangular)) for key,value in poly.items()}
    refreshed,_=jacobian_bands(image(h),image(c2),image(c3),{name:image(poly) for name,poly in outer.items()},6,S,first_band=0)
    for tp in range(7):
        assert all(sp.expand(cached[tp].get(k,sp.Integer(0)).xreplace(triangular)-refreshed[tp].get(k,0))==0 for k in set(cached[tp])|set(refreshed[tp]))
    return {'status':'PASS','all_rows_equal_to_independent_SymPy_sparse_formula':True,'zero_outer_zero_J':True,
        'cached_coefficients_commute_with_QQ_star_triangular_graph':True,'graph_control':str(b-a**2-sp.Rational(2,3)),'checks':checked}
