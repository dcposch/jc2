#!/usr/bin/env python3
"""Resume exact necessary equations from a complete saved minor phase.
...
def main():
    ...
    if args.phase in ('d1','both'):
        ...
        face,fmeta=d1_face_jacobian(hs,cs2,cs3,os)
        ...
        Pi=sp.Symbol('Pi')
        rows=[(f'gauge_D1_J_face_Pi{powers[0]}',value) for powers,value in sp.Poly(sp.expand(face-jc),Pi).terms() if value!=0]
