...
def main():
    ...
    def continuation_paths(state):
        ...
        if state.get('resumed_from'):
            ...
            assert M.digest(terminal)==link['terminal_phase_sha256']==antecedent['phase_certificates'][-1]['sha256']
            last=read(terminal)
            assert last['phase']==f'Jacobian_t{antecedent["last_completed_t"]}'
    ...
    terminal_regeneration=(quotient_audits[-1] if final['phase']=='certified_quotient_transition' else v.regenerate(final,a.jacobian_method))
