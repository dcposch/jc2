...
def main():
    ...
    from deep_oracle_dimension import polynomial as exact_ast_polynomial
    allowed=set(old['free_after']);symbols={v:sp.Symbol(v) for v in allowed}
    def fast_parse(expression):
        monomials=exact_ast_polynomial(expression,allowed)
        return sp.Add(*(sp.Rational(c.numerator,c.denominator)*sp.Mul(*(symbols[v]**int(n) for v,n in m)) for m,c in monomials.items()))
    cfg=old['coefficient_quotient'];q=fast_parse(cfg['polynomial']);d,e=map(sp.Symbol,(cfg['d'],cfg['e']))
    assert sp.expand(fast_parse(oracle['metadata']['relation'])-q)==0
    free={sp.Symbol(v) for v in old['free_after']};loc=sp.Symbol('rho' if branch=='delta2' else 'c')
    residual=[(label,fast_parse(value)) for label,value in old['residual_rows']]
    assert E.rows_hash(residual)==old['residual_hash'] and any(sp.expand(value-q)==0 for _,value in residual)
    raw=[(f'resumed_J_t{tp}_d{163-tp}_k{int(k)}',fast_parse(v)) for k,v in sorted(oracle['rows'].items(),key=lambda kv:int(kv[0]))]
    ...
    oldmap={sp.Symbol(v):fast_parse(expr) for v,expr in old['cumulative_map'].items()}
