...
def main():
    ...
    if args.resume_state:
        ...
        inherited_phase_record=(inherited['phase_certificates'][-1] if inherited['phase_certificates'] else
            {'path':inherited['resumed_from']['terminal_phase'],'sha256':inherited['resumed_from']['terminal_phase_sha256']})
        ...
        inherited_phase_kind=inherited_phase['phase']
        assert inherited_phase_kind==f'Jacobian_t{completed}' or (
            inherited.get('coefficient_quotient') and inherited_phase_kind=='certified_quotient_transition'
            and (not inherited['records'] or inherited['records'][-1]['phase']=='certified_quotient_transition'))
    ...
    if inherited and inherited.get('coefficient_quotient'):
        quotient_cfg=inherited['coefficient_quotient']
        quotient_proof=inherited['quotient_transition']
        assert quotient_proof['status']=='PASS'
        assert quotient_proof['exact_ideal_checks']=={'K_subset_I':True,'I_plus_K_equals_I_both_inclusions':True}
        assert quotient_cfg==quotient_proof['quotient']
        protected={sp.Symbol(quotient_cfg['d']),sp.Symbol(quotient_cfg['e'])}
        assert protected <= free-set(mapping),'retained quotient coordinates were eliminated'
        q=sp.sympify(quotient_cfg['polynomial'])
        assert any(sp.expand(row-q)==0 for _,row in residual),'inherited monic relation must be explicit'
        state['coefficient_quotient']=quotient_cfg
        state['quotient_transition']=quotient_proof
        state['quotient_transition_reused_from_hashed_ancestor']=True
    else:
