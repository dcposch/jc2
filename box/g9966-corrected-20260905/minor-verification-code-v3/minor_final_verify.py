...
class Verifier:
    def __init__(self,e,branch):
        ...
        self.cache={};self.symbols_cache={};self.extra_generators=set();self.graph_faces=None;self.weak_map=None;self.weak_residual=None
    ...
    def regenerate(self,cert,jacobian_method='direct'):
        ...
        if jrequests:
            ...
            if jacobian_method=='flint-direct':
                helper=importlib.import_module('minor_flint_direct')
                ...
                for tp in sorted({tp for _,tp,_,_ in jrequests}):
                    bands[tp],_metadata=helper.band(h,c2,c3,outer,tp,self.S)
            ...
            for label,tp,degree,k in jrequests:
                ...
                regenerated[label]=sp.expand(value)
            if full_j_phase:
                expected={label for label,_tp,_degree,_k in jrequests if regenerated[label]!=0}
        ...
        mismatches=[label for (label,a),(_,b) in zip(got,raw) if sp.expand(a-b)!=0]
        assert not mismatches,('raw source regeneration mismatch',mismatches[:5])
        assert self.e.rows_hash(got)==cert['raw_new_rows_hash']
        return {'raw_rows_regenerated_before_reduction':len(got),'all_images_equal':True,
                'row_hash':self.e.rows_hash(got),'jacobian_method':jacobian_method,
