...
class Verifier:
    ...
    def expr(self,text):
        ...
        if getattr(self,'native_map_verification',False):
            parser=importlib.import_module('minor_exact_parser')
            assert Path(parser.__file__).resolve().parent==Path(__file__).resolve().parent
            value=parser.parse(text,self.locals)
        else:value=sp.sympify(text,locals=self.locals)
    ...
    def replay_phase(self,cert):
        ...
        native_map=None
        if getattr(self,'native_map_verification',False):
            helper=importlib.import_module('minor_flint_map_verify')
            assert Path(helper.__file__).resolve().parent==Path(__file__).resolve().parent
            native_map=helper.verify(before,resolved,after,ambient-set(before),ambient-set(after))
        else:
        ...
        return {'phase':cert['phase'],'QQstar_pivots_verified':len(cert['pivot_steps']),
                'radical_power_memberships_verified':len(used_radicals),'native_map_composition':native_map,
