...
def main():
    ...
    for tp in range(completed+1,terminal+1):
        ...
        k2=R.plus(R.times(R.times(hh,hh,tp),hh,tp),R.times(c2,hh,tp),c3)
        oo={name:current(poly) for name,poly in base_outer.items()}
        jac=R.jacobian_factored(k2,oo,tp,expand=False)
        band={k:sp.expand(value) for (r,k),value in jac.items() if r==tp}
