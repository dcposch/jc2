...
def main():
    ...
    flint_spec=importlib.util.spec_from_file_location('exact_flint_jacobian',HERE/'deep_flint_jacobian.py')
    ...
    state={'branch':branch,'field':'Q','type':'FULL SOURCE GAUGE CHART, TWO CERTIFIED NECESSARY LOCI MERGED',
        ...
        'coefficient_backend_sha256':hashlib.sha256((HERE/'deep_flint_jacobian.py').read_bytes()).hexdigest(),
        'safe_exporter_sha256':hashlib.sha256((HERE/'deep_safe_singular.py').read_bytes()).hexdigest(),
        'weak_J_bands_already_proved':list(range(completed+1)),
    ...
    def add(rows,name):
        ...
        while True:
            remaining,new,pivots,zeros=E.qstar_reduce(pending,free-set(mapping)-{loc})
            if new:
    ...
    if add(merge_rows,'merge_weak_complete_graph'):
    ...
    for tp in range(completed+1,terminal+1):
        ...
        log(f'{branch}: generating exact w Jacobian band t{tp}, degree {jdegree-tp}')
        current=lambda poly:{pos:value for pos,expression in poly.items() if pos[0]<=tp and (value:=E.substitute_map(expression,mapping))!=0}
        hh,c2,c3=map(current,(base_h,base_c2,base_c3))
        oo={name:current(poly) for name,poly in base_outer.items()}
        band,backend_meta=flint_backend.jacobian_band(hh,c2,c3,oo,tp,S,progress=log)
