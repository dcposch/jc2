...
def main():
    ...
    flint_spec=importlib.util.spec_from_file_location('exact_flint_jacobian',HERE/'deep_flint_jacobian_batch.py')
    ...
    state={'branch':branch,'field':'Q','type':'FULL SOURCE GAUGE CHART, TWO CERTIFIED NECESSARY LOCI MERGED',
        ...
        'coefficient_backend_sha256':hashlib.sha256((HERE/'deep_flint_jacobian_batch.py').read_bytes()).hexdigest(),
    ...
    for tp in range(completed+1,terminal+1):
        ...
        if tp not in band_cache:
            ...
            band_cache,backend_meta=flint_backend.jacobian_bands(hh,c2,c3,oo,high,S,first_band=tp,progress=log)
