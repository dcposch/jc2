...
def run(branch,output,max_stage,finite_through=8,timeout_seconds=None,deep_hook=None):
    ...
    phase_certificate_paths=[]
    last_phase_certificate=None
    records=[]
    accelerated_metadata=None
    trgt=targets(branch)
    ...
    def save(final=False,reason=None):
        ...
        data={'type':('EXACT-Q GAUGE SLICE; FULL-CHART PROMOTION REQUIRES T_q ISOMORPHISM AUDIT'
                      if major.get('gauge_slice') else
                      'EXACT-Q FULL DERIVED CHART; INCREMENTAL SOUND LOCUS'),
              ...
              'accelerated_deep_block':accelerated_metadata,
              'cumulative_map':E.encode_map(mapping),'pivot_certificates':pivot_certificates,
              'radical_certificates':radical_certificates,
              'phase_certificate_paths':phase_certificate_paths,
              'last_phase_certificate':last_phase_certificate,
    ...
    def reduce_new(rows,phase):
        nonlocal mapping,residual,totalzero,last_phase_certificate
        current=[(label,E.substitute_map(sp.sympify(row),mapping)) for label,row in rows]
        phase_cert={'phase':phase,'branch':branch,'field':'Q','localizer':str(localizer),
            'free_before':sorted(map(str,free-set(mapping))),
            'map_before':E.encode_map(mapping),'residual_before':encoded_rows(residual),
            'raw_new_rows_before_reduction':encoded_rows(current),
        ...
        while True:
            ...
            if new:
                ...
                for pivot in pivots:
                    ...
                    phase_cert['pivot_steps'].append({'label':pivot.label,
                        'variable':str(pivot.variable),'rational_leader':str(pivot.coefficient),
                        'rhs_at_pivot':str(pivot.rhs),
                        'selected_equation_at_pivot':str(sp.expand(pivot.coefficient*(pivot.variable-pivot.rhs)))})
            residual=remain
            ...
            for certificate in certificates:
                if certificate['new_row'] in kept:
                    phase_cert['radical_steps'].append(dict(certificate,
                        source_polynomial=str(dict(residual)[certificate['source_row']])))
            pending=residual+roots
            rounds+=1
            assert rounds<=len(free),'radical pivot did not decrease variables'
        phase_cert.update({'map_after':E.encode_map(mapping),
            'residual_after':encoded_rows(residual),'residual_after_hash':E.rows_hash(residual),
            'free_after':sorted(map(str,free-set(mapping)))})
        certificate_path=outdir/f'{output.stem}_phase{len(phase_certificate_paths):04d}.json'
        certificate_path.write_text(json.dumps(jsonable(phase_cert),indent=2,sort_keys=True)+'\n')
        phase_certificate_paths.append(str(certificate_path))
        last_phase_certificate=str(certificate_path)
    ...
    for stage in range(requested+1):
        ...
        if deep_hook is not None and stage==finite_through+1:
            accelerated_metadata=deep_hook({
                'branch':branch,'h':h,'C2':C2,'C3':C3,'outer':baseouter,
                'state':lambda:(free,mapping,residual,all_raw),
                'reduce_new':reduce_new,'output':output,
                'rational_candidate':rational_candidate})
            save(False,'ACCELERATED_ALL_POLE_ROWS_EXHAUSTED')
            if accelerated_metadata.get('unit_ideal'):
                singular=E.singular_dimension(residual,branch,outdir/f'{output.stem}_deepblock.sing')
                assert singular['unit_ideal']
                records.append({'stage':stage,'phase':'deep_all_pole_block',
                    'dimension':-1,'singular':singular,'residual_count':len(residual),
                    'residual_rows':encoded_rows(residual),'cumulative_Qstar_pivots':len(mapping),
                    'free_after':len(free-set(mapping)),
                    'rational_point':{'status':'UNIT_NO_POINT'},
                    'elapsed_stage_seconds':round(time.monotonic()-tick,3)})
                save(True,'UNIT_CORRECTED_DERIVED_CHART_IN_ALL_POLE_BLOCK')
                return
            pole_done=pole_max
            h2_done=max(n for n,_ in trgt['H2'])
            for name,power in (('C2',2),('C3',S['inner_power'])):
                inner_remainder_done[name]=S['minor'][branch]['h3_local_floor']*power-1
            for name in outer_remainder_done:
                normalization_degree=S['outer_specs'][name][0]+1
                outer_remainder_done[name]=(normalization_degree//S['k2_degree'])*h2_done-S['minor'][branch]['cover']-1
        ...
        local_F,local_G=D.local_FG(E,k2,outer,branch,localcap) if pole_done<localcap else ({},{})
        rows=[]
        ...
        for name,table in (('F',local_F),('G',local_G)):
            for n in range(pole_done+1,localcap+1):
                ...
                for k in sorted(tags):
                    ...
                    counts[name]+=1
        pole_done=max(pole_done,localcap)
        if stage>finite_through:
            ...
            for name,poly,power in (('C2',c2,2),('C3',c3,S['inner_power'])):
                ...
                remainder_local=E.local_rows(poly,branch,remainder_cap) if inner_remainder_done[name]<remainder_cap else {}
                ...
                inner_remainder_done[name]=max(inner_remainder_done[name],remainder_cap)
            ...
            for name,poly in outer.items():
                ...
                remainder_cap=min(localcap,remainder_floor-1)
                remainder_local=E.local_rows(poly,branch,remainder_cap) if outer_remainder_done[name]<remainder_cap else {}
                for (n,k),value in sorted(remainder_local.items()):
                    if outer_remainder_done[name]<n<=remainder_cap:
                        rows.append((f'{phase}{stage}_{name}_minor_local{n}_coord{k}',value))
                        counts['outer_minor']+=1
                outer_remainder_done[name]=max(outer_remainder_done[name],remainder_cap)
            h2floor=max(n for n,_ in trgt['H2'])
            h2cap=min(localcap,h2floor)
            h2local=E.local_rows(k2,branch,h2cap) if h2_done<h2cap else {}
