...
def main():
 ...
 before=v.mapping(cert['map_before']);backend=None
 if cfg:
  import minor_flint_corner_direct as C
  assert Path(C.__file__).resolve().parent==HERE
  images={'h3':v.h,'C2':v.c2,'C3':v.c3,**v.outer};hd=v.S['h3_degree'];degrees={'h3':hd,'C2':2*hd,'C3':v.S['inner_power']*hd,**{name:spec[0] for name,spec in v.S['outer_specs'].items()}}
  jets={}
  for name,poly in images.items():
   degree=degrees[name];positions={(degree,0),(degree-1,0),(degree-1,1)};small=v.apply({pos:value for pos,value in poly.items() if pos in positions},before)
   jets[name]=(small.get((degree,0),sp.Integer(0)),sp.expand(small.get((degree-1,0),sp.Integer(0))-small.get((degree-1,1),sp.Integer(0))),small.get((degree-1,1),sp.Integer(0)))
  expected,backend=C.corner(jets,v.S,q,d,sp.Symbol('Jc').xreplace(before))
 else:expected=sp.expand(v.independent_origin_constant(before)-sp.Symbol('Jc').xreplace(before))
 ...
 'replay':replay,'Singular':singular,'independent_corner_backend':backend,'verifier_sha256_at_start':OWN_SHA256,'core_sha256_at_import':M.VERIFIER_SHA256_AT_IMPORT,
