...
def parse(raw, facts):
    ...
    for line in lines[12:-2]:
        ...
        need(len(files) <= 8192 and len(directories) <= 1000 and len(aliases) <= 4000, 'native record count ceiling')
    need(pending is None and 0 < len(files) <= 8192 and 0 < len(directories) <= 1000, 'complete nonempty native records')
    ...
    required = ['/usr/bin/python3', '/usr/bin/python3.12', '/usr/bin/setpriv', '/bin/ps', '/usr/bin/ps', '/usr/lib/python3.12', '/etc/ld.so.cache', '/usr/bin/env', '/usr/bin/systemctl', '/opt/jc2-mixed-20260912/science', '/opt/jc2-mixed-20260912/runtime', '/opt/jc2-mixed-20260912/lib']
    ...
    subset = {'schema': 'f10-mixed-native-files/v1', 'files': dict(files)}
...
def run():
    ...
    limits = {'native-manifest.json': 1048576, 'source-native-manifest.json': 2097152,
