...
done < <(printf '%s\n' "${!jc2_native_files[@]}" "${!jc2_native_aliases[@]}"; sed -n -E 's/^[a-f0-9]{64}  (\/.*)$/\1/p;s/^DIRECTORY [0-9]+ [0-9]+ [0-7]+ (\/.*)$/\1/p;s/^ALIAS (\/[^ ]+) .*$/\1/p' /home/ubuntu/jc2-closedchild-preflight9-20260911d/native.stdout)
