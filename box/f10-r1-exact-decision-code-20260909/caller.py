...
def main():
    ...
    def run(name,script,args,wall,cpu,rss=2147483648,defect=None,mathematical=False):
        ...
        if mathematical:
            ...
            hard=min(deadline,first_math+120,task_deadline)
    ...
    try:
        ...
        if reg['mode']=='validate':
            ...
            return
        run('engine','engine.py',[root/'exact.json',root/'engine.receipt.json'],60,50,mathematical=True)
        run('verify','certificate.py',[root/'exact.json',root/'engine.receipt.json',root/'decision.json'],30,25,mathematical=True)
