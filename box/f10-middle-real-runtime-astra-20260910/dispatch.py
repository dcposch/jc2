"""Fixed real-certificate dispatcher. Metadata only; DISABLED without ROOT authority."""
import argparse
import datetime
import hashlib
import json
...
PROFILES = {'preflight':[5,3,RSS], 'dummy':[5,3,33554432],
            'produce':[300,279,RSS], 'check':[45,49,RSS]}
...
class Batch:
    def __init__(self, path):
        ...
        r=self.r
        need(r.get('schema')=='f10-middle-real-runtime/v1' and r.get('enabled') is True,
        ...
        self.files={k:immutable(v,k in ('producer','checker','authority','probe','dispatcher','caprun'))
                    for k,v in r['files'].items()}
        need(set(self.files)=={'python','setpriv','caprun','producer','checker','authority','probe','dispatcher'},
        ...
        for role in ('producer','checker','authority','probe','dispatcher'):
    ...
    def run(self,label,role,tail,profile,mutation=None,input_pin=None,math_phase=False):
        ...
        authority_path,authority_sha=self.authority(label,args,'probe' if role=='probe' else
                                                  'produce' if role=='producer' else 'check',
                                                  [output],mutation,input_pin)
    ...
    def execute(self):
        ...
        self.normal(t,0)
        need(payload.is_file() and payload.stat().st_size<=LIMIT-4096,'complete producer output size')
        # Terminal-only ordinary metadata inspection; never interpret coefficients or perform arithmetic.
        candidate=read_json(payload,LIMIT)
        need(candidate.get('schema')=='f10-middle-real-certificate/v1' and candidate.get('status')=='CANDIDATE',
             'producer not a candidate; no checker run')
        target=self.frozen/'certificate.json'
        need(not target.exists() and not target.is_symlink(),'absent frozen input')
        before_sha=sha(payload); os.rename(payload,target); os.chown(target,0,0); os.chmod(target,0o444)
        os.chmod(self.frozen,0o555)
        need(sha(immutable(target,True))==before_sha,'frozen input move/hash')
        t,out,err,receipt=self.run('check','checker',['--job',JOB],'check',input_pin=target,math_phase=True)
        self.normal(t,0)
        result=read_json(receipt,4096)
        need(result.get('status')=='CERTIFIED_NONRESONANCE' and result.get('input_sha256')==before_sha,
             'independent checker did not certify')
        used=resource.getrusage(resource.RUSAGE_CHILDREN); own=resource.getrusage(resource.RUSAGE_SELF)
        return {'status':'CERTIFICATE_CHECKED','checker_receipt_sha256':sha(receipt),'input_sha256':before_sha,
                ...
                'scope':'accepted17zzi real triangle only; no source or JC2 promotion'}
...
def main():
    ...
    return 0 if result['status']=='CERTIFICATE_CHECKED' else 2
