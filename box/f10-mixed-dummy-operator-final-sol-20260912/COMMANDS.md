# Final one-shot v2 dummy commands — INERT / UNBOUND

Do not execute while any `__...__` token remains. A fresh ROOT registration
must bind every token, including exact executable hashes/native closure and
the relation `TERM_AT = admission+20s`, `KILL_AT = admission+25s`. Old worker,
unit, clock and cgroup facts are forbidden. `EXPECTED_CGROUP` is frozen before
launch as `/system.slice/UNIT.service`; a later blank ControlGroup is never
used as evidence. System `HOME` is neither assigned nor overwritten.

```sh
set -eu
TASK_INSTANCE='__NEW_INSTANCE__'; TASK_BOOT='__NEW_BOOT__'; TASK_HOST='__NEW_HOST__'
TASK_PID_NS='__NEW_PID_NS__'; TASK_CGROUP_NS='__NEW_CGROUP_NS__'
TASK_UID='__NEW_UID__'; TASK_GID='__NEW_GID__'; TASK_UNIT='__NEW_UNIT__'
TASK_TERM_JOB='__NEW_TERM_TIMER_UNIT__'; TASK_KILL_JOB='__NEW_KILL_TIMER_UNIT__'
TASK_TERM_AT='__NEW_ADMISSION_PLUS_20S_UTC__'; TASK_KILL_AT='__NEW_ADMISSION_PLUS_25S_UTC__'
TASK_MOUNT='__NEW_DUMMY_TMPFS__'; TASK_ADMIN="$TASK_MOUNT/admin"; TASK_OUTPUT="$TASK_MOUNT/output"
TASK_PYTHON='__PINNED_PYTHON__'; TASK_SETPRIV='__PINNED_SETPRIV__'
TASK_CAPRUN='__PINNED_CAPRUN__'; TASK_PROBE='__PINNED_PROBE__'; TASK_JQ='__PINNED_JQ__'
TASK_EXPECTED_CGROUP="/system.slice/$TASK_UNIT.service"
TASK_EXPECTED_CGROUP_FS="/sys/fs/cgroup$TASK_EXPECTED_CGROUP"
TASK_ARGV_SHA='__REGISTERED_CAPRUN_CHILD_ARGV_SHA256__'; TASK_ARGV_COUNT='__REGISTERED_ARGV_COUNT__'
...
test "$(cat "$TASK_OUTPUT/runner.stdout")" = "CAPRUN/v1 status=RESOURCE_CAP telemetry=$TASK_OUTPUT/dummy.telemetry.json"

if test -e "$TASK_EXPECTED_CGROUP_FS"; then
  test -r "$TASK_EXPECTED_CGROUP_FS/cgroup.procs"
  test ! -s "$TASK_EXPECTED_CGROUP_FS/cgroup.procs"
...
ROOT must additionally compare timer arming timestamps and total admission-to-
final-cgroup-proof time against the frozen25-second deadline, authenticate the
registered expected cgroup string and unit names, freeze/read back all evidence,
and perform durable transfer/retirement. Any command/predicate failure is STOP;
there is one attempt and no retry. This is not launch authority.
