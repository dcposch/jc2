#!/bin/bash
# ROOT released ONE worker allocation under ROOT-WORKER-RELEASE.md.
set -euo pipefail
cd /home/ubuntu/jc2
jc2_contact_release=RELEASED
...
test "$(date -u +%s)" -lt "$(date -ud '2026-09-10 21:20:00 UTC' +%s)"
...
/usr/local/bin/aws ec2 run-instances --profile personal --region us-east-1 \
 ...
 --client-token jc2-contact-gram-20260910T2110 \
 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=jc2-contact-gram-20260910T2110},{Key=jc2fleet,Value=1},{Key=market,Value=ondemand},{Key=Owner,Value=root-contact-gram-20260910T2110}]' \
...
systemd-run --user --unit=jc2-contact-gram-20260910T2110-worker-stop \
 --on-calendar='2026-09-10 21:51:00 UTC' --timer-property=AccuracySec=1s \
 /usr/local/bin/aws ec2 terminate-instances --profile personal --region us-east-1 \
 --instance-ids "$jc2_contact_instance"
systemctl --user is-active jc2-contact-gram-20260910T2110-worker-stop.timer
