...
def main():
    ...
    blocks0 = {n: {xy: dict(pl) for xy, pl in t.items()} for n, t in blocks.items()}
    ...
    # ---- independent end-to-end certificate of the lemma on this instance ----
    # A point w on the live coordinates lifts to z = images(w) on all 307 coordinates.
    # Two checks at a random w, neither of which needs a solution of either system:
    #   (a) every ORIGINAL row equals its COMPRESSED counterpart at the lifted point,
    #   (b) every row consumed as a pivot vanishes identically at the lifted point,
    # i.e. the graph of the composite substitution lies in V(I_original) over V(I_comp).
    import random
    ...
    def ev(poly, pt):
        ...
        for mon, co in poly.items():
            v = co
            for i in mon: v = v * pt[i]
            t = t + v
        return t
    pt = [K(rng.randrange(P)) for _ in names]
    for i in sorted(gone): pt[i] = K(0)
    for i in sorted(gone): pt[i] = ev(images[i], pt)
    original = regenerate(blocks0, e, q, ci, ell)
    consumed = [xy for xy in original if xy not in current]
    bad_all = sum(1 for poly in original.values() if ev(poly, pt))
    bad_res = sum(1 for xy, poly in original.items()
                  if ev(poly, pt) != ev(current.get(xy, {}), pt))
    bad_piv = sum(1 for xy in consumed if ev(original[xy], pt))
    print(f"CERT_LIFT rows={len(original)} consumed={len(consumed)} "
          f"original_vs_compressed_mismatch={bad_res} pivot_rows_nonzero={bad_piv} "
          f"original_rows_nonzero={bad_all}", flush=True)
    assert bad_res == 0, "compressed row differs from original row on the lifted graph"
    assert bad_piv == 0, "a consumed pivot row does not vanish on the lifted graph"
    print("CERT_GRAPH_IN_ORIGINAL_ZERO_SET=PASS", flush=True)
    with (out/"images.tsv").open("w") as f:
        f.write("variable|eliminated|image_terms|image_sha256\n")
        for i, nm in enumerate(names):
            t = text(images[i], names)
            f.write(f"{nm}|{int(i in gone)}|{len(images[i])}|"
