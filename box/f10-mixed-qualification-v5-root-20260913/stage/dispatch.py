...
def main():
    ...
    q=r["qualification"]; frozen(q["path"],q["sha256"],65536); n=r["native_manifest"]; frozen(n["path"],n["sha256"],2097152)
    ...
    capture_flags=os.O_WRONLY|os.O_CREAT|os.O_EXCL|os.O_NOFOLLOW
    stdout_fd=os.open(str(out/"runner.stdout"),capture_flags,0o600)
    try:
        stderr_fd=os.open(str(out/"runner.stderr"),capture_flags,0o600)
        os.fchmod(stdout_fd,0o600); os.fchmod(stderr_fd,0o600)
        os.dup2(stdout_fd,1); os.dup2(stderr_fd,2)
    finally:
        os.close(stdout_fd)
        if "stderr_fd" in locals(): os.close(stderr_fd)
    ...
    records=[]; env={"TMPDIR":str(out),"TMP":str(out),"TEMP":str(out),"XDG_CACHE_HOME":str(out),"PYTHONPYCACHEPREFIX":str(out/"pycache")}
    def run(label,role,input_path=None,input_sha=None,expect=0,rss=32212254720,cpu=3290,script=None,stdout=b"",stderr=b""):
        ...
        cmd=[p["python"],"-E","-s","-S","-B",p["caprun"],"--wall-seconds",str(max(1,int((utc(r["deadline"])-now()).total_seconds()))),"--cpu-seconds",str(cpu),"--rss-bytes",str(rss),"--stdout-file",base+"stdout","--stderr-file",base+"stderr","--telemetry-file",base+"telemetry.json","--cwd",str(out),"--",p["setpriv"],"--reuid="+str(r["uid"]),"--regid="+str(r["gid"]),"--clear-groups","--no-new-privs"]+command
