...
class Batch:
    def __init__(self, name):
        ...
        need(self.files['mutator'] == self.wrapper / 'mutate.py', 'exact mutator role')
    ...
    def size_budget(self):
        metadata = 0
        packet_paths = {d / n for d in (self.writer, self.frozen) for n in ('candidate.json', 'changed.json')}
        packet_bytes = 0
        for parent, dirs, names in os.walk(self.mount, followlinks=False):
            ...
            for name in names:
                ...
                if p in packet_paths:
                    need(size <= 2097152, '2MiB certificate cap')
                    packet_bytes += size
                else:
                    metadata += size
    ...
    def freeze(self, names):
        self.group_quiet(); self.revalidate()
        os.chmod(self.frozen, 0o755)
        for name in names:
            source, target = self.writer / name, self.frozen / name
            need(source.is_file() and not source.is_symlink() and not target.exists(), 'fresh freeze')
            digest = sha(source)
            os.rename(source, target); os.chown(target, 0, 0); os.chmod(target, 0o444)
            with open(target, 'rb') as f: os.fsync(f.fileno())
            need(sha(immutable(target)) == digest, 'freeze byte identity')
    ...
    def fixture_receipt(self):
        d = load(self.writer / 'mutate.receipt.json', 32768)
        ...
        need(d['sources']=={str(self.files['mutator']):MUTATOR_SHA,
             str(self.source/'arithmetic.py'):SCIENCE['arithmetic.py']}, 'mutation source binding')
        need(d['baseline_sha256']==BASELINE_SHA and d['baseline_bytes']==str(BASELINE_BYTES)
             and d['inverse_edit_restores_whole_input_bytes'] is True and d['checker_run'] is False,
             'mutation baseline/restoration')
        for field, path in (('input',self.frozen/'candidate.json'),('output',self.writer/'changed.json')):
            need(path.is_file() and not path.is_symlink() and 0<path.stat().st_size<=2097152, 'certificate regular size')
            need(d[field]=={'path':str(path),'sha256':sha(path),'bytes':str(path.stat().st_size)},
