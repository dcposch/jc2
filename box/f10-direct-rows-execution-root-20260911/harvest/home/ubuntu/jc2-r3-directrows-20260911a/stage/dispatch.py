...
JOB = 'f10-source-cone-r3-direct-rows-v1'
...
class Batch:
    def __init__(self, name):
        ...
        r = self.r
        need(r.get('schema') == 'f10-direct-rows-runtime/v1' and r.get('enabled') is True, 'disabled ROOT registration')
        ...
        need(set(preflight) == {'schema', 'source_files', 'entries'}
             and preflight['schema'] == 'f10-direct-rows-preflight-policy/v1'
    ...
    def authorize(self, label, mode, target, input_hash):
        if label in ('valid', 'dummy', 'mutate'):
            obj = {'schema': 'f10-direct-rows-root-policy/v1', 'root_registration_path': str(self.path),
    ...
    def execute(self):
        ...
        self.normal(t, 0)
        need(out.read_bytes() == b'FORMED_UNCHECKED_DIRECT_ROWS_25_NO_SOURCE_OUTCOME\n' and err.read_bytes() == b'', 'producer literal verdict')
        ...
        self.normal(t, 0)
        need(out.read_bytes() == b'CHECKED_DIRECT_ROWS_25_NO_SOURCE_OUTCOME\n'
             and err.read_bytes() == b'', 'independent checker literal verdict')
        phase = {'schema': 'f10-direct-rows-positive-phase/v1', 'status': 'POSITIVE_CHECKED_NO_SOURCE_OUTCOME',
        ...
        need(set(receipt) == {'schema', 'status', 'root_registration_sha256', 'input_sha256',
             ...
             and receipt['schema'] == 'f10-direct-rows-T-mutation/v1'
        ...
        result = {'status': 'DIRECT_ROWS_AND_T_REJECTION_CHECKED_NO_SOURCE_OUTCOME',
    ...
    def durable_copy(self, result):
        ...
        emit(self.durable / 'CUSTODY.json', {'schema': 'f10-direct-rows-runtime-custody/v1', 'result': result,
...
def main():
    ...
    return 0 if result['status'] == 'DIRECT_ROWS_AND_T_REJECTION_CHECKED_NO_SOURCE_OUTCOME' else 2
