...
def root_policy(root_name, role, label, actual):
    ...
    need(r.get('schema') == 'f10-direct-rows-runtime/v1' and r.get('enabled') is True
...
def startup():
    ...
    need(set(phase) == {'schema', 'status', 'root_registration_sha256', 'input', 'source_files', 'authorization', 'checker_stdout_sha256', 'policy'}
         and phase['schema'] == 'f10-direct-rows-positive-phase/v1'
         ...
         and phase['checker_stdout_sha256'] == hashlib.sha256(b'CHECKED_DIRECT_ROWS_25_NO_SOURCE_OUTCOME\n').hexdigest(), 'ROOT-derived genuine positive checker record')
...
def main():
    ...
    need(wire(doc) == raw and set(doc) == {'schema', 'job_tag', 'contract_sha256', 'source_pins', 'place', 'graph'}
         and doc['schema'] == 'f10-r3-direct-rows/v1' and doc['job_tag'] == JOB
    ...
    record = {'schema': 'f10-direct-rows-T-mutation/v1', 'status': 'FORMED_UNCHECKED',
