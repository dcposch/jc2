...
def main():
    ...
    expected = {'schema': 'f10-direct-rows-root-policy/v1', 'root_registration_path': str(pathlib.Path(sys.argv[2])),
                'job_tag': JOB, 'label': mode, 'artifact': str(output), 'science_outcome': 'NONE',
                'source_files': r['source_files'], 'limits': LIMITS, 'profile': r['profiles']['dummy' if mode == 'dummy' else 'control']}
