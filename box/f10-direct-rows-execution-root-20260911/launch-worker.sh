#!/bin/bash
# ROOT exact one-worker allocation release. No science, imports or provisioning.
set -euo pipefail
cd /home/ubuntu/jc2
...
test ! -e box/f10-direct-rows-execution-root-20260911/launch-response.json
sha256sum -c box/f10-direct-rows-execution-root-20260911/ROOT-READY-INPUTS.sha256
...
test "$(date -u +%s)" -lt "$(date -ud '2026-09-11 04:54:00 UTC' +%s)"
...
/usr/local/bin/aws ec2 run-instances --profile personal --region us-east-1 \
 ...
 --output json > box/f10-direct-rows-execution-root-20260911/launch-response.json
test "$(jq 'length' box/f10-direct-rows-execution-root-20260911/launch-response.json)" = 1
jc2_r3_instance=$(jq -r '.[0].ID' box/f10-direct-rows-execution-root-20260911/launch-response.json)
[[ "$jc2_r3_instance" =~ ^i-[0-9a-f]+$ ]]
systemd-run --user --unit=jc2-r3-directrows-20260911a-worker-stop \
 --on-calendar='2026-09-11 06:16:00 UTC' --timer-property=AccuracySec=1s \
...
sed -n '1,150p' box/f10-direct-rows-execution-root-20260911/launch-response.json
