...
INPUTS={
 ...
 'xmodel/golden-nonh-composition-gate-fable5-20260908.md':'ddd2077da0d63d53aea062373f82ce7bdb21d9df1fcf4180127789e98409b9e2'}
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
rows=[]
for name,expected in INPUTS.items():
 p=R/name
 if sha(p)!=expected:raise RuntimeError('input drift '+name)
 rows.append({'path':name,'sha256':expected,'bytes':p.stat().st_size,'body_read':'whole' if 'gate-fable5' not in name else 'lookup only; hash read, no body'})
...
cmd=['/usr/bin/python3','-I','-B',str(R/'ops/artifact_finalize.py'),'begin','--final',str(R/'xmodel/golden-hdiv-coalesced-m-source-astra-20260908.md'),'--basis','0d39df3c9fd69c939a8420c54d03228b9077777d','--owner','/root/nonemptiness_certificate']
r=subprocess.run(cmd,capture_output=True,timeout=25,check=True)
