...
test ! -e box/caprun-closed-scope-deployment-prep-root-20260911/launch-response.json
sha256sum --strict -c box/caprun-closed-scope-deployment-prep-root-20260911/ROOT-READY-INPUTS.sha256
...
/usr/local/bin/aws ec2 run-instances --profile personal --region us-east-1 \
 ...
 --output json > box/caprun-closed-scope-deployment-prep-root-20260911/launch-response.json
test "$(jq 'length' box/caprun-closed-scope-deployment-prep-root-20260911/launch-response.json)" = 1
jc2_r3_instance=$(jq -r '.[0].ID' box/caprun-closed-scope-deployment-prep-root-20260911/launch-response.json)
...
sed -n '1,150p' box/caprun-closed-scope-deployment-prep-root-20260911/launch-response.json
