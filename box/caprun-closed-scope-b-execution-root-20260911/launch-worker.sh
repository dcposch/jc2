...
test ! -e box/caprun-closed-scope-b-execution-root-20260911/launch-response.json
sha256sum --strict -c box/caprun-closed-scope-b-execution-root-20260911/ROOT-READY-INPUTS.sha256
...
test "$(date -u +%s)" -lt "$(date -ud '2026-09-11T12:05:00+00:00' +%s)"
...
/usr/local/bin/aws ec2 run-instances --profile personal --region us-east-1 \
 ...
 --client-token jc2-closedchild-preflight9-20260911b \
 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=jc2-closedchild-preflight9-20260911b},{Key=jc2fleet,Value=1},{Key=market,Value=ondemand},{Key=Owner,Value=root-closedchild-preflight9-20260911b}]' \
 ...
 --output json > box/caprun-closed-scope-b-execution-root-20260911/launch-response.json
test "$(jq 'length' box/caprun-closed-scope-b-execution-root-20260911/launch-response.json)" = 1
jc2_r3_instance=$(jq -r '.[0].ID' box/caprun-closed-scope-b-execution-root-20260911/launch-response.json)
[[ "$jc2_r3_instance" =~ ^i-[0-9a-f]+$ ]]
systemd-run --user --unit=jc2-closedchild-preflight9-20260911b-worker-stop \
 --on-calendar='2026-09-11 12:50:00 UTC' --timer-property=AccuracySec=1s \
 /usr/local/bin/aws ec2 terminate-instances --profile personal --region us-east-1 \
 --instance-ids "$jc2_r3_instance"
systemctl --user is-active jc2-closedchild-preflight9-20260911b-worker-stop.timer
date -u '+ORIGINAL_WORKER_TERMINATION_ARMED %Y-%m-%d %H:%M:%S.%N UTC'
sed -n '1,150p' box/caprun-closed-scope-b-execution-root-20260911/launch-response.json
