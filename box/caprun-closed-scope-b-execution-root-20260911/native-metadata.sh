...
# These are exact expected -I -S path entries; the actual caller compares sys.path.
...
for jc2_path in /usr/bin/python3 /usr/bin/python3.12 /usr/bin/setpriv /bin/ps /usr/bin/ps /usr/lib/python3.12 /etc/ld.so.cache; do
