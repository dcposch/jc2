...
def launch(authority_path, phase):
    ...
    artifacts = {name: file_sha(phase+'.'+name) for name in ('sing', 'identity.json')
                 if Path(phase+'.'+name).is_file()}
    after = {} if phase == 'control' else {name: file_sha(SOURCE.with_suffix('.'+name))
                                          for name in ('jsonl', 'sing')}
    ...
    if after:
        E.need(after == {'jsonl': E.JSON_SHA, 'sing': E.SING_SHA}, 'post-run frozen source drift')
