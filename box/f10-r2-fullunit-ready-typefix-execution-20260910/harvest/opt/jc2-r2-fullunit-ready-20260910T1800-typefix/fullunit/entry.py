...
TAG = 'f10-r2-fullunit-modular89-v1'
...
def enter(mode):
    ...
    if len(sys.argv) != 17 or tuple(sys.argv[1::2]) != flags or sys.argv[2] != TAG:
        raise SystemExit('REFUSED: exact registered modular-fullunit vector required')
...
def receipt(ctx, status, certificate_sha=None):
    ctx['recheck']()
    data = {'schema': 'f10-r2-fullunit-modular89-receipt/v1', 'status': status,
