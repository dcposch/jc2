...
def authorize(role):
    ...
    native = json.loads(frozen(m['path'], m['sha256'], 262144), object_pairs_hook=pairs)
    need(type(native) is dict and set(native) == {'schema','files'}
         and native['schema'] == 'f10-mixed-native-files/v1', 'native schema')
    need(type(native['files']) is dict and 1 <= len(native['files']) <= 1500, 'native inventory bound')
