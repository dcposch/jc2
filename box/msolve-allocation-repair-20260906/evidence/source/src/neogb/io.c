...
#include "../msolve/checked-input-size.h"
        ...
        )
{
    ...
    const size_t input_offset = msolve_input_exponent_offset(idx, nv);

    ev[0]   = 0;
    ev[ebl] = 0;

    for (i = 0; i < nev; ++i) {
        ev[i+1] = (exp_t)(iev+input_offset)[i];
        /* degree */
        ev[0]   +=  ev[i+1];
    }
    for (i = nev; i < nv; ++i) {
        ev[i+off] = (exp_t)(iev+input_offset)[i];
        /* degree */
        ev[ebl]    +=  ev[i+off];
    }
}
    ...
    )
{
  ...
  hm_t **phm = malloc((unsigned long)len * sizeof(*phm));
  if (phm == NULL) {
    fprintf(ERRSTREAM, "Could not allocate term-sort permutation.\n");
    exit(1);
  }
  for (i = 0; i < len; ++i) {
    phm[i]  = &hmo[i];
