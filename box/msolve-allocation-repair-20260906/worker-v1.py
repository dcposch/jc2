...
def tests():
    ...
    run(['cc','-std=c11','-O2','-Wall','-Wextra','-Werror','-fsanitize=undefined',
         '-fno-sanitize-recover=all','-I'+str(src/'src/msolve'),str(ROOT/'boundary.c'),'-o',str(ROOT/'boundary')])
    results={}
    ...
    for prime in (0,1073741827):
        for name,rows in controls.items():
            ...
            if name=='unit': need(polys==['1'],'tiny unit control')
            elif name=='rational': need(set(polys)=={'x','y'},'rational coefficients control')
            else: need(set(polys)=={'y^2-x','x*y-1','x^2-y'},'proper exact small basis')
            engine[label]={'argv':argv,'returncode':p.returncode,'polynomials':polys,
                          'input_sha256':sha(inp),'output_sha256':sha(out)}
    save('tests.json',{'boundary':results,'tiny_engine':engine,'boundary_binary_sha256':sha(ROOT/'boundary'),
