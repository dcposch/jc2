...
jc2_box=/home/ubuntu/jc2/box/caprun-lateflow-completion-sol-20260912
...
for jc2_entry in ADMIT ATTEST INSTALL RELEASE; do
 ...
 ( umask 077; sed "s/JC2_ENTRY_SELECTOR_PLACEHOLDER/$jc2_entry/" "$jc2_kernel" "$jc2_box/FLOW.tail.template.js" >"$jc2_target" )
 test "$(grep -c 'const JC2_ENTRY = ' "$jc2_target")" = 1
 test -z "$(grep 'JC2_ENTRY_SELECTOR_PLACEHOLDER' "$jc2_target" || true)"
