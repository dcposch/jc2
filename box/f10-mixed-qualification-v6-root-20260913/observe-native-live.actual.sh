...
task_boot=ea614786-3720-4b83-b098-89a305fe4087
task_host=ip-172-30-0-123
task_pid_ns=pid:[4026531836]
task_cgroup_ns=cgroup:[4026531835]
task_cutoff='2026-09-13 03:48:00 UTC'
...
printf '%s\n' "$task_limits" | grep -Eq '^Max file size[[:space:]]+4194304[[:space:]]+4194304[[:space:]]+bytes[[:blank:]]*$'
printf '%s\n' "$task_limits" | grep -Eq '^Max address space[[:space:]]+1073741824[[:space:]]+1073741824[[:space:]]+bytes[[:blank:]]*$'
