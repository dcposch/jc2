...
def snapshot():
    ...
    expected_times={20597:'2026-09-06T08:25:46+00:00',20637:'2026-09-06T08:25:49+00:00'}
    for pid,expected_time in expected_times.items():
        actual=datetime.datetime.fromisoformat(info['process_start_utc'][str(pid)])
        need(abs((actual-datetime.datetime.fromisoformat(expected_time)).total_seconds())<3,'start UTC differs from custody')
...
def post():
    ...
    info['source_inventory']=[{'path':str(p),'bytes':p.stat().st_size} for p in (LANE/'presentations/d2-z55-source').iterdir() if p.is_file()]
    info['retained_roots_unmodified']=[str(LANE),'/tmp/msolve-src-0101','/tmp/msolve-patched']
