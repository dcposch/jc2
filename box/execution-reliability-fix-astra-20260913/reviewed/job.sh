...
usage() {
  printf '%s\n' \
    'job.sh start INSTANCE TAG BUNDLE MANIFEST_SHA256' \
...
manager_state() {
  systemctl show "$unit" --no-pager \
    ...
    -p MemorySwapMax -p CPUQuotaPerSecUSec -p TasksMax
}
is_terminal() {
  local active
...
start() {
  require_root
  [[ $# == 2 ]] || die 'start requires bundle and manifest SHA256'
  local bundle=$1 expected=$2 manifest key value src dest wall mem cpu tasks fsize
  ...
  [[ -f $manifest && ! -L $manifest ]] || die 'missing regular job.json'
  ...
  exec > >(tee -a "$dir/admission.stdout") 2> >(tee -a "$dir/admission.stderr" >&2)
  ...
  while IFS=$'\t' read -r key value; do
    ...
    [[ -f $src && ! -L $src && $(realpath -e -- "$src") == "$bundle/"* ]] || die "unsafe bundle file: $key"
  ...
  install -m 0444 "$(realpath -e -- "$0")" "$dir/runner.sh"
...
run_batch() {
  ...
  check_existing
  local n name expected rc arg pid mem cpu tasks
...
collect() {
  ...
  # Serialize collectors; published archive/hash are reused byte-for-byte.
  ...
  if [[ ! -f $dir/evidence.tar ]]; then
    ...
    tar --exclude='./evidence.tar' --exclude='./evidence.tar.partial' \
      --exclude='./evidence.sha256' --exclude='./evidence.sha256.partial' --exclude='./collect.lock' \
