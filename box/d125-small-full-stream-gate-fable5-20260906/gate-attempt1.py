...
def compare(expected, got, authority_sha=None):
    """Byte-exact comparison of an independently reconstructed record list."""
    need(got[0].get('type') == 'header', 'header absent')
    hdr = dict(got[0])
    need(re.fullmatch(r'[0-9a-f]{64}', str(hdr.get('authority_sha256', ''))), 'authority sha absent')
    if authority_sha is not None:
        need(hdr['authority_sha256'] == authority_sha, 'header authority sha != sibling authority.json')
    hdr.pop('authority_sha256')
    need(canon(hdr) == canon(expected[0]), 'header mismatch')
...
def run_case(root, case, branch, results):
    ...
    for name, data2 in (('duplicate_json_key', data.replace(b'{"branch":', b'{"branch":"x","branch":', 1)),
        ...
        try:
            got2 = parse_jsonl(data2); compare(expected, got2, asha)
            controls.append({'control': name, 'rejected': False, 'error': None})
        except GateError as err:
