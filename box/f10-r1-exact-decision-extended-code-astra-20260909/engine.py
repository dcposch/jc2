...
def main():
    ...
    result={'schema':'F10-ENGINE/1','execution':custody,'artifact_sha256':ARTIFACT,
            ...
            'children_rusage':{'user_cpu_seconds':usage.ru_utime,'system_cpu_seconds':usage.ru_stime,
                               'maxrss_kib':usage.ru_maxrss,
                               'scope':'RUSAGE_CHILDREN cumulative waited-child accounting; Linux maxrss is largest child, not PGID sum'},
            'status':'ENGINE-FAILED-INCONCLUSIVE' if rc!=0 else 'CANDIDATE-ONLY-NOT-DECISION'}
