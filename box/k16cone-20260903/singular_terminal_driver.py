...
def emit(t: int, mode: str, prime: int | None, branch: int | None,
         ...
         dump_rows: bool, specialize_b4: int | None,
         var_order: str) -> tuple[str, dict[str, object]]:
    if t < 2:
        raise ValueError("the stable Laurent/Euler recurrence starts at t=2")
    if algorithm not in ("std", "slimgb", "modstd", "nfmodstd"):
        raise ValueError(algorithm)
    basis = (lambda ideal: f"modStd({ideal},1)" if algorithm == "modstd"
             else f"nfmodStd({ideal})" if algorithm == "nfmodstd"
    ...
    chart_full_residual = (list(reversed(["b3", "b4"] + [f"q{j}_0" for j in range(2, t)]))
                           if var_order == "reverse"
                           else ["b3", "b4"] + [f"q{j}_0" for j in range(2, t)])
    ...
    lines: list[str] = [
        ...
        *( ["LIB \"nfmodstd.lib\";"] if algorithm == "nfmodstd" else [] ),
    ...
    if charts != "none":
        full = ",".join(f"T{k}" for k in range(0, 2*t))
        fullone = ",".join(f"subst(T{k},b4,1)" for k in range(0, 2*t))
        ...
        lines += [f"ideal Rfull={full};", f"ideal Rfullone={fullone};", f"ideal Rtop={top};", f"ideal Rpositive={positive};",
                  f"ideal Rfullzero={fullzero};"]
        if charts in ("full", "full-all"):
            lines += chart_ring_header(t, mode, prime, branch, chart_full_residual)
            lines += ["ideal Ifull=imap(R,Rfull);",
                      f"ideal Gfull={basis('Ifull')};",
                      "if (reduce(1,Gfull)==0) { print(\"FULL_UNIT\"); }"
                      " else { print(\"FULL_NONUNIT\"); }",
                      "print(\"FULL_BASIS_SIZE\"); size(Gfull);",
                      "print(\"FULL_DIM\"); dim(Gfull);",
                      "print(\"FULL_VDIM\"); vdim(Gfull);",
                      "if (reduce(1,Gfull)==0) { print(\"FULL_BASIS\"); Gfull; }"]
            if charts == "full":
                ...
                metadata: dict[str, object] = {
                    ...
                    "chart_variable_order": chart_full_residual,
                ...
                return "\n".join(lines) + "\n", metadata
            lines += ["setring R;"]
        lines += chart_ring_header(t, mode, prime, branch, chart_residual)
        lines += ["ideal Ifullone=imap(R,Rfullone);",
                  "ideal Itop=imap(R,Rtop);", "ideal Ipositive=imap(R,Rpositive);",
                  "ideal Ifullzero=imap(R,Rfullzero);"]
        if charts in ("b4one",):
            lines += [f"ideal Gfullone={basis('Ifullone')};",
                      "if (reduce(1,Gfullone)==0) { print(\"B4ONE_FULL_UNIT\"); }"
                      " else { print(\"B4ONE_FULL_NONUNIT\"); }",
                      "print(\"FULLONE_BASIS_SIZE\"); size(Gfullone);",
                      "print(\"FULLONE_DIM\"); dim(Gfullone);",
                      "print(\"FULLONE_VDIM\"); vdim(Gfullone);",
                      "if (reduce(1,Gfullone)==0) { print(\"FULLONE_BASIS\"); Gfullone; }"]
        if charts in ("top", "all", "full-all"):
            lines += [f"ideal Gtop={basis('Itop')};",
                      ...
                      "print(\"TOP_DIM\"); dim(Gtop);",
                      "print(\"TOP_VDIM\"); vdim(Gtop);"]
        if charts in ("residual", "all", "full-all"):
            lines += [f"ideal Gpositive={basis('Ipositive')};",
                      "print(\"POSITIVE_BASIS_SIZE\"); size(Gpositive);",
                      "print(\"POSITIVE_DIM\"); dim(Gpositive);",
                      "print(\"POSITIVE_VDIM\"); vdim(Gpositive);"]
        ...
        if charts in ("b4zero", "residual", "all", "full-all"):
            lines += [f"ideal Gfullzero={basis('Ifullzero')};",
                      ...
                      "print(\"FULLZERO_DIM\"); dim(Gfullzero);",
                      "print(\"FULLZERO_VDIM\"); vdim(Gfullzero);",
                      "if (reduce(1,Gfullzero)==0) { print(\"FULLZERO_BASIS\"); Gfullzero; }"]
    lines += ["print(\"DRIVER_DONE\");", "quit;"]
    metadata: dict[str, object] = {
        "t": t,
        "mode": mode,
        ...
        "chart_variable_order": chart_residual if charts != "full" else chart_full_residual,
        "pivot_schedule": pivot_rows,
        "charts": charts,
        "algorithm": algorithm,
        "comparison_record": comparison_metadata,
...
def main() -> None:
    ...
    parser.add_argument("--charts", choices=("none", "top", "b4zero", "b4one", "residual", "all", "full", "full-all"),
                        default="none")
    parser.add_argument("--algorithm", choices=("std", "slimgb", "modstd", "nfmodstd"), default="std")
    ...
    source, metadata = emit(args.t, args.mode, args.prime, args.branch,
                            ...
                            args.dump_rows, args.specialize_b4, args.var_order)
    suffix = args.mode
    ...
    if args.run:
        ...
        try:
            result = subprocess.run(
                ["Singular", "--cpus=4", "--threads=4", "--flint-threads=4", "-q", str(source_path)], capture_output=True,
