#!/bin/bash
# ROOT one-worker allocation release; exact original clocks and ready pins required.
set -euo pipefail
cd /home/ubuntu/jc2
jc2_r3_release=RELEASED
test "$jc2_r3_release" = RELEASED
if grep -Eq 'JC2_[A-Z0-9_]+_PLACEHOLDER' "$0"; then
 printf '%s\n' 'DISABLED: unresolved ROOT release metadata' >&2
 exit 2
fi
test ! -e box/f10-necessary-rows-execution-root-20260911/launch-response.json
sha256sum -c box/f10-necessary-rows-execution-root-20260911/ROOT-READY-INPUTS.sha256
...
test "$(date -u +%s)" -lt "$(date -ud '2026-09-11 06:58:00 UTC' +%s)"
...
/usr/local/bin/aws ec2 run-instances --profile personal --region us-east-1 \
 ...
 --client-token jc2-r3-necessaryrows-20260911a \
 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=jc2-r3-necessaryrows-20260911a},{Key=jc2fleet,Value=1},{Key=market,Value=ondemand},{Key=Owner,Value=root-r3-necessaryrows-20260911a}]' \
 ...
 --output json > box/f10-necessary-rows-execution-root-20260911/launch-response.json
test "$(jq 'length' box/f10-necessary-rows-execution-root-20260911/launch-response.json)" = 1
jc2_r3_instance=$(jq -r '.[0].ID' box/f10-necessary-rows-execution-root-20260911/launch-response.json)
[[ "$jc2_r3_instance" =~ ^i-[0-9a-f]+$ ]]
systemd-run --user --unit=jc2-r3-necessaryrows-20260911a-worker-stop \
 --on-calendar='2026-09-11 08:16:00 UTC' --timer-property=AccuracySec=1s \
 /usr/local/bin/aws ec2 terminate-instances --profile personal --region us-east-1 \
 --instance-ids "$jc2_r3_instance"
systemctl --user is-active jc2-r3-necessaryrows-20260911a-worker-stop.timer
date -u '+ORIGINAL_WORKER_TERMINATION_ARMED %Y-%m-%d %H:%M:%S.%N UTC'
sed -n '1,150p' box/f10-necessary-rows-execution-root-20260911/launch-response.json
