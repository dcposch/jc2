...
test 'ROOT_RELEASED_EXACT_NECESSARY_ROWS_BATCH' = ROOT_RELEASED_EXACT_NECESSARY_ROWS_BATCH
...
test "$(date -u +%s)" -lt "$(date -ud '2026-09-11 07:20:00 UTC' +%s)"
...
systemd-run --unit=jc2-r3-necessaryrows-20260911a-term \
 --on-calendar='2026-09-11 08:10:00 UTC' --timer-property=AccuracySec=1s \
 /usr/bin/systemctl kill --kill-whom=all --signal=TERM "$jc2_unit"
systemd-run --unit=jc2-r3-necessaryrows-20260911a-kill \
 --on-calendar='2026-09-11 08:10:05 UTC' --timer-property=AccuracySec=1s \
