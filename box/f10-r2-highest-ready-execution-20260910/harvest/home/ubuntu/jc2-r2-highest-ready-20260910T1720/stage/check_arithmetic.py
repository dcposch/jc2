"""Independent checker arithmetic; does not import producer arithmetic."""
...
def inverse(v):
    ...
    if g != [Rational(1)]:
        raise ValueError('independent nonunit')
    inv = F(cofactor)
...
def symbol(j):
    e = tuple(1 if k == j else 0 for k in range(6))
    return {e: F(1)}
...
def product(a, b):
    ...
    for ex, x in a.items():
        for ey, y in b.items():
            key = tuple(ex[j] + ey[j] for j in range(6))
            buckets[key] = fsum(buckets.get(key, F(0)), fproduct(x, y))
    return {key: val for key, val in buckets.items() if any(val)}
...
def fractional_series(c, alpha, n):
    # Differential identity c*f'=alpha*c'*f; coefficient recurrence,
    # independent of the producer's binomial expansion.
    cs = [value(extract(c, 5, i)) for i in range(4)]
    if cs[0] != F(1):
    ...
    for k in range(1, n + 1):
        ...
        fs.append(val)
    return sum_poly(*(move(constant(v), 5, i) for i, v in enumerate(fs)))
...
def primitive(p):
    ...
    for ex, v in p.items():
        key = list(ex)
        key[5] += 1
        out[tuple(key)] = tuple(q / key[5] for q in v)
    return out
