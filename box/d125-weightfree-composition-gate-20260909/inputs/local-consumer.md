...
## 5. The equality endpoint and two incompatible scalar values

Only d1,j12,coalesced remains. Section3 forces r=12/5. Since
ord c1=12 and r=min(ord c0/6,12/5), this entails ord c0>=72/5,
hence ord c0>=15 by integrality; infinity is included. Coalescence
gives kappa>=24/5, hence kappa>=5, and therefore b=0 because
kappa>2r. These assertions use the integer s-orders BEFORE any
Puiseux rescaling. No static branch substitution is made in place of
the moving critical point.

Suppose now F12=lambda*pLM, lambda!=0. This polynomial has transverse
multiplicity1 at BOTH p and M, so the preceding classification must
hold in each actual chart separately. The two kappa values need not
be equal; each is >=5. Let a be the constant Morse square root, so
y_zeta at s=zeta=0 is 1/(a*X^(3/2)). The exact leading data are:

| chart (y,X) | root y0 | a² | (F12)_y at root | source bracket sign/value |
|---|---|---|---|---|
| (p,g) |0|rho=t²|lambda*t*X²|-c*X²|
| (g,p) |-X/t|t(t-1)|lambda*(t-1)*X²|c*X²/t²|

For example at p=0, H=p²*(p+g)*(p+tg)² has quadratic coefficient
t²g³; differentiating lambda*pLM in p gives lambda*t*g². At M=0,
L=(t-1)X/t and M_y=t give the displayed derivative, and H has
quadratic coefficient t(t-1)X³. These are degree<=5 factored source
projections, not an expanded degree15/25 pair.

Write k=lambda*t/a in the first chart, and k=lambda*(t-1)/a in the
second. It is nonzero in each. Combined degree gives the leading
c1 coefficient k*X^(1/2). At zeta=s^(12/5)Y the actual initials are

    P=Y^6+k*X^(1/2)*Y                       at s^(72/5),
    Q=Y^10+(5/3)k*X^(1/2)*Y^5+e*X          at s^24.     (13)

Here is why these are COMPLETE initials. Higher F Taylor coefficients
have order>=12 and n>=2, strictly beyond72/5; c0 has order>=15.
xi is later than24/5. The A scalars are later. Every B scalar kernel
is later than24 since its order is25-5i+24i/5=25-i/5>24 for i<=4.
The leading qF term has precisely order24; the other q terms are
later. The ENTIRE G has order>=24 before the nonnegative-order Morse
inverse, so its initial at24 is independent of Y. Its combined degree
is25-24=1; hence it is e*X with e a field constant, not an arbitrary
function of X. There are no other terms at these orders.

The initial bracket order in (zeta,X) is15r=36, EQUAL to the source
target order. It must not be set to zero. Formal differentiation of
the five displayed monomials, collecting only their scalar products,
gives

    [P,Q]_(Y,X)=(6e-10k²/3)*Y^5+e*k*X^(1/2).       (14)

The Y10 terms cancel: their scalar coefficients are5k on each side.
The Y5 quadratic-in-k contributions are5k²/6 and25k²/6, whose
difference is-10k²/3. This is a scalar projection calculation; no
degree6/10 pair or source high power was materialized.

The chain rule in the M chart gives the target coefficient
c*X^(1/2)/(t²*a). Equation(14) has no target Y5 term, so
e=5k²/9; comparing constants then gives

    c=t²*a*(5k³/9)=5*lambda³*t*(t-1)²/9.           (15M)

In the p chart the order of the two ORIGINAL coordinates is reversed:
[A,B]_(p,g)=-c*g². Its target is therefore
-c*X^(1/2)/a, not +c*X^(1/2)/a. Once again e=5k²/9, and

    c=-a*(5k³/9)=-5*lambda³*t/9.                  (15p)

The two e and k values are chart-local and are not equated. Only the
single original c and lambda are shared. Their equality in(15M)/(15p)
requires (t-1)²+1=0. But (t-1)²+1=rho²+1=3rho, a unit in the
field. Since lambda,t and3rho are nonzero, this is impossible. Both
rho embeddings are covered. Replacing a by -a changes k by -k but
leaves a*k³ unchanged, so neither Morse sign is an additional branch.
This contradiction uses TWO actual double charts, not a receiver
weight, prescribed lower-face coefficient or unreviewed weight filter.

## 6. Exact conditional composition, attacks and limitations

The reference-source report proves the following separate implications
from the unweighted polynomial hypotheses: H² does not divide F_j,
H divides F_j², D=rad H divides F_j, j<=11 in Q and j<=12 in
golden; if H divides F_j then F_j=H*C with j<=9 and H not dividing C.
Conditionally on those reference assertions, the case cover now closes:

1. If H divides F_j, section2 forces EVERY simple factor of H to
   divide C. Section3/4's d2 statement forces every double factor to
   divide C as well. Thus D divides C, giving j<=6 (Q) or j<=7
   (golden). Since H does not divide C, at least one double factor
   has multiplicity EXACTLY1 in C, and hence exactly3 in F_j.
   Section3/4's d3 statement excludes it, including golden j7.
2. If H does not divide F_j, every simple factor already divides
   F_j and every double factor divides it at least once. At least
   one double factor has multiplicity exactly1. Sections3/4 exclude
   it for j<=11. The only possible remaining case is golden j12.
   Degree and D-divisibility then give F12=lambda*pLM, and section5
   excludes it by using BOTH double factors, not just the one first
   selected.

Thus the framework plus this local consumer, IF confirmed, prove
nonexistence of ordinary A15,B25 with the two exact tops and nonzero
J=c*g² WITHOUT any receiver weights. The separate conditional client
interface dcb3a30a48c5977005083a3ad7bbc5935e1f2f74ade6c3569a4b45751c213276
would attach all six named receiver/full-source clients. Its existing
standard-F2(75,125) necessary arrow remains conditional on named
external reductions; arbitrary75/125-to-standardF2, every historical
D125 tag, arbitrary maximum-degree125 and JC2 are NOT supplied here.
Any finite guarded ideal unit consequence is an existential maximal-ideal
argument after theorem acceptance, not a computed cofactor certificate.

Producer attacks/scope controls, all prose or scalar reasoning:

- Moving constants and xi=0 are included through infinite orders. In
  the separated regime xi cannot be0. Each eta is finite because an
  EXACT source Taylor coefficient forces a finite base; no genericity
  assumption is used to avoid cancellation.
- A single vanishing sheet does not kill the tuple: a=b!=0 yields
  (2a,0). Identifying the two signs would incorrectly allow the
  tuple(a+b,a+b) to vanish at a=-b!=0. Both signs are load-bearing.
- The separated proof fails at eta=kappa, where the binomial shift
  is zero. That equality is not silently assigned to this regime;
  it is excluded by(8)/(10), derived from kappa<2r.
- The coalesced leading-B argument would fail if B-R^5 were allowed
  an order0 term -zeta^10. That violates the exact common-top
  hypothesis. The separated proof does NOT reuse that leading-B
  degree bound after a change of valuation: it controls the whole G.
- Allowing correction degree4 at a coalesced initial defeats the
  Y4 comparison: the common quadratic Y²+b+v with v!=0 has a cube
  differing from (Y²+b)³ in degree4. Hence the d<=3 restriction is
  structural, not a cosmetic bound. No such d4 source is excluded.
- The target is deliberately retained at order36 in section5; below
  that order it is legitimately absent from the initial bracket.
  Field/UFD arguments are not extended unchanged to dual numbers.
  The simple consumer's degree0-Q step never treats a degree0 P as
  monic-positive-degree. No unweighted bound deg_g C<=2 appears.

No mathematical subprocess or expanded source/high power/6-10 pair,
CAS/AWS/SSH, external source or new instrument was used. The evidence
is the complete formal proof above, not finite empirical tests. Root
reread the whole body, checked unchanged framework/client pins and ran
own-body raised-OPEN extraction before sealing at00:39. No corpus scan
was performed. The local proof and the complete conditional composition
remain UNREVIEWED until a different-model gate checks all arrows.
...
- Body bytes: `20391`.
- Body SHA-256:
  `a8c6b2bc70269d3225064fe20369a1862ae6607f49efafbf07b2ad70e7182453`.
