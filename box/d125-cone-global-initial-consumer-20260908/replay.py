...
for opt in (False,True):
    for mode in ('normal','change-q0','drop-d0-shift','change-quartic','replace-odd-e'):
        ...
        if p.returncode!=(0 if mode=='normal' else 1):
            raise ValueError((mode,opt,p.returncode,p.stderr.decode()))
        if mode=='normal':
            if witness is not None and witness!=p.stdout: raise ValueError('optimized mismatch')
            witness=p.stdout
        rows.append({'mode':mode,'optimized':opt,'returncode':p.returncode,
